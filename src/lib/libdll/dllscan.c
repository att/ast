...
Dllscan_t *dllsopen(const char *lib, const char *name, const char *version) {
    ...
    if (!(vm = vmopen(Vmdcheap, Vmlast, 0))) return 0;
    if (lib && *lib && (*lib != '-' || *(lib + 1))) {
        /*
         * grab the local part of the library id
         */

        if (s = strrchr(lib, ':')) lib = (const char *)(s + 1);
    ...
    if (!name || !*name || *name == '-' && !*(name + 1)) {
        ...
        if (!(scan->pb = vmnewof(vm, 0, char, t - (char *)name, 2))) goto bad;
    ...
    if (name) {
        ...
        if (!j || i > j && strneq(name, info->prefix, j)) {
            ...
            if (i > k && streq(name + i - k, info->suffix)) {
                i -= j + k;
                if (!(t = vmnewof(vm, 0, char, i, 1))) goto bad;
                memcpy(t, name + j, i);
                t[i] = 0;
                name = (const char *)t;
            }
        ...
        if (!version)
            for (t = (char *)name; *t; t++)
                if ((*t == '-' || *t == '.' || *t == '?') && isdigit(*(t + 1))) {
                    ...
                    version = t + 1;
                    if (!(s = vmnewof(vm, 0, char, t - (char *)name, 1))) goto bad;
    ...
    if (!version) {
        ...
        if (!(s = sfstruse(scan->tmp))) goto bad;
