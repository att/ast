...
static int getquery(void) {
    ...
    for (;;) {
        if (!ld_info) return 1;
        if (!loadquery(L_GETINFO, ld_info, ld_info_size)) return 0;
...
static NSModule multiple(NSSymbol sym, NSModule om, NSModule nm) { return om; }

static void linkedit(NSLinkEditErrors c, int n, const char *f, const char *m) { dlmessage = m; }
    ...
    if (!path) {
        ...
        switch (NSCreateObjectFileImageFromFile(path, &image)) {
            case NSObjectFileImageSuccess:
                dll->module =
                    NSLinkModule(image, path, (mode & RTLD_LAZY) ? 0 : NSLINKMODULE_OPTION_BINDNOW);
...
int dlclose(void *handle) {
    ...
    if (dll->module) NSUnLinkModule(dll->module, 0);
    free(dll);
    return 0;
}

static NSSymbol lookup(Dll_t *dll, const char *name) {
    unsigned long pun;
    void *address;
    ...
    if (dll == DL_NEXT) {
        ...
        if (!NSIsSymbolNameDefined(name)) return 0;
        address = NSLookupAndBindSymbol(name);
    }
    if (address) address = NSAddressOfSymbol(address);
    return address;
}
