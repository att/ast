...
long mtchstring(char *src, long n_src, char *tar, long n_tar, char **match) {
    ...
    if (src != Cursrc) {
        if (Alloced) freemem(N_index, Index);
        Alloced = 0;
        Cursrc = 0;
    }

    /* nothing to do */
    if (!src || n_src <= 0 || !tar || n_tar <= 0) return 0;
        ...
        if (N_index = (long *)malloc(ALPHA * sizeof(long))) {
            ...
            if (!(Index = (char ***)malloc(ALPHA * sizeof(char **)))) {
                ...
                for (sp = src; sp < endsrc; ++sp) {
                    ...
                    if ((n_ind % ALPHA) == 0) {
                        ind = n_ind == 0 ? (char **)malloc(ALPHA * sizeof(char *))
    ...
    while (1) {
        ...
        if (ind) {
            ...
            for (; src + n_match < endsrc; ++src)
                if (*src == *tar) break;
        if (src + n_match >= endsrc) break;

        if ((m = domatch(src, endsrc, tar, endtar, n_match)) > n_match) {
            n_match = m;
            *match = src;
            if (tar + n_match >= endtar) break;
        }
    }
