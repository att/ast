...
Suftree *bldsuftree(Element *src, long len) {
    ...
    n = 2 * len + 1;
    if (n > ALLOCSIZE) n = ALLOCSIZE;
    if (!(list = getmem(root, n))) return 0;
    endlist = list + n;
    ...
    for (; src < endsrc; ++src) {
        ...
        while (sp < endsrc) {
            ...
            if (!(match = child_find(locus, *sp))) break;
...
long mtchsuftree(Suftree *tree, Element *str, long len, Element **mtchp) {
    ...
    endstr = str + len;
    while (1) {
        if (!(match = child_find(tree, *str))) break;
    ...
    if (mlen == 0) /* no match */
        *mtchp = 0;
        ...
        while (CHILD(tree)) tree = CHILD(tree);
        *mtchp = SUFFIX(tree);
    }
