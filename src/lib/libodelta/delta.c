...
static int delputl(register int n, register long v) {
    ...
    for (i = n - 1; i >= 0; --i)
        if (delputc((char)c[i]) < 0) return -1;
    return 0;
}
...
static int delputs(register long n, register long addr) {
    if (n < (Dend - Dnext)) {
        ...
        if (delflush() < 0) return -1;
        if (write(Dfd, Btar + addr, n) != n) return -1;
    }
    return 0;
}
...
static Move *makeAdd(char *beg, char *end, Move *last) {
    ...
    while (last) {
        ...
        if (last->type == DELTA_ADD) break;

        cost_m = NBYTE(last->size) + NBYTE(last->addr) + NBYTE(ip->size) + ip->size;
        a_size = ip->size + last->size;
        cost_a = NBYTE(a_size) + a_size;
        if (cost_m < cost_a) break;
...
static Move *optMove(register Move *s) {
    ...
    for (ip = s; ip; ip = ip->next) {
        ...
        if (ip->type == DELTA_ADD || ip->size > (M_MAX + A_MAX)) break;
...
/* the real thing */
int delta(char *src, long n_src, char *tar, long n_tar, int delfd) {
    ...
    if (delputc(inst) < 0) return -1;
    if (delputl(NBYTE(n_src), n_src) < 0 || delputl(NBYTE(n_tar), n_tar) < 0) return -1;
    ...
    while (n_tar > 0) {
        ...
        if (size < 0) return -1;
        if (size > 0) size = chkMove(size, (long)(match - Bsrc), (long)(tp ? tp - tar : 0));
    ...
    if (tree)
        ...
        mtchstring(NULL, 0L, NULL, 0L, NULL);
    ...
    if (moves) {
        ...
        ip = moves;
        while (ip->last) ip = ip->last;
        for (; ip; ip = ip->next)
            if (ip->type == DELTA_MOVE && ip->size <= (M_MAX + A_MAX)) moves = ip = optMove(ip);

        while (moves->last) moves = moves->last;
    }

    /* write out the move instructions */
    addr = 0L;
    while (moves) {
        if (moves->type == DELTA_ADD) moves->addr = addr;
        addr += moves->size;
        if (putMove(moves) < 0) return -1;
        moves = delMove(moves);
    }
