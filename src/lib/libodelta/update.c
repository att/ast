...
static int delgetc(void) {
    if (Dnext >= Dend) {
        register int n;
        if ((n = read(Dfd, (char *)Ddata, BUFSIZE)) <= 0) return -1;
...
static int filetransfer(int fd, long n) {
    while (n > 0) {
        ...
        if (Tnext >= Tend)
            if (tarflush() < 0) return -1;
        r = n > (Tend - Tnext) ? (Tend - Tnext) : n;
        if (read(fd, (char *)Tnext, r) != r) return -1;
        Tnext += r;
        n -= r;
    }
    return 0;
...
int update(int srcfd, long offset, int delfd, int tarfd) {
    register int i;
    ...
    /* read the file sizes */
    if ((i = delgetc()) < 0 || (i & DELTA_TYPE) != DELTA_TYPE) return -1;
    if ((n_src = delgetl((i >> 3) & 07)) < 0 || (n_tar = delgetl(i & 07)) < 0) return -1;
    ...
    if (src = (unsigned char *)malloc(n_src)) /* assignment = */
    {
        ...
        if (read(srcfd, (char *)src, n_src) != n_src) return -1;
    }

    while ((i = delgetc()) >= 0) {
        register long size, addr;

        if ((size = delgetl((i >> 3) & 07)) < 0) return -1;
        switch (i & DELTA_TYPE) {
            default:
                return -1;
            case DELTA_TYPE:
                /* sync delta file pointer */
                if ((addr = Dend - Dnext) > 0) lseek(Dfd, -addr, 1);
                /* flush output buffer */
                if (tarflush() < 0) return -1;
                /* free space used */
                if (src) free(src);
                ...
                return 0;
            case DELTA_MOVE:
                if ((addr = delgetl(i & 07)) < 0) return -1;
                if (src) {
                    if (memtransfer(src + addr, size) < 0) return -1;
                } else {
                    if (lseek(srcfd, offset + addr, 0) < 0) return -1;
                    if (filetransfer(srcfd, size) < 0) return -1;
                }
                break;
            case DELTA_ADD:
                if (deltransfer(size) < 0) return -1;
                break;
        }
    }
