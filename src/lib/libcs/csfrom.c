...
ssize_t csfrom(register Cs_t *state, int fd, void *buf, size_t siz, Csaddr_t *addr) {
#if CS_LIB_V10
    ...
    if ((n = read(fd, buf, siz)) < 0)
        messagef((state->id, NULL, -1, "from: %d: udp data read error", fd));
    else
        messagef((state->id, NULL, -8, "from(%d,*,%d) = %d, data = `%-.*s', addr = %s, port = %u",
                  fd, siz, n, n, buf, csntoa(state, udp.host), udp.port));
    return n;
    ...
    messagef((state->id, NULL, -8, "from(%d,*,%d) = %d, data = `%-.*s', addr = %s, port = %u", fd,
              siz, n, n, buf, csntoa(state, (unsigned long)nam.sin_addr.s_addr),
...
ssize_t _cs_from(int fd, void *buf, size_t siz, Csaddr_t *addr) {
