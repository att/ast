...
static int sockrecv(int fd, Csid_t *id, int *fds, int n) {
    struct iovec iov;
    struct msghdr msg;
...
#define msg_controllen msg_accrightslen
    ...
    id->gid = id->gid;
#if _mem_msg_control_msghdr
    if (ctl.hdr.cmsg_level != SOL_SOCKET || ctl.hdr.cmsg_type != SCM_RIGHTS) return -1;
    n = (ctl.hdr.cmsg_len - sizeof(ctl.hdr)) / sizeof(fds[0]);
    for (fd = 0; fd < n; fd++) fds[fd] = ctl.fds[fd];
    return n;
#else
...
int csrecv(register Cs_t *state, int fd, Csid_t *id, int *fds, int n) {
    int oerrno;
    Csid_t ignore;
    ...
    if ((fds[0] = accept(fd, (struct sockaddr *)&nam, &namlen)) >= 0) {
        ...
        if (nam.sin_family == AF_UNSPEC) nam.sin_family = AF_UNIX;
#endif
        if (nam.sin_family == AF_UNIX) {
            ...
            if (fstat(rfd[0], &st)) {
                messagef(
                    (state->id, NULL, -1, "recv: %d: %d: authentication stat error", fd, rfd[0]));
            drop:
                close(rfd[0]);
            eperm:
                close(fds[0]);
            ...
            if ((st.st_mode & CS_AUTH_MASK) != CS_AUTH_MODE) {
                messagef((state->id, NULL, -1, "recv: %d: %d: invalid authentication mode %04o", fd,
                          rfd[0], st.st_mode & CS_AUTH_MASK));
                goto drop;
            }
            close(rfd[0]);
#else
    ...
    if ((st.st_mode & CS_AUTH_MASK) != CS_AUTH_MODE) {
        messagef((state->id, NULL, -1, "recv: %d: %d: invalid authentication mode %04o", fd, rfd[0],
    ...
    if (!access(CS_PROC_FD_TST, F_OK)) {
        ...
        if (i >= sizeof(state->temp)) i = sizeof(state->temp) - 1;
        s[i] = 0;
        messagef((state->id, NULL, -8, "recv: `%s'", s));
        pid = strtol(s, &s, 0);
        i = strtol(s, &s, 0);
        if (i < n) n = i;
        for (i = 0; i < n; i++) fds[i] = strtol(s, &s, 0);
        s = state->temp;
        for (i = j = 0; i < n; i++) {
            sfsprintf(s, sizeof(state->temp), CS_PROC_FD_FMT, pid, fds[i]);
            if (!stat(s, &st) &&
                (fds[i] = open(s, (st.st_mode & (S_IRUSR | S_IWUSR)) == (S_IRUSR | S_IWUSR)
                                      ? O_RDWR
                                      : (st.st_mode & S_IWUSR) ? O_WRONLY : O_RDONLY)) >= 0)
                j++;
        }
        if (id) {
            id->hid = 0;
...
int _cs_recv(int fd, Csid_t *id, int *fds, int n) { return csrecv(&cs, fd, id, fds, n); }
