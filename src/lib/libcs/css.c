...
static void done(void) { cssclose(NULL); }

/*
 * drop fd ip
 */

static void drop(Css_t *css, register Cspoll_t *pp) {
    register Cssfd_t *ip;
    register int fd;
    register int user;

    if (!css) css = state.main;
    fd = pp->fd;
    ip = &state.fdinfo[fd];
    if (ip->css == css) {
        user = (ip->events & CS_POLL_USER) != 0;
        ip->events = 0;
        if (css->auth) state.auth[fd].seq = 0;
        if (ip->actionf) {
            ip->status = CS_POLL_CLOSE;
            if ((*ip->actionf)(css, ip, css->disc) > 0) fd = -1;
        }
        ...
        css->fdpolling--;
        if (ip->events & CS_POLL_CONNECT) css->fdlistening--;
        if (user)
            css->fduser--;
        else if (fd >= 0)
            close(fd);
...
Css_t *cssopen(const char *path, Cssdisc_t *disc) {
    register Css_t *css;
    int type;
    int n;
    char *s;
    ...
    if (!state.servers) {
        csprotect(&cs);
        if ((n = (int)strtol(astconf("OPEN_MAX", NULL, NULL), NULL, 0)) < OPEN_MIN) n = OPEN_MIN;
        ...
        state.fdnext = -1;
        state.pid = getpid();
        if (!(css = newof(0, Css_t, 1, sizeof(Cssdisc_t)))) return 0;
        css->id = cs.id;
    ...
    if (!path || (n = strlen(path) + 2) < 2 * CS_NAME_MAX) n = 2 * CS_NAME_MAX;
    if (!(css = newof(0, Css_t, 1, n))) return 0;
    n = n / 2 + 1;
    css->next = state.servers;
    state.servers = css;
    if (!(css->state = csalloc(NULL))) goto bad;
    ...
    if (path && strchr(path, '/')) {
        ...
        if ((css->fd = csopen(css->state, path, CS_OPEN_CREATE)) < 0) {
            if (css->disc->errorf) {
                if (errno == EEXIST)
                    ...
                    (*css->disc->errorf)(css, css->disc, ERROR_SYSTEM | 3,
                                         "%s: cannot create connect stream", path);
            }
            goto bad;
        }
        ...
        if ((css->disc->flags & CSS_AUTHENTICATE) &&
            csopen(css->state, css->service, CS_OPEN_CREATE | CS_OPEN_MOUNT)) {
            if (css->disc->errorf)
                (*css->disc->errorf)(css, css->disc, ERROR_SYSTEM | 3,
                                     "%s: cannot create authentication directory", css->service);
            goto bad;
        }
    }
    strcpy(css->path, path);
    if ((css->disc->flags & CSS_DAEMON) &&
        csdaemon(css->state,
                 (css->disc->flags & CSS_PRESERVE) ? ~0L : ((1 << css->fd) | (1 << 2)))) {
        if (css->disc->errorf)
    ...
    if (css->state->control) {
        ...
        *s = 0;
        if (stat(css->mount, &st)) {
            if (css->disc->errorf)
                (*css->disc->errorf)(css, css->disc, 3, "%s: cannot stat service directory",
                                     css->mount);
            goto bad;
        }
        if ((css->disc->flags & CSS_DAEMON) && chdir(css->mount)) {
            if (css->disc->errorf)
                (*css->disc->errorf)(css, css->disc, 3, "%s: cannot set service directory",
                                     css->mount);
            goto bad;
        }
        *s = '/';
        ...
        *s = 0;
        if (lstat(css->mount, &st)) {
            if (css->disc->errorf)
                (*css->disc->errorf)(css, css->disc, 3,
                                     "%s: invalid global authentication directory", css->mount);
            goto bad;
        }
        *s = '/';
        ...
        if (lstat(s, &st) && (mkdir(s, S_IRWXU | S_IRWXG | S_IRWXO) ||
                              chmod(s, S_ISVTX | S_IRWXU | S_IRWXG | S_IRWXO) || lstat(s, &st))) {
            if (css->disc->errorf)
                (*css->disc->errorf)(css, css->disc, 3,
                                     "%s: invalid local authentication directory", s);
            goto bad;
        }
        ...
        css->uid = st.st_uid;
        css->gid = st.st_gid;
        if (type == 't' && *(css->control - 2) != CS_MNT_OTHER && *(css->control - 2) != '.' &&
            *(css->control - 2) != '*') {
            css->auth = 1;
            state.nauth++;
            CSTIME();
            ...
            *css->control = CS_MNT_AUTH;
            ...
            if (close(open(css->mount, O_WRONLY | O_CREAT | O_TRUNC,
            ...
            for (n = 0; n < (css->newkey & 0xff); n++) CSTOSS(css->conkey, n);
        ...
        *css->control = CS_MNT_PROCESS;
        ...
        s = css->buf;
        if (type != 'f')
            s += sfsprintf(s, sizeof(css->buf) - (s - css->buf), "/n/%s", csname(css->state, 0));
    ...
    if ((css->disc->flags & CSS_INTERRUPT) && !(state.flags & CSS_INTERRUPT)) {
        ...
        for (n = 0; n < elementsof(signals); n++)
            if (signal(signals[n], SIG_IGN) != SIG_IGN) signal(signals[n], interrupt);
    }
    cssfd(css, css->fd,
          type == 'u' ? CS_POLL_READ : (CS_POLL_AUTH | CS_POLL_CONNECT | CS_POLL_READ));
    CSTIME();
    return css;
bad:
    cssclose(css);
...
int cssclose(register Css_t *css) {
    register Css_t *pss;
    register Css_t *xss;
    register Css_t *oss;
    register int i;
    ...
    static char mnt[] = {CS_MNT_AUTH, CS_MNT_LOCK, CS_MNT_PROCESS, CS_MNT_STREAM};
...
Cssfd_t *cssfd(register Css_t *css, register int fd, unsigned long op) {
    register Cssfd_t *ip;
    ...
    if (!css) css = state.main;
again:
    if (fd < 0 || fd >= state.fdmax) return 0;
    ip = state.fdinfo + fd;
    if (op == 0) return ip->events ? ip : (Cssfd_t *)0;
    if (op & CS_POLL_ARG) {
        cmd = op & CS_POLL_MASK;
    ...
    if (op != CS_POLL_CLOSE) {
        ...
        for (n = 0;; n++) {
            if (n >= state.fdpolling) {
                if (!(op & CS_POLL_AUTH | CS_POLL_CONNECT | CS_POLL_READ | CS_POLL_WRITE)) return 0;
                if (fcntl(fd, F_SETFD, FD_CLOEXEC) && errno == EBADF) return 0;
                ...
                ip->set = 0;
                css->fdpolling++;
                if (ip->events & CS_POLL_BEFORE) state.fdbefore++;
                if (ip->events & CS_POLL_CONNECT) css->fdlistening++;
                if (ip->events & CS_POLL_USER) css->fduser++;
#if DEBUG
                goto show;
#else
                return ip;
            ...
            if (state.fdpoll[n].fd == fd) {
                switch (cmd) {
                            ...
                            if (op & CS_POLL_WRITE) {
                                ...
                                if (!(ip->set & CS_POLL_WRITE)) {
                                    ...
                                    if ((flags = fcntl(fd, F_GETFL, 0)) >= 0) {
                                        ...
                                        messagef(
                                            (state.main->id, NULL, -4, "cssfd: %d O_NONBLOCK", fd));
                                    }
                                }
#endif
                    ...
                    case CS_POLL_DUP:
                        ...
                        goto again;
                    case CS_POLL_MOVE:
                        if (fcntl(nfd, F_SETFD, FD_CLOEXEC) && errno == EBADF) return 0;
                        state.fdpoll[n].fd = nfd;
                        if (fd == css->fd) css->fd = nfd;
                        state.fdinfo[nfd] = *ip;
                        ip = state.fdinfo + nfd;
                        ip->fd = nfd;
                        break;
            ...
            show:
                if (error_info.trace <= -9) {
                    errorf(state.main->id, NULL, -9,
                           "cssfd: pending=%d polling=%d next=%d fd=%d op=0x%08lx", state.fdpending,
                           state.fdpolling, state.fdnext, ofd, oop);
                    for (n = 0; n < state.fdpolling; n++)
                        errorf(state.main->id, NULL, -9,
                               "cssfd: index=%d fd=%d events=%06o actionf=%p", n,
                               state.fdpoll[n].fd, state.fdpoll[n].events,
...
Cssfd_t *csspoll(unsigned long ms, unsigned long flags) {
    register Css_t *css;
    register Cspoll_t *pp;
    register Cssfd_t *ip;
    register Auth_t *ap;
    ...
    for (;;) {
        while (state.fdpending <= 0) {
            ...
            for (css = state.servers; css; css = css->next) {
                ...
                if (css->disc->timeout) {
                    ...
                    if (to > css->timeout_remain) to = css->timeout_remain;
                }
                if (css->disc->wakeup) {
                    if (css->wakeup_last != css->disc->wakeup)
                        css->wakeup_last = css->wakeup_remain = css->disc->wakeup;
                    if (to > css->wakeup_remain) to = css->wakeup_remain;
                }
            }
            if (state.fdbefore)
                for (pp = state.fdpoll; pp < state.fdpoll + state.fdpolling;) {
            ...
            for (css = state.servers; css; css = css->next)
                if (css->disc->exceptf) {
                    if (css->disc->timeout) {
                        if (css->fdpending)
                            ...
                            if ((css->disc->flags & CSS_DORMANT) &&
                                css->fdpolling <= (css->fdlistening + css->fduser))
                    ...
                    if (css->disc->wakeup) {
                        messagef((state.main->id, NULL, -4,
                                  "csspoll: pending=%d polling=%d z=%I*d wakeup=%I*d remain=%I*d",
                                  state.fdpending, state.fdpolling, sizeof(z), z,
                                  sizeof(css->disc->wakeup), css->disc->wakeup,
                                  sizeof(css->wakeup_remain), css->wakeup_remain));
            ...
            if (err &&
                (ms != CS_NEVER || (flags & CSS_INTERRUPT) && (sig || err == EINTR) && !clrsig ||
        ...
        do {
            if (++state.fdnext >= state.fdpolling) {
                if (state.fdloop-- < 0) break;
                state.fdnext = 0;
            }
            pp = state.fdpoll + state.fdnext;
            fd = pp->fd;
            messagef((state.main->id, NULL, -9,
                      "csspoll: pending=%d polling=%d next=%d fd=%d status=%06o", state.fdpending,
                      state.fdpolling, state.fdnext, fd, pp->status));
            ip = state.fdinfo + fd;
            css = ip->css;
            ...
            switch (status) {
                ...
                case CS_POLL_CONNECT | CS_POLL_READ:
                    if (csrecv(css->state, fd, &id, &fdnew, 1) == 1 &&
                        (ip = cssfd(css, fdnew, CS_POLL_READ)) && css->disc->acceptf &&
                        (fdnew = (*css->disc->acceptf)(css, ip, &id, NULL, css->disc)) != ip->fd)
                        cssfd(css, ip->fd,
                              fdnew >= 0 ? CS_POLL_CMD(fdnew, CS_POLL_MOVE) : CS_POLL_CLOSE);
                    break;
                case CS_POLL_AUTH | CS_POLL_READ:
                    switch ((ap = state.auth + fd)->seq) {
                        case 1:
                            if ((n = csread(css->state, fd, css->buf, sizeof(css->buf), CS_LINE)) <=
                                0)
                                goto reject;
                            css->buf[n - 1] = 0;
                            if (tokscan(css->buf, &t, "%lu", &key) != 1) goto reject;
                            switch (key) {
                                case CS_KEY_SEND:
                                    n = sfsprintf(css->tmp, sizeof(css->tmp), "%s\n", css->mount);
                                    if (cswrite(css->state, fd, css->tmp, n) != n) goto reject;
                                    break;
                                default:
                                    if (key <= CS_KEY_MAX ||
                                        key != css->newkey && key != css->oldkey ||
                                        tokscan(t, NULL, "%ld", &ap->id.pid) != 1)
                                        goto reject;
                        ...
                        case 2:
                            ...
                            if (cs.time > ap->expire ||
                                (n = csread(css->state, fd, css->buf, sizeof(css->buf), CS_LINE)) <=
                                    1)
                                goto reject;
                            css->buf[n - 1] = 0;
                            if (t = strchr(css->buf, ' ')) *t++ = 0;
                            if (!(s = strrchr(css->buf, '/'))) goto reject;
                            *s = 0;
                            if (lstat(css->buf, &st)) goto reject;
                            *s = '/';
                            for (n = 0; n < elementsof(css->fid); n++)
                                if (css->fid[n].dev == st.st_dev && css->fid[n].ino == st.st_ino)
                                    break;
                            if (n >= elementsof(css->fid)) goto reject;
                            if (lstat(css->buf, &st)) {
                                /*
                            ...
                            if ((st.st_mode & CS_AUTH_MODE) != CS_AUTH_MODE ||
                                ap->atime != st.st_atime || ap->mtime != st.st_mtime)
                                goto reject;
                        ok:
                            ...
                            if (css->disc->acceptf) {
                                ...
                                while ((vp < &va[elementsof(va) - 1]) && (*vp++ = t) &&
                                    ...
                                    *t++ = 0;
                                *vp = 0;
                                if ((fdnew = (*css->disc->acceptf)(
                                         css, ip, &ap->id, *va ? va : (char **)0, css->disc)) !=
                                    ip->fd)
                                    cssfd(css, ip->fd,
                ...
                case CS_POLL_READ:
                    ip->status = status;
                    messagef((state.main->id, NULL, -9, "csspoll: status=%06o fd=%d actionf=%p",
                              status, fd, ip->actionf));
