...
ssize_t msgvcall(int fd, unsigned long channel, unsigned long call, Msg_return_t *ret, void **xp,
                 va_list ap) {
    register long at;
    register long n;
    register char *p;
    ...
    if (call & MSG_ACK) {
        if (msg_ack.fd < 0) return -1;
        if (!msg_ack.fd) {
            if ((msg_ack.fd = csbind(&cs, "udp", 0, CS_PORT_NORMAL, 0L)) < 0) return -1;
        ...
        switch (at & ((1 << MSG_ARG_TYPE) - 1)) {
            ...
            case MSG_ARG_file:
                if (ret) {
                    ...
                    for (n = 0; n < sizeof(*ip) / sizeof(ip->fid[0]); n++) msgputu(&b, e, 0);
                break;
            default:
    ...
    for (;;) {
        switch ((at >>= MSG_ARG_TYPE) & ((1 << MSG_ARG_TYPE) - 1)) {
            case MSG_ARG_array:
                ...
                msgputu(&b, e, n);
                np = va_arg(ap, long *);
                while (n-- > 0) msgputu(&b, e, *np++);
                continue;
            case MSG_ARG_file:
                ip = (xp && *xp) ? (Msg_file_t *)(*xp++) : va_arg(ap, Msg_file_t *);
                for (n = 0; n < sizeof(*ip) / sizeof(ip->fid[0]); n++) msgputu(&b, e, ip->fid[n]);
                continue;
            case MSG_ARG_input:
                p = va_arg(ap, char *);
                n = va_arg(ap, long);
            ...
            case MSG_ARG_vector:
                if (vp = va_arg(ap, char **)) {
                    ...
                    if (nv++ && !coexport && !(coexport = getenv("COEXPORT"))) coexport = "";
                    n = 0;
...
ssize_t msgcall(int fd, unsigned long channel, unsigned long call, Msg_return_t *ret, ...) {
    ssize_t n;
    va_list ap;
...
long msgvreturn(int fd, unsigned long call, void **xp, va_list ap) {
    register long n;
...
long msgreturn(int fd, unsigned long call, ...) {
    long n;
    va_list ap;
