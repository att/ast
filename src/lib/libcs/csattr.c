...
static Hash_table_t *load(register Cs_t *state) {
    register char *s;
    char *h;
    register Info_t *ip;
    register Sfio_t *sp;
            ...
            if (*s) {
                ...
                *s++ = 0;
                if (!(ip = (Info_t *)hashlook(
                          tp, h, HASH_CREATE | HASH_SIZE(sizeof(Info_t) + strlen(s) + 1), NULL))) {
                    hashfree(tp);
                    tp = 0;
                    break;
                }
...
char *csattr(register Cs_t *state, const char *name, const char *attr) {
    register char *s;
    register char *b;
    register char *x;
    register char *v;
    ...
    if (!tp && !(tp = load(state))) return 0;
    if (attr && (!*attr || *attr == '-' && !*(attr + 1))) attr = 0;
    b = buf;
    x = &buf[sizeof(buf) - 1];
    if (!name || *name == '-' && !*(name + 1)) {
        name = 0;
    scan:
        if (!pt && !(pt = hashscan(tp, 0))) return 0;
        ...
        if (n) return ip->name;
        if (attr && streq(attr, "*")) return buf;
        b += sfsprintf(b, x - b, "%s", ip->name);
    } else {
        if (streq(name, CS_HOST_LOCAL)) name = (const char *)csname(state, 0);
        if (!(ip = (Info_t *)hashlook(tp, name, HASH_LOOKUP, NULL))) return 0;
        if (attr) {
            if (streq(attr, "*")) return buf;
            if (streq(attr, "name")) return csaddr(state, ip->name) ? state->host : ip->name;
    ...
    if (!attr) {
        ...
        if (addr = csaddr(state, ip->name)) {
            if (!streq(ip->name, state->host)) b += sfsprintf(b, x - b, " host=%s", state->host);
...
char *_cs_attr(const char *name, const char *attr) { return csattr(&cs, name, attr); }
