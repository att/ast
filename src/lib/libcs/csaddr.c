...
static unsigned long realaddr(register Cs_t *state, const char *name) {
    register int n;
    ...
    if (streq(s, CS_HOST_LOCAL)) {
        messagef((state->id, NULL, -8, "realaddr(%s) = %s, flags = |%s%s%s", name,
                  csntoa(state, local), (state->flags & CS_ADDR_LOCAL) ? "LOCAL|" : "",
    ...
    if (!*s && n == 4) {
        ...
        state->flags |= CS_ADDR_NUMERIC;
        messagef((state->id, NULL, -8, "realaddr(%s) = %s, flags = |%s%s%s", name,
                  csntoa(state, addr.l), (state->flags & CS_ADDR_LOCAL) ? "LOCAL|" : "",
                  (state->flags & CS_ADDR_REMOTE) ? "REMOTE|" : "",
                  (state->flags & CS_ADDR_SHARE) ? "SHARE|" : ""));
        return addr.l;
    }
...
#if CS_LIB_V10
    if ((addr.l = (unsigned long)in_address(name)) == INADDR_ANY) addr.l = 0;
#else
...
unsigned long csaddr(register Cs_t *state, const char *aname) {
    ...
    if (!local) {
#if CS_LIB_SOCKET
        if (!state->db) state->db = -1;
#endif
        local = CS_LOCAL;
        if (addr = realaddr(state, csname(state, 0))) local = addr;
    }
    if (!name) {
        addr = local;
        goto ok;
            ...
            case 0:
                ...
                if (sp = csinfo(state, name, NULL)) {
                    while (name = sfgetr(sp, '\n', 1))
                        if (addr = realaddr(state, name)) goto ok;
                    sfclose(sp);
                    sp = 0;
                }
                if (!(addr = realaddr(state, name = CS_HOST_GATEWAY))) addr = local;
                goto ok;
            case '.':
                flags |= CS_ADDR_SHARE;
    ...
    return 0;
ok:
    if (state->flags & CS_ADDR_NUMERIC) flags &= ~CS_ADDR_SHARE;
    if (addr == local) {
        flags |= CS_ADDR_LOCAL;
        name = csname(state, 0);
    ...
    if (sp) sfclose(sp);
    state->flags &= ~(CS_ADDR_LOCAL | CS_ADDR_NOW | CS_ADDR_REMOTE | CS_ADDR_SHARE |
                      CS_DAEMON_SLAVE | CS_ADDR_TEST | CS_ADDR_TRUST);
    state->flags |= flags;
    messagef((state->id, NULL, -8, "addr(%s) = %s, flags = |%s%s%s", aname, csntoa(state, addr),
...
unsigned long _cs_addr(const char *aname) { return csaddr(&cs, aname); }
