...
void cssetdb(register Cs_t *state) {
    register int fd;
    register int i;

    if (state->db <= 0 && ++state->db) {
        if ((fd = dup(0)) >= 0) close(fd);
        sethostent(1);
...
char *csname(register Cs_t *state, unsigned long addr) {
    register char *s;
...
char *_cs_name(unsigned long addr) { return csname(&cs, addr); }
