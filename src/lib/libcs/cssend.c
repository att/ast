...
int cssend(register Cs_t *state, int fd, int *fds, int n) {
...
#define msg_controllen msg_accrightslen
    ...
    if (eaccess(s, X_OK) && (mkdir(s, S_IRWXU | S_IRWXG | S_IRWXO) ||
    ...
    ctl.fds[0] = i;
    for (i = 0; i < n; i++) ctl.fds[i + 1] = fds[i];
    id.hid = 0;
    id.pid = getpid();
    id.uid = geteuid();
    id.gid = getegid();
...
#ifdef EMSGSIZE
    n = 0;
    while ((i = sendmsg(fd, &msg, 0) < 0 ? -1 : 0) < 0 && errno == EMSGSIZE && n++ < 5) sleep(1);
#else
...
int _cs_send(int fd, int *fds, int n) { return cssend(&cs, fd, fds, n); }
