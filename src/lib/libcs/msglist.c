...
static int buffer(register Sfio_t *sp, register char *s, int n) {
    register char *se;
    register int c;
    register int r = 0;
...
int msglist(Sfio_t *sp, register Msg_call_t *msg, int flags, unsigned long terse) {
    ...
    if (flags & (MSG_LIST_ID | MSG_LIST_STAMP)) {
        ...
        if (flags & MSG_LIST_ID)
            r += sfprintf(sp, " %5d %5d", MSG_CHANNEL_USR(msg->channel),
    ...
    for (;;) {
        switch ((at >>= MSG_ARG_TYPE) & ((1 << MSG_ARG_TYPE) - 1)) {
            case MSG_ARG_array:
                ...
                r += sfprintf(sp, " [");
                ne = np + n;
                while (np < ne) r += sfprintf(sp, " %lu", *np++);
                r += sfprintf(sp, " ]");
                continue;
            case MSG_ARG_file:
                ip = (ap++)->file;
                ...
                if ((msg->call & MSG_VALUE) && !(MSG_MASK(msg->call) & terse)) {
                    switch (MSG_CALL(msg->call)) {
                        ...
                        case MSG_CALL(MSG_stat):
                            ...
                            r += sfprintf(
                                sp, " [ dev=%d ino=%u mode=0%o nlink=%d uid=%d gid=%d size=%lu",
...
#endif
                            r += sfprintf(sp, " atime=%lu mtime=%lu ctime=%lu ]", st->st_atime,
                                          st->st_mtime, st->st_ctime);
                            break;
                        case MSG_CALL(MSG_statfs):
                            fp = (struct statvfs *)(ap++)->pointer;
                            r += sfprintf(sp,
                                          " [ bsize=%d frsize=%d blocks=%d bfree=%d bavail=%d "
                                          "files=%d ffree=%d favail=%d",
                                          fp->f_bsize, fp->f_frsize, fp->f_blocks, fp->f_bfree,
                                          fp->f_bavail, fp->f_files, fp->f_ffree, fp->f_favail);
            ...
            case MSG_ARG_vector:
                ...
                if (MSG_MASK(msg->call) & terse)
                    ...
                    r += sfprintf(sp, " [");
                    while (p = *vp++) r += buffer(sp, p, strlen(p));
                    r += sfprintf(sp, " ]");
                }
                continue;
        }
