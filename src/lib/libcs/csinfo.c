...
Sfio_t *csinfo(register Cs_t *state, const char *file, int *line) {
    int n;
    Sfio_t *sp = 0;
    char buf[PATH_MAX];
    char tmp[PATH_MAX];
    ...
    if (strmatch(file, "*[ \t\n=]*"))
        ...
        for (n = 0; n <= 1; n++) {
            sfsprintf(tmp, sizeof(tmp), "%s/%s", n ? csvar(state, CS_VAR_SHARE, 0) : CS_SVC_DIR,
                      file);
...
Sfio_t *_cs_info(const char *file, int *line) { return csinfo(&cs, file, line); }
