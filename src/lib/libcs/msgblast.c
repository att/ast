...
ssize_t msgblast(register Msg_call_t *msg) {
    register long n;
    ...
    e = msg->data + msg->size;
    if ((msg->version = msggetu(&b, e)) != MSG_VERSION) return -1;
    ...
    while (ap < msg->argv + elementsof(msg->argv)) {
        switch ((at >>= MSG_ARG_TYPE) & ((1 << MSG_ARG_TYPE) - 1)) {
            case MSG_ARG_array:
                ...
                while (r-- > 0) {
                    n = msggetu(&b, e);
                    if (np < (((long *)&msg->value[sizeof(msg->value)]) - 1)) *np++ = n;
                    if (e - b >= n) b += n;
                }
            ...
            case MSG_ARG_file:
                ...
                for (n = 0; n < sizeof(*ip) / sizeof(ip->fid[0]); n++) ip->fid[n] = msggetu(&b, e);
            ...
            case MSG_ARG_vector:
                (ap++)->vector = vp;
                while (n = msggetu(&b, e)) {
                    if (vp < (((char **)&msg->value[sizeof(msg->value)]) - 1)) *vp++ = b;
                    if (e - b >= n) b += n;
                }
                *vp = 0;
