...
int csdaemon(register Cs_t *state, int fds) {
    register int fd;
    register int i;
    int oerrno;
    ...
    if ((fds & ((1 << 0) | (1 << 1) | (1 << 2))) != ((1 << 0) | (1 << 1) | (1 << 2)) &&
    ...
    for (i = 3; i < 20; i++)
        if (!(fds & (1 << i)) && !fcntl(i, F_GETFD, 0)) close(i);
...
int _cs_daemon(int fds) { return csdaemon(&cs, fds); }
