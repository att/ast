...
static int initiate(register Cs_t *state, const char *name, char *buf) {
    char *av[3];

    if (!pathpath(CS_STAT_DAEMON, "", PATH_REGULAR | PATH_EXECUTE, buf, PATH_MAX)) {
        messagef(
            (state->id, NULL, -1, "stat: %s: %s: cannot find stat daemon", name, CS_STAT_DAEMON));
        return -1;
    }
    av[0] = buf;
    av[1] = (char *)name;
...
int csstat(register Cs_t *state, const char *name, register Csstat_t *sp) {
    register int n;
    register int i;
    char *s;
    unsigned long a;
...
int _cs_stat(const char *name, Csstat_t *sp) { return csstat(&cs, name, sp); }
