...
static int mkmount(register Cs_t *state, int mode, int uid, int gid, char *endserv, char *endhost,
    ...
    *(state->control - 1) = 0;
    if (eaccess(state->mount, R_OK | W_OK | X_OK)) {
        ...
        if (!endserv && !(endserv = strrchr(state->mount, '/'))) goto bad;
        *endserv = 0;
        if (eaccess(state->mount, X_OK)) {
            if (!endhost && !(endhost = strrchr(state->mount, '/'))) goto bad;
            *endhost = 0;
            if (eaccess(state->mount, X_OK)) {
                if (!endtype && !(endtype = strrchr(state->mount, '/'))) goto bad;
                *endtype = 0;
                if (eaccess(state->mount, X_OK) &&
        ...
        *endserv = '/';
        if (mkdir(state->mount, mode)) goto bad;
        if (mode != (S_IRWXU | S_IRWXG | S_IRWXO) && (uid >= 0 || gid >= 0 && (mode |= S_ISGID)) &&
    ...
    *(state->control - 1) = '/';
    return 0;
bad:
    if (endtype && !*endtype) *endtype = '/';
    if (endhost && !*endhost) *endhost = '/';
    if (endserv && !*endserv) *endserv = '/';
    *(state->control - 1) = '/';
    messagef(
        (state->id, NULL, -1, "mkmount: %s: cannot access physical mount directory", state->mount));
    return -1;
}
...
static void remote(register Cs_t *state, const char *host, const char *user, const char *path,
                   int agent, register char **av, register char *fv) {
    register char *t;
    ...
    *av = 0;
    *fv++ = '-';
    if (error_info.trace < 0) fv += sfsprintf(fv, 4, "%c%d", CS_REMOTE_DEBUG, -error_info.trace);
    *fv++ = CS_REMOTE_OPEN;
    t = fv;
    if (agent) *fv++ = CS_REMOTE_OPEN_AGENT;
    if (state->flags & CS_ADDR_LOCAL) *fv++ = CS_REMOTE_OPEN_LOCAL;
    if (state->flags & CS_ADDR_NOW) *fv++ = CS_REMOTE_OPEN_NOW;
    if (state->flags & CS_ADDR_SHARE) *fv++ = CS_REMOTE_OPEN_SHARE;
    if (state->flags & CS_ADDR_TEST) *fv++ = CS_REMOTE_OPEN_TEST;
    if (state->flags & CS_ADDR_TRUST) *fv++ = CS_REMOTE_OPEN_TRUST;
    if (fv == t) *fv++ = CS_REMOTE_OPEN_READ;
    *fv = 0;
}
...
static int initiate(register Cs_t *state, char *user, char *path, char *service, char *name) {
    register char *s;
    char *on;
    char *local;
    Sfio_t *sp;
    ...
    *s++ = '/';
    if (!(state->flags & CS_ADDR_SHARE)) {
        ...
        if (!(sp = tokline(buf, SF_STRING, NULL))) return -1;
    ...
    while (s = sfgetr(sp, '\n', 1))
        if (tokscan(s, NULL, " %s ", &on) == 1) {
            if (streq(on, CS_HOST_LOCAL) || streq(on, local)) {
                ...
                av[1] = path;
                av[2] = 0;
                return procclose(procopen(av[0], av, NULL, NULL, PROC_PRIVELEGED | PROC_ZOMBIE)) < 0
                           ? -1
                           : 0;
...
static int reopen(register Cs_t *state, char *path) {
    int ret;
    Cs_t tmp;
...
static int agent(register Cs_t *state, const char *host, const char *user, const char *path) {
    register int n;
    register int m;
    register char *s;
    Proc_t *proc;
    ...
    if (!(proc = procopen(av[0], av, NULL, NULL, PROC_READ | PROC_WRITE | PROC_PRIVELEGED)))
        goto sorry;
    if ((m = csread(state, proc->rfd, buf, sizeof(buf), CS_LINE)) <= 1) goto sorry;
    buf[--m] = 0;
    if (buf[--m] == 'A')
        buf[--m] = 0;
    else
        m = 0;
    if ((fd = reopen(state, buf)) < 0) goto sorry;
    if (m) {
        if ((m = csread(state, proc->rfd, buf, sizeof(buf), CS_LINE)) <= 1) goto sorry;
        if (cswrite(state, fd, buf, m) != m) goto sorry;
        if ((n = csread(state, fd, num, sizeof(num), CS_LINE)) <= 0) goto sorry;
        if (*num != '\n') {
            buf[m - 1] = 0;
            num[n - 1] = 0;
            n = sfsprintf(tmp, sizeof(tmp), "%s %s\n", num, (s = strchr(buf, ' ')) ? (s + 1) : buf);
            if (cswrite(state, proc->wfd, tmp, n) != n) goto sorry;
            if ((m = csread(state, proc->rfd, buf, sizeof(buf), CS_LINE)) <= 0) goto sorry;
            if (cswrite(state, fd, buf, m) != m) goto sorry;
            if (csread(state, fd, num, 1, CS_LINE) != 1) goto sorry;
            if (cswrite(state, proc->wfd, num, 1) != 1) goto sorry;
        }
    }
    procclose(proc);
    return fd;
...
static int doattach(register Cs_t *state, const char *path, int op, int mode, char *user,
                    char *opath, char *tmp, char *serv, char *b) {
    register int n;
    int fd;
    char *s;
    ...
    if (op & CS_OPEN_CREATE) {
        ...
        if (chmod(path, mode)) {
            ...
            if ((fd = open(path, O_WRONLY | O_CREAT | O_TRUNC, mode)) < 0) {
                messagef(
                    (state->id, NULL, -1, "open: %s: %s: creat %o error", state->path, path, mode));
                return -1;
            }
            close(fd);
            chmod(path, mode);
        ...
        {
            messagef((state->id, NULL, -1, "open: %s: %s: %s error", state->path, path,
                      n ? "connld" : "fattach"));
            close(fds[0]);
            close(fds[1]);
            errno = ENXIO;
        ...
        for (;;) {
            if ((fd = open(path, O_RDWR)) >= 0) {
                if (!fstat(fd, &st) && !S_ISREG(st.st_mode)) break;
                close(fd);
                remove(path);
            ...
            if (initiate(state, user, opath, tmp, serv)) {
                messagef((state->id, NULL, -1, "open: %s: %s: service initiate error", state->path,
                          path));
                return -1;
            }
            op = CS_OPEN_TEST;
    ...
    messagef((state->id, NULL, -8,
              "%s:%d state.path=`%s' state.mount=`%s' path=`%s' opath=`%s' user=`%s' serv=`%s'",
              __FILE__, __LINE__, state->path, state->mount, path, opath, user, serv));
    ...
    for (n = 0;; n++) {
        ...
        if ((fd = socket(AF_UNIX, SOCK_STREAM, 0)) < 0) {
            messagef(
                (state->id, NULL, -1, "open: %s: %s: AF_UNIX socket error", state->path, path));
            return -1;
        }
        if (!connect(fd, (struct sockaddr *)&nam, namlen)) {
            ...
            if (read(fd, &c, 1) == 1 && !cssend(state, fd, NULL, 0)) break;
#else
            break;
#endif
        } else {
        ...
        if (op & CS_OPEN_CREATE) {
            ...
            if (!bind(fd, (struct sockaddr *)&nam, namlen)) {
                ...
                if (listen(fd, 32)) {
                    messagef(
                        (state->id, NULL, -1, "open: %s: %s: listen error", state->path, path));
                    n = errno;
                    remove(path);
                    errno = n;
                    goto badcon;
            ...
            if (errno != EADDRINUSE || n && remove(path) && errno != ENOENT) goto badcon;
            close(fd);
        } else if (op & CS_OPEN_TEST)
            return -1;
        else if (!n && initiate(state, user, opath, tmp, serv)) {
            messagef(
                (state->id, NULL, -1, "open: %s: %s: service initiate error", state->path, path));
            return -1;
        } else
            sleep(2);
    }
...
int csattach(register Cs_t *state, const char *path, int op, int mode) {
    return doattach(state, path, op, 0, 0, 0, 0, 0, 0);
}

int _cs_attach(const char *path, int op, int mode) { return csattach(&cs, path, op, mode); }

/*
 * access/open/create service on path
 */

int csopen(register Cs_t *state, const char *apath, int op) {
    ...
    strcpy(state->path, path);
    b = path;
    if ((*b++ != '/') || !(s = strchr(b, '/'))) return -1;
    *s++ = 0;
    if (!streq(b, "dev")) return -1;
    if (b = strchr(s, '/')) *b++ = 0;
    if (streq(s, "fdp")) {
#if !(CS_LIB_SOCKET_UN || CS_LIB_STREAM || CS_LIB_V10)
                ...
                do {
                    ...
                    switch (n) {
                        ...
                        case HASHKEY4('u', 's', 'e', 'r'):
                            ...
                            break;
                        default:
                            qual += sfsprintf(qual, sizeof(state->qual) - (qual - state->qual) - 1,
                                              "%s%s", qual == state->qual ? "" : "-", b);
                            if (s) *(s - 1) = '=';
                            break;
                    }
                } while (b = u);
    ...
    messagef(
        (state->id, NULL, -8, "open: type=%s host=%s serv=%s qual=%s", type, host, serv, qual));
    if (host) {
        /*
         * validate host
         */
        ...
        if (!(state->addr = addr = csaddr(state, host))) {
            if (serv && !(op & CS_OPEN_CREATE) && *type == 't' &&
                (port = csport(state, type, serv)) >= CS_PORT_MIN && port <= CS_PORT_MAX) {
                /*
                ...
                if ((fd = state->proxy.addr
                              ? csbind(state, type, state->proxy.addr, state->proxy.port, 0L)
                              : reopen(state, csvar(state, CS_VAR_PROXY, 0))) >= 0) {
                    ...
                    if (cswrite(state, fd, tmp, n) == n &&
                        (n = csread(state, fd, tmp, sizeof(tmp), CS_LINE)) >= 2) {
                        if (tmp[0] == '0' && tmp[1] == '\n') return fd;
                        if (error_info.trace <= -4 && n > 2) {
                            ...
                            while (*s && *s++ != '\n')
                                ;
                            messagef((state->id, NULL, -4, "%s error message `%s'",
                                      csvar(state, CS_VAR_PROXY, 0), s));
                        }
                    }
                    close(fd);
                }
        ...
        if (op & CS_OPEN_NOW) state->flags |= CS_ADDR_NOW;
        if ((op & (CS_OPEN_AGENT | CS_OPEN_REMOTE)) == CS_OPEN_REMOTE) {
            state->flags |= CS_ADDR_REMOTE;
            state->flags &= ~CS_ADDR_LOCAL;
        }
        if (op & CS_OPEN_SHARE) state->flags |= CS_ADDR_SHARE;
        if (op & CS_OPEN_SLAVE) state->flags |= CS_DAEMON_SLAVE;
        if (op & CS_OPEN_TEST) state->flags |= CS_ADDR_TEST;
        if (op & CS_OPEN_TRUST) state->flags |= CS_ADDR_TRUST;
        if ((state->flags & CS_ADDR_REMOTE) &&
            (!serv ||
             !strneq(serv, CS_SVC_INET, sizeof(CS_SVC_INET) - 1) && (strtol(serv, &t, 0), *t)))
        ...
        if (s = group) {
            ...
            if (*s) {
                ...
                if (geteuid() && gid != n) {
                    ...
                    if ((g = getgroups(0, NULL)) <= 0) g = getconf("NGROUPS_MAX");
                    if (groups = newof(0, gid_t, g, 0)) {
                        for (n = getgroups(g, groups); n >= 0; n--)
                            if (gid == groups[n]) break;
                        free(groups);
                    } else
                        n = -1;
                    if (n < 0) {
    ...
    if (*type == 'f') {
        ...
        if (op & CS_OPEN_TRUST) {
            if (!pathaccess(csvar(state, CS_VAR_TRUST, 1), csvar(state, CS_VAR_SHARE, 1), NULL,
                            PATH_EXECUTE, b, sizeof(state->mount) - (b - state->mount)))
                goto bad;
        } else if (!pathpath(csvar(state, CS_VAR_SHARE, 0), "", PATH_EXECUTE, b,
    ...
    if (!host) {
        *(state->control = b + 1) = 0;
        ...
        if (fd < 0) messagef((state->id, NULL, -1, "open: %s: %s: open error", state->path, path));
        return fd;
    }
    endtype = b;
    ...
    if (strlen(host) <= CS_MNT_MAX)
        ...
        if (strlen(s) <= CS_MNT_MAX)
            ...
            b += sfsprintf(b, sizeof(state->mount) - (b - path), "/0x%X.%X.%X.%X", a[0], a[1], a[2],
                           a[3]);
        }
    }
    ...
    sfsprintf(b, sizeof(state->mount) - (b - path), "%s/%s/%s/%s%s", CS_SVC_DIR, type, serv, serv,
              CS_SVC_SUFFIX);
    if (!pathpath(b, "", PATH_ABSOLUTE | PATH_EXECUTE, tmp, sizeof(tmp)) || stat(tmp, &st))
        op |= CS_OPEN_TEST;
    else {
        *strrchr(tmp, '/') = 0;
        if (!(op & CS_OPEN_TRUST)) sid = st.st_uid;
        if (!st.st_size) op |= CS_OPEN_TEST;
    }
    ...
    *b = CS_MNT_STREAM;
    ...
    if (op & CS_OPEN_MOUNT) {
        messagef(
            (state->id, NULL, -1, "open(%s,%o) = %d, mount = %s", state->path, op, state->mount));
        return 0;
    }
    if (*type == 'f') {
        /*
        ...
        if ((fd = doattach(state, path, op, mode, user, opath, tmp, serv, b)) < 0) return -1;
    } else {
        /*
         * {tcp,udp}
         */
            ...
            *state->control = CS_MNT_LOCK;
            if ((fd = open(path, O_WRONLY | O_CREAT | O_TRUNC, 0)) < 0) {
                ...
                if ((CSTIME() - (unsigned long)st.st_ctime) < 2 * 60) {
                    errno = EEXIST;
                    messagef((state->id, NULL, -1, "open: %s: %s: another server won the race",
                              state->path, path));
                    goto unblock;
                }
                if (remove(path)) {
                    messagef(
                        (state->id, NULL, -1, "open: %s: %s: remove error", state->path, path));
                    goto unblock;
                }
                if ((fd = open(path, O_WRONLY | O_CREAT | O_TRUNC, 0)) < 0) {
                    messagef((state->id, NULL, -1, "open: %s: %s: creat error", state->path, path));
            ...
            if ((fd = csbind(state, type, 0L, port, 0L)) >= 0) {
                *state->control = CS_MNT_STREAM;
                ...
                if (pathsetlink(cspath(state, fd, 0), path)) {
                    messagef((state->id, NULL, -1, "open: %s: %s: link error", cspath(state, fd, 0),
                              path));
                    close(fd);
                    fd = -1;
                }
        ...
        unblock:
            ...
            *state->control = CS_MNT_STREAM;
            if (fd < 0) return -1;
        } else if (fd < 0 && ((op & CS_OPEN_TEST) || initiate(state, user, opath, tmp, serv) ||
                              (fd = reopen(state, path)) < 0)) {
            messagef(
                (state->id, NULL, -1, "open: %s: %s: reopen/initiate error", state->path, path));
            return -1;
            ...
            *state->control = CS_MNT_STREAM;
            if (n) {
                close(fd);
                messagef(
                    (state->id, NULL, -1, "open: %s: %s: authentication error", state->path, path));
                return -1;
            }
        }
    }
