...
static int portbind(register Cs_t *state, const char *type, unsigned long addr, unsigned int port) {
#if CS_LIB_V10
    ...
    if (streq(type, "udp")) {
        ...
        messagef(
            (state->id, NULL, -1, "bind: %s %s error", type, addr ? "tcp_connect" : "tcp_listen"));
        close(fd);
    }
...
#undef connect
        ...
        if (errno == EADDRNOTAVAIL || errno == ECONNREFUSED) errno = ENOENT;
    } else {
#ifdef SO_REUSEADDR
        ...
        if (!bind(fd, (struct sockaddr *)&nam, sizeof(nam)) &&
            ...
            if (!getsockname(fd, (struct sockaddr *)&nam, &namlen) && namlen == sizeof(nam)) {
                ...
                messagef((state->id, NULL, -1, "bind: %s: getsockname error", type));
            return fd;
        }
...
int csbind(register Cs_t *state, const char *type, unsigned long addr, unsigned long port,
           unsigned long clone) {
    int fd;

    messagef(
        (state->id, NULL, -8, "bind(%s,%s,%u,%lu) call", type, csntoa(state, addr), port, clone));
    if (port == CS_PORT_INVALID) return -1;
    if (port == CS_PORT_RESERVED) {
        static unsigned int last = IPPORT_RESERVED;

        if (addr) {
            errno = EROFS;
            messagef((state->id, NULL, -1, "bind: %s: explicit reserved port invalid",
                      csntoa(state, addr)));
            return -1;
        }
        port = last;
        do {
            if (--last <= IPPORT_RESERVED / 2) port = IPPORT_RESERVED - 1;
            if (last == port) {
                errno = ENOSPC;
                messagef((state->id, NULL, -1, "bind: no more reserved ports"));
                break;
            }
            if ((fd = portbind(state, type, 0L, last)) >= 0) goto ok;
        } while (errno != EACCES);
    ...
    if ((fd = portbind(state, type, addr, port)) < 0) return -1;
    if (clone) {
        int n;
        char buf[16];
...
ok:
    messagef((state->id, NULL, -8, "bind(%s,%s,%u,%lu) = %d", type, csntoa(state, addr), port,
              clone, fd));
    return fd;
}

int _cs_bind(const char *type, unsigned long addr, unsigned long port, unsigned long clone) {
