...
int csauth(register Cs_t *state, int fd, const char *path, const char *arg) {
    register char *s;
    register char *t;
    char *b;
    char *key = 0;
    ...
    *s++ = '\n';
    ...
    if (*num != '\n') {
        n = 0;
        if (state->addr == csaddr(state, NULL))
            b = tmp +
                sfsprintf(tmp, sizeof(tmp), "%s/AUTH.%05d.", csvar(state, CS_VAR_LOCAL, 0), m);
        else {
        ...
        if (s = strrchr(tmp, '/')) {
            *s = 0;
            if (eaccess(tmp, X_OK) && (mkdir(tmp, S_IRWXU | S_IRWXG | S_IRWXO) ||
        ...
        for (;;) {
            ...
            if ((stat(tmp, &st) || t1 <= (unsigned long)st.st_ctime && !remove(tmp)) &&
                !close(open(tmp, O_CREAT | O_TRUNC | O_cloexec,
        ...
        if (cschallenge(state, tmp, &t1, &t2)) goto sorry;
...
int cschallenge(Cs_t *state, const char *path, unsigned long *v1, unsigned long *v2) {
    struct stat st;
    ...
    if (v1) *v1 = st.st_atime;
    if (v2) *v2 = st.st_mtime;
    return 0;
}

int _cs_auth(int fd, const char *path, const char *arg) { return csauth(&cs, fd, path, arg); }

int _cs_challenge(const char *path, unsigned long *v1, unsigned long *v2) {
