...
int cspoll(Cs_t *state, Cspoll_t *fds, int num, int ms) {
    ...
    if (!(events & CS_POLL_WRITE)) wp = 0;
#if CS_LIB_SOCKET
    if (!(events & CS_POLL_READ)) rp = 0;
    if (!(events & CS_POLL_CONTROL)) ep = 0;
    messagef((state->id, NULL, -6, "poll: %s num=%d ms=%d sec=%d usec=%d", fmttime("%K", CSTIME()),
              num, ms, tp ? tp->tv_sec : 0, tp ? tp->tv_usec : 0));
    num = select(width + 1, rp, wp, ep, tp);
#else
    if (!(events & (CS_POLL_READ | CS_POLL_CONTROL))) rp = 0;
    ...
    if (num < 0)
        ...
        for (num = 0, pp = fds; pp < mp; pp++) {
            ...
            if (pp->fd >= 0) {
                ...
                if (wp && FD_ISSET(pp->fd, wp)) pp->status |= CS_POLL_WRITE;
#if CS_LIB_SOCKET
                if (ep && FD_ISSET(pp->fd, ep)) pp->status |= CS_POLL_CONTROL;
#endif
                if (pp->status) {
        ...
        for (mp = (pp = fds) + num, i = n; pp < mp; pp++) {
            if (pp->status) {
                ...
                if (pp->status & CS_POLL_CONTROL) {
                    ...
                    if (getmsg(pp->fd, NULL, &buf, &f)) pp->status &= ~CS_POLL_CONTROL;
                }
#endif
                if (--i <= 0) break;
            }
...
int _cs_poll(Cspoll_t *fds, int num, int ms) { return cspoll(&cs, fds, num, ms); }
