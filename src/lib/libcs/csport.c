...
unsigned long csport(register Cs_t *state, const char *type, const char *serv) {
    unsigned long n;
    char *t;

    if (streq(serv, "reserved")) return CS_PORT_RESERVED;
    if (streq(serv, "normal")) return CS_PORT_NORMAL;
    n = strtol(serv, &t, 0);
    if (t > serv && *t == '.') strtol(t + 1, &t, 0);
    if (*t) {
...
#if CS_LIB_V10
                if (sp = in_service(serv, type, 0)) return sp->port;
#endif
#if CS_LIB_SOCKET
                if (sp = getservbyname(serv, type)) return ntohs(sp->s_port);
#endif
            }
#endif
            switch (strkey(serv)) {
                ...
                case HASHKEY5('p', 'r', 'o', 'x', 'y'):
                    return 402;
                case HASHKEY3('a', 't', 'x'):
                    if (access("/etc/in.atxd", F_OK)) break;
                    return 512;
...
unsigned long _cs_port(const char *type, const char *serv) { return csport(&cs, type, serv); }
