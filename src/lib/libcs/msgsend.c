...
ssize_t msgsend(int fd, register Msg_call_t *msg, unsigned long call, long ret, int err,
                void *data) {
    ...
    if (call & MSG_ACK) {
        if ((fd = csbind(&cs, "udp", msg->ack.addr, msg->ack.port, 0L)) < 0) return -1;
    ...
    if (cswrite(&cs, fd, msg->data, ret) != ret) ret = -1;
    if (call & MSG_ACK) close(fd);
    return ret;
}
