...
static int initiate(Cs_t *state, const char *svc, char *cmd) {
    pid_t pid;
    pid_t n;
    char *av[3];
...
int cslocal(register Cs_t *state, const char *path) {
    register char *s;
    register char *p;
    register char *t;
    register char *v;
    ...
    for (;;) {
        ...
        if (!p) {
            messagef((state->id, NULL, -1, "local: %s: %s: cannot locate service on ../lib/cs/fdp",
                      path, cmd));
            goto sorry;
        }
    }
    *t = 0;
    sfsprintf(tmp, sizeof(tmp), "%s/fdp/%s/%s/%d-%d-/%c%s", csvar(state, CS_VAR_LOCAL, 0),
              csname(state, 0), cmd, st.st_uid, geteuid(), CS_MNT_STREAM, CS_MNT_TAIL);
    ...
    for (n = 0; (fd = open(tmp, O_RDWR)) < 0; n++)
        if (n || errno == EACCES) {
            ...
            messagef(
                (state->id, NULL, -1, "local: %s: %s: cannot initiate service %s", path, tmp, exe));
            return -1;
        }
        ...
        for (;;) {
            ...
            if (!connect(fd, (struct sockaddr *)&nam, namlen)) {
                ...
                if (read(fd, cmd, 1) != 1)
                    ...
                    messagef((state->id, NULL, -1, "local: %s: connect authentication send error",
                              path));
                else
#endif
                    return fd;
                ...
                messagef((state->id, NULL, -1, "local: %s: connect error", path));
...
int _cs_local(const char *path) { return cslocal(&cs, path); }
