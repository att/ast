...
/* exchanges columns in each of 4 rows
	...
	row3 - shifted left 3
*/
static void ShiftRows (Vcchar_t* state)
{
	Vcchar_t tmp;
...
*/
static void InvShiftRows (Vcchar_t * state)
{
	Vcchar_t tmp;
...
static void MixSubColumns (Vcchar_t * state)
{
	int		i;
	Vcchar_t	tmp[AES_STATE]; /**/DEBUG_ASSERT(AES_STATE == 16);
...
static void InvMixSubColumns (Vcchar_t* state)
{
	int		i;
	Vcchar_t	tmp[AES_STATE]; /**/DEBUG_ASSERT(AES_STATE == 16);
...
static void AddRoundKey (Vcchar_t* state, const Vcchar_t* key)
{
	int	i;
...
static void ExpandKey(Aes_t* aes, Vcchar_t* ckey, Vcchar_t* ekey)
{
	int		i;
...
static int AesEncrypt (Aes_t* aes, Vcchar_t* ekey, Vcchar_t* in, Vcchar_t* out)
{
	int	r;
	...
	memcpy(out, in, AES_STATE); /**/DEBUG_ASSERT(AES_STATE == 16);
...
static int AesDecrypt (Aes_t* aes, const Vcchar_t* ekey, const Vcchar_t* in, Vcchar_t* out)
{
	int		r;
...
static ssize_t aes_init(Vcx_t* xx, Vcxmethod_t* meth, Vcchar_t* key, ssize_t keyz)
{
	Aes_t		*aes;
	...
	if(meth != Vcxaes128 && meth != Vcxaes192 &&
	   meth != Vcxaes256 && meth != Vcxaessum )
		return -1;
...
static ssize_t aes_encode(Vcx_t* xx, const Void_t* buf, ssize_t size, Vcchar_t** out)
{
	...
	{	if(aes->tdsz > 0) /* deal with buffered data as appropriate */
		...
		{	obsz = ((aes->pdsz + size + AES_BLOCK-1)/AES_BLOCK) * AES_BLOCK;
			if(obsz > aes->obsz)
	...
	{	/* encode the size of unprocessed data (even if zero) */
		...
		*out = aes->obuf;
		obsz = aes->pdsz; /**/DEBUG_ASSERT(obsz <= aes->obsz);
	}
...
static ssize_t aes_decode(Vcx_t* xx, const Void_t* buf, ssize_t size, Vcchar_t** out)
{
	const Vcchar_t	*data = buf;
	ssize_t		k;
	const Vcchar_t	*salt;
	...
	k = (ssize_t)aes->obuf[aes->obsz-1]; /**/DEBUG_ASSERT(k < AES_BLOCK);

	*out = aes->obuf;
	return aes->obsz - (AES_BLOCK - k);
}

static Vcxmethod_t _Vcxaes128 = { aes_init, aes_stop, aes_encode, aes_decode };
static Vcxmethod_t _Vcxaes192 = { aes_init, aes_stop, aes_encode, aes_decode };
static Vcxmethod_t _Vcxaes256 = { aes_init, aes_stop, aes_encode, aes_decode };
