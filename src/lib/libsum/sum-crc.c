...
static Sum_t *crc_open(const Method_t *method, const char *name) {
    register Crc_t *sum;
    ...
    while (*(t = s)) {
        ...
        if (isdigit(*t) || v && i >= 4 && strneq(t, "poly", 4) && (t = v + 1))
            ...
            sum->addsize = 1;
            if (v) sum->xorsize = strtoul(v + 1, NULL, 0);
        }
        if (*s == '-') s++;
    }
    if (sum->rotate) {
        ...
        p[0] = polynomial;
        for (i = 1; i < 8; i++) p[i] = (p[i - 1] << 1) ^ ((p[i - 1] & 0x80000000) ? polynomial : 0);
        ...
        for (i = 0; i < elementsof(sum->tab); i++) {
            x = i;
            for (j = 0; j < 8; j++) x = (x >> 1) ^ ((x & 1) ? polynomial : 0);
            sum->tab[i] = x;
        }
    }
    return (Sum_t *)sum;
...
static int crc_block(Sum_t *p, const void *s, size_t n) {
    Crc_t *sum = (Crc_t *)p;
    ...
    if (sum->rotate)
        while (b < e) CRCROTATE(sum, c, *b++);
    else
        while (b < e) CRC(sum, c, *b++);
    sum->sum = c;
    return 0;
}

static int crc_done(Sum_t *p) {
