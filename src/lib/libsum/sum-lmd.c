...
static int lmd_block(Sum_t *p, const void *s, size_t n) {
    Lmd_t *lmd = (Lmd_t *)p;
...
static int lmd_done(Sum_t *p) {
    register Lmd_t *lmd = (Lmd_t *)p;
    register int i;

    (*lmd->finalf)(lmd->data, &lmd->context);
    for (i = 0; i < lmd->datasize; i++) lmd->total[i] ^= lmd->data[i];
    return 0;
}

static int lmd_print(Sum_t *p, Sfio_t *sp, register int flags, size_t scale) {
    register Lmd_t *lmd = (Lmd_t *)p;
    register unsigned char *d;
    register int i;

    d = (flags & SUM_TOTAL) ? lmd->total : lmd->data;
    for (i = 0; i < lmd->datasize; i++) sfprintf(sp, "%02x", d[i]);
    return 0;
}

static int lmd_data(Sum_t *p, Sumdata_t *data) {
    Lmd_t *lmd = (Lmd_t *)p;
...
static Sum_t *md4_open(const Method_t *method, const char *name) {
    Md4_t *lmd;
...
static Sum_t *md5_open(const Method_t *method, const char *name) {
    Md5_t *lmd;
