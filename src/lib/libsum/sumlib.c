...
static Sum_t *long_open(const Method_t *method, const char *name) {
    Integral_t *p;
...
static int long_init(Sum_t *p) {
    ((Integral_t *)p)->sum = 0;
    return 0;
}

static int long_done(Sum_t *p) {
...
static int long_print(Sum_t *p, Sfio_t *sp, register int flags, size_t scale) {
    register Integral_t *x = (Integral_t *)p;
    register uint32_t c;
    ...
    if (flags & SUM_SIZE) {
        ...
        if ((flags & SUM_SCALE) && ((n = scale) || (n = x->method->scale))) z = SCALE(z, n);
        sfprintf(sp, " %*I*u", (flags & SUM_LEGACY) ? 6 : 0, sizeof(z), z);
    }
    if (flags & SUM_TOTAL)
        sfprintf(sp, " %*I*u", (flags & SUM_LEGACY) ? 6 : 0, sizeof(x->total_count),
                 x->total_count);
    return 0;
}

static int long_data(Sum_t *p, Sumdata_t *data) {
    register Integral_t *x = (Integral_t *)p;
...
#define METHOD(x)                                                                                \
    x##_match, x##_description, x##_options, x##_open, x##_init, x##_block, x##_data, x##_print, \
        x##_done, x##_scale, x##_flags
...
static const Map_t maps[] = {
    ...
    {"ast|strsum", "The \bast\b \bstrsum\b(3) PRNG hash.", "prng-0x63c63cd9-add=0x9c39c33d"},
};
...
static int match(register const char *s, register const char *p) {
...
Sum_t *sumopen(register const char *name) {
    Sum_t *sum;
    int n;

    if (!name || !name[0] || name[0] == '-' && !name[1]) name = "default";
    ...
    for (n = 0; n < elementsof(methods); n++)
        if (match(name, methods[n].match)) {
            if (sum = (*methods[n].open)(&methods[n], name)) sum->flags = methods[n].flags;
            return sum;
        }
    return 0;
}
...
int sumblock(Sum_t *p, const void *buf, size_t siz) {
    p->size += siz;
...
int sumprint(Sum_t *p, Sfio_t *sp, int flags, size_t scale) {
...
int sumdata(Sum_t *p, Sumdata_t *d) { return (*p->method->data)(p, d); }

/*
 * close an open sum handle
 */

int sumclose(Sum_t *p) {
    free(p);
    return 0;
}
...
int sumusage(Sfio_t *sp) {
    register int i;
    register int n;

    for (i = n = 0; i < elementsof(methods); i++) {
        n += sfprintf(sp, "[+%s?%s]", methods[i].match, methods[i].description);
        if (methods[i].options) n += sfprintf(sp, "{\n%s\n}", methods[i].options);
    }
    for (i = 0; i < elementsof(maps); i++)
        n += sfprintf(sp, "[+%s?%s Shorthand for \b%s\b.]", maps[i].match, maps[i].description,
                      maps[i].map);
    return n;
}
