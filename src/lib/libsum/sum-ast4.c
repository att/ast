...
static Sum_t *ast4_open(const Method_t *method, const char *name) {
    Ast4_t *p;
...
static int ast4_block(Sum_t *p, const void *s, size_t n) {
...
static int ast4_print(Sum_t *p, Sfio_t *sp, int flags, size_t scale) {
...
static int ast4_data(Sum_t *p, Sumdata_t *data) {
    data->size = sizeof(((Ast4_t *)p)->cur);
    data->num = 0;
#if _ast_intswap
    swapmem(_ast_intswap, data->buf = ((Ast4_t *)p)->buf, &((Ast4_t *)p)->cur,
            sizeof(((Ast4_t *)p)->cur));
#else
