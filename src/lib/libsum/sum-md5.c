...
#define md5_options \
    "[+(version)?md5 (RSA Data Security, Inc. MD5 Message-Digest, 1991-2) 1996-02-29]"
...
static void md5_encode(register unsigned char *output, register UINT4 *input, unsigned int len) {
...
static void md5_decode(register UINT4 *output, register unsigned char *input, unsigned int len) {
    unsigned int i;
    unsigned int j;
...
static int md5_block(Sum_t *p, const void *s, size_t inputLen) {
    register Md5_t *context = (Md5_t *)p;
...
static int md5_print(Sum_t *p, Sfio_t *sp, register int flags, size_t scale) {
    register Md5_t *x = (Md5_t *)p;
    register unsigned char *d;
    register int n;

    d = (flags & SUM_TOTAL) ? x->digest_sum : x->digest;
    for (n = 0; n < elementsof(x->digest); n++) sfprintf(sp, "%02x", d[n]);
    return 0;
}

static int md5_data(Sum_t *p, Sumdata_t *data) {
    register Md5_t *x = (Md5_t *)p;
