...
static Sum_t *prng_open(const Method_t *method, const char *name) {
    register Prng_t *sum;
    ...
    if (!sum->mpy) {
        sum->mpy = FNV_MULT;
        if (!sum->init) sum->init = FNV_INIT;
    }
    return (Sum_t *)sum;
}

static int prng_init(Sum_t *p) {
    Prng_t *sum = (Prng_t *)p;
