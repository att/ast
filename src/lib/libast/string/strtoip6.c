...
int strtoip6(const char *s, char **e, unsigned char *addr, unsigned char *bits) {
    ...
    if (!lex[0]) {
        for (c = 0; c < sizeof(lex); ++c) lex[c] = END;
        lex['0'] = 0;
        lex['1'] = 1;
        lex['2'] = 2;
        lex['3'] = 3;
                ...
                case DOT:
                    ...
                    *b++ = ((a >> 8) & 0xf) * 100 + ((a >> 4) & 0xf) * 10 + (a & 0xf);
                    ...
                    if (c == COL) {
                        ...
                        if ((b - addr) == 6 && addr[0] == 0x20 && addr[1] == 0x02) continue;
                    }
                    break;
                default:
                    a = (a << 4) | c;
    ...
    if (b == addr)
        ...
        if (bits) {
            if (c == PFX) {
                a = 0;
                while ((c = lex[*((unsigned char *)s++)]) < 10) a = a * 10 + c;
            } else
                a = 0xff;
            *bits = a;
        }
