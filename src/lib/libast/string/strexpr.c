...
static long seterror(Expr_t *ex, char *msg) {
...
static long expr(Expr_t *ex, int precedence) {
    int c;
    long n;
    long x;
    char *pos;
...
long strexpr(const char *s, char **end, long (*convert)(const char *, char **, void *),
             void *handle) {
    long n;
    Expr_t ex;
    ...
    ex.handle = handle;
    n = expr(&ex, 0);
    if (peekchr(&ex) == ':') seterror(&ex, "invalid use of :");
    if (ex.errmsg) {
