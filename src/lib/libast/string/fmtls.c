...
char *fmtls(char *buf, const char *name, struct stat *st, const char *info, const char *link,
            int flags) {
    char *s;
    time_t tm;
    Sfoff_t n;
    ...
    if (flags & LS_LONG) {
        s += sfsprintf(s, LS_W_MAX, "%s%3u", fmtmode(st->st_mode, flags & LS_EXTERNAL),
                       (unsigned int)st->st_nlink);
        if (!(flags & LS_NOUSER)) {
            if (flags & LS_NUMBER)
                s += sfsprintf(s, LS_W_MAX, " %-*I*d", LS_W_NAME - 1, sizeof(st->st_uid),
                               st->st_uid);
            else
        ...
        if (!(flags & LS_NOGROUP)) {
            if (flags & LS_NUMBER)
                s += sfsprintf(s, LS_W_MAX, " %-*I*d", LS_W_NAME - 1, sizeof(st->st_gid),
                               st->st_gid);
            else
