...
static size_t debug_mbrtowc(wchar_t *p, const char *s, size_t n, mbstate_t *ms) {
    const char *q;
    const char *r;
    int w;
    int dr;
    ...
    switch (((unsigned char *)s)[0]) {
        ...
        default:
        single:
            if (p) *p = ((unsigned char *)s)[0] & ((1 << DC) - 1);
            return 1;
    }
    if (n < 2) return -1;
    if ((w = ((unsigned char *)s)[1]) < '0' || w > ('0' + DX)) goto single;
    if ((w -= '0' - DD) > n) return -1;
    r = s + w - 1;
    q = s += 2;
    while (q < r && *q) q++;
    if (q != r || *((unsigned char *)q) != dr) return -1;
    if (p) {
        c = 1;
        while (--q >= s) {
            c <<= DC;
...
static int debug_mbtowc(wchar_t *p, const char *s, size_t n) {
    static mbstate_t q;
...
static int debug_wctomb(char *s, wchar_t c) {
    static mbstate_t ms;
...
static int debug_wcwidth(wchar_t c) {
    if (c >= 0 && c <= UCHAR_MAX) return 1;
    if ((c &= ((1 << DZ) - 1)) > DX) return -1;
    return c + DD;
}

static int debug_alpha(wchar_t c) { return isalpha((c >> DZ) & ((1 << DC) - 1)); }
...
static int default_wcwidth(wchar_t w) { return w >= 0 && w <= 255 && !iscntrl(w) ? 1 : -1; }
...
static int utf8_wcwidth(wchar_t c) {
    int n;
...
static int utf8_alpha(wchar_t c) { return !!(utf8_wam[(c >> 3) & 0x1fff] & (1 << (c & 0x7))); }
...
static int set_ctype(Lc_category_t *cp) {
    ...
    } else if ((locales[cp->internal]->flags & LC_default) || (ast.mb_cur_max = MB_CUR_MAX) <= 1 ||
    ...
    if ((ast.locale.set & (AST_LC_debug | AST_LC_setlocale)) && !(ast.locale.set & AST_LC_internal))
        sfprintf(
            sfstderr,
            "locale setf %17s %16s %16s %16s codeset=%s mb_cur_max=%d byte_max=0x%02x set=0x%08x\n",
            cp->name, locales[cp->internal]->name, "", "", codeset(CODESET_ctype), ast.mb_cur_max,
            ast.byte_max, ast.locale.set);
#endif
    return 0;
}
...
static char *single(int category, Lc_t *lc, unsigned int flags) {
    ...
    if ((ast.locale.set & (AST_LC_debug | AST_LC_setlocale)) &&
        ...
        header();
        sfprintf(sfstderr, "locale set  %17s %16s %16s %16s", lc_categories[category].name,
                 lc->name, sys,
                 lc_categories[category].prev ? lc_categories[category].prev->name : NULL);
        if (category == AST_LC_CTYPE)
            sfprintf(sfstderr, " MB_CUR_MAX=%d%s%s%s%s%s", ast.mb_cur_max,
                     ast.mb_len == debug_mblen
                         ? " debug_mbrlen"
                     ...
                     ast.mb_conv == debug_wctomb
                         ? " debug_wcrtomb"
                         : ast.mb_conv == utf8_wctomb ? " utf8_wctomb"
            ...
            sfprintf(sfstderr, " decimal='%c' thousands='%c'", dp->decimal,
