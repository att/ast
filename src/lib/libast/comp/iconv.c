...
int _ast_iconv_name(const char *m, char *b, size_t n) {
    ...
    if (error_info.trace < DEBUG_TRACE)
        sfprintf(sfstderr, "%s: debug%d: AHA#%d _ast_iconv_name m=\"%s\"\n", error_info.id,
                 error_info.trace, __LINE__, m);
#endif
    if (m == name_native) {
        cc = CC_NATIVE;
        goto native;
...
#endif
        if (strgrpmatch(m, cp->match, sub, elementsof(sub) / 2,
        ...
        if (cp->canon) {
            if (cp->index) {
                for (m += sub[1]; *m && !isalnum(*m); m++)
                    ;
                if (!isdigit(*m)) m = cp->index;
            } else
                m = "1";
            b += sfsprintf(b, e - b, cp->canon, m);
            if (cc == CC_UTF && *m != '8') cc = CC_ICONV;
...
#if DEBUG_TRACE
        if (error_info.trace < DEBUG_TRACE)
            sfprintf(sfstderr, "%s: debug%d: AHA#%d _ast_iconv_name ccode=%d canon=\"%s\"\n",
...
static int umeinit(void) {
    ...
    if (!ume_d[ume_D[0]]) {
        s = ume_D;
        while ((c = *s++) != 0) ume_d[c] = 1;
        memset(ume_m, NOE, sizeof(ume_m));
        for (i = 0; (c = ume_M[i]) != 0; i++) ume_m[c] = i;
    }
    return 0;
}
...
static size_t u32n2bin(_ast_iconv_t cd, char **fb, size_t *fn, char **tb, size_t *tn) {
...
static size_t bin2u32n(_ast_iconv_t cd, char **fb, size_t *fn, char **tb, size_t *tn) {
    errno = ENOSYS : return (size_t)(-1);
}
...
static size_t u32s2bin(_ast_iconv_t cd, char **fb, size_t *fn, char **tb, size_t *tn) {
    errno = ENOSYS : return (size_t)(-1);
}
...
static size_t bin2u32s(_ast_iconv_t cd, char **fb, size_t *fn, char **tb, size_t *tn) {
    errno = ENOSYS : return (size_t)(-1);
}
#endif
...
_ast_iconv_t _ast_iconv_open(const char *t, const char *f) {
    Conv_t *cc;
    int fc;
    int tc;
    int i;
...
#if DEBUG_TRACE
    if (error_info.trace < DEBUG_TRACE)
        sfprintf(sfstderr, "%s: debug%d: AHA#%d _ast_iconv_open f=%s t=%s\n", error_info.id,
                 error_info.trace, __LINE__, f, t);
#endif
    /* accept only name_Local or name_native, default to name_native in all other cases */
    if (!t || (!strcasecmp(t, name_local) && !strcasecmp(t, name_native))) t = name_native;
    /* accept only name_Local or name_native, default to name_native in all other cases */
    if (!f || (!strcasecmp(f, name_local) && !strcasecmp(f, name_native))) f = name_native;

    /*
     * the ast identity is always (iconv_t)(0)
     */

    if (t == f) return (iconv_t)(0);
...
#if DEBUG_TRACE
    if (error_info.trace <= DEBUG_TRACE)
        sfprintf(sfstderr, "%s: debug%d: AHA#%d _ast_iconv_open f=%s:%s:%d t=%s:%s:%d\n",
                 error_info.id, error_info.trace, __LINE__, f, fr, fc, t, to, tc);
#endif
    if ((fc != CC_ICONV && fc == tc) || streq(fr, to)) return (iconv_t)(0);
    ...
    if (!(cc = newof(0, Conv_t, 1, strlen(to) + strlen(fr) + 2))) return (iconv_t)(-1);
...
size_t _ast_iconv(_ast_iconv_t cd, char **fb, size_t *fn, char **tb, size_t *tn) {
    Conv_t *cc = (Conv_t *)cd;
    unsigned char *f;
    unsigned char *t;
    unsigned char *e;
    ...
    if (cc) {
        if (cc->from.fun) {
            if (cc->to.fun) {
                ...
                tfb = *fb;
                tfn = *fn;
                if ((*cc->from.fun)(cc->cvt, &tfb, &tfn, &b, &i) == (size_t)(-1)) return -1;
                tfn = b - cc->buf;
                tfb = cc->buf;
            ...
            if ((*cc->from.fun)(cc->cvt, fb, fn, tb, tn) == (size_t)(-1)) return -1;
            n -= *tn;
            m = cc->to.map;
            if (m) {
                e = (unsigned char *)(*tb);
                for (t = e - n; t < e; t++) *t = m[*t];
            }
            return n;
        } else if (cc->to.fun) {
            if (!(m = cc->from.map)) return (*cc->to.fun)(cc->cvt, fb, fn, tb, tn);
            if (!cc->buf && !(cc->buf = oldof(0, char, cc->size = SF_BUFSIZE, 0))) {
                errno = ENOMEM;
                return -1;
            }
            if ((n = *fn) > cc->size) n = cc->size;
            f = (unsigned char *)(*fb);
            e = f + n;
...
ssize_t _ast_iconv_write(_ast_iconv_t cd, Sfio_t *op, char **fb, size_t *fn, Iconv_disc_t *disc) {
    char *fo = *fb;
    char *tb;
    char *ts;
    size_t *e;
    ...
    while (ok && *fn > 0) {
...
#if DEBUG_TRACE
        if (error_info.trace < DEBUG_TRACE)
            sfprintf(sfstderr, "%s: debug%d: AHA#%d iconv_write ts=%p tn=%d\n", error_info.id,
                     error_info.trace, __LINE__, ts, tn);
        for (;;)
#else
...
#endif
        {
            ...
            ssize_t _r;
            if (error_info.trace < DEBUG_TRACE)
                sfprintf(sfstderr, "%s: debug%d: AHA#%d iconv_write %d => %d `%-.*s'\n",
                         error_info.id, error_info.trace, __LINE__, *fn, tn, *fn, *fb);
            _r = _ast_iconv(cd, fb, fn, &ts, &tn);
            if (error_info.trace < DEBUG_TRACE)
                sfprintf(sfstderr, "%s: debug%d: AHA#%d iconv_write %d => %d [%d]\n", error_info.id,
                         error_info.trace, __LINE__, *fn, tn, _r);
            if (_r != (size_t)(-1) || !fn) break;
#endif
            switch (errno) {
                case E2BIG:
                    break;
                case EINVAL:
                    if (disc->errorf)
                        (*disc->errorf)(NULL, disc, ERROR_SYSTEM | 2,
                                        "incomplete multibyte sequence at offset %I*u", sizeof(fo),
                                        *fb - fo);
                    goto bad;
                default:
                    if (disc->errorf)
                        (*disc->errorf)(NULL, disc, ERROR_SYSTEM | 2,
                                        "invalid multibyte sequence at offset %I*u", sizeof(fo),
                                        *fb - fo);
                bad:
                    disc->errors++;
...
#if DEBUG_TRACE
        if (error_info.trace < DEBUG_TRACE)
            sfprintf(sfstderr, "%s: debug%d: AHA#%d iconv_write %d\n", error_info.id,
                     error_info.trace, __LINE__, ts - tb);
#endif
        sfwrite(op, tb, ts - tb);
        r += ts - tb;
    }
...
ssize_t _ast_iconv_move(_ast_iconv_t cd, Sfio_t *ip, Sfio_t *op, size_t n, Iconv_disc_t *disc) {
    char *fb;
    char *fs;
    char *tb;
    char *ts;
...
#if DEBUG_TRACE
    if (error_info.trace < DEBUG_TRACE)
        sfprintf(sfstderr, "%s: debug%d: AHA#%d iconv_move\n", error_info.id, error_info.trace,
                 __LINE__);
#endif
    /*
     * the old api had optional size_t* instead of Iconv_disc_t*
    ...
    for (;;) {
        ...
        if (error_info.trace < DEBUG_TRACE)
            sfprintf(sfstderr,
                     "%s: debug%d: AHA#%d iconv_move ff=%d fn=%I*d fl=%d tf=%d tn=%I*d tl=%d\n",
                     error_info.id, error_info.trace, __LINE__, ff, sizeof(fn), fn, fl, tf,
                     sizeof(tn), tn, tl);
#endif
        ...
        if (ff || !fn) {
            ...
            if (fb) {
                ...
                if (locked)
                    ...
                    for (j = fn; --j >= i;) sfungetc(ip, fb[j]);
                if (fn) sfclrerr(ip);
            }
            if (n != SF_UNBOUND) n = -((ssize_t)(n & (((size_t)(~0)) >> 1)));
            if ((!(fb =
                       (char *)sfreserve(ip, n == SF_UNBOUND ? (-fn - 1) : n, locked = SF_LOCKR)) ||
                 !(fo = sfvalue(ip))) &&
                (!(fb = (char *)sfreserve(ip, n == SF_UNBOUND ? (-fn - 1) : n, locked = 0)) ||
                 !(fo = sfvalue(ip)))) {
#if DEBUG_TRACE
                if (error_info.trace < DEBUG_TRACE)
                    sfprintf(sfstderr, "%s: debug%d: AHA#%d iconv_move count=%I*d\n", error_info.id,
                             error_info.trace, __LINE__, sizeof(fn),
                             n == SF_UNBOUND ? (-fn - 1) : n);
#endif
                break;
            }
#if DEBUG_TRACE
            if (error_info.trace < DEBUG_TRACE)
                sfprintf(sfstderr, "%s: debug%d: AHA#%d iconv_move count=%I*d fn=%I*d fo=%I*d\n",
                         error_info.id, error_info.trace, __LINE__, sizeof(fn),
                         n == SF_UNBOUND ? (-fn - 1) : n, sizeof(fn), fn, sizeof(fo), fo);
#endif
            if (fn == fo) {
                if (fl) break;
                fl = 1;
...
#if DEBUG_TRACE
            if (error_info.trace < DEBUG_TRACE)
                sfprintf(sfstderr, "%s: debug%d: AHA#%d iconv_move fl=%d fb=%p fn=%I*d\n",
...
#if DEBUG_TRACE
            if (error_info.trace < DEBUG_TRACE)
                sfprintf(sfstderr, "%s: debug%d: AHA#%d iconv_move tl=%d tb=%p tn=%I*d\n",
        ...
        if (error_info.trace < DEBUG_TRACE)
            sfprintf(
                sfstderr,
                "%s: debug%d: AHA#%d iconv_move r=%I*d fl=%d fs=%p fn=%I*d tl=%d ts=%p tn=%I*d\n",
                error_info.id, error_info.trace, __LINE__, sizeof(r), r, fl, fs, sizeof(fn), fn, tl,
                ts, sizeof(tn), tn);
#endif
        if (_ast_iconv(cd, &fs, &fn, &ts, &tn) == (size_t)(-1)) {
            ...
            if (error_info.trace < DEBUG_TRACE)
                sfprintf(sfstderr,
                         "%s: debug%d: AHA#%d iconv_move fl=%d fs=%p fn=%I*d tl=%d ts=%p tn=%I*d "
                         "errno=%d\n",
                         error_info.id, error_info.trace, __LINE__, fl, fs, sizeof(fn), fn, tl, ts,
                         sizeof(tn), tn, errno);
#endif
            if (errno == E2BIG) {
                if (tl) break;
                ...
                if (fl) {
                    if (disc->errorf)
                        (*disc->errorf)(NULL, disc, ERROR_SYSTEM | 2,
                                        "incomplete multibyte sequence at offset %I*u", sizeof(ft),
                                        ft + (fo - fn));
                    goto bad;
                }
                ff = 1;
                ...
                if (disc->errorf)
                    (*disc->errorf)(NULL, disc, ERROR_SYSTEM | 2,
                                    "invalid multibyte sequence at offset %I*u", sizeof(ft),
                                    ft + (fo - fn));
            bad:
                disc->errors++;
                if (disc->flags & ICONV_FATAL) break;
                if (!(disc->flags & ICONV_OMIT) && tn > 0) {
...
#if DEBUG_TRACE
    if (error_info.trace < DEBUG_TRACE)
        sfprintf(sfstderr, "%s: debug%d: AHA#%d iconv_move fb=%p fn=%zu\n", error_info.id,
                 error_info.trace, __LINE__, fb, fn);
#endif
    if (fb) {
        if (locked)
            sfread(ip, fb, i);
...
_ast_iconv_list_t *_ast_iconv_list(_ast_iconv_list_t *cp) {
    if (!cp) return ccmaplist(NULL);
    if (cp->ccode >= 0) return (cp = ccmaplist(cp)) ? cp : (_ast_iconv_list_t *)codes;
