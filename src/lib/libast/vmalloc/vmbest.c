...
#define APPEND(e, b) \
    (((LINK(b) = LINK(e)) ? LEFT(LINK(b)) = (b) : NULL), (LEFT(b) = (e)), LINK(e) = (b), SETLIST(b))
#define DELETE(b) \
    ((LEFT(b) ? (LINK(LEFT(b)) = LINK(b)) : NULL), (LINK(b) ? (LEFT(LINK(b)) = LEFT(b)) : NULL))
#define REMOVE(pk, b)                        \
    ((b) == (pk)->wild ? ((pk)->wild = NULL) \
...
static int bestfree(Vmalloc_t *vm, Void_t *data, int local) {
    ...
    /* quick sanity check */
    if ((Vmuchar_t *)data < vm->data->segmin || (Vmuchar_t *)data >= vm->data->segmax) return -1;
    ...
    {
        LINK(blk) = head = *listp;
        if (asocasptr(listp, head, blk) == head) break;
    }

    if (!local && _Vmtrace) (*_Vmtrace)(vm, (Vmuchar_t *)data, NULL, TRUESIZE(sz), 0);
...
static Pack_t *bestpackget(Vmalloc_t *vm, unsigned int ppos, unsigned int tid) {
    ...
    if ((pack = asocasptr(&best->pack[ppos], NULL, PK_INITIALIZING)) != NULL) {
        asospindecl(); /* pack is initializing -- wait until its done */
        for (asospininit();;
             asospinnext()) { /* if we are inititializing then a signal hit -- skip it or deadlock
                               */
            if (best->tid[ppos] == tid) return NULL;
            if ((pack = best->pack[ppos]) != PK_INITIALIZING) return pack;
        }
    } /**/
    DEBUG_ASSERT(pack == NULL && best->pack[ppos] == PK_INITIALIZING);
...
#define EXTZ (8 * 1024) /* extra on each memory extension */
    if (!(blk = (*_Vmsegalloc)(vm, NULL, sizeof(Pack_t) + EXTZ, VM_SEGEXTEND))) {
        best->nomem = 1; /**/
        DEBUG_ASSERT(best->list[0]);
        asocasptr(&best->pack[ppos], PK_INITIALIZING, NULL);
        return best->list[0];
    }
    ...
    if (_Vmassert & VM_debug)
        debug_printf(2, "%s:%d: PACK(%p) WILD(%p) => WILD(%p)=%zd\n", __FILE__, __LINE__, pack,
                     pack->wild, nb, BDSZ(nb));
    pack->wild = nb; /**/
    DEBUG_ASSERT(PACKWILD(pack, pack->wild));

    lpos = asoaddint(&best->pkcnt, 1); /* get position in best->list[] to insert */
    pack->lpos = lpos;                 /**/
    DEBUG_ASSERT(lpos < best->pkmax && best->list[lpos] == NULL);
    asocasptr(&best->list[lpos], NULL, pack);
...
static Block_t *bestpackextend(Vmalloc_t *vm, Pack_t *pack, Block_t *wild, ssize_t size,
                               int segtype) {
    size_t blkz;
    Block_t *pblk, *endb;
    ...
    if (_Vmassert & VM_debug)
        debug_printf(2, "%s:%d: PACK(%p) WILD(%p)=%zd BDSZ(%p)=%zd blkz=%zd size=%zu\n", __FILE__,
                     __LINE__, pack, wild, wild ? BDSZ(wild) : 0, pack->pblk, BDSZ(pack->pblk),
                     blkz, size);
    if (!(pblk = (*_Vmsegalloc)(vm, pack->pblk, size, segtype))) pblk = pack->pblk;
    if (_Vmassert & VM_debug)
        debug_printf(2, "%s:%d: PACK(%p) WILD(%p)=%zd BDSZ(%p)=%zd blkz=%zd size=%zu\n", __FILE__,
                     __LINE__, pack, wild, wild ? BDSZ(wild) : 0, pblk, BDSZ(pblk), blkz, size);
    ...
    if (BDSZ(pblk) > blkz) /* superblock increased in size */
    {
        if (!wild) /* new wild starts at the previous end block */
        {
            wild = pack->endb;
            if (_Vmassert & VM_debug)
                debug_printf(2, "%s:%d: PACK(%p) WILD(%p)=%zd\n", __FILE__, __LINE__, pack, wild,
                             BDSZ(wild));
        }
        if ((_Vmassert & VM_debug) &&
            (Vmuchar_t *)wild >
                ((Vmuchar_t *)ENDB(pblk) - sizeof(Head_t))) /* heisus in _Vmsegalloc */
        {
        ...
        if (_Vmassert & VM_debug)
            debug_printf(2, "%s:%d: PACK(%p) pblk=%p endb=%p wild=%p size=%zu\n", __FILE__,
                         __LINE__, pack, pblk, endb, wild, size);
        PACK(wild) = pack;
...
static int bestlistreclaim(Vmalloc_t *vm, Pack_t *pack, Block_t *volatile *listp) {
    ...
    {
        if (!(list = *listp)) return 0;
        if (asocasptr(listp, list, NULL) == list) break;
    }

    /* merge all adjacent free blocks */
    for (fp = list; fp; fp = LINK(fp)) SIZE(fp) |= MARK;
    for (adj = NULL, fp = list;;) { /**/
        DEBUG_ASSERT(!(SIZE(fp) & SMALL));
        if (SIZE(fp) & PFREE) /* merge with a preceding free block */
        {
            np = PREV(fp); /**/
            DEBUG_ASSERT(!(SIZE(np) & SMALL) && NEXT(np) == fp && PACK(np) == pack);
            REMOVE(pack, np); /**/
    ...
    for (fp = list; fp != NULL; fp = adj) {
        ...
        *s = fp; /* self-pointer for a free block */
        ...
        if (PACKWILD(pack, fp)) /* wilderness preservation */
        {
            if (_Vmassert & VM_debug)
                debug_printf(2, "%s:%d: PACK(%p) WILD(%p) => WILD(%p)=%zd\n", __FILE__, __LINE__,
                             pack, pack->wild, fp, BDSZ(fp));
            pack->wild = fp;
        } else /* insert into free tree */
        {
            LEFT(fp) = RGHT(fp) = LINK(fp) = NULL;
...
static Block_t *bestpackalloc(Vmalloc_t *vm, Pack_t *pack, size_t size, size_t minz) {
    ...
    if ((tp = pack->alloc)) /* fast allocation from recent memory */
    {
        ...
        if (SIZE(tp) >= size) {
            if (_Vmassert & VM_debug)
                debug_printf(2, "%s:%d: PACK(%p) SIZE(%p)=%zd size=%zu minz=%zu\n", __FILE__,
                             __LINE__, pack, tp, SIZE(tp), size, minz);
            goto a_lloc;
        }
    ...
    if (!(tp = bestpackextract(pack, size))) {
        ...
        if (!(tp = bestpackextract(pack, size)) && minz < size) tp = bestpackextract(pack, minz);
    }
    if (tp) /* got one from the free tree */
    {
        if (_Vmassert & VM_debug)
            debug_printf(2, "%s:%d: PACK(%p) SIZE(%p)=%zd size=%zu minz=%zu\n", __FILE__, __LINE__,
                         pack, tp, SIZE(tp), size, minz);
        SIZE(tp) |= BUSY; /**/
    ...
    if (vm->data->lock == 0) /* could be asothreadid() */
    {
        if (!(tp = pack->wild) || SIZE(tp) < size) /* check wilderness */
        {
            if (tp) {
                ...
                if (!PACKWILD(pack, tp)) {
                    if (_Vmassert & VM_debug)
                        debug_printf(2, "%s:%d: PACK(%p) WILD(%p)=%zd orphaned wild\n", __FILE__,
                                     __LINE__, pack, tp, SIZE(tp));
                    pack->wild = tp = NULL;
                }
            }
    ...
    if (!(pblk = (*_Vmsegalloc)(vm, NULL, size + EXTRA(pack), VM_SEGEXTEND))) {
        best->nomem = 1;
        return NULL;
    } else /* got a new superblock */
    {
        ...
        if ((_Vmassert & 2) && pack->wild) {
            if (_Vmassert & VM_debug)
                debug_printf(2, "%s:%d: PACK(%p) WILD(%p) => WILD(%p)\n", __FILE__, __LINE__, pack,
                             pack->wild, tp);
            pack->wild = tp; /**/
            DEBUG_ASSERT(PACKWILD(pack, pack->wild));
        }
...
static Block_t *bestsmallalloc(Vmalloc_t *vm, Pack_t *pack, ssize_t size) {
    ...
    if (asocaschar(&small->lock, 0, 1) != 0)     /* lock to prevent concurrent allocs */
        return NULL; /* list is busy; return to explore a different pack */

    for (asospininit();; asospinnext()) /* loop is needed because bestfree() is lock-free */
        if (!(blk = small->free) || asocasptr(&small->free, blk, LINK(blk)) == blk) break;
    ...
    if (!blk && asocasint(&pack->lock, 0, KEY_BEST) == 0) {
        ...
        if ((blk = bestpackalloc(vm, pack, memz, SM_MAKE * blkz))) {
            ...
            for (asospininit();; asospinnext()) /* prepend list to cache */
            {
                ...
                if (asocasptr(&small->free, head, list) == head) break;
            }
...
static Void_t *bestalloc(Vmalloc_t *vm, size_t size, int local) {
    ...
    if (!(pk = best->pack[ppos]) || pk == PK_INITIALIZING) pk = bestpackget(vm, ppos, tid); /**/
    ...
    for (asospininit();;) {
        if (pk) {
            if (smsz > 0 && (blk = bestsmallalloc(vm, pk, smsz))) break;
        ...
        do /* get next pack to allocate from */
        {
            if (!best->pkcnt || (lpos = (lpos + 1) % best->pkcnt) == begp) /* cycling over list[] */
            {
                ...
                do { /* conflict at ppos so move it forward to reduce contention */
    ...
    if (_Vmtrace && !local) (*_Vmtrace)(vm, NULL, (Vmuchar_t *)DATA(blk), size, 0);
    return DATA(blk);
}
...
static Void_t *bestresize(Vmalloc_t *vm, Void_t *data, size_t size, int type, int local) {
    ...
    /* quick sanity check */
    if ((Vmuchar_t *)rsdt < vm->data->segmin || (Vmuchar_t *)rsdt >= vm->data->segmax) return NULL;
    ...
    if (!local && _Vmtrace) (*_Vmtrace)(vm, (Vmuchar_t *)data, (Vmuchar_t *)rsdt, size, 0);
    return rsdt;
}
...
static Void_t *bestalign(Vmalloc_t *vm, size_t size, size_t align, int local) {
    ...
    if (METHOD(best) != VM_MTBEST && vm->meth.eventf) {
        ...
        while (algn < extra || (algn - extra) < sizeof(Block_t)) algn *= 2;
    } else
        extra = 0;

    sz = algz + 2 * (algn + sizeof(Head_t) + extra);
    if (!(data = (Vmuchar_t *)KPVALLOC(vm, sz, bestalloc))) return NULL;

    tp = BLOCK(data);
    pack = PACK(tp);
    for (asospininit();; asospinnext()) /* acquire lock to alter arena */
        if (asocasint(&pack->lock, 0, KEY_BEST) == 0) break;
...
static int beststat(Vmalloc_t *vm, Vmstat_t *st, int local) {
    ...
    for (seg = best->vmdt.seg; seg; seg = seg->next)
        for (sgb = (Block_t *)SEGDATA(seg); sgb < seg->endb; sgb = NEXT(sgb)) { /**/
            ...
            for (k = 0; k < best->pkcnt; ++k)
                if (pack == best->list[k]) break;
            if (k >= best->pkcnt || pack->lock && !local) return -1;
    ...
    for (k = 0; k < best->pkcnt; ++k) {
        if (!(pack = best->list[k]) || pack->lock && !local) return -1;
        ...
        for (ty = 0; ty < SM_SLOT; ++ty) {
            if (pack->small[ty].lock) return -1;
...
static int bestevent(Vmalloc_t *vm, int event, Void_t *arg) {
    ...
    if (event == VM_OPEN) /* return the size of Vmbest_t */
    {
        ...
        *((ssize_t *)arg) = sizeof(Vmbest_t);
        ...
        if ((best->pkmax = asoactivecpu()) < 2) best->pkmax = 2;
        if ((best->pkmax *= 2) > PK_ARRAY) best->pkmax = PK_ARRAY;
        if (best->pkmax < PK_ALLOW) best->pkmax = PK_ALLOW;
    } else if (event == VM_CHECKARENA) /* check integrity of the arena */
    {
        if (chkregion((Vmbest_t *)vm->data, 0) < 0) return -1;
    }
...
void _vmchkaddress(Vmalloc_t *vm, Vmuchar_t *addr, int tag) {
    if (vm && addr && (addr < vm->data->segmin || addr >= vm->data->segmax))
        _vmmessage("Address not belonging to a region", 0, tag ? "Tag" : NULL, tag);
}
...
void _vmchkall(int tag) /* check to see if some region may have a bad free list or cache */
{
    ...
    for (vh = _Vmhold; vh; vh = vh->next) {
        if (!vh->vm || !(vh->vm->meth.meth & VM_MTBEST)) continue;

        best = (Vmbest_t *)vh->vm->data;
        for (p = 0; p < best->pkcnt; ++p) {
            if (!(pack = best->list[p])) continue;
...
#define VMHOLD(vm, b) \
    ((Vmuchar_t *)(b) >= (vm)->data->segmin && (Vmuchar_t *)(b) < (vm)->data->segmax)
...
#define CAREFUL 1 /* 1 for any system where different malloc packages may be mixed together */
    ...
    if (VMHOLD(Vmheap, addr))
        if ((pack = (Pack_t *)PACK(blk))->best == (Vmbest_t *)Vmheap->data) return Vmheap;

    for (vh = _Vmhold; vh; vh = vh->next)
        if (VMHOLD(vh->vm, addr))
            if (pack->best == (Vmbest_t *)vh->vm->data) return vh->vm;
#endif

    if (Vmregion->meth.meth == VM_MTDEBUG) return Vmregion;
