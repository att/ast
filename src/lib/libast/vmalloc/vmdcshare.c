...
#endif
    for (hash = FNV_OFFSET; *str; ++str) hash = (hash ^ str[0]) * FNV_PRIME;
...
static int mminit(Mmdisc_t *mmdc) {
    ...
    if ((size = mmdc->size) < MM_MINSIZE) size = MM_MINSIZE;
    ...
    {                   /* this can be done in multiple processes */
        ...
        if ((mmdc->shmkey = mmkey(mmdc->name, mmdc->proj)) < 0) goto done;
    ...
    if (asocasint(&mmvm->magic, 0, MM_JUST4US) == 0) /* lucky winner: us! */
    {
        ...
        if (!base || !(mmvm = mmfix(mmvm, mmdc, fd))) { /* remove any resource just created */
            if (mmdc->proj >= 0)
                shmctl(mmdc->shmid, IPC_RMID, 0);
            else
                unlink(mmdc->name);
...
#endif
{
    ...
    if (!(mmvm = mmdc->mmvm)) return 0;

    if (mmdc->proj < 0) {
        (void)msync(mmvm->base, mmvm->size, MS_ASYNC);
        if (mmdc->mode & MM_DETACH) {
            if (mmvm->base) (void)munmap(mmvm->base, mmvm->size);
        }
        if (mmdc->mode & MM_REMOVE) (void)unlink(mmdc->name);
    } else {
        if (mmdc->mode & MM_DETACH) {
            if (mmvm->base) (void)shmdt(mmvm->base);
        }
        if (mmdc->mode & MM_REMOVE) {
            if (mmdc->shmid >= 0) (void)shmctl(mmdc->shmid, IPC_RMID, &shmds);
        }
    }
