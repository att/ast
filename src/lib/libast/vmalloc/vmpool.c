...
#endif
{
    ...
    {
        if (!(list = pool->free)) break;
        if (asocasptr(&pool->free, list, NULL) == list) break;
    }

    if (!list) /* need new memory */
    {
        size = POOLSIZE(pool->size);
        if (!(blk = (*_Vmsegalloc)(vm, NULL, ROUND(2 * size, pool->vmdt.incr),
    ...
    if ((list = list->next)) {
        if (asocasptr(&pool->free, NULL, list) != NULL) {
            ...
            for (;;) {
                ...
                if (asocasptr(&pool->free, free, list) == free) break;
            }
        }
    }
...
#endif
{
    ...
    for (;;) {
        ...
        if (asocasptr(&pool->free, free, pl) == free) break;
    }

    if (!local && _Vmtrace) (*_Vmtrace)(vm, (Vmuchar_t *)data, NULL, pool->size, 0);
...
static int poolstat(Vmalloc_t *vm, Vmstat_t *st, int local) {
    ...
    for (pl = pool->free; pl; pl = pl->next) st->n_free += 1;
...
static int poolevent(Vmalloc_t *vm, int event, Void_t *arg) {
    ...
    {
        if (!(pool = (Vmpool_t *)vm->data)) return -1;
        pool->size = 0;
        pool->free = NULL;
    }
    return 0;
