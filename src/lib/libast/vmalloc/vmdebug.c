...
static void dbwarn(Vmalloc_t *vm, Void_t *data, int where, char *file, int line, Void_t *func,
                   int type)
#else
static void dbwarn(vm, data, where, file, line, func,
...
#else
static void dbsetinfo(data, size, file,
                      line) Vmuchar_t *data; /* real address not the one from Vmbest	*/
...
#endif
{
    ...
    if (vm->data->mode & VM_DBCHECK) vmdbcheck(vm);
    ...
    if (Dbnwatch > 0) dbwatch(vm, data, file, line, func, DB_ALLOC);
...
#endif
{
    ...
    if (asocasint(&vm->data->dlck, 0, KEY_DEBUG)) { /* prepend to delayed free list -- handled by
                                                       another free() that gets the lock */
        asospindecl();
        item = (Free_t *)data;
        for (asospininit();; asospinnext()) {
            item->next = list = vm->data->delay;
            if (asocasptr(&vm->data->delay, list, item) == (void *)list) break;
        }
        return 0;
    }
    ...
    for (;;) { /* check to see if memory is from vm */
        for (seg = vm->data->seg; seg; seg = seg->next)
            if ((Vmuchar_t *)data >= seg->base && (Vmuchar_t *)data < seg->base + seg->size) break;
        ...
        if (Dbnwatch > 0) dbwatch(vm, data, file, line, func, DB_FREE);
...
#else
static Void_t *dbresize(vm, addr, size, type, local) Vmalloc_t *vm; /* region allocating from	*/
...
#endif
{
    ...
    for (seg = vm->data->seg; seg; seg = seg->next)
        if ((Vmuchar_t *)addr >= seg->base && (Vmuchar_t *)addr < seg->base + seg->size) break;
    ...
    if (Dbnwatch > 0) dbwatch(vm, addr, file, line, func, DB_RESIZE);
    ...
    if (!data) /* failed, reset data for old block */
    {
        ...
        if (Dbnwatch > 0) dbwatch(vm, data, file, line, func, DB_RESIZED);
    }
...
#endif
{
    ...
    if (vm->data->mode & (VM_MTDEBUG | VM_MTBEST)) {
        if ((*vm->meth.eventf)(vm, VM_CHECKARENA, NULL) < 0) return -1;
        if (!(vm->data->mode & VM_MTDEBUG)) return 0;
    } else
        return -1;
    ...
    for (seg = vm->data->seg; seg; seg = seg->next) {
        ...
        for (; sgb < seg->endb; sgb = NEXT(sgb)) {
            ...
            for (; bp < endbp; bp = NEXT(bp)) {
                ...
                set_bad:
                    dbwarn(vm, data, begp - data, NULL, 0, 0, DB_CHECK);
                    DBSETBAD(data);
                    rv += 1;
                    break;
                }
...
#endif
{
    ...
    if ((sz = ROUND(size, MEM_ALIGN) + DB_EXTRA) < sizeof(Body_t)) sz = sizeof(Body_t);
