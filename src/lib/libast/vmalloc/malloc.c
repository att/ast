...
#endif
{
    ...
    if ((pid = getpid()) < 0) return NULL;

    s = ends;
    do {
        if (s == begs) return NULL;
        *--s = '0' + pid % 10;
    } while ((pid /= 10) > 0);
    while (s < ends) *begs++ = *s++;
...
void _vmoptions(int boot) {
    ...
    if (boot & 1) {
        if (_Vmassert & VM_GETMEMORY) return;
    ...
    if (options && options[0]) { /* copy option string to a writable buffer */
        ...
        *s = 0;
        ...
        for (s = buf;;) { /* skip blanks to option name */
            ...
            switch (t[0]) {
                ...
                case 'm': /* method=<method> */
                    if ((boot & 2) && v && !vm) {
                        if ((v[0] == 'V' || v[0] == 'v') && (v[1] == 'M' || v[1] == 'm')) v += 2;
                        switch (v[0]) {
                            case 'b':
                                vm = Vmheap;
                                break;
                ...
                case 'p': /* pagesize=<size> period=<count> */
                    switch (t[1]) {
                        case 'a':
                            if (boot & 1) _Vmpagesize = atou(&v);
                            break;
                        case 'e':
                            if (boot & 2) {
                                if (!vm) vm = vmopen(Vmdcsystem, Vmdebug, 0);
                                if (v && vm && vm->meth.meth == VM_MTDEBUG) _Vmdbcheck = atou(&v);
                            }
                            break;
                    }
                    break;
                case 's': /* segsize=<size> start=<count> */
                    switch (t[1]) {
                        case 'e':
                            if (boot & 1) _Vmsegsize = atou(&v);
                            break;
                        case 't':
                            if (boot & 2) {
                                if (!vm) vm = vmopen(Vmdcsystem, Vmdebug, 0);
                                if (v && vm && vm->meth.meth == VM_MTDEBUG) _Vmdbstart = atou(&v);
                            }
                            break;
                    }
                    break;
                case 't': /* test || trace=<path> */
                    ...
                    break;
                case 'u': /* usage */
                    if (boot & 1) _Vmassert |= VM_usage;
                    break;
                case 'v': /* verbose */
                    if (boot & 1) _Vmassert |= VM_verbose;
                    break;
                case 'w':
                    if ((boot & 2) && t[1] == 'a') {
                        switch (t[2]) {
        ...
        {
            if (vm->meth.meth == VM_MTDEBUG && _Vmdbcheck == 0) _Vmdbcheck = 1;
            Vmregion = vm;
        }

        /* enable tracing */
        if (trace && (fd = createfile(trace)) >= 0) vmtrace(fd);
    }
    if ((boot & 1) && !(_Vmassert & VM_GETMEMORY)) _Vmassert |= VM_GETMEMORY;
}
...
static char *lcl_getenv(const char *s) {
    int n;
    static char buf[512];

    if (!(n = GetEnvironmentVariable(s, buf, sizeof(buf))) || n > sizeof(buf)) return 0;
    return buf;
}
...
#define VMEPILOGUE(f)                                                                        \
    {                                                                                        \
        if (_Vmdbcheck && Vmregion->meth.meth == VM_MTDEBUG) vmset(Vmregion, VM_DBCHECK, 0); \
    }
...
static int _vmstart(int freeing) {
    ...
    if (_Vmassert & VM_debug)
        debug_printf(2, "%s:%d: _Vmstart=%s\n",
                     _Vmstart == 0
                         ? "UNINITIALIZED"
                         : _Vmstart == VM_STARTING ? "STARTING"
    ...
    else if (start == VM_STARTING) /* wait until initialization is done */
    {
        ...
        for (asospininit();; asospinnext()) {
            if ((start = asogetint(&_Vmstart)) == VM_STARTED) return 0;
            if (freeing && ++i >= 10) return 0;
        }
    }
    ...
    if (_Vmassert & VM_verbose)
        debug_printf(2, "vmalloc: method=%s\n",
                     Vmregion->meth.meth == VM_MTBEST
                         ...
                         : Vmregion->meth.meth == VM_MTDEBUG
                               ...
                               : Vmregion->meth.meth == VM_MTLAST
                                     ? "last"
                                     : Vmregion->meth.meth == VM_MTPOOL ? "pool" : "unknown");
...
char *strdup(const char *s) {
    ...
    if (!s)
        ...
        n = strlen(s);
        if ((ns = malloc(n + 1))) memcpy(ns, s, n + 1);
        return ns;
    }
}
#endif /* _PACKAGE_ast */
...
extern Void_t *alloca(size_t size) {
    ...
    while (Frame) /* free unused frames */
    {
        if ((_stk_down && &array[0] > Frame->head.head.addr) ||
            ...
            if ((vm = vmregion(f))) (void)(*vm->meth.freef)(vm, f, 0);
...
static void vm_free_hook(void *ptr, const void *caller) { free(ptr); }
