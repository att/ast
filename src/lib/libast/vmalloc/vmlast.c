...
static Void_t *lastalloc(Vmalloc_t *vm, size_t size, int local) {
    ...
    last->last = NULL; /* wipe record of last allocation */
    ...
    if (last->size < size) {
        ...
        {
            if ((blk = (*_Vmsegalloc)(vm, NULL, size, VM_SEGALL | VM_SEGEXTEND))) { /**/
    ...
    if (last->last && !local && _Vmtrace) (*_Vmtrace)(vm, NULL, last->last, origsz, 0);
...
static int lastfree(Vmalloc_t *vm, Void_t *data, int local) {
    ...
    if (data != (Void_t *)last->last)
        ...
        if (!local && _Vmtrace) (*_Vmtrace)(vm, (Vmuchar_t *)data, NULL, size, 0);
    }
...
static Void_t *lastresize(Vmalloc_t *vm, Void_t *data, size_t size, int type, int local) {
    ...
    if (!data) {
        ...
        if (data && (type & VM_RSZERO)) memset(data, 0, size);
        return data;
    } else if (size <= 0) {
    ...
    if (data != (Void_t *)last->last)
        ...
        } else /* getting larger */
        {
            if ((oldz + last->size) < size &&
                (blk = last->blk) != NULL) { /* try to extend in place */
                blksz = SIZE(blk) & ~BITS;
            ...
            if ((oldz + last->size) < size && (type & VM_RSMOVE)) { /* try to get new memory */
                if ((blk = (*_Vmsegalloc)(vm, NULL, size, VM_SEGALL | VM_SEGEXTEND))) { /**/
                    DEBUG_ASSERT((SIZE(blk) & ~BITS) >= size);
                    last->size = SIZE(blk) & ~BITS;
...
static Void_t *lastalign(Vmalloc_t *vm, size_t size, size_t align, int local) {
    ...
    if (data && !local && _Vmtrace) (*_Vmtrace)(vm, NULL, data, orgsize, orgalign);
...
static int lastevent(Vmalloc_t *vm, int event, Void_t *arg) {
    ...
    {
        if (!(last = (Vmlast_t *)vm->data)) return -1;
        last->lock = 0;
        last->blk = NULL;
        last->data = NULL;
        last->size = 0;
