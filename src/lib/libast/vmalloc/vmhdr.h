...
#define ASSERT(p) \
    ((p) ? 0 : (MESSAGE("Assertion failed"), (ABORT() ? (abort(), 0) : PAUSE() ? (pause(), 0) : 0)))
#define COUNT(n) ((n) += 1)
#define ACCOUNT(a, b) ((a) += (b))
#define INITMEMORY(m, z) \
    ((m) ? (memset((m), 'i', (z) > 2 * MEM_ALIGN ? 2 * MEM_ALIGN : (z)), 0) : 0)
#define SETBUSYMEM(m, z) \
    (memset(((char *)(m)) + 2 * MEM_ALIGN, 'b', (z) <= 2 * MEM_ALIGN ? 0 : MEM_ALIGN))
#define CHKBUSYMEM(m, z) \
    (memcmp(((char *)(m)) + 2 * MEM_ALIGN, "bbbbbbbb", (z) <= 2 * MEM_ALIGN ? 0 : 8) == 0 ? 1 : 0)
#define SETFREEMEM(m, z) \
    (memset(((char *)(m)) + 2 * MEM_ALIGN, 'f', (z) <= 2 * MEM_ALIGN ? 0 : MEM_ALIGN))
#define CHKFREEMEM(m, z) \
    (memcmp(((char *)(m)) + 2 * MEM_ALIGN, "ffffffff", (z) <= 2 * MEM_ALIGN ? 0 : 8) == 0 ? 1 : 0)
...
#define VMFLF(vm, fi, ln, fn)                                                                    \
    ((fi) = (vm)->file, (vm)->file = NULL, (ln) = (vm)->line, (vm)->line = 0, (fn) = (vm)->func, \
     (vm)->func = NULL)
...
#define SEGROOT(sg) \
    ((Vmuchar_t *)(sg)->vmdt >= (sg)->base && (Vmuchar_t *)(sg)->vmdt < (Vmuchar_t *)(sg))
