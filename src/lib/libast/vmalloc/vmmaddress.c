...
static int _vmchkmem(Vmuchar_t *area, size_t size) {
    ...
    if (!(_Vmassert & VM_check_seg)) return 1;
    oldsigsegv = (Sighandler_f)signal(SIGSEGV, sigsegv);
    if (!(available = sigsetjmp(jmp, 1))) peek = area[0];
    if (available && !(available = sigsetjmp(jmp, 1))) peek = area[size - 1];
    if (available && !(available = sigsetjmp(jmp, 1))) peek = area[size / 2];
...
ssize_t _vmpagesize(void) {
    if (_Vmpagesize <= 0) {
        if ((_Vmpagesize = getpagesize()) <= 0) _Vmpagesize = VM_PAGESIZE;
...
int _vmboundaries(void) {
    ...
    for (; memz >= _Vmpagesize; memz /= 2) {
        ...
        if ((shmid = shmget(IPC_PRIVATE, z, IPC_CREAT | 0600)) >= 0) break;
    }
