...
void vmclrlock(int all) {
    ...
    for (vh = _Vmhold;; vh = vh->next) {
        if ((vm = vh ? vh->vm : Vmheap) == NULL) continue;

        if (threadid) {
            asocasint(&vm->data->lock, threadid, 0); /* lock on segments */
            asocasint(&vm->data->ulck, threadid, 0); /* lock on user data */
        } else {
            while (vm->data->lock) asosubint(&vm->data->lock, vm->data->lock);
            while (vm->data->ulck) asosubint(&vm->data->ulck, vm->data->ulck);
        }
    ...
    for (vh = _Vmhold;; vh = vh->next) {
        if ((todo = vm ? vm : vh ? vh->vm : Vmheap) && todo->data)
            for (seg = todo->data->seg; seg; seg = seg->next)
                if ((rv = (*segf)(todo, seg->base, seg->size, todo->disc, handle)) < 0) return -1;
        if (vm || !vh) break;
    }
    return rv;
}
...
static Block_t *_vmseginit(Vmdata_t *vmdt, Seg_t *seg, Vmuchar_t *base, ssize_t size, int insert) {
    ...
    base = seg->base;
    while (!vmdt->segmin || base < vmdt->segmin) asocasptr(&vmdt->segmin, vmdt->segmin, base);
    base += seg->size;
    while (!vmdt->segmax || base > vmdt->segmax) asocasptr(&vmdt->segmax, vmdt->segmax, base);
...
#define RETURN(x)     \
    ...
    if (Segunit == 0) Segunit = ROUND(8 * 1024, _Vmpagesize);
    ...
    if (vmdt->lock == key) {
        ...
        if (blk && (type & VM_SEGEXTEND)) /* no physical extension when he is us */
        {
            if (_Vmassert & VM_debug)
                debug_printf(2, "%s:%d: VM_SEGEXTEND: %s\n", __FILE__, __LINE__,
        ...
        if (blk) /* try extending an existing block in place */
        {
            ...
            if (BDSZ(blk) >= size) RETURN(blk);
            if (NEXT(blk) != ((Seg_t *)SEG(blk))->endb) RETURN(blk = NULL);
            ...
            {                                      /**/
                ...
                */
                ...
                {                      /* keeping segment size right is first priority */
                    ...
                    if (!vmdt->segmin || base < vmdt->segmin) vmdt->segmin = base;
                    if (!vmdt->segmax || (base + segsz) > vmdt->segmax) vmdt->segmax = base + segsz;
...
re_turn:
    if (blk) /* keep any excess memory for future allocations */
    {
        ...
        if (!(type & VM_SEGALL) &&
            (sz - size) > (Segunit + sizeof(Block_t))) { /* make block of unused space */
...
static char *_vmitoa(Vmulong_t v, int type) {
    ...
    *(s = &buf[255]) = 0;
    ...
    } else /* signed base-10 */
    {
        ...
        if (negative) v = (Vmulong_t)(-((long)v));
        do
            *--s = (char)('0' + (v % 10));
        while ((v /= 10) > 0);
        if (negative) *--s = '-';
