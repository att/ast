...
Vmalloc_t *_vmopen(Vmalloc_t *vmo, Vmdisc_t *disc, Vmethod_t *meth, int mode) {
    ...
    if (!meth || !disc || !disc->memoryf) return NULL;
    ...
    if (disc->exceptf) {
        ...
        } else if (rv > 0) /* region opened before and now being restored */
        {
            ...
            {
                if (initheap) write(9, "vmalloc: panic: heap initialization error #2\n", 45);
                return NULL;
            }
    ...
    if (!vd) { /* amount to round to when getting raw memory via discipline */
        ...
        if (!meth->eventf || (*meth->eventf)(vmp, VM_OPEN, &vdsz) < 0 || vdsz <= 0) {
            if (initheap) write(9, "vmalloc: panic: heap initialization error #3\n", 45);
            return NULL;
        }
        ...
        if (!(base = (Vmuchar_t *)(*disc->memoryf)(vmp, NULL, 0, size, disc))) {
            if (initheap) write(9, "vmalloc: panic: heap initialization error #4\n", 45);
            return NULL;
        }
        if (_Vmassert & 0x10)
            memset(base, 0, size);
    ...
    if (vmo)
        ...
        else if (!(vm = (Vmalloc_t *)vmalloc(Vmheap, sizeof(Vmalloc_t)))) /* on the heap */
        {
            ...
            if (initheap) write(9, "vmalloc: panic: heap initialization error #5\n", 45);
            return NULL;
        }
        *vm = *vmp;
    }
    ...
    } else if (vm != Vmheap) /* locklessly adding vm to list of regions */
    {
        ...
        for (vh = _Vmhold; vh; vh = vh->next) /* try adding to an existing slot */
            if (asocasptr(&vh->vm, NULL, vm) == NULL) break;
        if (!vh) /* need a new slot */
        {
            if (!(vh = vmalloc(Vmheap, sizeof(Vmhold_t)))) {
                vmclose(vm);
                if (initheap) write(9, "vmalloc: panic: heap initialization error #6\n", 45);
                return NULL;
            } else
                vh->vm = vm;
            for (;;) {
                vh->next = cvh = _Vmhold;
                if (asocasptr(&_Vmhold, cvh, vh) == cvh) break;
            }
        }
    }
