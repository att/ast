...
Lc_category_t *lccategories(void) { return (Lc_category_t *)&lc_categories[0]; }

/*
 * return the current info for category
 */

Lc_info_t *lcinfo(int category) {
...
static int match(const char *s, const char *p, int minimum, int standard) {
    const char *t;
    const char *x;
    int w;
    int z;
    ...
    do {
        ...
        if (*p) {
            ...
            if ((!*t || *t == ',') && (!*p || *p == '|' || w)) return p - x;
            if (minimum && z < (p - x) && (p - x) >= minimum) z = p - x;
        }
        while (*p && *p != '|') p++;
    } while (*p++);
    return z;
...
static size_t canonical(const Lc_language_t *lp, const Lc_territory_t *tp, const Lc_charset_t *cp,
                        const Lc_attribute_list_t *ap, unsigned long flags, char *buf, size_t siz) {
    int c;
    int u;
    char *s;
    char *e;
    ...
    if (s < e) {
        if (tp && tp != &lc_territories[0]) {
            r = 0;
            if (lp) {
                if ((flags & (LC_abbreviated | LC_default)) && streq(lp->code, tp->code)) r = s;
                *s++ = '_';
            }
            if (flags & LC_verbose) {
                u = 1;
                ...
                *s = 0;
                if ((p = setlocale(LC_MESSAGES, 0)) && (p = strdup(p))) {
                    if (!setlocale(LC_MESSAGES, buf)) {
                        *r = 0;
                        if (!setlocale(LC_MESSAGES, buf)) *r = '_';
                    }
...
size_t lccanon(Lc_t *lc, unsigned long flags, char *buf, size_t siz) {
    if ((flags & LC_local) && (!lc->language || !(lc->language->flags & (LC_debug | LC_default)))) {
        ...
        if (lc->index && GetLocaleInfo(lc->index, LOCALE_SENGLANGUAGE, lang, sizeof(lang)) &&
...
Lc_t *lcmake(const char *name) {
    ...
    if (!(t = name) || !*t) return &default_lc;
    for (lc = lcs; lc; lc = lc->next)
        if (!strcasecmp(t, lc->code) || !strcasecmp(t, lc->name)) return lc;
    ...
    *s = 0;
    ...
    if (n == 2)
        ...
        for (lp = lc_languages; lp->code && (!lp->alternates || !match(s, lp->alternates, n, 0));
             lp++)
            ;
        if (!lp->code) {
            c = s[2];
    ...
    if (!lp || !lp->code) {
        ...
        if (!lp || !lp->code) {
            ...
            if (!lp || !lp->code) {
                ...
                s = (char *)name;
                z = strlen(s) + 1;
                if (!(lp = newof(0, Lc_language_t, 1, z))) return 0;
                name = ((Lc_language_t *)lp)->code = ((Lc_language_t *)lp)->name =
                    (const char *)(lp + 1);
                ...
                for (tp = lc_territories; tp->code; tp++)
                    if (streq(s, tp->code)) {
                        if (lp != &lc_languages[0]) {
                            for (i = 0; i < elementsof(tp->languages) && lp != tp->languages[i];
                                 i++)
                                ;
                            if (i >= elementsof(tp->languages)) tp = 0;
                        }
                        break;
                    }
            } else {
                for (tp = lc_territories; tp->code; tp++)
                    if (match(s, tp->name, 3, 0)) {
                        for (i = 0; i < elementsof(tp->languages) && lp != tp->languages[i]; i++)
                            ;
                        if (i < elementsof(tp->languages)) break;
                    }
            }
            if (tp && !tp->code) tp = 0;
        }
            ...
            *s = 0;
            ...
            if (!cp->code) {
                ...
                if (i >= elementsof(lp->attributes) &&
                    (ap = newof(0, Lc_attribute_t, 1, sizeof(Lc_attribute_list_t) + s - w + 1))) {
    ...
    if ((ast.locale.set & AST_LC_debug) && !(ast.locale.set & AST_LC_internal))
        sfprintf(sfstderr, "locale make %s charset_name=%s cp=%s ppa=%s lp=%s\n", name,
                 charset_name, cp ? cp->code : 0, ppa, lp->charset);
#endif
    if (!cp || !cp->code) cp = ppa ? ppa : lp->charset;
mapped:
...
override:
    ...
    if (!(lc = newof(0, Lc_t, 1, n + z))) return 0;
    ...
    if ((ast.locale.set & AST_LC_debug) && !(ast.locale.set & AST_LC_internal))
        sfprintf(sfstderr,
                 ...
                 lc->name, lc->code, "", lc->language->name, lc->territory->name, lc->charset->code,
...
Lc_t *lcscan(Lc_t *lc) {
    ...
    if (!(ls = (Lc_scan_t *)lc)) {
        if (!(ls = newof(0, Lc_scan_t, 1, 0))) return 0;
...
#endif
    canonical(ls->lc.language, ls->lc.territory, ls->lc.charset, ls->lc.attributes, 0, ls->buf,
              sizeof(ls->buf));
    return (Lc_t *)ls;
}
