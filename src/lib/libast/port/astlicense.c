...
#define END(b) \
    (*((b)->nxt >= (b)->end ? ((b)->nxt = (b)->end - 1) : (b)->nxt) = 0, (b)->nxt - (b)->buf)
...
static const Item_t key[] = {
    KEY("author"),      KEY("class"),  KEY("company"), KEY("component"),     KEY("contributor"),
    KEY("corporation"), KEY("domain"), KEY("id"),      KEY("incorporation"), KEY("license"),
    KEY("location"),    KEY("name"),   KEY("notice"),  KEY("organization"),  KEY("package"),
    KEY("parent"),      KEY("query"),  KEY("since"),   KEY("source"),        KEY("start"),
    KEY("type"),        KEY("url"),    KEY("urlmd5"),  KEY("version"),       {0}};
...
static int lookup(const Item_t *item, const char *name, int size) {
    int c;
    int i;
...
static void copy(Buffer_t *b, char *s, int n) {
    if (n < 0) n = strlen(s);
    while (n--) PUT(b, *s++);
}
...
static void comment(Notice_t *notice, Buffer_t *b, char *s, int n, int u) {
    int i;
    int m;
    int x;
    int cc;
    ...
    if (!s) {
        if (n) {
            ...
            for (i = 0; i < COMDATA; i++) PUT(b, cc);
            PUT(b, notice->cc[n > 0 ? 1 : 2]);
        } else
            s = "";
    }
    if (s) {
        if (n > COMDATA) n = COMDATA;
        PUT(b, cc);
        m = (u < 0) ? 1 : (COMDATA - n) / 2;
        if ((x = COMDATA - m - n) < 0) n--;
        while (m-- > 0) PUT(b, ' ');
        while (n-- > 0) {
            i = *s++;
            if (u > 0 && i >= 'a' && i <= 'z') i = i - 'a' + 'A';
            PUT(b, i);
        }
        while (x-- > 0) PUT(b, ' ');
        PUT(b, cc);
...
static void expand(Notice_t *notice, Buffer_t *b, const Item_t *item) {
    char *t;
    char *e;
    int q;
    char *x;
...
static void copyright(Notice_t *notice, Buffer_t *b) {
    char *x;
    char *t;
    time_t clock;
    ...
    if ((x = notice->item[START].data) && strncmp(t, x, 4) < 0) t = x;
...
static int push(Stack_t *sp, char *file, char *parent, char *info, int size, Buffer_t *buf) {
    char *s;
    char *t;
    int i;
    int n;
...
int astlicense(char *p, int size, char *file, char *options, int cc1, int cc2, int cc3) {
    char *s;
    char *v;
    char *x;
    int c;
    ...
    if (file && *file) {
        if (push(&input[++level], file, 0, data, &info[sizeof(info)] - data, &buf)) return -1;
    ...
    notice.cc[1] = cc2;
    notice.cc[2] = cc3;
    for (i = 0; i < ITEMS; i++) notice.item[i].data = 0;
                ...
                *s = 0;
                ...
                if (*file) {
                    ...
                    if (++level >= (sizeof(input) / sizeof(input[0])) ||
                        push(&input[level], file, input[level - 1].file, data,
                             &info[sizeof(info)] - data, &buf))
                        return -1;
                    data += input[level].size;
                    s = input[level].info;
                }
            ...
            if (c == '=' || first) {
                ...
                if (contributor) {
                    ...
                    if (i < IDS) {
                        ...
                        if (notice.ids <= i) notice.ids = i + 1;
                    }
                } else if (h == QUERY) {
    ...
    if (!k) return 0;
    if (notice.type == INLINE && (!notice.verbose || !notice.item[NOTICE].data)) return 0;
    if (notice.type != USAGE) {
        if (!notice.type) notice.type = SPECIAL;
        ...
        if (notice.type >= OPEN) {
            ...
            comment(&notice, &buf, BUF(&tmp), USE(&tmp), 0);
            if (notice.type >= SPECIAL) COMMENT(&notice, &buf, "All Rights Reserved", 0);
        }
        if (notice.type == CPL || notice.type == EPL) {
            ...
            if (notice.item[URL].data) {
                ...
                copy(&tmp, "at the ", -1);
                if (i >= 0) expand(&notice, &tmp, &notice.item[i]);
            ...
            COMMENT(&notice, &buf, "modify it under the terms of the GNU General Public License",
                    0);
            ...
            comment(&notice, &buf, NULL, 0, 0);
            COMMENT(&notice, &buf, "Redistribution and use in source and binary forms, with or",
                    -1);
            COMMENT(&notice, &buf,
                    "without modification, are permitted provided that the following", -1);
            COMMENT(&notice, &buf, "conditions are met:", -1);
            comment(&notice, &buf, NULL, 0, 0);
            COMMENT(&notice, &buf, "   1. Redistributions of source code must retain the above",
                    -1);
            COMMENT(&notice, &buf, "      copyright notice, this list of conditions and the", -1);
            COMMENT(&notice, &buf, "      following disclaimer.", -1);
            comment(&notice, &buf, NULL, 0, 0);
            COMMENT(&notice, &buf, "   2. Redistributions in binary form must reproduce the above",
                    -1);
            COMMENT(&notice, &buf, "      copyright notice, this list of conditions and the", -1);
            COMMENT(&notice, &buf, "      following disclaimer in the documentation and/or other",
                    -1);
            ...
            COMMENT(&notice, &buf, "CONTRIBUTORS \"AS IS\" AND ANY EXPRESS OR IMPLIED WARRANTIES,",
                    -1);
            ...
            COMMENT(&notice, &buf, "TO, PROCUREMENT OF SUBSTITUTE GOODS OR SERVICES; LOSS OF USE,",
                    -1);
            COMMENT(&notice, &buf,
            ...
            COMMENT(&notice, &buf, "OR TORT (INCLUDING NEGLIGENCE OR OTHERWISE) ARISING IN ANY WAY",
                    -1);
            ...
            COMMENT(&notice, &buf, "warranty. In no event will the authors be held liable for any",
                    -1);
            COMMENT(&notice, &buf, "damages arising from the use of this software.", -1);
            comment(&notice, &buf, NULL, 0, 0);
            COMMENT(&notice, &buf, "Permission is granted to anyone to use this software for any",
                    -1);
            COMMENT(&notice, &buf,
            ...
            comment(&notice, &buf, NULL, 0, 0);
            COMMENT(&notice, &buf, " 1. The origin of this software must not be misrepresented;",
                    -1);
            COMMENT(&notice, &buf,
            ...
            comment(&notice, &buf, NULL, 0, 0);
            COMMENT(&notice, &buf, " 2. Altered source versions must be plainly marked as such,",
                    -1);
            COMMENT(&notice, &buf, "    and must not be misrepresented as being the original", -1);
            COMMENT(&notice, &buf, "    software.", -1);
            comment(&notice, &buf, NULL, 0, 0);
            COMMENT(&notice, &buf, " 3. This notice may not be removed or altered from any source",
                    -1);
            COMMENT(&notice, &buf, "    distribution.", -1);
            comment(&notice, &buf, NULL, 0, 0);
        } else if (notice.type == MIT) {
            comment(&notice, &buf, NULL, 0, 0);
            COMMENT(&notice, &buf, "Permission is hereby granted, free of charge, to any person",
                    0);
            ...
            COMMENT(&notice, &buf, "the rights to use, copy, modify, merge, publish, distribute,",
                    0);
            ...
            comment(&notice, &buf, NULL, 0, 0);
            COMMENT(&notice, &buf, "The above copyright notice and this permission notice shall",
                    0);
            COMMENT(&notice, &buf, "be included in all copies or substantial portions of the", 0);
            COMMENT(&notice, &buf, "Software.", 0);
            comment(&notice, &buf, NULL, 0, 0);
            COMMENT(&notice, &buf, "THE SOFTWARE IS PROVIDED \"AS IS\", WITHOUT WARRANTY OF ANY",
                    0);
            ...
            if (notice.type == PROPRIETARY) {
                ...
                if (notice.item[URL].data) {
                    ...
                    if (SIZ(&tmp)) comment(&notice, &buf, BUF(&tmp), USE(&tmp), 1);
                ...
                if (notice.item[CORPORATION].data || notice.item[COMPANY].data) {
                    ...
                    if ((notice.item[PARENT].size +
                         (notice.item[CORPORATION].data
                              ? (notice.item[CORPORATION].size + notice.item[INCORPORATION].size)
                              : notice.item[COMPANY].size)) >= (COMLONG - 6))
                        comment(&notice, &buf, BUF(&tmp), USE(&tmp), 0);
                    else
                        PUT(&tmp, ' ');
                    if (notice.item[PARENT].data) {
        ...
        if (v = notice.item[NOTICE].data) {
            ...
            if (item.size) comment(&notice, &buf, NULL, 0, 0);
        }
        if (notice.item[ORGANIZATION].data) {
            ...
            if (SIZ(&tmp)) comment(&notice, &buf, BUF(&tmp), USE(&tmp), 0);
    ...
    if (v = notice.item[AUTHOR].data) {
        ...
        for (;;) {
            ...
            h = 0;
            for (i = 0; i < notice.ids; i++)
                if (q || item.size == notice.id[i].name.size &&
                             !strncmp(item.data, notice.id[i].name.data, item.size)) {
                    h = 1;
                    if (notice.type == USAGE) {
                        copy(&buf, "[-author?", -1);
        ...
        if (k > 0) comment(&notice, &buf, NULL, 0, 0);
    }
    if (notice.type == USAGE) {
