...
static void detrie(Trie_node_t *x, Sfio_t *sp, char *b, char *p, char *e, int delimiter) {
    Trie_node_t *y;
    char *o;
    int k;
...
static int decomp(Rex_t *e, Rex_t *parent, Sfio_t *sp, int type, int delimiter, regflags_t flags) {
    ...
    do {
        switch (e->type) {
            case REX_ALT:
                if (decomp(e->re.group.expr.binary.left, e, sp, type, delimiter, flags)) return 1;
                sfputc(sp, '|');
                if (e->re.group.expr.binary.right &&
                    decomp(e->re.group.expr.binary.right, e, sp, type, delimiter, flags))
                    return 1;
                break;
            case REX_BACK:
                sfprintf(sp, "\\%d", e->lo);
            ...
            case REX_REP:
                if (type >= SRE) {
                    ...
                    if (e->hi == RE_DUP_INF) {
                        ...
                        c = 0;
                    if (e->re.group.expr.rex && e->re.group.expr.rex->type == REX_GROUP) c = 0;
                }
                switch (e->type) {
                    case REX_REP:
                        if (decomp(e->re.group.expr.rex, e, sp, type, delimiter, flags)) return 1;
                        break;
                    case REX_CLASS:
                        sfputc(sp, '[');
                        nb = ne = ib = ie = -2;
            ...
            case REX_CONJ:
                if (decomp(e->re.group.expr.binary.left, e, sp, type, delimiter, flags)) return 1;
                sfputc(sp, '&');
                if (decomp(e->re.group.expr.binary.right, e, sp, type, delimiter, flags)) return 1;
                break;
            case REX_GROUP:
                if (type >= SRE && parent->type != REX_REP) sfputc(sp, '@');
                meta(sp, '(', type, 1, delimiter);
                if (decomp(e->re.group.expr.rex, e, sp, type, delimiter, flags)) return 1;
                meta(sp, ')', type, 1, delimiter);
                break;
            ...
            case REX_GROUP_COND:
                ...
                if (q) {
                    ...
                    if (q->re.group.expr.binary.right &&
                        decomp(q->re.group.expr.binary.right, q, sp, type, delimiter, flags))
                        return 1;
                }
                meta(sp, ')', type, 1, delimiter);
                break;
...
size_t regdecomp(regex_t *p, regflags_t flags, char *buf, size_t n) {
    Sfio_t *sp;
    char *s;
    int type;
    int delimiter;
    ...
    if (flags == (regflags_t)~0) flags = p->env->flags;
    ...
    if (decomp(p->env->rex, p->env->rex, sp, type, delimiter, flags))
        ...
        if (delimiter > 0) sfputc(sp, delimiter);
