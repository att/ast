...
static int subold(Sfio_t *dp, const char *op, const char *sp, size_t nmatch, regmatch_t *match,
                  regflags_t flags, int sre) {
    int c;
    char *s;
    char *e;
    const char *b;
    ...
    for (;;) {
        ...
        if (sre) {
            if (c < 0 || *sp != ')') {
                for (; b < sp; b++) sfputc(dp, *b);
                continue;
            }
            sp++;
        }
        if (c >= nmatch) return REG_ESUBREG;
...
int regsub(const regex_t *p, Sfio_t *dp, const char *op, const char *sp, size_t nmatch,
           regmatch_t *match, regflags_t flags) {
    int m;
    int r;
    int sre;

    if ((p->env->flags & REG_NOSUB) || !nmatch) return fatal(p->env->disc, REG_BADPAT, NULL);
    m = (flags >> 16) & 0x3fff;
    ...
    do {
        ...
        op += match->rm_eo;
             ...
             !(r = regexec(p, op, nmatch, match,
                           p->env->flags | (match->rm_so == match->rm_eo ? REG_ADVANCE : 0))));
    if (r && r != REG_NOMATCH) return fatal(p->env->disc, r, NULL);
    sfputr(dp, op, -1);
    return 0;
}
