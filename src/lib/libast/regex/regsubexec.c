...
static int sub(const regex_t *p, regsub_t *b, const char *ss, regsubop_t *op, size_t nmatch,
               regmatch_t *match) {
    char *s;
    char *e;
    int c;
    ...
    for (;; op++) {
        switch (op->len) {
            case -1:
                break;
            case 0:
                if (op->off >= nmatch) return REG_ESUBREG;
                if ((c = match[op->off].rm_so) < 0) continue;
                s = (char *)ss + c;
                if ((c = match[op->off].rm_eo) < 0) continue;
                e = (char *)ss + c;
...
int regsubexec(const regex_t *p, const char *s, size_t nmatch, regmatch_t *match) {
    int c;
    regsub_t *b;
    const char *e;
    int m;
    ...
    for (;;) {
        ...
        if (c = regnexec(p, s, e - s, nmatch, match,
                         p->env->flags | (match->rm_so == match->rm_eo ? REG_ADVANCE : 0))) {
            if (c != REG_NOMATCH) return fatal(p->env->disc, c, NULL);
            break;
        }
