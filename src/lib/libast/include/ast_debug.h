...
static void *_ast_newof(void *_ast_p, ssize_t _ast_t, ssize_t _ast_n, ssize_t _ast_x,
    ...
    r = newof(p, t, n, x);
    error(-2, "%s:%lu: newof(%p,%zd,%zd,%zd)=%p", _ast_f, _ast_l, _ast_p, _ast_t, _ast_n, _ast_x,
...
static void *_ast_oldof(void *_ast_p, ssize_t _ast_t, ssize_t _ast_n, ssize_t _ast_x,
                        const char *_ast_f, unsigned long _ast_l) {
    void *_ast_r;

    r = oldof(p, t, n, x);
    error(-2, "%s:%lu: oldof(%p,%zd,%zd,%zd)=%p", _ast_f, _ast_l, _ast_p, _ast_t, _ast_n, _ast_x,
          _ast_r) return _ast_r;
}
