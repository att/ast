        ...
        switch (pc = mbgetchar(p)) {
            ...
            case '!':
                if (pc == '(' || *p == '(') {
                    ...
                    if (pc == '*' || pc == '?' || pc == '+' && oldp == r) {
                        if (onematch(mp, g, s, p, e, NULL, flags)) return 1;
                        if (!sc || !getsource(s, e)) {
                    ...
                    switch (pc = mbgetchar(p)) {
                        ...
                        case ')':
                            ...
                            if (!pc &&
                                (!mp->best.next_s ||
                                 (flags & STR_MAXIMAL) && mp->current.next_s > mp->best.next_s ||
                                 !(flags & STR_MAXIMAL) && mp->current.next_s < mp->best.next_s))
                                mp->best = mp->current;
                            return 1;
                        case '\\':
                    ...
                    p = oldp;
                    for (;;) {
                        if ((n || pc == sc) && onematch(mp, g, olds, p, e, NULL, flags)) return 1;
                        if (!sc) return 0;
                        olds = s;
                        sc = getsource(s, e);
            ...
            case ')':
                ...
                if (!pc && (!mp->best.next_s || (flags & STR_MAXIMAL) && olds > mp->best.next_s ||
...
static int grpmatch(Match_t *mp, int g, char *s, char *p, char *e, int flags) {
    char *a;
