...
static int rofs(const char *path) {
    struct statvfs vfs;
    struct stat st;

    if (!statvfs(path, &vfs)) {
#if defined(ST_RDONLY)
        if (vfs.f_flag & ST_RDONLY) return 1;
#endif
...
char *pathprobe_20100601(const char *lang, const char *tool, const char *aproc, int op, char *path,
                         size_t pathsize, char *attr, size_t attrsize) {
    ...
    if (op >= 0 && !stat(path, &st)) {
                                ...
                                if (pp) {
                                    ...
                                    *k = 0;
                                    if (strcmp(ver, e)) {
                                        force = 1;
                                        error(0,
                                              "probe processor %s version \"%s\" changed -- "
                                              "expected \"%s\"",
                                              proc, ver, e);
                                    }
                                    procclose(pp);
        ...
        if (op >= 0 && (st.st_mode & S_IWUSR)) {
            if (op == 0)
                error(0,
                      "%s probe information for %s language processor %s must be manually "
                      "regenerated",
                      tool, lang, proc);
            op = -1;
            force = 0;
