...
int pathcheck(const char *package, const char *tool, Pathcheck_t *pc) {
    ...
    if (strmatch(tool, PARANOID) && environ && (s = *environ) && *s++ == '_' && *s++ == '=' &&
        !stat(s, &st)) {
        unsigned long n;
        unsigned long o;
        Sfio_t *sp;
        ...
        if (n > o && (n - o) > (unsigned long)(60 * 60 * 24 * 90) &&
            ...
            while (n++ < 64 && (s = sfgetr(sp, '\n', 0)))
                if (strmatch(s, PARANOID_PAY)) {
                    error(1,
                          "licensed for external use -- %s employees should contact %s for the "
                          "internal license",
                          PARANOID_COMPANY, PARANOID_MAIL);
                    break;
