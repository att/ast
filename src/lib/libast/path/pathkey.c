...
char *pathkey_20100601(const char *lang, const char *tool, const char *apath, char *key,
    ...
    if (tool && streq(tool, "mam")) {
        for (n = 0; *path; path++) n = n * 0x63c63cd9L + *path + 0x9c39c33dL;
        k = key;
        for (n &= 0xffffffffL; n; n >>= 4) *k++ = let[n & 0xf];
        *k = 0;
    } else {
        ...
        if (attr && (getpreroot(attr, path) || getpreroot(attr, NULL))) attr += strlen(attr);
#else
        ...
        c = 0;
        usr[c++] = "OBJTYPE";
        if (!(k = getenv("PROBE_ATTRIBUTES"))) k = getenv("VERSION_ENVIRONMENT");
        if (k)
