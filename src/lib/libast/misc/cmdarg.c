...
Cmdarg_t *cmdopen(char **argv, int argmax, int size, const char *argpat, int flags) {
    Cmddisc_t disc;
...
Cmdarg_t *cmdopen_20110505(char **argv, int argmax, int size, const char *argpat, int flags,
                           Error_f errorf) {
    Cmddisc_t disc;
...
Cmdarg_t *cmdopen_20120411(char **argv, int argmax, int size, const char *argpat, Cmddisc_t *disc) {
    Cmdarg_t *cmd;
    int n;
    char **p;
    char *s;
    ...
    if (*argv) {
        ...
        argc = 0;
    for (p = environ; *p; p++) n += sizeof(char **) + strlen(*p) + 1;
    if ((x = strtol(astconf("ARG_MAX", NULL, NULL), NULL, 0)) <= 0) x = ARG_MAX;
    if (size <= 0 || size > x) size = x;
    sh = pathshell();
    m = n + (argc + 4) * sizeof(char **) + strlen(sh) + 1;
    m = roundof(m, sizeof(char **));
    if (size < m) {
        if (disc->errorf) (*disc->errorf)(NULL, sh, 2, "size must be at least %d", m);
        return 0;
    }
    if ((m = x / 10) > 2048) m = 2048;
    if (size > (x - m)) size = x - m;
    n = size - n;
    m = ((disc->flags & CMD_INSERT) && argpat) ? (strlen(argpat) + 1) : 0;
    if (!(cmd = newof(0, Cmdarg_t, 1, n + m))) {
        if (disc->errorf) (*disc->errorf)(NULL, sh, ERROR_SYSTEM | 2, "out of space");
        return 0;
    }
    cmd->id = lib;
    cmd->disc = disc;
    cmd->errorf = disc->errorf;
    if (!(cmd->runf = disc->runf)) cmd->runf = cmdrun;
    c = n / sizeof(char **);
    if (argmax <= 0 || argmax > c) argmax = c;
    s = cmd->buf;
    if (!(exe = argv[0])) {
        exe = *(argv = (char **)echo);
        cmd->echo = 1;
        ...
        if (!pathpath(exe, NULL, PATH_REGULAR | PATH_EXECUTE, s, n + m)) {
            ...
            if (disc->flags & CMD_EXIT) (*error_info.exit)(n);
            free(cmd);
            return 0;
        }
        exe = s;
...
int cmdflush(Cmdarg_t *cmd) {
    ...
    if (cmd->echo) {
        ...
        if (cmd->flags & CMD_EXIT) (*error_info.exit)(n);
    } else if (n >= EXIT_NOTFOUND - 1) {
        if (cmd->flags & CMD_EXIT) (*error_info.exit)(n);
    } else if (!(cmd->flags & CMD_IGNORE)) {
        if (n == EXIT_QUIT && (cmd->flags & CMD_EXIT)) (*error_info.exit)(2);
        if (n) error_info.errors++;
    }
    return n;
}
...
int cmdarg(Cmdarg_t *cmd, const char *file, int len) {
    int i;
    int r;
    ...
    if (len > 0) {
        ...
        *cmd->nextarg++ = cmd->nextstr;
        ...
        cmd->nextstr[len] = 0;
        cmd->argcount++;
        if (cmd->argcount >= cmd->argmax && (i = cmdflush(cmd)) > r) r = i;
    } else
        cmd->argcount += len;
    return r;
}
