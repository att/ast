...
static int lower(int c) { return isupper(c) ? tolower(c) : c; }
...
static void dropcap(Cap_t *cap) {
    while (cap->att.next) {
        Att_t *att = cap->att.next;
...
int mimeset(Mime_t *mp, char *s, unsigned long flags) {
    Ent_t *ent;
    Cap_t *cap;
    Att_t *att;
    char *t;
        ...
        if (*v) {
            *v++ = 0;
            do {
                ...
                if (t <= v && (!cap || !k)) break;
                if (!cap) {
                    if (!(cap = newof(0, Cap_t, 1, strlen(v) + 1))) return -1;
                    if (*t) *t++ = 0;
                    tta = &cap->att;
                    tta->name = "default";
                    ...
                    if (*t) *t++ = 0;
                    if (!(att = newof(0, Att_t, 1, 0))) return -1;
                    x = stpcpy(att->name = x, k) + 1;
                    x = stpcpy(att->value = x, v) + 1;
                    tta = tta->next = att;
                    if (!strcasecmp(k, "test")) cap->test = att->value;
                }
            } while (*(v = t));
        }
                    ...
                    if ((!cap->test && !dup->test) ||
                        ...
                        if (flags & MIME_REPLACE) {
                            if (pud)
                                ...
                                ent->cap = cap;
                            if (!(cap->next = dup->next)) ent->pac = cap;
                            cap = dup;
                        }
                        dropcap(cap);
                        return 0;
...
int mimeload(Mime_t *mp, const char *file, unsigned long flags) {
    char *s;
    char *t;
    char *e;
    int n;
    ...
    if (!(s = (char *)file)) {
        flags |= MIME_LIST;
        if (!(s = getenv(MIME_FILES_ENV))) s = MIME_FILES;
    }
    for (;;) {
        if (!(flags & MIME_LIST)) {
            e = NULL;
            ...
            e = strchr(s, ':');
            if (e) {
                // Ok, so ~ won't work for the last list element we do it for MIME_FILES_ENV anyway.
                ...
                if (!(s = sfstruse(mp->buf))) return -1;
            }
        }
        fp = tokline(s, SF_READ, NULL);
        if (fp) {
            while ((t = sfgetr(fp, '\n', 1))) {
                if (mimeset(mp, t, flags)) break;
            }
            sfclose(fp);
...
static Ent_t *find(Mime_t *mp, const char *type) {
    char *lp;
    char *rp;
    char *rb;
    char *rv;
    ...
    if ((ent = (Ent_t *)dtmatch(mp->cap, type)) || !(rp = strchr(lp = (char *)type, '/')) ||
...
int mimelist(Mime_t *mp, Sfio_t *fp, const char *pattern) {
    Ent_t *ent;
    Walk_t ws;
...
static int arg(Parse_t *pp, int first) {
    char *s;
    int c;
    int q;
    int x;
    ...
    if (first >= 0 && pp->name.size > 0 && pp->name.data[pp->name.size - 1] == ':') return 0;
...
static char *expand(Mime_t *mp, char *s, const char *name, const char *type, const char *opts) {
    char *t;
    char *v;
    int c;
    int e;
...
char *mimeview(Mime_t *mp, const char *view, const char *name, const char *type, const char *opts) {
    Ent_t *ent;
    Cap_t *cap;
    Att_t *att;
    char *s;
    ...
    if (ent) {
        ...
        if (!view || strcasecmp(view, "test"))
            while (cap->test) {
                s = expand(mp, cap->test, name, type, opts);
                if (s) {
                    Parse_t a1;
                    Parse_t a2;
                    Parse_t a3;
                    ...
                    if (arg(&a1, -1)) {
                        ...
                        if ((a1.name.size == 6 && strneq(a1.name.data, "strcmp", 6)) ||
                            ...
                            if (a2.name.size != a3.name.size)
                                ...
                                c ^= (a1.name.size == 6 ? strncmp : strncasecmp)(
                                         a2.name.data, a3.name.data, a2.name.size) == 0;
                            if (c) break;
                            goto skip;
                            ...
                            if (*a1.name.data == '`' && *(a1.name.data + a1.name.size - 1) == '`') {
                                ...
                                if (!arg(&a1, -1) || a1.name.size != 4 ||
                                    !strneq(a1.name.data, "echo", 4) || !arg(&a1, -1))
                                    goto lose;
                                a4.next = a1.next;
                                if (!arg(&a4, 1) || a4.name.size < 21 ||
                                    !strneq(a4.name.data, "| tr '[A-Z]' '[a-z]'`", 21))
                                    goto lose;
                            } else
                                a4.name.size = 0;
                            c = *a2.name.data == '!';
                            if (a1.name.size != a3.name.size)
                                ...
                                c ^= (a4.name.size ? strncasecmp : strncmp)(
                                         a1.name.data, a3.name.data, a1.name.size) == 0;
                            if (c) break;
                            goto skip;
                        }
                    }
...
int mimehead(Mime_t *mp, void *tab, size_t num, size_t siz, char *s) {
    void *p;
    char *e;
    Parse_t pp;
    Mimevalue_f set;
...
Mime_t *mimeopen(Mimedisc_t *disc) {
    Mime_t *mp;

    if (!(mp = newof(0, Mime_t, 1, 0))) return 0;
    mp->id = lib;
    mp->disc = disc;
...
int mimeclose(Mime_t *mp) {
    if (mp) {
        if (mp->buf) sfclose(mp->buf);
        if (mp->cap) dtclose(mp->cap);
        if (mp->freef) (*mp->freef)(mp);
        free(mp);
    }
    return 0;
}
