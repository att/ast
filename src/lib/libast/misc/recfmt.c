...
Recfmt_t recfmt(const void *buf, size_t size, off_t total) {
    unsigned char *s;
    unsigned char *t;
    Sample_t *q;
    unsigned int *h;
    ...
    while ((k = (t - s)) >= 4 && !s[2] && !s[3]) {
        if ((i = (s[0] << 8) | s[1]) > k) break;
        s += i;
    }
    if (!k || size > 2 * k) return REC_V_TYPE(4, 0, 2, 0, 1);
    s = (unsigned char *)buf;
    ...
    for (i = 0; i < elementsof(terminators); i++)
        if ((t = (unsigned char *)memchr((void *)s, k = terminators[i], size / 2)) &&
            ...
            if (n) return REC_D_TYPE(terminators[i]);
        }
    ...
    if (!(q = newof(0, Sample_t, 1, 0))) return REC_N_TYPE();
    x = 0;
    for (i = 0; i < size; i++) {
        h = q->hit + s[i];
        m = i - *h;
