...
#define VASIGNED(a, n) \
    (((n) > SIZEOF(long)) ? va_arg(a, int64_t) : ((n) ? va_arg(a, long) : va_arg(a, int)))
#define VAUNSIGNED(a, n)                                 \
    (((n) > SIZEOF(unsigned long)) ? va_arg(a, uint64_t) \
                                   : ((n) ? va_arg(a, unsigned long) : va_arg(a, unsigned int)))

ssize_t debug_vsprintf(char *buf, size_t siz, const char *format, va_list ap) {
    int c;
    char *p;
    char *e;
    int64_t n;
        ...
        switch (c = *format++) {
            ...
            case '%':
				...
				else if (streq(t, "foo"))
				{
                    ...
                    hex:
                        do
                            *b++ = ((c = (char)(u & 0xf)) >= 0xa) ? c - 0xa + 'a' : c + '0';
                        while (u >>= 4);
                        break;
                    case 'z':
                        u = VAUNSIGNED(ap, n);
                        do
                            *b++ = ((c = (char)(u & 0x1f)) >= 0xa) ? c - 0xa + 'a' : c + '0';
                        while (u >>= 5);
                        break;
                    default:
                        if (p >= e) goto done;
...
ssize_t debug_sprintf(char *buf, size_t siz, const char *format, ...) {
    ssize_t n;
    va_list ap;
...
ssize_t debug_vprintf(int fd, const char *format, va_list ap) {
    ssize_t n;
    char buf[4 * 1024];
...
ssize_t debug_printf(int fd, const char *format, ...) {
    ssize_t n;
    va_list ap;
...
void debug_fatal(const char *file, int line, const char *text) {
    char *s;
#ifdef SIG_BLOCK
    sigset_t ss;
    sigfillset(&ss);
