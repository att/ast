    ...
    switch (type) {
        case VM_NOMEM:
            ...
            error(ERROR_SYSTEM | 3,
                  "storage allocator out of space on %zu byte request ( region %zu segments %zu "
                  "busy %zu:%zu free %zu:%zu )",
                  (size_t)obj, st.extent, st.n_seg, st.n_busy, st.s_busy, st.n_free, st.s_free);
            return -1;
#ifdef VM_BADADDR
        case VM_BADADDR:
...
void memfatal(void) { (void)memfatal_20130509(NULL); }
...
int memfatal_20130509(Vmdisc_t *disc) {
    if (!disc) {
        malloc(0);
        if (!(disc = vmdisc(Vmregion, NULL))) return -1;
    }
