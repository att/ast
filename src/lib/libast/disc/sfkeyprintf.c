...
static char *getfield(Field_t *f, int restore) {
    char *s;
    int n;
    int c;
    int lp;
...
static int getfmt(Sfio_t *sp, void *vp, Sffmt_t *dp) {
    ...
    switch (fp->fmt.fmt) {
        ...
        case 's':
                            ...
                            if (strmatch(s, a)) {
                                ...
                                *(v - 1) = d;
                                ...
                                for (h = 0; h < elementsof(fmt.tmp); h++)
                                    if (fmt.tmp[h]) sfclose(fmt.tmp[h]);
                                h = 1;
                                break;
                            }
                            *(v - 1) = d;
                    ...
                    case FMT_edit:
                        for (x = 0; *f.next; x ^= 1) {
                            if (fp->re[x])
                                ...
                                fp->re[x] = &fp->red[x];
                            if (regcomp(fp->re[x], f.next, REG_DELIMITED | REG_NULL)) break;
                            f.next += fp->re[x]->re_npat;
                            if (regsubcomp(fp->re[x], f.next, NULL, 0, 0)) break;
                            f.next += fp->re[x]->re_npat;
                            if (!regexec(fp->re[x], s, elementsof(match), match, 0) &&
                                !regsubexec(fp->re[x], s, elementsof(match), match)) {
                                s = fp->re[x]->re_sub->re_buf;
                                if (fp->re[x]->re_sub->re_flags & REG_SUB_STOP) break;
                            }
                        }
                        h = 1;
                        break;
            ...
            value->s = s;
            if (fp->level == 1)
                while ((s = strchr(s, CC_esc)) && *(s + 1) == '[') do
                        fp->invisible++;
                    while (*s && !islower(*s++));
            break;
        case 'Z':
...
int sfkeyprintf_20000308(Sfio_t *sp, void *handle, const char *format, Sf_key_lookup_t lookup,
                         Sf_key_convert_t convert) {
    int i;
    int r;
    Fmt_t fmt;
    ...
    for (i = 0; i < elementsof(fmt.tmp); i++)
        if (fmt.tmp[i]) sfclose(fmt.tmp[i]);
    return r;
}

#undef sfkeyprintf

int sfkeyprintf(Sfio_t *sp, void *handle, const char *format, Sf_key_lookup_t lookup,
                Sf_key_convert_t convert) {
    int i;
    int r;
    Fmt_t fmt;
