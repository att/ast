...
Sfio_t *sfpopen(Sfio_t *f, const char *command, const char *mode) {
    ...
    /* sanity check */
    if (!command || !command[0] || !mode) return NULL;
    sflags = _sftype(mode, NULL, NULL, NULL);

    /* make pipes */
    parent[0] = parent[1] = child[0] = child[1] = -1;
    if (sflags & SF_RDWR) {
        if (syspipef(parent) < 0) goto error;
        if ((sflags & SF_RDWR) == SF_RDWR && syspipef(child) < 0) goto error;
    }
    ...
    switch ((pid = fork())) {
        default: /* in parent process */
            ...
            /* make the streams */
            if (!(f = sfnew(f, NULL, (size_t)SF_UNBOUND, parent[pkeep],
                            sflags | ((sflags & SF_RDWR) ? 0 : SF_READ))))
                goto error;
            if (sflags & SF_RDWR) {
                CLOSE(parent[!pkeep]);
        ...
        case 0: /* in child process */
            ...
            if ((sflags & SF_RDWR) == SF_RDWR && pkeep == child[ckeep])
                if ((child[ckeep] = sysdupf(pkeep)) < 0) _exit(EXIT_NOTFOUND);
