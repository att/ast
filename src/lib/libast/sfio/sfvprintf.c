...
#define SMnputc(f, c, n)                               \
    {                                                  \
        if ((o = SFNPUTC(f, c, n)) > 0) n_output += 1; \
        if (o != n) {                                  \
            SFBUF(f);                                  \
            goto done;                                 \
        }                                              \
...
loop_fmt:
    ...
    loop_flags: /* LOOP FOR \0, %, FLAGS, WIDTH, PRECISION, BASE, TYPE */
        switch ((fmt = *form++)) {
            ...
            case '-':
                ...
                goto loop_flags;
            case '0':
                if (!(flags & (SFFMT_LEFT | SFFMT_CENTER))) flags |= SFFMT_ZERO;
                goto loop_flags;
            case ' ':
            ...
            case '.':
                ...
                if (dot == 1) { /* so base can be defined without setting precis */
                    if (*form != '.' && !(flags & SFFMT_CHOP)) precis = 0;
                } else if (dot == 2) {
                    base = 0; /* for %s,%c */
                    v = form[0] == 'l' ? form[1] : form[0];
                    if (v == 'c' || v == 'C' || v == 's' || v == 'S') goto loop_flags;
        ...
        } else if (ft && ft->extf) /* extended processing */
        {
            ...
            } else /* extf did not output */
            {
                ...
                if (!(ft->flags & SFFMT_VALUE))
                    ...
                    if (size == sizeof(float)) argv.d = argv.f;
                } else if (_Sftype[fmt] & SFFMT_CHAR) {
                    if (base < 0) argv.i = (int)argv.c;
                }
            }
        } else {
        arg_list:
    ...
    do_output:
        ...
        if (flags &
            (SFFMT_FLOAT | SFFMT_LEFT)) { /* SFFMT_FLOAT: right padding for float precision */
