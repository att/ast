...
static void dumpflags(int flags) {
...
static void dumpbucket(Hash_table_t *tab, int flags) {
    Hash_bucket_t **sp;
    Hash_bucket_t *b;
    Hash_bucket_t **sx;
    int n;
    ...
    for (sp = tab->table; sp < sx; sp++) {
        n = 0;
        for (b = *sp; b; b = b->next)
            if (!(b->hash & HASH_DELETED) && (!(tab->flags & HASH_VALUE) || b->value)) n++;
        if (n) {
...
static void dumptable(Hash_table_t *tab, int flags) {
    Hash_table_t *scope;
    int level;
...
static void dumproot(Hash_root_t *root, int flags) {
    Hash_table_t *tab;
    ...
    if (root->local->compare)
        sfprintf(sfstderr, "compare=0x%08lx ", (unsigned long)root->local->compare);
    ...
    if (root->local->region)
        sfprintf(sfstderr, "region=0x%08lx handle=0x%08lx ", (unsigned long)root->local->region,
    ...
    sfprintf(sfstderr, "\n");
    for (tab = root->references; tab; tab = tab->next) dumptable(tab, flags);
}
...
void hashdump(Hash_table_t *tab, int flags) {
    ...
    if (tab)
        ...
        for (root = hash_info.list; root; root = root->next) dumproot(root, flags);
    sfsync(sfstderr);
}
