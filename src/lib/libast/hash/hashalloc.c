...
Hash_table_t *hashalloc(Hash_table_t *ref, ...) {
    Hash_table_t *tab;
    Hash_table_t *ret = 0;
    int internal;
    int n;
    ...
    if (!ref && n == HASH_region) {
        ...
        n = va_arg(ap, int);
        if (!(tab = (Hash_table_t *)(*region)(handle, NULL, sizeof(Hash_table_t), 0))) goto out;
    ...
    for (;;) {
        switch (n) {
            ...
            case 0:
                ...
                if (!tab->table) {
                    if (region) {
                        if (!(tab->table = (Hash_bucket_t **)(*region)(
                                  handle, NULL, sizeof(Hash_bucket_t *) * tab->size, 0)))
                            goto out;
                        memset(tab->table, 0, sizeof(Hash_bucket_t *) * tab->size);
                    } else if (!(tab->table = newof(0, Hash_bucket_t *, tab->size, 0)))
                        goto out;
                }
                if (!ref) {
