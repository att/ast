...
char *hashlook(Hash_table_t *tab, const char *name, long flags, const char *value) {
    Hash_bucket_t *b;
    unsigned int n;
    Hash_last_t *last;
    Hash_table_t *top;
    ...
    if ((flags & (HASH_LOOKUP | HASH_INTERNAL)) == (HASH_LOOKUP | HASH_INTERNAL)) {
        ...
        for (;;) {
            ...
            if (!(tab = tab->scope) || (flags & HASH_NOSCOPE)) return (0);
            n = i;
        }
    }
    tab->root->accesses++;
    ...
    if (name) {
        ...
        for (;;) {
            ...
            for (prev = 0, b = tab->table[n]; b; prev = b, b = b->next) {
                if (i == HASHVAL(b->hash) &&
                    ((b->hash & (HASH_DELETED | HASH_OPAQUED)) != HASH_DELETED ||
                    ...
                    if (!tab->root->local->compare) {
                        ...
                        if (!(*tab->root->local->compare)(hashname(b), name, tab->root->namesize))
                            goto found;
            ...
            if (!tab->scope ||
                (flags & (HASH_CREATE | HASH_INSTALL | HASH_NOSCOPE)) == HASH_NOSCOPE)
                break;
            tab = tab->scope;
            n = i;
        }
            ...
            switch (flags & (HASH_CREATE | HASH_DELETE | HASH_INSTALL | HASH_RENAME)) {
                        ...
                        if (tab->frozen || (b->hash & HASH_OPAQUED))
                            ...
                            if (tab->root->local->free && (tab->root->flags & HASH_BUCKET))
                                ...
                                if (tab->root->local->region)
                                    (*tab->root->local->region)(tab->root->local->handle, b, 0, 0);
                                else
                                    free(b);
                            }
                            if (name) {
                ...
                case HASH_RENAME:
                    if (tab != top || tab->frozen || (b->hash & (HASH_KEEP | HASH_OPAQUED)) ||
                        hashlook(top, value, (flags & (HASH_HASHED | HASH_INTERNAL)) | HASH_LOOKUP,
                                 NULL))
                        return (0);
                    name = (char *)b->name;
                        ...
                        if (b->name == ((char *)b + i) && strlen(b->name) <= m) {
                            ...
                            if (!(t = tab->root->local->region
                                          ? (char *)(*tab->root->local->region)(
                                                tab->root->local->handle, NULL, m, 0)
                                          : (char *)malloc(m)))
                                return (0);
...
create:
    ...
    if (flags & HASH_INSTALL) {
        ...
        if (!prev && (tab->flags & HASH_ALLOCATE)) {
            ...
            if (tab->root->local->region) {
                if (!(b = (Hash_bucket_t *)(*tab->root->local->region)(tab->root->local->handle,
                                                                       NULL, n + m, 0)))
                    return (0);
                memset(b, 0, n + m);
...
exists:
    ...
    switch (flags & (HASH_CREATE | HASH_VALUE)) {
        case HASH_CREATE | HASH_VALUE:
            if (tab->root->local->free && !(tab->root->flags & HASH_BUCKET) && b->value)
                (*tab->root->local->free)(b->value);
            if (value && tab->root->local->alloc)
                value = (*tab->root->local->alloc)((unsigned int)integralof(value));
            b->value = (char *)value;
