...
extern pid_t spawnve(const char *cmd, char *const argv[], char *const envv[]) {
...
extern pid_t spawnvpe(const char *name, char *const argv[], char *const envv[]) {
    ...
    if ((pid = spawnve(path, argv, envv)) >= 0) return pid;
    if (errno == ENOEXEC) {
        char **newargv;
        char **ov;
        char **nv;
...
extern pid_t spawnvp(const char *name, char *const argv[]) { return spawnvpe(name, argv, environ); }
...
extern pid_t spawnlp(const char *name, const char *arg, ...) {
    va_list ap;
    pid_t pid;
