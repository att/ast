...
static int key(void *handle, Sffmt_t *fp, const char *arg, char **ps, Sflong_t *pn) {
    ...
    if (!(s = fp->t_str) || streq(s, "size"))
        ...
        *ps = s = fmtbuf(32);
        if (state->vs.mode & VM_DBCHECK) s = stpcpy(s, "DBCHECK|");
        if (state->vs.mode & VM_DBABORT) s = stpcpy(s, "DBABORT|");
        if (state->vs.mode & VM_HEAPINIT) s = stpcpy(s, "HEAPINIT|");
        if (state->vs.mode & VM_MEMORYF) s = stpcpy(s, "MEMORYF|");
        if (s > *ps)
            *(s - 1) = 0;
        else
            strcpy(s, "0");
...
static int visit(Vmalloc_t *vm, void *addr, size_t size, Vmdisc_t *disc, void *handle) {
    State_t *state = (State_t *)handle;
...
int b_vmstate(int argc, char **argv, Shbltin_t *context) {
    int i;
    State_t state;
    ...
    argv += opt_info.index;
    if (error_info.errors || *argv) error(ERROR_USAGE | 4, "%s", optusage(NULL));
    if (!state.format) state.format = FORMAT;
