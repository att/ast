...
static void *labelcomp(const regex_t *re, const char *s, size_t len, regdisc_t *disc) {
    const char *e = s + len;
    uintmax_t n;
...
static int labelexec(const regex_t *re, void *data, const char *xstr, size_t xlen, const char *sstr,
                     size_t slen, char **snxt, regdisc_t *disc) {
    ((State_t *)disc)->hit = (Item_t *)data;
    return 0;
}

static int addre(State_t *state, char *s) {
    int c;
    int r;
    char *b;
    Item_t *x;
    ...
    if (state->label) {
        ...
        if (c) memcpy(x->string, b, c);
        x->string[c] = 0;
    }
    if (sfstrtell(state->tmp)) sfputc(state->tmp, '\n');
    if (state->words) {
        if (!(state->options & REG_AUGMENTED)) sfputc(state->tmp, '\\');
        sfputc(state->tmp, '<');
    }
    ...
    if (r && x) vmfree(state->vm, x);
    return r;
}

static int addstring(State_t *state, List_t *p, char *s) {
    Item_t *x;
...
static int compile(State_t *state) {
    int line;
    int c;
    int r;
    size_t n;
    ...
    if (state->number || state->before || state->after) state->byline = 1;
    for (x = state->patterns.head; x; x = x->next)
        if (addre(state, x->string)) return r;
    ...
    if (!regrecord(&state->re)) state->byline = 1;
    if (!state->label) {
...
static int hit(State_t *state, const char *prefix, int sep, int line, const char *s, size_t len) {
    regmatch_t *pos;
    ...
    state->hit->hits++;
    if (state->query || state->list) return -1;
    if (!state->count) {
    another:
        if ((pos = state->pos) && (state->before || state->after) &&
            (regnexec(&state->re, s, len, state->posnum, state->pos, 0) == 0) != state->match) {
            if (state->only) return 0;
            pos = 0;
        }
        if (state->prefix) sfprintf(sfstdout, "%s%c", prefix, sep);
        if (state->number && line) sfprintf(sfstdout, "%d%c", line, sep);
        if (state->label) sfprintf(sfstdout, "%s%c", state->hit->string, sep);
        if (!pos)
            ...
            do {
                ...
                s += state->pos[0].rm_eo;
                if (!(len -= state->pos[0].rm_eo)) break;
...
static int record(void *handle, const char *s, size_t len) {
    return hit((State_t *)handle, error_info.file, ':', 0, s, len);
}

static int list(Context_line_t *lp, int show, int group, void *handle) {
    if (group) sfputr(sfstdout, "--", '\n');
    return hit((State_t *)handle, error_info.file, show ? ':' : '-', lp->line, lp->data,
               lp->size - 1);
}

static int execute(State_t *state, Sfio_t *input, char *name, Shbltin_t *context) {
    char *s;
    char *file;
    Item_t *x;
    size_t len;
    ...
    if (state->buffer.noshare) sfset(input, SF_SHARE, 0);
    if (state->buffer.size) sfsetbuf(input, state->buffer.base, state->buffer.size);
    if (!name) name = "(standard input)"; /* posix! (ast prefers /dev/stdin) */
    file = error_info.file;
    ...
    if (state->before || state->after) {
        ...
        while (lp = context_line(cp)) {
            if ((result =
                     regnexec(&state->re, lp->data, lp->size - 1, state->posnum, state->pos, 0)) &&
                result != REG_NOMATCH) {
                regfatal(&state->re, 2, result);
                goto bad;
            }
            if ((result == 0) == state->match) context_show(cp);
        }
        context_close(cp);
        ...
        for (;;) {
            ...
            if (s < e) {
                ...
                for (;;) {
                    ...
                    if (state->spansize < len) {
                        ...
                        len = t - state->span;
                        if (!(state->span =
                                  vmnewof(state->vm, state->span, char, state->spansize, 0))) {
                            error(ERROR_SYSTEM | 2, "%s: line longer than %lu characters", name,
                                  len + e - s);
                            goto bad;
                        }
                        t = state->span + len;
                ...
                *t = '\n';
                if (!(len = t - state->span)) len++;
                if (result = regrexec(&state->re, state->span, len, state->posnum, state->pos,
                                      state->options, '\n', (void *)state, record)) {
            ...
            while (t > s)
                if (*--t == '\n') {
                    len = t - s;
                    if (!len || t > s && *(t - 1) == '\n') len++;
                    if (result = regrexec(&state->re, s, len, state->posnum, state->pos,
                                          state->options, '\n', (void *)state, record)) {
                        if (result < 0) goto done;
                        if (result != REG_NOMATCH) {
                            regfatal(&state->re, 2, result);
            ...
            if (!state->list) {
                if (state->count) {
                    if (state->count & 2)
                        ...
                        if (state->prefix) sfprintf(sfstdout, "%s:", name);
                        if (*x->string) sfprintf(sfstdout, "%s:", x->string);
                        sfprintf(sfstdout, "%I*u\n", sizeof(x->hits), x->hits);
                    }
                }
            } else if ((x->hits != 0) == (state->list > 0)) {
                if (state->list < 0) state->any = 1;
                if (*x->string)
...
static int grep(char *id, int options, int argc, char **argv, Shbltin_t *context) {
    int c;
    char *s;
    char *h;
    Sfio_t *f;
    ...
    if (strcmp(astconf("CONFORMANCE", NULL, NULL), "standard")) state.options |= REG_LENIENT;
    error_info.id = id;
    h = 0;
    fts = 0;
        ...
        switch (c) {
            case 'A':
                if (opt_info.arg) {
                    ...
                    if (*s) error(3, "%s: invalid after-context line count", s);
                } else
                    state.after = 2;
                break;
            case 'B':
                if (opt_info.arg) {
                    ...
                    if (*s) error(3, "%s: invalid before-context line count", s);
                } else
                    state.before = 2;
                break;
            case 'C':
                if (opt_info.arg) {
                    ...
                    if (*s) error(3, "%s: invalid context line count", s);
                } else
            ...
            case 'T':
                ...
                switch (*s) {
                    ...
                    case 'm':
                        ...
                        if (c == 'b' &&
                            !(state.buffer.base = newof(0, char, state.buffer.size, 0))) {
            ...
            case 'c':
                ...
                break;
            case 'e':
                if (addstring(&state, &state.patterns, opt_info.arg)) goto done;
                break;
            case 'f':
                if (addstring(&state, &state.files, opt_info.arg)) goto done;
                break;
            case 'h':
                state.prefix = 2;
                break;
    ...
    if (!state.files.head && !state.patterns.head) {
        ...
        if (addstring(&state, &state.patterns, *argv++)) goto done;
    }
    ...
    if (r = compile(&state)) goto done;
    sfset(sfstdout, SF_LINE, 1);
    if (!argv[0]) {
        if (state.prefix != 1) state.prefix = h ? 1 : 0;
        if (r = execute(&state, sfstdin, h, context)) goto done;
    }
    if (state.prefix > 1)
        state.prefix = 0;
        ...
        switch (ent->fts_info) {
            case FTS_F:
                if (f = sfopen(NULL, ent->fts_accpath, "r")) {
                    ...
                    sfclose(f);
                    if (r) goto done;
                    if (state.query && state.any) goto quit;
                    break;
                }
                /*FALLTHROUGH*/
            case FTS_NS:
            case FTS_SLNONE:
                state.notfound = 1;
                if (!state.suppress) error(ERROR_SYSTEM | 2, "%s: cannot open", ent->fts_path);
                break;
            case FTS_DC:
...
quit:
    ...
    if (sfsync(sfstdout)) error(ERROR_SYSTEM | 2, "write error");
...
int main(int argc, char **argv) { return b_grep(argc, argv, 0); }
