...
static const char usage_1[] = "[-?@(#)$Id: chgrp (AT&T Research) 2012-04-20 $\n]" USAGE_LICENSE;
...
static void getids(char *s, char **e, Key_t *key, int options) {
    char *t;
    int n;
    int m;
    char *z;
    ...
    if (n) {
        options |= OPT_CHOWN;
        if ((n = t++ - s) >= sizeof(buf)) n = sizeof(buf) - 1;
...
int b_chgrp(int argc, char **argv, Shbltin_t *context) {
    int options = 0;
    char *s;
    Map_t *m;
    FTS *fts;
    ...
    for (;;) {
        switch (optget(argv, usage)) {
            ...
            case 'm':
                ...
                if (!(map = dtopen(&mapdisc, Dtset))) error(ERROR_exit(1), "out of space [id map]");
                continue;
            case 'n':
                options |= OPT_SHOW;
                continue;
    ...
    argv += opt_info.index;
    argc -= opt_info.index;
    if (error_info.errors || argc < 2) error(ERROR_usage(2), "%s", optusage(NULL));
    s = *argv;
    ...
    if (map) {
        ...
        if (sp != sfstdin) sfclose(sp);
        keys[1].gid = keys[2].uid = NOID;
    } else if (!(options & (OPT_UID | OPT_GID))) {
        getids(s, NULL, &key, options);
        if ((uid = key.uid) != NOID) options |= OPT_UID;
        if ((gid = key.gid) != NOID) options |= OPT_GID;
    }
    ...
    while (!sh_checksig(context) && (ent = fts_read(fts))) switch (ent->fts_info) {
            ...
            case FTS_SLNONE:
                if (options & OPT_LCHOWN) {
                    ...
                    if (!(options & OPT_FORCE)) {
                        errno = ENOSYS;
                        error(ERROR_system(0), "%s: cannot change symlink owner/group",
                              ent->fts_path);
                    }
#endif
                }
            ...
            anyway:
                ...
                op = "chown";
            commit:
                if ((unsigned long)ent->fts_statp->st_ctime >= before) break;
                if (map) {
                    options &= ~(OPT_UID | OPT_GID);
                    uid = gid = NOID;
                    ...
                    if (!(options & OPT_UID)) uid = ent->fts_statp->st_uid;
                    if (!(options & OPT_GID)) gid = ent->fts_statp->st_gid;
                }
                ...
                if (uid != ent->fts_statp->st_uid && uid != NOID ||
                    ...
                    if (options & (OPT_SHOW | OPT_VERBOSE)) {
                        ...
                        sfprintf(sfstdout, "%s uid:%05d->%05d gid:%05d->%05d %s\n", op,
                                 ent->fts_statp->st_uid, uid, ent->fts_statp->st_gid, gid,
                                 ent->fts_path);
                    }
