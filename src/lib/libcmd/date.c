...
static const char usage[] =
    ...
    "		[+K?all numeric date; equivalent to \b%Y-%m-%d+%H:%M:%S\b; \b%_[EO]]K\b for space "
    "separator, %OK adds \b.%N\b, \b%EK\b adds \b%.N%z\b, \b%_EK\b adds \b.%N %z\b]"
...
static int settime(Shbltin_t *context, const char *cmd, Time_t now, int adjust, int network) {
    char *s;
    char **argv;
    char *args[5];
    char buf[1024];

    if (!adjust && !network) return tmxsettime(now);
    argv = args;
    s = "/usr/bin/date";
        ...
        *argv++ = s;
        if (streq(astconf("UNIVERSE", NULL, NULL), "att")) {
            ...
            tmxfmt(buf, sizeof(buf),
                   ...
                   now);
            if (network) *argv++ = "-n";
            if (tm_info.flags & TM_UTC) *argv++ = "-u";
        }
        *argv++ = buf;
        *argv = 0;
        if (!sh_run(context, argv - args, args)) return 0;
    }
    return -1;
}
...
static Time_t convert(Fmt_t *f, char *s, Time_t now) {
    char *t;
    char *u;

    do {
        now = tmxscan(s, &t, f->format, &u, now, 0);
        if (!*t && (!f->format || !*u)) break;
    } while (f = f->next);
    if (!f || *t) error(3, "%s: invalid date specification", f ? t : s);
    return now;
}

int b_date(int argc, char **argv, Shbltin_t *context) {
    int n;
    char *s;
    Fmt_t *f;
    char *t;
    ...
    for (;;) {
        switch (optget(argv, usage)) {
            ...
            case 'n':
                ...
                continue;
            case 'p':
                if (!(f = newof(0, Fmt_t, 1, 0))) error(ERROR_SYSTEM | 3, "out of space [format]");
                f->next = fmts;
    ...
    if (listzones) {
        ...
        while (listzones->standard) {
            if (listzones->type) s = listzones->type;
            sfprintf(sfstdout, "%3s %4s %4s %4d %4d\n", s,
                     *listzones->standard ? listzones->standard : "-",
                     listzones->daylight ? listzones->daylight : "-", listzones->west,
                     listzones->dst);
            listzones++;
            show = 1;
        }
        ...
        while (s = *argv++) {
            ...
            if (*t) error(3, "%s: invalid elapsed time", s);
            sfprintf(sfstdout, "%lu\n", u);
        }
        show = 1;
    } else if (filetime) {
        if (!*argv) error(ERROR_USAGE | 4, "%s", optusage(NULL));
        n = argv[1] != 0;
        while (s = *argv++) {
            if (stat(s, &st))
        ...
        if (s || (s = string)) {
            if (*argv && string) error(ERROR_USAGE | 4, "%s", optusage(NULL));
    ...
    tm_info.flags = 0;
    if (show && sfsync(sfstdout)) error(ERROR_system(0), "write error");
