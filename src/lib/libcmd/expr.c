...
static const struct Optable_s {
    const char opname[3];
    int op;
} optable[] = {"|",  '|',    "&",  '&',    "=",  OP_EQ, "==", OP_EQ,   ">", OP_GT,    "<", OP_LT,
               ">=", OP_GE,  "<=", OP_LE,  "!=", OP_NE, "+",  OP_PLUS, "-", OP_MINUS, "*", OP_MULT,
               "/",  OP_DIV, "%",  OP_MOD, ":",  ':',   "(",  '(',     ")", ')'};
        ...
        switch (cp[0]) {
            case 'i':
                if (cp[1] == 'n' && !strcmp(cp, "index")) {
                    ...
                    if (!(ep = *state->arglist++)) error(ERROR_exit(2), "chars argument expected");
            ...
            case 's':
                if (cp[1] == 'u' && !strcmp(cp, "substr")) {
                    if (!(sp = *state->arglist++)) error(ERROR_exit(2), "string argument expected");
                    if (!(cp = *state->arglist++))
                        error(ERROR_exit(2), "position argument expected");
                    i = strtol(cp, &ep, 10);
                    if (*ep || --i < 0) i = -1;
                    if (!(cp = *state->arglist++)) error(ERROR_exit(2), "length argument expected");
                    j = strtol(cp, &ep, 10);
                    if (*ep) j = -1;
                    k = strlen(sp);
                    if (i < 0 || i >= k || j < 0)
...
next:
    if (!(cp = *state->arglist)) return 0;
    state->arglist++;
    for (i = 0; i < sizeof(optable) / sizeof(*optable); i++)
        if (*cp == optable[i].opname[0] && cp[1] == optable[i].opname[1]) return optable[i].op;
...
static int expr_cond(State_t *state, Node_t *np) {
    ...
    while (tok == ':') {
        ...
        np->num = 0;
        np->type = T_NUM;
        if (n = regcomp(&re, rp.str, REG_LEFT | REG_LENIENT)) regfatal(&re, ERROR_exit(2), n);
        if (!(n = regexec(&re, cp, elementsof(match), match, 0))) {
            if (re.re_nsub > 0) {
                ...
                if (match[1].rm_so >= 0) {
                    ...
                    if (cp != np->str && *cp == 0) np->type |= T_NUM;
                } else
                    np->str = "";
            } else
...
static int expr_mult(State_t *state, Node_t *np) {
    ...
    while ((tok & ~T_OP) == T_MULT) {
        ...
        if (!numeric(np) || !numeric(&rp)) error(ERROR_exit(2), "non-numeric argument");
        if (op && rp.num == 0) error(ERROR_exit(2), "division by zero");
        switch (op) {
            case 0:
                np->num *= rp.num;
                break;
...
static int expr_or(State_t *state, Node_t *np) {
    ...
    while (tok == '|') {
        ...
        if ((numeric(np) && np->num == 0) || *np->str == 0) *np = rp;
    }
    return tok;
}

int b_expr(int argc, char **argv, Shbltin_t *context) {
    State_t state;
    Node_t node;
    int n;
...
#endif
    {
        while (n = optget(argv, usage)) {
             ...
             *	 unknown - options
             */
            if (n == '?') error(ERROR_usage(2), "%s", opt_info.arg);
            if (opt_info.option[1] != '?') break;
            error(ERROR_usage(2), "%s", opt_info.arg);
        }
        if (error_info.errors) error(ERROR_usage(2), "%s", optusage((char *)0));
        state.arglist = argv + opt_info.index;
    }
    if (expr_or(&state, &node)) error(ERROR_exit(2), "syntax error");
    if (node.type & T_STR) {
        if (*node.str) sfprintf(sfstdout, "%s\n", node.str);
    } else
        sfprintf(sfstdout, "%d\n", node.num);
