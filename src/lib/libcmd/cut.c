...
static void cutcols(Cut_t *cut, Sfio_t *fdin, Sfio_t *fdout) {
    ...
    for (;;) {
        ...
        len = sfvalue(fdin);
        xx = 0;
        if (!(ncol = skip = *(lp = cut->list))) ncol = *++lp;
        must = 1;
        do {
            if (cut->nosplit) {
                const char *s = bp;
                ...
                while (w > 0 && ncol > 0) {
                    ncol--;
                    if (!(*s & 0x80) || (z = mbtsize(s, w, &cut->q)) <= 0) z = 1;
                    s += z;
                    w -= z;
                }
                c = s - bp;
...
static void cutfields(Cut_t *cut, Sfio_t *fdin, Sfio_t *fdout) {
    ...
    while ((bp = (unsigned char *)sfreserve(fdin, SF_UNBOUND, -1)) && (c = sfvalue(fdin)) > 0) {
        cp = bp;
        ep = cp + --c;
        if ((lastchar = cp[c]) != cut->eob) *ep = cut->eob;
        ...
        while (cp <= ep) {
            ...
            do {
                        ...
                        switch (c = sp[*(unsigned char *)cp++]) {
                            ...
                            case SP_WIDE:
                                ...
                                while ((c = mbchar(&w, cp, ep - cp, &cut->q)),
                                    ...
                                    if (lastchar != cut->eob) {
                                        *ep = lastchar;
                                        if ((c = mbchar(&w, cp, ep - cp + 1, &cut->q)),
                                            mberrno(&cut->q) != E2BIG)
                                            break;
                                    }
                                    if (copy) {
                                    ...
                                    if (!(bp = (unsigned char *)sfreserve(fdin, SF_UNBOUND, -1)) ||
                                        (c = sfvalue(fdin)) <= 0)
                                        goto failed;
                                    cp = bp;
                                    ep = cp + --c;
                ...
                if (copy) {
                    empty = 0;
                    if ((c = wp - copy) > 0 && sfwrite(fdout, (char *)copy, c) < 0) goto failed;
                    copy = 0;
                } else
