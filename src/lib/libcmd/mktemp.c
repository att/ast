...
int b_mktemp(int argc, char **argv, Shbltin_t *context) {
    mode_t mode = 0;
    mode_t mask;
    int fd;
    int i;
    ...
    for (;;) {
        switch (optget(argv, usage)) {
            ...
            case 'm':
                ...
                if (*opt_info.arg) error(ERROR_exit(0), "%s: invalid mode", pfx);
                continue;
            case 'p':
