...
static const Tty_t Ttable[] = {
    ...
    {"parenb", BIT, C_FLAG, 0, PARENB, PARENB,
    ...
    {"brkint", BIT, I_FLAG, SS, BRKINT, BRKINT,
     C("Generate (do not generate) INTR signal on break")},
    {"ignpar", BIT, I_FLAG, 0, IGNPAR, IGNPAR,
    ...
    {"istrip", BIT, I_FLAG, 0, ISTRIP, ISTRIP,
     C("Clear (do not clear) high bit of input characters")},
    {"inlcr", BIT, I_FLAG, US, INLCR, INLCR,
...
#ifdef IMAXBEL
    {"imaxbel", BIT, I_FLAG, SS, IMAXBEL, IMAXBEL,
     C("Beep (do not beep) if a character arrives with full input buffer")},
#endif /* IMAXBEL */
    {"icrnl", BIT, I_FLAG, NL | SS, ICRNL, ICRNL,
     C("Translate (do not translate) carriage return to newline")},

    {"isig", BIT, L_FLAG, SS, ISIG, ISIG,
...
#define ECHOPRT 0
    ...
    {"ek", MIXED, C_FLAG, IG, 0, 0,
     C("Reset the \berase\b and \bkill\b special characters to their default values")},
...
#else
#define cntl(x)                                       \
    (((x) == '?') ? ccmapc(0177, CC_ASCII, CC_NATIVE) \
...
static void gout(struct termios *sp) {
    ...
    for (i = 0; i < NCCS; i++) sfprintf(sfstdout, ":%x", sp->c_cc[i]);
#if _mem_c_line_termios
...
static void output(struct termios *sp, int flags) {
    ...
    for (i = 0; i < elementsof(Ttable); i++) {
        tp = &Ttable[i];
        if (tp->flags & IG) {
            if (tp->flags & NL) sfputc(sfstdout, '\n');
            continue;
        }
        switch (tp->type) {
            case BIT:
            case BITS:
                ...
                switch (tp->field) {
                    case C_FLAG:
                        if (sp->c_cflag & tp->mask) off = 0;
                        if (tty.c_cflag & tp->mask) off2 = 0;
                        break;
                    case I_FLAG:
                        if (sp->c_iflag & tp->mask) off = 0;
                        if (tty.c_iflag & tp->mask) off2 = 0;
                        break;
                    case O_FLAG:
                        if ((sp->c_oflag & tp->mask) == tp->val) off = 0;
                        if (tty.c_oflag & tp->mask) off2 = 0;
                        break;
                    case L_FLAG:
                        if (sp->c_lflag & tp->mask) off = 0;
                        if (tty.c_lflag & tp->mask) off2 = 0;
                }
            ...
            case CHAR:
...
#else
                {
                    ...
                    sfprintf(sfstdout, "%s = ^%c;%c", tp->name,
                             off == 0177 ? '?' : ccmapc(off ^ 0100, CC_ASCII, CC_NATIVE), delim);
                }
#endif
                delim = ' ';
                break;
            case SIZE:
                if ((sp->c_cflag & CSIZE) != tp->mask) continue;
    ...
    while (cp = *argv++) {
        ...
        switch (tp->type) {
            ...
            case NUM:
                ...
                if (*ep) error(ERROR_exit(1), "%s: %s: numeric argument expected", tp->name, cp);
                switch (tp->field) {
#if _mem_c_line_termios
                    case C_LINE:
                        sp->c_line = c;
...
static void listgroup(Sfio_t *sp, int type, const char *description) {
    ...
    sfprintf(sp, "[+");
    for (i = 0; i < elementsof(Ttable); i++) {
        if (Ttable[i].type == type) sfprintf(sp, "%s ", Ttable[i].name);
    }
...
static void listmask(Sfio_t *sp, unsigned int mask, const char *description) {
    ...
    sfprintf(sp, "[+");
    for (i = 0; i < elementsof(Ttable); i++) {
        if (Ttable[i].mask == mask && Ttable[i].type == BITS) sfprintf(sp, "%s ", Ttable[i].name);
    }
    sfprintf(sp, "?%s.]", description);
}
...
static int infof(Opt_t *op, Sfio_t *sp, const char *s, Optdisc_t *dp) {
    ...
    listgroup(sp, SPEED,
              "Attempt to set input and output baud rate to number given.  A value of \b0\b causes "
              "immediate hangup");
    listchars(sp, NUM);
