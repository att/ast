...
Wc_t *wc_init(int mode) {
    ...
    if (!(wp = (Wc_t *)stakalloc(sizeof(Wc_t)))) return 0;
    if (!mbwide()) wp->mb = 0;
#if _hdr_wchar && _hdr_wctype && _lib_iswctype
...
static void invalid(const char *file, int byte, int nlines) {
            ...
            if ((o = endbuff - cp) <= 0 || (mbchar(&x, cp, o, &wp->q), mberrno(&wp->q))) {
                if (eof) {
                    ...
                    if (!buff)
                        ...
                        memcpy(side, cp, o);
                    cp = side + o;
                    if (!(buff = (unsigned char *)sfreserve(fd, SF_UNBOUND, 0)) ||
                        (n = sfvalue(fd)) <= 0) {
                ...
                if (x == -1) {
                    ...
                    if (eline != nlines) {
                        if (!(wp->mode & (WC_INVAL | WC_QUIET))) eline = nlines;
                        if (!(wp->mode & WC_QUIET)) invalid(file, *(cp - 1), nlines);
                    }
                }
            }
            if (x == '\n') {
