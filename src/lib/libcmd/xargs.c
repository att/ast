...
static int run(int argc, char **argv, Cmddisc_t *disc) {
    if (((Xargs_t *)disc)->context) return sh_run(((Xargs_t *)disc)->context, argc, argv);
}

int b_xargs(int argc, char **argv, Shbltin_t *context) {
    int c;
    int q;
    char *s;
    Sfio_t *sp;
    ...
    if (argv[0]) {
        if (context) {
            ...
            av[2] = argv[0];
            av[3] = 0;
            if (run(3, av, &xargs.disc)) error(3, "%s: command not found", argv[0]);
        } else {
            char buf[PATH_MAX];
    ...
    if (xargs.cmd = cmdopen(argv, argmax, size, insert, &xargs.disc)) {
        ...
        if (term >= 0)
            while (!sh_checksig(context)) {
                if (!(s = sfgetr(sfstdin, term, 0))) {
                    if (sfvalue(sfstdin) > 0) error(2, "last argument incomplete");
                    break;
                }
                error_info.line++;
            ...
            while (!sh_checksig(context)) {
                switch (c = sfgetc(sfstdin)) {
                    ...
                    arg:
                        c = sfstrtell(sp);
                        if (!(s = sfstruse(sp))) error(ERROR_SYSTEM | 3, "out of space");
                        if (eof && streq(s, eof)) break;
                        if (c || insert) {
                            if (lines && c > 1 && isspace(s[c - 2])) cmdarg(xargs.cmd, 0, -1);
                            cmdarg(xargs.cmd, s, c);
                        }
                        continue;
                    default:
                        if (isspace(c)) goto arg;
                        sfputc(sp, c);
                        continue;
                }
                break;
        ...
        if (sferror(sfstdin)) error(ERROR_SYSTEM | 2, "input read error");
        cmdclose(xargs.cmd);
