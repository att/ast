...
#define setchar(p, s, t)                                                                          \
    ((p)->type = (t), (p)->prev = (p)->last = (-1), (p)->isit = 0, (p)->iseq = 0, (p)->count = 0, \
     (p)->base = (p)->next = (s))
...
next:
    if (tr->last >= 0) {
        while (++tr->prev <= tr->last) {
            if (tr->isit) {
                if (!(*tr->isit)(tr->prev)) continue;
            }
            if (tr->iseq) {
                if ((c = mbtconv(buf, tr->prev, &tr->q)) <= 0 ||
                    regnexec(tr->iseq, buf, c, 0, NULL, 0))
                    continue;
            }
            return (!tr->type || !tr->convert)
                       ? tr->prev
                       : tr->convert == 'l' ? towlower(tr->prev) : towupper(tr->prev);
        }
        if (tr->isit)
            tr->isit = 0;
        else if (tr->iseq) {
    ...
    switch (c = mbtchar(&w, tr->next, MB_LEN_MAX, &tr->q)) {
        case -1:
            c = *(tr->next - 1);
            if (tr->warn)
                error(2, "%s operand: \\x%02x: invalid multibyte character byte",
                      typename[tr->type], c);
            break;
        case 0:
            tr->next--;
        ...
        case '[':
            switch (*tr->next) {
                ...
                case '.':
                case '=':
                    if ((q = regcollate((char *)tr->next, (char **)&e, buf, sizeof(buf), &w)) >=
                        0) {
                        if (*tr->next == '=' && q > 0) {
                ...
                default:
                    if (!tr->level) {
                        ...
                        if (tr->type == 1 && *tr->next == '*') {
                            e = tr->next + 1;
                            if (!(tr->count =
                                      (int)strtol((char *)tr->next + 1, (char **)&tr->next, 0)) &&
                                tr->next == e)
                                tr->count = -1;
                            if (*tr->next++ != ']') return -2;
                            if (tr->count < 0) {
                                ...
                                peek.count = 0;
                                peek.last = -1;
                                while (nextchar(&peek) >= 0) peek.dst++;
                                tr->count = tr->src - peek.dst;
                            } else if (tr->count > tr->chars)
                                tr->count = tr->chars;
                            if (!tr->count) goto next;
                            tr->count--;
                            tr->level--;
                        }
                    }
...
static Tr_t *tropen(unsigned char *src, unsigned char *dst, unsigned int flags,
                    Shbltin_t *context) {
    Tr_t *tr;
    int c;
    int n;
    ...
    if (flags & TR_COMPLEMENT) {
        ...
        if (flags & TR_COLLATE) {
            ...
            for (n = 0; n < c; n++) set[n] = col[n].chr;
        }
#endif
        tr->src = c;
    } else
        tr->src = n;
    if (tr->convert == '?') tr->convert = 0;
    setchar(tr, dst, 1);
...
static ssize_t trcopy(Tr_t *tr, Sfio_t *ip, Sfio_t *op, ssize_t ncopy) {
    int c;
    int oldc = -1;
    ...
    if (tr->mb) {
        ...
        eline = 0;
        eof = 0;
        if ((m = mbmax()) > (sizeof(side) / 2)) m = sizeof(side) / 2;
        inp = inend = 0;
            ...
            if ((o = inend - inp) <= 0 || (mbchar(&w, inp, o, &tr->q), mberrno(&tr->q))) {
                if (o >= m || o && mberrno(&tr->q) == EILSEQ)
                    ...
                    if (eof) {
                        ...
                        w = -1;
                    else if (!(pushbuf = (unsigned char *)sfreserve(ip, SF_UNBOUND, 0)) ||
                             (n = sfvalue(ip)) <= 0) {
                        eof = 1;
                        w = -1;
                    } else {
                    ...
                    if (inp) {
                        if (o) memcpy(side, inp, o);
                        mbinit(&tr->q);
                    } else
                        o = 0;
                    if (!(inp = (unsigned char *)sfreserve(ip, SF_UNBOUND, 0)) ||
                        (n = sfvalue(ip)) <= 0) {
                        eof = 1;
                        inp = side;
                        inend = inp + o;
                        ...
                        pushend = pushbuf + n;
                        inp = side + o;
                        if ((c = sizeof(side) - o) > n) c = n;
                        if (c) memcpy(inp, pushbuf, c);
                        inend = inp + c;
                        inp = side;
                    } else
                        inend = inp + n;
            ...
            while (inp < inend) {
                if (outp >= outend) {
                    ...
                    if (!(outbuf = (unsigned char *)sfreserve(
                              op, (ncopy < 0) ? SF_UNBOUND : (ncopy - nwrite), SF_LOCKR)))
                        break;
        ...
        if (inbuf && (c = inp - inbuf) > 0) sfread(ip, inbuf, c);
        if (outbuf && (c = outp - outbuf) >= 0) sfwrite(op, outbuf, c);
    }
    if (sfsync(op)) {
        if (!ERROR_PIPE(errno)) error(ERROR_SYSTEM | 2, "write error [%d]", c);
        return -1;
    }
    return nwrite;
}

int b_tr(int argc, char **argv, Shbltin_t *context) {
    ...
    if (tr = tropen((unsigned char *)argv[0],
                    (unsigned char *)argv[0] ? (unsigned char *)argv[1] : (unsigned char *)0, flags,
                    context)) {
