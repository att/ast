...
static void aform(State_t *state, Format_t *fp, Sfio_t *op, unsigned char *u) {
    int c = *u;
...
static void bform(State_t *state, Format_t *fp, Sfio_t *op, unsigned char *u) {
    sfprintf(op, "%08..2u", *u);
}

static void cform(State_t *state, Format_t *fp, Sfio_t *op, unsigned char *u) {
    unsigned char *v;
    char *s;
    wchar_t w;
    int i;
    ...
    if ((w = mbtchar(&w, u, state->eob - u, &state->q)) > 0 && (i = u - v) > 1 && !iswprint(w)) {
        state->mb = i - 1;
        for (j = 3 - mbwidth(w); j > 0; j--) sfputc(op, ' ');
        while (i--) sfputc(op, *v++);
        return;
    }
    buf[1] = 0;
...
static void mform(State_t *state, Format_t *fp, Sfio_t *op, unsigned char *u) {
    char *s;
    char buf[2];
...
static void Oform(State_t *state, Format_t *fp, Sfio_t *op, unsigned char *u) {
    unsigned char *v;
    char *s;
    wchar_t w;
    int i;
...
static void form_int8(State_t *state, Format_t *fp, Sfio_t *op, unsigned char *u) {
    sfprintf(op, fp->form, *u);
}

static void form_int16(State_t *state, Format_t *fp, Sfio_t *op, unsigned char *u) {
    sfprintf(op, fp->form, (int16_t)swapget(state->swap, u, fp->size.external));
}

static void form_int32(State_t *state, Format_t *fp, Sfio_t *op, unsigned char *u) {
    sfprintf(op, fp->form, (int32_t)swapget(state->swap, u, fp->size.external));
}

static void form_intmax(State_t *state, Format_t *fp, Sfio_t *op, unsigned char *u) {
    sfprintf(op, fp->form, (intmax_t)swapget(state->swap, u, fp->size.external));
}

static void form_uint8(State_t *state, Format_t *fp, Sfio_t *op, unsigned char *u) {
    sfprintf(op, fp->form, *u);
}

static void form_uint16(State_t *state, Format_t *fp, Sfio_t *op, unsigned char *u) {
    sfprintf(op, fp->form, (uint16_t)swapget(state->swap, u, fp->size.external));
}

static void form_uint32(State_t *state, Format_t *fp, Sfio_t *op, unsigned char *u) {
    sfprintf(op, fp->form, (uint32_t)swapget(state->swap, u, fp->size.external));
}

static void form_uintmax(State_t *state, Format_t *fp, Sfio_t *op, unsigned char *u) {
    sfprintf(op, fp->form, (uintmax_t)swapget(state->swap, u, fp->size.external));
}

static void form_float(State_t *state, Format_t *fp, Sfio_t *op, unsigned char *u) {
    Mem_t mem;
...
static void form_double(State_t *state, Format_t *fp, Sfio_t *op, unsigned char *u) {
    Mem_t mem;
...
static void form_long_double(State_t *state, Format_t *fp, Sfio_t *op, unsigned char *u) {
    Mem_t mem;
...
static void format(State_t *state, char *t) {
    int c;
    const Type_t *tp;
    const Size_t *zp;
    Format_t *fp;
    ...
    while (c = *t++) {
        ...
        for (;;) {
            ...
            if (n) {
                if (n == zp->size) break;
                zp++;
                if (n > (zp - 1)->size && n < zp->size) xp = zp;
            } else if (c == zp->name) {
                if (c != *t) break;
                t++;
                zp++;
        ...
        if (tp->mb && (i = mbmax()) > 1) state->mbmax = i;
        i = zp - tp->size;
        ...
        if (state->size < fp->size.external) state->size = fp->size.external;
        if (zp->exponent) {
            if (zp->map == 'a') {
        ...
        if (!(fp->fun = tp->fun)) {
            ...
            *e++ = '%';
            ...
            if (prec) e += sfsprintf(e, sizeof(fp->form) - (e - fp->form), ".%d", prec);
            if (base)
                e +=
                    sfsprintf(e, sizeof(fp->form) - (e - fp->form), "%s.%d", prec ? "" : ".", base);
            if (s = (char *)zp->qual)
                while (*e = *s++) e++;
...
static Sfio_t *init(State_t *state, char ***p) {
    Sfio_t *ip;
    intmax_t offset;
    ...
    for (;;) {
        ...
        if (state->buffer.noshare) sfset(ip, SF_SHARE, 0);
        if (state->buffer.size) sfsetbuf(ip, state->buffer.base, state->buffer.size);
        if (state->skip) {
            if ((offset = sfseek(ip, (off_t)0, SEEK_END)) > 0) {
                ...
                for (;;) {
                    if (!(state->peek.data = sfreserve(ip, SF_UNBOUND, 0))) {
                        if (sfvalue(ip)) error(ERROR_system(2), "%s: read error", state->file);
                        goto next;
                    }
...
static int block(State_t *state, Sfio_t *op, char *bp, char *ep, intmax_t base) {
    Format_t *fp;
    unsigned char *u;
    ssize_t z;
    unsigned long n;
    ...
    for (fp = state->form; fp; fp = fp->next) {
                        ...
                        for (u = (unsigned char *)bp; u < (unsigned char *)ep; u++) {
                            if (state->mbp) {
                                ...
                                sfputc(op, '.');
                                       ...
                                       (w = mbtchar(&w, v, state->eob - u, &state->q)) > 0 &&
                                       (c = v - u) > 1 && !iswprint(w)) {
                                sfwrite(op, u, c);
                                state->mbp = c - 1;
                            } else {
                                if ((c = ccmapchr(state->map, *u)) < 040 || c >= 0177) c = '.';
                                sfputc(op, c);
                            }
                        }
                    else
        ...
        if (sferror(op)) return -1;
    }
    return 0;
}

static int od(State_t *state, char **files) {
    char *s;
    char *e;
    char *x;
    char *span = 0;
    ...
    if (!(ip = init(state, &files))) return 0;
    for (;;) {
        if (sh_checksig(state->context)) goto bad;
        if (s = state->peek.data) {
            ...
            for (;;) {
                ...
                n = sfvalue(ip);
                if (s) break;
                if (n) error(ERROR_system(2), "%s: read error", state->file);
                if (ip != sfstdin) sfclose(ip);
        ...
        respan:
            ...
            if ((m = span - state->span) > state->block) m = state->block;
            if (block(state, sfstdout, state->span, state->span + m, state->offset)) goto bad;
            state->offset += m;
            if (!state->verbose)
        ...
        if (state->strings) {
            ...
            for (;;) {
                if (s >= x || (c = *s++) == 0 || c == '\n' || !isprint(c)) {
                    if (n >= state->strings) {
                        if (*state->base &&
                            sfprintf(sfstdout, state->base, state->offset - (x - s) - n - 1) < 0)
                            break;
            ...
            if (state->mbmax) e -= state->block + state->mbmax;
            if (s < e) {
                do {
                    if (block(state, sfstdout, s, s + state->block, state->offset)) goto bad;
...
static int optsize(Sfio_t *sp, const Size_t *zp) {
    int n;
    ...
    switch (*s) {
        case 'c':
            for (ic = iconv_list(NULL); ic; ic = iconv_list(ic))
                if (ic->ccode >= 0)
                    n += sfprintf(sp, "[%c:%s?%s]", ic->match[ic->match[0] == '('], ic->name,
                                  ic->desc);
            break;
        case 't':
...
int b_od(int argc, char **argv, Shbltin_t *context) {
    int n;
    char *s;
    Format_t *fp;
    char *e;
            ...
            case 'T':
                ...
                switch (*s) {
                    ...
                    case 'm':
                        ...
                        if (n == 'b' && !(state.buffer.base =
                                              vmnewof(state.vm, 0, char, state.buffer.size, 0))) {
                            error(ERROR_SYSTEM | 2, "out of space");
                            goto done;
                        }
                        if (*e) error(2, "%s: invalid characters after test", e);
                        break;
                    case 'n':
    ...
    if (error_info.errors) goto done;
    for (fp = state.form; fp; fp = fp->next)
        if ((n = (state.size / fp->size.external) * (fp->width + 1) - 1) > state.width)
            state.width = n;
    for (fp = state.form; fp; fp = fp->next) fp->per = state.width * fp->size.external / state.size;
    n = LINE_LENGTH - state.printable;
    if (*state.base) n -= (BASE_WIDTH + 1);
    if (!(n /= (state.width + state.printable * state.size + 1))) n = 1;
    if (state.addr) {
        while (n > state.addr) state.addr *= 2;
        while (n < state.addr) state.addr /= 2;
        if (state.addr) n = state.addr;
    }
    if (per) n = per;
    ...
    if (sfsync(sfstdout) && !ERROR_PIPE(errno)) error(ERROR_SYSTEM | 2, "write error");
done:
    vmclose(state.vm);
