...
static int uniq(Sfio_t *fdin, Sfio_t *fdout, int fields, int chars, int width, int mode, int *all,
                Compare_f compare) {
    ...
    if (mode & C_FLAG) cwidth = CWIDTH + 1;
    while (1) {
        if (bufp = sfgetr(fdin, '\n', 0))
            n = sfvalue(fdin);
        ...
        if (n) {
            ...
            if (chars) {
                if (mb)
                    for (f = chars; f; f--) mbchar(&w, cp, MB_LEN_MAX, &q);
                else
                    cp += chars;
            }
        ...
        if (reclen == oreclen && (!reclen || !(*compare)(cp, orecp, reclen))) {
            ...
            if (outsize > 0) {
                if (((mode & D_FLAG) && count == 0) || ((mode & U_FLAG) && count)) {
                    if (outp != sbufp) sfwrite(fdout, outp, 0);
                } else {
                    if (cwidth) {
                        if (count < 9) {
                            f = 0;
                            while (f < CWIDTH - 1) outp[f++] = ' ';
                            outp[f++] = '0' + count + 1;
                            outp[f] = ' ';
                            ...
                            outsize -= (CWIDTH + 1);
                            if (outp != sbufp) {
                                if (!(sbufp = fmtbuf(outsize))) return (1);
                    ...
                    if (sfwrite(fdout, outp, outsize) != outsize) return (1);
                }
            }
        }
        if (n == 0) break;
        if (count = next) {
            ...
            sep = all && *all > 0;
        /* save current record */
        if (!(outbuff = sfreserve(fdout, 0, 0)) || (outsize = sfvalue(fdout)) < 0) return (1);
        outp = outbuff;
        if (outsize < n + cwidth + sep) {
            /* no room in outp, clear lock and use side buffer */
            sfwrite(fdout, outp, 0);
            if (!(sbufp = outp = fmtbuf(outsize = n + cwidth + sep))) return (1);
        } else
...
int b_uniq(int argc, char **argv, Shbltin_t *context) {
    int mode = 0;
    char *cp;
    ...
    argv += opt_info.index;
    if (all && (mode & C_FLAG)) error(2, "-c and -D are mutually exclusive");
    if (error_info.errors) error(ERROR_usage(2), "%s", optusage(NULL));
    if ((cp = *argv) && (argv++, !streq(cp, "-"))) {
        if (!(fpin = sfopen(NULL, cp, "r"))) error(ERROR_system(1), "%s: cannot open", cp);
    } else
        fpin = sfstdin;
    if (cp = *argv) {
        argv++;
        if (!(fpout = sfopen(NULL, cp, "w"))) error(ERROR_system(1), "%s: cannot create", cp);
    } else
        fpout = sfstdout;
    if (*argv) {
    ...
    if (fpin != sfstdin) sfclose(fpin);
    if (fpout != sfstdout) sfclose(fpout);
