...
#if _lib_fsid
static void getfsids(Sfio_t *sp, const char *name, int flags, int lastchar) {
    struct fsg *fs;
    char *s;
    char **p;
    char **x;
        ...
        if (!isfsg(fs)) {
            if (p = fs->fs_mem) {
                if (flags > 0)
                    ...
                    n = 0;
                    q = p;
                    while (s = *q++) n += strlen(s) + 1;
                    if (!(x = newof(0, char *, q - p, n))) break;
                    s = (char *)(x + (q - p));
                    q = x;
                    while (t = *p++) {
                        *q++ = s;
...
static void putid(Sfio_t *sp, int flags, const char *label, const char *name, long number) {
    sfprintf(sp, "%s=", label);
    if (flags & O_FLAG) {
        if (name)
            sfputr(sp, name, -1);
...
static int getids(Sfio_t *sp, const char *name, int flags) {
    struct passwd *pw;
    struct group *grp;
    int i;
    int j;
    ...
    if (flags & GG_FLAG) {
        ...
        if (maxgroups < 0) {
            ...
            if ((maxgroups = getgroups(0, groups)) <= 0) maxgroups = getconf("NGROUPS_MAX");
    ...
    if (name) {
        ...
        if (!(flags & N_FLAG) || (flags & (G_FLAG | GG_FLAG))) {
            if (!(pw = getpwnam(name))) {
                ...
                if (*s || !(pw = getpwuid(user))) error(ERROR_exit(1), "%s: name not found", name);
                name = pw->pw_name;
            }
            user = pw->pw_uid;
            group = pw->pw_gid;
        ...
        if (!(flags & N_FLAG) || (flags & S_FLAG)) {
            setfsgent();
            do {
                if (!(fs = getfsgnam(name))) error(ERROR_exit(1), "%u: fss name not found", name);
            } while (isfsg(fs));
            fs_id = fs->fs_id;
        }
#endif
    } else {
        if (flags & G_FLAG) group = (flags & R_FLAG) ? getgid() : getegid();
        if (flags & (GG_FLAG | N_FLAG | U_FLAG)) user = (flags & R_FLAG) ? getuid() : geteuid();
#if _lib_fsid
        if (flags & S_FLAG) fs_id = fsid(0);
#endif
        if (flags & N_FLAG) name = (pw = getpwuid(user)) ? pw->pw_name : (char *)0;
    }
    if (ngroups == 1 && groups[0] == group) ngroups = 0;
        ...
        if ((flags & X_FLAG) && name) {
            ...
            while (grp = getgrent())
                if (p = grp->gr_mem)
                    while (s = *p++)
                        if (streq(s, name)) {
                            if (lastchar == '=') sfputr(sp, " groups", -1);
                            sfputc(sp, lastchar);
                            lastchar = ',';
                            if (flags & O_FLAG)
...
int b_id(int argc, char **argv, Shbltin_t *context) {
    int flags = 0;
    int n;
    ...
    if (!power2(n)) error(2, "incompatible options selected");
