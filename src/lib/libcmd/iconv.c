...
static int listall(void *context) {
    iconv_list_t *ic;
    const char *p;
...
static int checksig(void *context) { return sh_checksig(context); }

int b_iconv(int argc, char **argv, Shbltin_t *context) {
    char *file;
    char *from;
    char *to;
    iconv_t cvt;
    ...
    if (list) return listall(context);
    ...
    if (sfsync(sfstdout)) error(ERROR_SYSTEM | 3, "conversion write error");
