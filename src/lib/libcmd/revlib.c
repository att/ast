...
int rev_line(Sfio_t *in, Sfio_t *out, off_t start) {
    ...
    if (sfseek(in, (off_t)0, SEEK_CUR) < 0) {
        ...
        if (!tmp) return (-1);
        if (start > 0 && sfmove(in, (Sfio_t *)0, start, -1) != start) return (-1);
        if (sfmove(in, tmp, SF_UNBOUND, -1) < 0 || !sfeof(in) || sferror(tmp)) return (-1);
        in = tmp;
        start = 0;
    }
    if ((offset = sfseek(in, (off_t)0, SEEK_END)) <= start) return (0);
        ...
        while (1) {
            ...
            if (cp == buff && n != '\n') {
                *cp = n;
                ...
                cp++;
            if (sfwrite(out, cp, cpold - cp) < 0) return (-1);
            if (nleft) {
                if (nleft == 1)
                    sfputc(out, '\n');
    ...
    if (nleft) {
        ...
        if (sfmove(in, out, nleft, -1) != nleft) return (-1);
    }
    return (0);
}
