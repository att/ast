...
static Sfoff_t tailpos(Sfio_t *fp, Sfoff_t number, int delim) {
    size_t n;
    Sfoff_t offset;
    Sfoff_t first;
    Sfoff_t last;
    ...
    if (delim < 0) {
        if ((offset = last - number) < first) return first;
        return offset;
    }
    incomplete = 1;
    for (;;) {
        if ((offset = last - SF_BUFSIZE) < first) offset = first;
        sfseek(fp, offset, SEEK_SET);
        n = last - offset;
        if (!(s = sfreserve(fp, n, SF_LOCKR))) return -1;
        t = s + n;
        if (incomplete) {
...
static void pipetail(Sfio_t *infile, Sfio_t *outfile, Sfoff_t number, int delim) {
    Sfio_t *out;
    Sfoff_t n;
    ...
    if (offset[fno]) {
        ...
        if ((n = number - nleft) > 0) sfmove(tmp[!fno], NULL, n, delim);
        if ((n = offset[!fno] - sftell(tmp[!fno])) > 0) sfmove(tmp[!fno], outfile, n, -1);
    } else
        fno = !fno;
...
static int init(Tail_t *tp, Sfoff_t number, int delim, int flags, const char **format) {
    Sfoff_t offset;
    Sfio_t *op;
    struct stat st;
    ...
    error(-1, "AHA#%d offset=%I*d number=%I*d", __LINE__, sizeof(offset), offset, sizeof(number),
          number);
    if (offset) {
    ...
    return 0;
bad:
    if (tp->sp != sfstdin) sfclose(tp->sp);
    tp->sp = 0;
    return -1;
}
...
static intmax_t num(const char *s, char **e, int *f, int o) {
    intmax_t number;
    char *t;
    int c;
...
int b_tail(int argc, char **argv, Shbltin_t *context) {
    Sfio_t *ip;
    int n;
    int i;
    int delim;
    ...
    for (;;) {
        switch (n = optget(argv, usage)) {
            ...
            case 'b':
                blocks = 512;
                flags &= ~LINES;
                if (opt_info.option[0] == '+') number = -number;
                continue;
            case 'c':
                flags &= ~LINES;
            ...
            case 'n':
                ...
                if (flags & ERROR) continue;
                if (flags & (NEGATIVE | POSITIVE)) number = -number;
                if (opt_info.option[0] == '+') number = -number;
                continue;
            case 'f':
                flags |= FOLLOW;
            ...
            case 't':
                ...
                if (*s) error(ERROR_exit(1), "%s: invalid elapsed time [%s]", opt_info.arg, s);
                continue;
            case 'v':
                flags |= VERBOSE;
                continue;
            case ':':
                ...
                if (i = *(s - 1) == '-' || *(s - 1) == '+') s--;
                if ((number = num(s, &t, &flags, 0)) && i) number = -number;
                goto compatibility;
            suffix:
                r = 0;
                if (opt_info.option[0] == '+') number = -number;
    ...
    if (!*argv) {
        ...
        flags &= ~HEADERS;
    delim = (flags & LINES) ? '\n' : -1;
    if (blocks) number *= blocks;
    if (flags & REVERSE) {
        if (delim < 0) error(2, "--reverse requires line mode");
        if (!(flags & COUNT)) number = -1;
        flags &= ~FOLLOW;
    }
    ...
    if (flags & FOLLOW) {
        ...
        if (!files) return error_info.errors != 0;
        pp->next = 0;
        hp = 0;
        n = 1;
        tv.tv_sec = 1;
        ...
        while (fp = files) {
            ...
            while (fp) {
                if (fstat(sffileno(fp->sp), &st))
                    ...
                    n = 1;
                    if (timeout) fp->expire = NOW + timeout;
                    z = fp->fifo ? SF_UNBOUND : st.st_size - fp->cur;
                    i = 0;
                    if ((s = sfreserve(fp->sp, z, SF_LOCKR)) ||
                        (z = sfvalue(fp->sp)) && (s = sfreserve(fp->sp, z, SF_LOCKR)) && (i = 1)) {
                        z = sfvalue(fp->sp);
                        for (r = s + z; r > s && *(r - 1) != '\n'; r--)
                            ;
                        if ((w = r - s) || i && (w = z)) {
                    ...
                    if (flags & LOG) {
                        ...
                        while (--i && stat(fp->name, &st))
                            if (sh_checksig(context)) {
                                ...
                                tvsleep(&tv, NULL);
                        if (i && (fp->dev != st.st_dev || fp->ino != st.st_ino) &&
                ...
                if (fp->sp && fp->sp != sfstdin) sfclose(fp->sp);
                if (pp)
                    pp = pp->next = fp->next;
                else
                    files = files->next;
            ...
            if (sfsync(sfstdout)) error(ERROR_system(1), "write error");
        }
    done:
        ...
        do {
            ...
            if (number < 0 || !number && (flags & POSITIVE)) {
                sfset(ip, SF_SHARE, 1);
                if (number < -1 && (moved = sfmove(ip, NULL, -(number + 1), delim)) >= 0 &&
