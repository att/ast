...
static void fold(Sfio_t *in, Sfio_t *out, int width, const char *cont, size_t contsize,
                 char *cols) {
    char *cp, *first;
    int n, col = 0, x = 0;
    char *last_space = 0;
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            while ((cp - first) > (width - col)) {
                ...
                col = 0;
                last_space = 0;
                if (cp > first + 1 || (n != T_NL && n != T_BS)) sfwrite(out, cont, contsize);
            }
            switch (n) {
                case T_NL:
                    if (x) *(cp - 1) = x;
...
int b_fold(int argc, char **argv, Shbltin_t *context) {
    int n, width = WIDTH;
    Sfio_t *fp;
    char *cp;
    char *cont = "\n";
