...
static int read_real_link_path_canon(int argc, char **argv, Shbltin_t *context, int readlink_mode) {
    char *file;
    char *path;
    ssize_t n;
    int r = 0;
    ...
    do {
        if (readlink_mode) {
            if ((n = readlinkat(dirfd, file, linkbuf, sizeof(linkbuf))) >= 0 &&
                ...
                if (verbose) error(ERROR_SYSTEM | 2, "%s: cannot read link text", file);
                r = 1;
                path = (char *)fail;
            }
        } else
        ...
        if ((flags & PATH_CANON) && path != fail) {
            if (pathdev(dirfd, path, canonbuf, sizeof(canonbuf), flags, NULL))
                ...
                if (verbose)
                    error(ERROR_SYSTEM | 2, "%s: canonicalization error%s%s", file,
                          *canonbuf ? " at " : "", canonbuf);
                r = 1;
                path = (char *)fail;
            }
        }
        if (argc > 1 || path != (char *)fail) sfputr(sfstdout, path, separator);
    } while (file = *argv++);
    if (sfsync(sfstdout)) {
        if (verbose) error(ERROR_SYSTEM | 2, "write error");
        r = 2;
    }
    return r;
}

int b_readlink(int argc, char **argv, Shbltin_t *context) {
    return read_real_link_path_canon(argc, argv, context, READLINK_COMMAND);
}

int b_realpath(int argc, char **argv, Shbltin_t *context) {
