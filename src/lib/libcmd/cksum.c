...
static Sfio_t *openfile(const char *path, const char *mode) {
    Sfio_t *sp;
...
static int closefile(Sfio_t *sp) { return sp == sfstdin ? 0 : sfclose(sp); }

/*
 * compute and print sum on an open file
 */

static void pr(State_t *state, Sfio_t *op, Sfio_t *ip, char *file, int perm, struct stat *st,
               Sfio_t *check) {
    char *p;
    char *r;
    char *e;
    ...
    if (check) {
        ...
        while (p = sfgetr(ip, '\n', 1)) verify(state, p, file, check);
        state->sum = state->oldsum;
        if (state->warn && !sfeof(ip)) error(2, "%s: last line incomplete", file);
        return;
    }
    suminit(state->sum);
    if (state->text) {
        ...
        if (peek) sumblock(state->sum, "\r", 1);
    } else
        while (p = sfreserve(ip, SF_UNBOUND, 0)) sumblock(state->sum, p, sfvalue(ip));
    if (sfvalue(ip)) error(ERROR_SYSTEM | 2, "%s: read error", file);
    sumdone(state->sum);
        ...
        if (perm >= 0) {
            if (perm) {
                if (!st && fstat(sffileno(ip), st = &ss))
                    ...
                    sfprintf(sfstdout, " %04o %s %s", modex(st->st_mode & S_IPERM),
                             (st->st_uid != state->uid &&
                              ((st->st_mode & S_ISUID) ||
                               (st->st_mode & S_IRUSR) && !(st->st_mode & (S_IRGRP | S_IROTH)) ||
                               (st->st_mode & S_IXUSR) && !(st->st_mode & (S_IXGRP | S_IXOTH))))
                                 ? fmtuid(st->st_uid)
                                 : "-",
                             (st->st_gid != state->gid &&
                              ((st->st_mode & S_ISGID) ||
                               (st->st_mode & S_IRGRP) && !(st->st_mode & S_IROTH) ||
            ...
            if (ip != sfstdin)
                sfprintf(op, " %s%s",
                         (state->sum->flags & SUM_INDICATOR) ? indicator[state->text] : "", file);
            sfputc(op, '\n');
        }
    }
}
...
static void verify(State_t *state, char *s, char *check, Sfio_t *rp) {
    char *t;
    char *e;
    char *file;
    int attr;
    ...
    if (!*s || *s == '#' && (!*(s + 1) || *(s + 1) == ' ' || *(s + 1) == '\t')) return;
    if (t = strchr(s, ' ')) {
        if ((t - s) > 10 || !(file = strchr(t + 1, ' '))) file = t;
        *file++ = 0;
        ...
        if (sp = openfile(file, text ? "rt" : "rb")) {
            ...
            if (!(t = sfstruse(rp))) error(ERROR_SYSTEM | 3, "out of space");
            if (!streq(s, t)) {
                if (state->silent)
                    error_info.errors++;
                else
                ...
                if (fstat(sffileno(sp), &st)) {
                    ...
                    if (state->permissions && (uid >= 0 || gid >= 0)) {
                        if (chown(file, uid, gid) < 0) {
                            if (uid < 0)
                                error(ERROR_SYSTEM | 2, "%s: cannot change group to %s", file,
                                      fmtgid(gid));
                            else if (gid < 0)
                                error(ERROR_SYSTEM | 2, "%s: cannot change user to %s", file,
                                      fmtuid(uid));
                            else
                                error(ERROR_SYSTEM | 2,
                                      "%s: cannot change user to %s and group to %s", file,
                            ...
                            if (uid < 0)
                                ...
                                error(1, "%s: changed user to %s and group to %s", file,
                                      fmtuid(uid), fmtgid(gid));
                        }
                    }
                    if ((st.st_mode & S_IPERM) ^ mode) {
                        if (state->permissions) {
                            if (chmod(file, mode) < 0)
                                error(ERROR_SYSTEM | 2, "%s: cannot change mode to %s", file,
                                      fmtmode(mode, 0));
                            else
                                error(ERROR_SYSTEM | 1, "%s: changed mode to %s", file,
                                      fmtmode(mode, 0));
        ...
        s += 7;
        if (state->sum != state->oldsum) sumclose(state->sum);
        if (!(state->sum = sumopen(s))) error(3, "%s: %s: unknown checksum method", check, s);
...
static void list(State_t *state, Sfio_t *lp) {
    char *file;
    Sfio_t *sp;
...
static int optinfo(Opt_t *op, Sfio_t *sp, const char *s, Optdisc_t *dp) {
    if (streq(s, "methods")) return sumusage(sp);
    return 0;
}

int b_cksum(int argc, char **argv, Shbltin_t *context) {
    int flags;
    char *file;
    char *method;
    Sfio_t *sp;
    ...
    for (;;) {
        switch (optget(argv, usage)) {
            ...
            case 'B':
                ...
                continue;
            case 'c':
                if (!(state.check = sfstropen())) error(3, "out of space [check]");
                continue;
            case 'h':
                state.header = 1;
                continue;
    ...
    if (method && !(state.sum = sumopen(method))) error(3, "%s: unknown checksum method", method);
    if (!state.sum && !(state.sum = sumopen(error_info.id)) &&
        !(state.sum = sumopen(astconf("UNIVERSE", NULL, NULL))))
    ...
    if (!state.check && (state.header || state.permissions)) {
        ...
        if (state.permissions) sfprintf(sfstdout, "permissions\n");
        if (state.text) sfprintf(sfstdout, "text\n");
    }
    if (state.list) {
        if (*argv) {
            while (file = *argv++)
        ...
        while (!sh_checksig(context) && (ent = fts_read(fts))) switch (ent->fts_info) {
                ...
                case FTS_F:
                    if (sp = openfile(ent->fts_accpath, "rb")) {
                        pr(&state, sfstdout, sp, ent->fts_path, state.permissions, ent->fts_statp,
                           state.check);
                        closefile(sp);
                    }
                    break;
    ...
    sumclose(state.sum);
    if (state.check) sfclose(state.check);
