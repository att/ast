...
static void split(Fmt_t *fp, char *buf, int splice) {
    char *cp;
    char *ep;
    char *qp;
    int c = 1;
    ...
    if (fp->prefix < prefix && !isoption(fp, 'c')) outline(fp);
    if (!fp->outp || prefix < fp->prefix) fp->prefix = prefix;
    while (c) {
        cp = ep;
        while (*ep == ' ') ep++;
        if (cp != ep && isoption(fp, 'u')) cp = ep - 1;
        while (c = *ep) {
            if (c == ' ') break;
            ep++;
        ...
        if (fp->nwords > 0 && &fp->outp[n] >= fp->endbuf && !fp->retain && !q) outline(fp);
    skip:
        if (fp->nwords == 0) {
            if (fp->prefix) memset(fp->outbuf, ' ', fp->prefix);
    ...
    if (isoption(fp, 's') || *buf == 0)
        ...
        if (!isoption(fp, 'o') && strchr(".:!?", fp->outp[-1])) *fp->outp++ = ' ';
        if (!splice && !fp->retain &&
            (!fp->quote || (fp->outp - fp->outbuf) < 2 || fp->outp[-2] != '\\' ||
             fp->outp[-1] != 'n' && fp->outp[-1] != 't' && fp->outp[-1] != ' '))
            *fp->outp++ = ' ';
    }
}
...
static int dofmt(Fmt_t *fp) {
    ...
    cp = 0;
    while (cp ||
           (cp = sfgetr(fp->in, '\n', 0)) && !(splice = 0) && (lp = cp + sfvalue(fp->in) - 1) ||
        ...
        for (b = 1;; b = 0) {
            ...
            if (isoption(fp, 'o')) {
                if (c == '\\') {
                    ...
                    if (c == ' ' && (dp == buf || *(dp - 1) == ' ')) continue;
                } else if (c == '"') {
                    if (b || cp >= lp) {
                        if (fp->quote) continue;
                        fp->section = 0;
                    }
                } else if (c == '\a') {
                    *dp++ = '\\';
                    ...
                    *dp++ = '\\';
                    ...
                    if (fp->retain) {
                        ...
                        while (cp < lp && *cp != ' ' && *cp != '\t' && *cp != ']' &&
                               dp < &buf[sizeof(buf) - 3])
                            *dp++ = *cp++;
                        if (cp < lp && (*cp == ' ' || *cp == '\t')) *dp++ = *cp++;
                        *dp = 0;
                        split(fp, buf, 0);
                        dp = buf;
                        ep = 0;
                        fp->retain = 0;
                        if (fp->outp >= fp->endbuf) outline(fp);
                        continue;
                    }
                } else if (c == ' ' || c == '\t')
                    for (c = ' '; *cp == ' ' || *cp == '\t'; cp++)
...
int b_fmt(int argc, char **argv, Shbltin_t *context) {
    ...
    if (isoption(&fmt, 'o')) setoption(&fmt, 'c');
    if (isoption(&fmt, 's')) clroption(&fmt, 'u');
    if (cp = *argv) argv++;
    do {
        if (!cp || streq(cp, "-"))
            fmt.in = sfstdin;
        ...
        dofmt(&fmt);
        if (fmt.in != sfstdin) sfclose(fmt.in);
    } while (cp = *argv++);
    outline(&fmt);
