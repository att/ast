...
static int rm(State_t *state, FTSENT *ent) {
    char *path;
    int n;
    int v;
    struct stat st;
        ...
        switch (ent->fts_info) {
                        ...
                        if (v)
                            v = st.st_nlink <= 2 ||
                                st.st_ino == ent->fts_parent->fts_statp->st_ino &&
                                    st.st_dev == ent->fts_parent->fts_statp->st_dev ||
                                strchr(astconf("PATH_ATTRIBUTES", path, NULL), 'l');
                    } else
                        v = 1;
                    if (v) {
                        ...
                        if (!state->directory && ent->fts_info == FTS_D) break;
                    } else {
                        ent->fts_info = FTS_DC;
            ...
            case FTS_DP:
                if (isempty(ent) || state->directory) {
                    ...
                    if (path[0] != '.' || path[1]) {
                        path = ent->fts_accpath;
                        if (state->verbose) sfputr(sfstdout, ent->fts_path, '\n');
                        if ((state->recursive || state->directory) ? rmdir(path) : unlink(path))
                            switch (errno) {
                                ...
                                default:
                                    nonempty(ent);
                                    if (!state->force)
                                        error(ERROR_SYSTEM | 2, "%s: directory not removed",
                                              ent->fts_path);
                                    else
                                        error_info.errors++;
                                    break;
            ...
            default:
                ...
                } else if (!(ent->fts_info & FTS_SL) && !state->force && state->terminal &&
                           eaccess(path, W_OK)) {
...
int b_rm(int argc, char **argv, Shbltin_t *context) {
    State_t state;
    FTS *fts;
    FTSENT *ent;
    int set3d;
    ...
    argv += opt_info.index;
    if (*argv && streq(*argv, "-") && !streq(*(argv - 1), "--")) argv++;
    if (error_info.errors || !*argv && !state.force) error(ERROR_USAGE | 4, "%s", optusage(NULL));
    if (!*argv) return 0;
