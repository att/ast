...
static long mypathconf(const char *path, int op) {
    ...
    errno = 0;
    if ((r = strtol(astconf(ops[op], path, NULL), NULL, 0)) < 0 && !errno) return LONG_MAX;
    return r;
}
...
static int pathchk(char *path, int mode) {
    ...
    if (!*path) {
        if (mode & PATH) error(2, "path is empty");
        return -1;
    }
    if (mode & COMPONENTS) {
        ...
        tmp[0] = (*cp == '/' ? '/' : '.');
        tmp[1] = 0;
        if ((r = mypathconf(tmp, 0)) > _POSIX_NAME_MAX) name_max = r;
        if ((r = mypathconf(tmp, 1)) > _POSIX_PATH_MAX) path_max = r;
        if (*cp != '/') {
            if (name_max == 0 || path_max == 0) {
                if (!(cpold = getcwd((char *)0, 0)) && errno == EINVAL &&
                    (cpold = newof(0, char, PATH_MAX, 0)) && !getcwd(cpold, PATH_MAX)) {
                    free(cpold);
                    cpold = 0;
                }
                if (cpold) {
        ...
        if (name_max == 0) name_max = _POSIX_NAME_MAX;
        if (path_max == 0) path_max = _POSIX_PATH_MAX;
        while (*(cpold = cp)) {
            while ((c = *cp++) && c != '/')
                ;
            if ((cp - cpold) > name_max) goto err;
            errno = 0;
            cp[-1] = 0;
            r = mypathconf(path, 0);
            if ((cp[-1] = c) == 0)
    ...
    while (*(cpold = cp)) {
        ...
        while ((c = *cp++) && c != '/')
            if ((mode & COMPONENTS) && !isport(c)) {
                buf[0] = c;
                buf[1] = 0;
                error(2, "%s: '%s' not in portable character set", path,
                      fmtquote(buf, NULL, "'", 1, 0));
                return -1;
            }
        if ((cp - cpold) > name_max) goto err;
        if (c == 0) break;
...
int b_pathchk(int argc, char **argv, Shbltin_t *context) {
    int mode = 0;
    char *s;
    ...
    argv += opt_info.index;
    if (!*argv || error_info.errors) error(ERROR_usage(2), "%s", optusage(NULL));
    while (s = *argv++) pathchk(s, mode);
