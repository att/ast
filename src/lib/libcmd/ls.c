...
static const char usage[] =
    ...
    "[N:literal|show-controls-chars?Print raw entry names (don't treat e.g. control characters "
    "specially).]"
...
#define VISIBLE(s, f)                                                                     \
    ((f)->fts_level <= 0 ||                                                               \
     (!(s)->ignore || !strmatch((f)->fts_name, (s)->ignore)) &&                           \
         (!((s)->lsflags & LS_NOBACKUP) || (f)->fts_name[(f)->fts_namelen - 1] != '~') && \
         (((s)->lsflags & LS_ALL) || (f)->fts_name[0] != '.' ||                           \
          ((s)->lsflags & LS_MOST) &&                                                     \
              ((f)->fts_name[1] && (f)->fts_name[1] != '.' || (f)->fts_name[2])))
...
static char *printable(State_t *state, char *s) {
    char *t;
    char *p;
    int c;
    wchar_t w;
    ...
    if (state->lsflags & LS_ESCAPE) {
        if (!(state->lsflags & LS_QUOTE)) return fmtesc(s);
    ...
    t = state->prdata;
    if (state->lsflags & LS_QUOTE) *t++ = '"';
    if (!mbwide())
        while (c = *s++) *t++ = (iscntrl(c) || !isprint(c)) ? '?' : c;
    else {
        mbinit(&q);
...
static int key(void *handle, Sffmt_t *fp, const char *arg, char **ps, Sflong_t *pn) {
    ...
    if (!(kp = (Key_t *)dtmatch(state->keys, fp->t_str))) {
        ...
        if (!(kp = vmnewof(state->vm, 0, Key_t, 1, strlen(fp->t_str) + 1))) goto nospace;
        kp->name = (char *)(kp + 1);
    ...
    if (kp->macro && !kp->disable) {
        kp->disable = 1;
        if (!mp && !(mp = sfstropen())) goto nospace;
        ...
        switch (kp->index) {
            case KEY_atime:
                ...
                if (!arg) arg = state->timefmt;
                break;
            case KEY_blocks:
                if (st) n = BLOCKS(state, st);
                break;
            case KEY_ctime:
                if (st) {
                    n = st->st_ctime;
            ...
            case KEY_devmajor:
                if (st)
                    n = (S_ISBLK(st->st_mode) || S_ISCHR(st->st_mode)) ? major(idevice(st))
                                                                       : major(st->st_dev);
                break;
            case KEY_devminor:
                if (st)
                    n = (S_ISBLK(st->st_mode) || S_ISCHR(st->st_mode)) ? minor(idevice(st))
                                                                       : minor(st->st_dev);
                break;
            case KEY_dir_blocks:
                if (!state->scale) {
            ...
            case KEY_linkpath:
                if (ent && (ent->fts_info & FTS_SL)) {
                    ...
                    if ((st->st_size + 1) > state->txtsize) {
                        ...
                        if (!(state->txtdata =
                                  vmnewof(state->vm, state->txtdata, char, state->txtsize, 0)))
                            goto nospace;
                    }
                    if (*ent->fts_name == '/' || !state->list.dirnam)
                        dirnam = ent->fts_name;
                    else {
                        sfprintf(state->tmp, "%s/%s",
                                 state->list.dirnam + streq(state->list.dirnam, "/"),
                                 ent->fts_name);
                        if (!(dirnam = sfstruse(state->tmp))) goto nospace;
                    }
            ...
            case KEY_mark:
                ...
                break;
            case KEY_markdir:
                if (!st || !S_ISDIR(st->st_mode)) return 0;
                s = "/";
                break;
            case KEY_mode:
                if (st) n = st->st_mode;
            ...
            case KEY_mtime:
                ...
                break;
            case KEY_name:
                if (ent) s = PRINTABLE(state, ent->fts_name);
                break;
            case KEY_nlink:
                if (st) n = st->st_nlink;
                break;
...
static void pr(State_t *state, FTSENT *ent, int fill) {
#ifdef S_ISLNK
    /*
     * -H == --hairbrained
    ...
    if (ent->fts_level == 0 &&
        (state->ftsflags & (FTS_META | FTS_PHYSICAL)) == (FTS_META | FTS_PHYSICAL) &&
        !(ent->fts_info & FTS_D) &&
        !lstat(ent->fts_path ? ent->fts_path : ent->fts_name, ent->fts_statp) &&
...
static void col(State_t *state, FTSENT *ent, int length) {
    FTSENT *p;
    int i;
    int n;
    int files;
    ...
    if ((files = state->list.count.files) > 0) {
        ...
        if (state->lsflags & LS_COMMAS) {
            ...
            for (p = ent->fts_link; p; p = p->fts_link)
                if (p->fts_number != INVISIBLE) pr(state, p, 0);
        } else {
            FTSENT **x;
            int c;
            int j;
            ...
            if (files > state->sizsiz) {
                ...
                if (!(state->siz =
                          vmnewof(state->vm, state->siz, unsigned short, state->sizsiz, 0)))
                    error(ERROR_SYSTEM | 2, "out of space");
            }
            ...
            i = 0;
            for (p = ent->fts_link; p; p = p->fts_link)
                if (p->fts_number != INVISIBLE) x[i++] = p;
            ...
            if ((state->lsflags & LS_ACROSS) && n > 1) {
                ...
                do {
                    ...
                    for (j = 0; j < c; j++) {
                        ...
                        for (l = 0, r = j; l < n && r < i; r += c, l++)
                            if (z < (x[r]->fts_namelen + a)) z = x[r]->fts_namelen + a;
                        w += z + BETWEEN;
                    }
                    if (w <= state->width) o = n;
                ...
                for (j = 0; j < c; j++) {
                    state->siz[k] = 0;
                    for (l = 0, r = j; l < n && r < i; r += c, l++)
                        if (state->siz[k] < x[r]->fts_namelen) state->siz[k] = x[r]->fts_namelen;
                ...
                if (n > 1) {
                    ...
                    for (c = q; (c - q) < 2 && c <= state->width / (BETWEEN + 1); ++c) {
                        n = m = (i + c - 1) / c;
                        if ((r = i - m * c) > state->height) n -= (r + c - 1) / c;
                        for (; n <= m; n++) {
                            w = -AFTER;
                            j = 0;
                            while (j < i) {
                                z = 0;
                                for (l = 0; l < n && j < i; j++, l++)
                                    if (z < x[j]->fts_namelen) z = x[j]->fts_namelen;
                                w += z + a + BETWEEN;
                            }
                            if (w <= state->width) {
                                q = c;
                ...
                while (j < i) {
                    state->siz[k] = 0;
                    for (l = 0; l < n && j < i; j++, l++)
                        if (state->siz[k] < x[j]->fts_namelen) state->siz[k] = x[j]->fts_namelen;
                    state->siz[k] += a + BETWEEN;
                    k++;
                }
                for (j = 0; j < n; j++)
...
static int order_none(FTSENT *f1, FTSENT *f2) { return 0; }

static int order_blocks(FTSENT *f1, FTSENT *f2) {
    if (f1->fts_statp->st_size < f2->fts_statp->st_size) return 1;
    if (f1->fts_statp->st_size > f2->fts_statp->st_size) return -1;
    return 0;
}

static int order_atime(FTSENT *f1, FTSENT *f2) {
    Time_t t1;
    Time_t t2;
...
static int order_ctime(FTSENT *f1, FTSENT *f2) {
    Time_t t1;
    Time_t t2;
...
static int order_mtime(FTSENT *f1, FTSENT *f2) {
    Time_t t1;
    Time_t t2;
...
static int order_extension(FTSENT *f1, FTSENT *f2) {
    int n;
    char *x1;
    char *x2;
    ...
    if (x1) {
        ...
        n = 0;
    if (!n) n = strcoll(f1->fts_name, f2->fts_name);
    return n;
}

static int order_version(FTSENT *f1, FTSENT *f2) { return strvcmp(f1->fts_name, f2->fts_name); }

static int order_name(FTSENT *f1, FTSENT *f2) { return strcoll(f1->fts_name, f2->fts_name); }

/*
 * order child entries
 */

static int order(FTSENT *const *f1, FTSENT *const *f2) {
...
static void dir(State_t *state, FTSENT *ent) {
    FTSENT *p;
    int length;
    int top = 0;
    ...
    for (p = ent->fts_link; p; p = p->fts_link) {
        if (p->fts_level == 0 && p->fts_info == FTS_D && !(state->lsflags & LS_DIRECTORY)) {
            ...
            if (p->fts_namelen > length) length = p->fts_namelen;
            if (!(state->lsflags & LS_RECURSIVE)) fts_set(NULL, p, FTS_SKIP);
        } else {
        invisible:
            p->fts_number = INVISIBLE;
    ...
    if (top) {
        ...
        for (p = ent->fts_link; p; p = p->fts_link)
            if (p->fts_level == 0 && p->fts_info == FTS_D) p->fts_number = 0;
    }
}
...
static int ls(State_t *state, FTSENT *ent) {
    ...
    switch (ent->fts_info) {
        case FTS_NS:
            if (ent->fts_parent->fts_info == FTS_DNX) break;
            error(2, "%s: not found", ent->fts_path);
            return 1;
        case FTS_DC:
            if (state->lsflags & LS_DIRECTORY) break;
        ...
        case FTS_D:
        case FTS_DNX:
            if ((state->lsflags & LS_DIRECTORY) && ent->fts_level >= 0) break;
            if (!(state->lsflags & LS_RECURSIVE))
                fts_set(NULL, ent, FTS_SKIP);
            else if (ent->fts_info == FTS_DNX) {
                error(2, "%s: cannot search directory", ent->fts_path, ent->fts_level);
                fts_set(NULL, ent, FTS_SKIP);
                if (ent->fts_level > 0 && !(state->lsflags & LS_NOSTAT)) return 0;
            }
            if (ent->fts_level >= 0) fts_children(ent->fts, 0);
            dir(state, ent);
            return 0;
    }
    fts_set(NULL, ent, FTS_SKIP);
...
static int walk(const char *path, int (*userf)(State_t *, FTSENT *), int flags,
                int (*comparf)(FTSENT *const *, FTSENT *const *), State_t *state) {
    FTS *f;
    FTSENT *e;
    int rv;
    int oi;
    ...
    if (!(f = fts_open((char *const *)path, flags, comparf))) {
        if (!path || !(path = (const char *)(*((char **)path)))) return -1;
        ns = strlen(path) + 1;
        if (!(e = vmnewof(state->vm, 0, FTSENT, 1, ns))) return -1;
    ...
    if (e = fts_children(f, 0)) {
        ...
        for (x = e; x; x = x->fts_link)
            if (x->fts_info & FTS_DD) {
                x->fts_info &= ~FTS_DD;
                dd[nd++] = x;
                if (nd >= elementsof(dd)) break;
            }
        e->fts_parent->fts_link = e;
        rv = (*userf)(state, e->fts_parent);
        e->fts_parent->fts_link = 0;
        while (nd > 0) dd[--nd]->fts_info |= FTS_DD;
        for (x = e; x; x = x->fts_link)
            if (!(x->fts_info & FTS_D)) x->_fts_status = FTS_SKIP;
    }
    while (!rv && !sh_checksig(state->context) && (e = fts_read(f))) {
        ...
        rv = (*userf)(state, e);
        e->fts_info = oi;
        if (e->_fts_status == ns) e->_fts_status = os;
        while (nd > 0) dd[--nd]->fts_info |= FTS_DD;
    }
    fts_close(f);
    return rv;
...
int b_ls(int argc, char **argv, Shbltin_t *context) {
    int n;
    char *s;
    char *e;
    FTS *fts;
    ...
    static char fmt_color[] =
        "%(mode:case:d*:\\E[01;34m%(name)s\\E[0m:l*:\\E[01;36m%(name)s\\E[0m:*x*:\\E[01;32m%(name)"
        "s\\E[0m:*:%(name)s)s";
    ...
    for (n = 1; n < elementsof(keys); n++) dtinsert(state.keys, keys + n);
    if (streq(s, "lc"))
        ...
        state.lsflags |= LS_COLUMNS;
        if (!strmatch(setlocale(LC_ALL, NULL), "*[Uu][Tt][Ff]?(-)8")) state.lsflags |= LS_PRINTABLE;
    }
    ...
    while (n = optget(argv, usage)) {
        switch (n) {
            ...
            case 'c':
                ...
                if (!state.order) state.order = order_ctime;
                break;
            case 'd':
            ...
            case 's':
                ...
                break;
            case 't':
                if (set(&state, LS_TIME) && !state.order) state.order = order_mtime;
                break;
            case 'u':
                state.lsflags &= ~LS_CTIME;
                state.lsflags |= LS_ATIME;
                if (!state.order) state.order = order_atime;
                break;
            case 'w':
                ...
                if (*e) error(2, "%s: invalid screen width specification at `%s'", opt_info.arg, e);
                break;
            case 'x':
            ...
            case 'z':
                switch (opt_info.num) {
                    case -10:
                        if (!strcmp(setlocale(LC_TIME, NULL), "C")) break;
                        /*FALLTHROUGH*/
                    case 'i':
            ...
            case 'D':
                ...
                if (kp->macro = s) {
                    stresc(s);
                    if (strmatch(s, "*:case:*")) state.lsflags |= LS_STAT;
                }
                break;
            case 'E':
                state.lsflags |= LS_LONG;
            ...
            case 'Y':
                ...
                break;
            case 'Z':
                if (!sfstrtell(fmt)) state.lsflags &= ~LS_COLUMNS;
            ...
            case '1':
                ...
                break;
            case -101:
                if (opt_info.num <= 0) error(3, "%ld: invalid block size", opt_info.num);
            ...
            case -104:
                ...
                if (*e) error(3, "%s: invalid date string", opt_info.arg, opt_info.option);
                break;
            case -105:
        ...
        if (!strchr(state.flags, n)) *state.endflags++ = n;
    }
    argv += opt_info.index;
    if (error_info.errors) error(ERROR_USAGE | 4, "%s", optusage(NULL));
    if (state.lsflags == (lsflags | LS_TIME))
        state.ftsflags |= FTS_SEEDOTDIR; /* keep configure happy */
    if (state.lsflags & LS_DIRECTORY) state.lsflags &= ~LS_RECURSIVE;
    if (!state.order) state.order = order_name;
    if (!state.timeflags) state.timeflags = state.lsflags;
    if (state.lsflags & (LS_COLUMNS | LS_COMMAS)) {
        if (state.lsflags & LS_LONG)
            ...
            if (!state.width) {
                ...
                if (state.width <= 20) state.width = 80;
            }
            if (state.height <= 4) state.height = 24;
        }
    }
    ...
    if (!sfstrtell(fmt)) {
        if (state.lsflags & LS_INUMBER) sfputr(fmt, "%6(ino)u ", -1);
        if (state.lsflags & LS_BLOCKS) sfputr(fmt, "%5(blocks)u ", -1);
        if (state.lsflags & LS_LONG) {
        ...
        if (state.lsflags & LS_LONG) sfputr(fmt, "%(linkop:case:?*: %(linkop)s %(linkpath)s)s", -1);
    } else
    ...
    if (dump)
        ...
        if (argv[1]) state.lsflags |= LS_LABEL;
    ...
    if (fmt) sfstrclose(fmt);
    if (state.tmp) sfstrclose(state.tmp);
    if (state.vm) vmclose(state.vm);
