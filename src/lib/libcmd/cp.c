...
static const char usage_head[] = "[-?@(#)$Id: cp (AT&T Research) 2013-07-16 $\n]" USAGE_LICENSE;
...
static void preserve(State_t *state, const char *path, struct stat *ns, struct stat *os) {
    int n;
        ...
        if (n && chown(state->path, os->st_uid, os->st_gid)) {
            switch (n) {
                case 01:
                    ...
                    break;
                case 02:
                    error(ERROR_SYSTEM | 2, "%s: cannot reset owner to %s", path,
                          fmtuid(os->st_uid));
                    break;
                case 03:
                    error(ERROR_SYSTEM | 2, "%s: cannot reset owner to %s and group to %s", path,
...
static int visit(State_t *state, FTSENT *ent) {
    char *base;
    int n;
    int len;
    int rm;
    ...
    if (ent->fts_level == 0) {
        ...
        if (state->hierarchy)
            ...
            while (base > ent->fts_name && *(base - 1) == '/') base--;
            while (base > ent->fts_name && *(base - 1) != '/') base--;
            len -= base - ent->fts_name;
            if (state->directory) state->presiz -= len + 1;
        }
    } else {
    ...
    if (state->directory) {
        if ((state->postsiz + len) > state->pathsiz &&
            !(state->path = newof(state->path, char,
                                  state->pathsiz = roundof(state->postsiz + len, PATH_CHUNK), 0)))
                ...
                *e = 0;
                if (access(state->path, F_OK)) {
                    ...
                    if (mkdir(state->path, st.st_mode & S_IPERM)) {
                        error(ERROR_SYSTEM | 2, "%s: cannot create directory -- %s ignored",
                              state->path, ent->fts_path);
                        fts_set(NULL, ent, FTS_SKIP);
                        return 0;
                    }
                }
    ...
    switch (ent->fts_info) {
        case FTS_DP:
            if (state->preserve && state->op != LN ||
                ent->fts_level > 0 && (ent->fts_statp->st_mode & S_IRWXU) != S_IRWXU) {
                ...
                if (stat(state->path, &st))
                    ...
                    if ((ent->fts_statp->st_mode & S_IPERM) != (st.st_mode & S_IPERM) &&
                        chmod(state->path, ent->fts_statp->st_mode & S_IPERM))
                        error(ERROR_SYSTEM | 2, "%s: cannot reset directory mode to %s",
                              state->path, fmtmode(st.st_mode & S_IPERM, 0) + 1);
            ...
            if (!state->recursive) {
                ...
                switch (ent->fts_info) {
                    ...
                    case FTS_DNX:
                        ...
                        /*FALLTHROUGH*/
                    case FTS_D:
                        if (state->directory) memcpy(state->path + state->postsiz, base, len);
                        if (!(*state->stat)(state->path, &st)) {
                            if (!S_ISDIR(st.st_mode)) {
                                error(2, "%s: not a directory -- %s ignored", state->path,
                                      ent->fts_path);
                                return 0;
                            }
                        } else if (mkdir(state->path, (ent->fts_statp->st_mode & S_IPERM) |
    ...
    if ((*state->stat)(state->path, &st))
        ...
        if (!rm || !state->force) {
            if (S_ISLNK(st.st_mode) && (n = -1) ||
                (n = open(state->path, O_RDWR | O_BINARY | O_CLOEXEC)) >= 0) {
                if (n >= 0) close(n);
                if (state->force)
                    /* ok */;
                else if (state->interactive) {
                    if ((n = astquery(-1, "%s %s? ", state->opname, state->path)) < 0 ||
                ...
                if (state->interactive) {
                    if ((n = astquery(-1, "override protection %s for %s? ", protection,
                                      state->path)) < 0 ||
                        sh_checksig(state->context))
                        return -1;
                    if (n) return 0;
        ...
        switch (state->backup) {
            ...
            case BAK_number:
                ...
                if (fts = fts_open((char **)e,
                                   FTS_NOCHDIR | FTS_ONEPATH | FTS_PHYSICAL | FTS_NOPOSTORDER |
                                       FTS_NOSTAT | FTS_NOSEEDOTDIR,
                                   NULL)) {
                    while (sub = fts_read(fts)) {
                        if (strneq(s, sub->fts_name, n) && sub->fts_name[n] == '.' &&
                            strneq(sub->fts_name + n + 1, state->suffix, state->suflen) &&
                            (m = strtol(sub->fts_name + n + state->suflen + 1, &e, 10)) &&
                            streq(e, state->suffix) && m > v)
                            v = m;
                        if (sub->fts_level) fts_set(NULL, sub, FTS_SKIP);
                    }
                    fts_close(fts);
                }
                if (s != state->path) *--s = '/';
                if (v || state->backup == BAK_number) {
                    sfprintf(state->tmp, "%s.%s%d%s", state->path, state->suffix, v + 1,
                             state->suffix);
                    goto backup;
                }
                /*FALLTHROUGH*/
...
operate:
    switch (state->op) {
        case MV:
            for (;;) {
                if (!rename(ent->fts_path, state->path)) return 0;
                if (errno == ENOENT)
                    rm = 1;
        ...
        case CP:
            if (S_ISLNK(ent->fts_statp->st_mode)) {
                ...
                if (pathsetlink(state->text, state->path)) {
                    error(ERROR_SYSTEM | 2, "%s: cannot copy symbolic link to %s", ent->fts_path,
                          state->path);
                    return 0;
                }
                ...
                if ((!S_ISREG(ent->fts_statp->st_mode) || ent->fts_statp->st_size > 0) &&
                    ...
                    if (!(op = sfnew(NULL, NULL, SF_UNBOUND, wfd, SF_WRITE))) {
                        error(ERROR_SYSTEM | 2, "%s: %s write stream error", ent->fts_path,
                              state->path);
                        close(wfd);
                        sfclose(ip);
                        return 0;
                    ...
                    n = 0;
                    if (sfmove(ip, op, (Sfoff_t)SF_UNBOUND, -1) < 0) n |= 3;
                    if (!sfeof(ip)) n |= 1;
                    if (sfsync(op) || state->sync && fsync(wfd) || sfclose(op)) n |= 2;
                    if (sfclose(ip)) n |= 1;
                    if (n) {
                        error(ERROR_SYSTEM | 2, "%s: %s %s error", ent->fts_path, state->path,
                ...
                if (mknod(state->path, ent->fts_statp->st_mode, idevice(ent->fts_statp))) {
                    error(ERROR_SYSTEM | 2, "%s: cannot copy special file to %s", ent->fts_path,
                          state->path);
                    return 0;
                }
            } else {
                error(2, "%s: cannot copy -- unknown file type 0%o", ent->fts_path,
                ...
                if (ent->fts_info != FTS_SL) {
                    if (stat(state->path, &st))
                        ...
                        if ((state->preserve & PRESERVE_PERM) &&
                            (ent->fts_statp->st_mode & state->perm) != (st.st_mode & state->perm) &&
                            chmod(state->path, ent->fts_statp->st_mode & state->perm))
                            error(ERROR_SYSTEM | 2, "%s: cannot reset mode to %s", state->path,
...
int b_cp(int argc, char **argv, Shbltin_t *context) {
    char *file;
    char *s;
    char **v;
    char *backup_type;
    ...
    if (!(sh = CMD_CONTEXT(context)) || !(state = (State_t *)sh->ptr)) {
        if (!(state = newof(0, State_t, 1, 0))) error(ERROR_SYSTEM | 3, "out of space");
        if (sh) sh->ptr = state;
    } else
    ...
    if (!(usage = sfstruse(state->tmp))) error(ERROR_SYSTEM | 3, "%s: out of space", state->path);
    ...
    for (;;) {
        switch (optget(argv, usage)) {
            ...
            case 'f':
                state->force = 1;
                if (state->op != CP || !standard) state->interactive = 0;
                continue;
            case 'h':
                state->hierarchy = 1;
                continue;
            case 'i':
                state->interactive = 1;
                if (state->op != CP || !standard) state->force = 0;
                continue;
            case 'l':
                state->op = LN;
                state->link = link;
            ...
            case 'r':
                state->recursive = 1;
                if (path_resolve < 0) path_resolve = 0;
                continue;
            case 's':
                state->op = LN;
        ...
        if (!(file = backup_type) && !(backup_type = getenv("VERSION_CONTROL")))
            ...
            switch (strkey(backup_type)) {
                ...
                case HASHKEY1('t'):
                    ...
                    break;
                default:
                    if (file) error(2, "%s: unknown backup type", backup_type);
                    break;
            }
    ...
    if (argc <= 0 || error_info.errors) error(ERROR_USAGE | 4, "%s", optusage(NULL));
    if (!path_resolve) state->flags |= fts_flags() | FTS_META;
    file = argv[argc];
    argv[argc] = 0;
    if (s = strrchr(file, '/')) {
        while (*s == '/') s++;
    ...
    if (file != (char *)dot) pathcanon(file, 0, 0);
    ...
    if (state->pathsiz < roundof(state->postsiz + 2, PATH_CHUNK) &&
        !(state->path = newof(state->path, char,
                              state->pathsiz = roundof(state->postsiz + 2, PATH_CHUNK), 0)))
        error(ERROR_SYSTEM | 3, "out of space");
    memcpy(state->path, file, state->postsiz + 1);
    if (state->directory && state->path[state->postsiz - 1] != '/')
        state->path[state->postsiz++] = '/';
    if (state->hierarchy) {
        if (!state->directory) error(3, "%s: last argument must be a directory", file);
        state->missmode = st.st_mode;
    }
    state->perm = state->uid ? S_IPERM : (S_IPERM & ~S_ISVTX);
    if (!state->recursive) state->flags |= FTS_TOP;
