...
static int paste(int nstream, Sfio_t *streams[], Sfio_t *out, const char *delim, int dsiz, int dlen,
                 Delim_t *mp) {
    const char *cp;
    int d, n, i, z, more = 1;
    Sfio_t *fp;
                ...
                if (cp = sfgetr(fp, '\n', 0)) {
                    ...
                    else if (!more) /* first stream with output */
                    {
                        if (dsiz == 1)
                            ...
                            for (d = n; d > dlen; d -= dlen) sfwrite(out, delim, dsiz);
                            if (d) {
                                if (mp)
                                    for (i = z = 0; i < d; i++) z += mp[i].len;
                                else
                                    z = d;
                                sfwrite(out, delim, z);
                            }
                    ...
                    if (sfwrite(out, cp, sfvalue(fp) - ((n + 1) < nstream)) < 0) return (-1);
                } else
                    streams[n] = 0;
            }
...
static int spaste(Sfio_t *in, Sfio_t *out, const char *delim, int dsiz, int dlen, Delim_t *mp) {
    const char *cp;
    int d = 0;
    if ((cp = sfgetr(in, '\n', 0)) && sfwrite(out, cp, sfvalue(in) - 1) < 0) return (-1);
    while (cp = sfgetr(in, '\n', 0)) {
        if (dlen) {
            int c;
            if (d >= dlen) d = 0;
        ...
        if (sfwrite(out, cp, sfvalue(in) - 1) < 0) return (-1);
    }
    sfputc(out, '\n');
    return (0);
}

int b_paste(int argc, char **argv, Shbltin_t *context) {
    int n, sflag = 0;
    Sfio_t *fp, **streams;
    char *cp, *delim;
    char *ep;
    ...
    if (!(delim = strdup(delim))) error(ERROR_system(1), "out of space");
    ...
    if (!sflag) {
        if (error_info.errors == 0 && paste(n, streams, sfstdout, delim, dsiz, dlen, mp) < 0)
            error(ERROR_system(0), "write failed");
        while (--n >= 0)
            if ((fp = streams[n]) && fp != sfstdin) sfclose(fp);
    }
    if (mp) free(mp);
    free(delim);
