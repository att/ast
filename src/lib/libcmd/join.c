...
static void done(Join_t *jp) {
    if (jp->file[0].iop && jp->file[0].iop != sfstdin) sfclose(jp->file[0].iop);
    if (jp->file[1].iop && jp->file[1].iop != sfstdin) sfclose(jp->file[1].iop);
    if (jp->outlist) free(jp->outlist);
    if (jp->file[0].fields) free(jp->file[0].fields);
    if (jp->file[1].fields) free(jp->file[1].fields);
    if (jp->same) free(jp->same);
    free(jp);
}

static Join_t *init(void) {
    Join_t *jp;
    int i;

    if (jp = newof(0, Join_t, 1, 0)) {
        if (jp->mb = mbwide())
            for (i = 0x80; i <= 0xff; i++) jp->state[i] = S_WIDE;
 ...
 */
static unsigned char *getrec(Join_t *jp, int index, int discard) {
    ...
    if (sh_checksig(jp->context)) return 0;
    if (discard && fp->discard) sfraise(fp->iop, SFSK_DISCARD, NULL);
    fp->spaces = 0;
    fp->hit = 0;
    if (!(cp = sfgetr(fp->iop, '\n', 0))) {
        jp->outmode &= ~(1 << index);
...
#define getrec(p, n, d)                                                      \
    (u1 = getrec(p, n, d),                                                   \
     sfprintf(sfstdout, "[G%d#%d@%I*d:%-.8s]", __LINE__, n, sizeof(Sfoff_t), \
              sftell(p->file[n].iop), u1),                                   \
     u1)
#endif
 ...
 */
static int outfield(Join_t *jp, int index, int n, int last) {
    File_t *fp = &jp->file[index];
    char *cp;
    char *cpmax;
    int size;
    ...
    if (n == -1) {
        if (size <= 1) {
            if (jp->nullfield && sfputr(iop, jp->nullfield, -1) < 0) return -1;
        } else if (sfwrite(iop, cp, size) < 0)
            return -1;
        if (sfwrite(iop, jp->delimstr, jp->delimlen) < 0) return -1;
    } else if (size <= 1) {
        if (!jp->nullfield)
            sfputc(iop, n);
        ...
        last = cp[size - 1];
        cp[size - 1] = n;
        if (sfwrite(iop, cp, size) < 0) return -1;
        cp[size - 1] = last;
    }
    return 0;
}
...
#define outfield(p, i, n, f)                                                 \
    (sfprintf(sfstdout, "[F%d#%d:%d,%d]", __LINE__, i1 = i, i2 = n, i3 = f), \
     outfield(p, i1, i2, i3))
#endif

static int outrec(Join_t *jp, int mode) {
    File_t *fp;
    int i;
    int j;
    int k;
    ...
    if (mode < 0 && jp->file[0].hit++) return 0;
    ...
    if (out = jp->outlist) {
        while ((n = *out++) >= 0) {
            if (n == JOINFIELD) {
                ...
                i = n & 1;
                j = (mode < 0 && i || mode > 0 && !i) ? jp->file[i].nfields : n >> 2;
            }
            if (outfield(jp, i, j, *out < 0) < 0) return -1;
        }
        return 0;
    }
    k = jp->file[0].nfields;
    if (mode >= 0) k += jp->file[1].nfields - 1;
    for (i = 0; i < 2; i++) {
        fp = &jp->file[i];
        if (mode > 0 && i == 0) {
            k -= (fp->nfields - 1);
        ...
        if (mode || i == 0) {
            ...
            if (outfield(jp, i, n, !--k) < 0) return -1;
            if (!k) return 0;
            for (j = 0; j < n; j++) {
                if (outfield(jp, i, j, !--k) < 0) return -1;
                if (!k) return 0;
            }
            j = n + 1;
        } else
            j = 0;
        for (; j < fp->nfields; j++) {
            if (j != n && outfield(jp, i, j, !--k) < 0) return -1;
            if (!k) return 0;
        }
    }
    return 0;
...
#if DEBUG_TRACE
#define outrec(p, n)                                                                         \
    (sfprintf(sfstdout, "[R#%d,%d,%lld,%lld:%-.*s{%d}:%-.*s{%d}]", __LINE__, i1 = n, lo, hi, \
              jp->file[0].fieldlen, cp1, jp->file[0].hit, jp->file[1].fieldlen, cp2,         \
              jp->file[1].hit),                                                              \
     outrec(p, i1))
#endif
        ...
        for (;;) {
            ...
            if (!n && !(cmp = n1 < n2 ? -1 : (n1 > n2)) ||
                n && !(cmp = (int)*cp1 - (int)*cp2) &&
                    !(cmp = jp->ignorecase ? strncasecmp((char *)cp1, (char *)cp2, n)
                                           : memcmp(cp1, cp2, n)))
                cmp = n1 - n2;
            sfprintf(sfstdout, "[C#%d:%d(%c-%c),%d,%lld,%lld%s]", __LINE__, cmp, *cp1, *cp2, same,
                     lo, hi, (jp->outmode & C_COMMON) ? ",COMMON" : "");
            if (!cmp)
#else
            if (!n && !(cmp = n1 < n2 ? -1 : (n1 > n2)) ||
                n && !(cmp = (int)*cp1 - (int)*cp2) &&
...
#endif
            {
                if (!(jp->outmode & C_COMMON)) {
                    ...
                    if ((jp->ooutmode & (C_FILE1 | C_FILE2)) != C_FILE2) break;
                    if (sfseek(jp->file[0].iop, (Sfoff_t)-jp->file[0].reclen, SEEK_CUR) < 0 ||
                        !(cp1 = getrec(jp, 0, 0))) {
                ...
                next:
                    ...
                    if (!(cp2 = getrec(jp, 1, 0))) break;
                    n2 = jp->file[1].fieldlen;
                    if (n2 == o2 && *cp2 == *jp->same && !memcmp(cp2, jp->same, n2)) goto next;
                    continue;
                }
                if (hi >= 0) {
                ...
                same = 0;
                if (!(cp1 = getrec(jp, 0, 0))) break;
                n1 = jp->file[0].fieldlen;
                continue;
            }
            if (lo >= 0) {
                if ((hi = sfseek(jp->file[1].iop, (Sfoff_t)0, SEEK_CUR)) < 0 ||
                    (hi -= jp->file[1].reclen) < 0 || sfseek(jp->file[1].iop, lo, SEEK_SET) != lo ||
                    !(cp2 = getrec(jp, 1, 0))) {
                    error(ERROR_SYSTEM | 2, "%s: seek error", jp->file[1].name);
                    return -1;
                }
                n2 = jp->file[1].fieldlen;
                lo = -1;
                if (jp->file[1].discard) sfseek(jp->file[1].iop, (Sfoff_t)-1, SEEK_SET);
            } else if (!cp2)
                break;
            else if ((jp->outmode & C_FILE1) && outrec(jp, -1) < 0)
                return -1;
            if (!(cp1 = getrec(jp, 0, 1))) break;
            n1 = jp->file[0].fieldlen;
        }
    }
#if DEBUG_TRACE
    sfprintf(sfstdout, "[X#%d:?,%p,%p,%d,%d,%d%s]", __LINE__, cp1, cp2, cmp, lo, hi,
...
#if DEBUG_TRACE
    sfprintf(sfstdout, "[X#%d:%d,%p,%p,%d,%02o,%02o%s]", __LINE__, n, cp1, cp2, cmp, jp->ooutmode,
             jp->outmode, (jp->outmode & C_COMMON) ? ",COMMON" : "");
#endif
    if (!cp1 || !(jp->outmode & (1 << n))) {
        if (cp1 && jp->file[n].iop == sfstdin) sfseek(sfstdin, (Sfoff_t)0, SEEK_END);
        return 0;
    }
    if (outrec(jp, cmp) < 0) return -1;
    do {
        if (!getrec(jp, n, 1)) return 0;
    } while (outrec(jp, cmp) >= 0);
    return -1;
}

int b_join(int argc, char **argv, Shbltin_t *context) {
    int n;
    char *cp;
    Join_t *jp;
    char *e;
...
#endif
    setlocale(LC_ALL, "");
    if (!(jp = init())) error(ERROR_system(1), "out of space");
    jp->context = context;
    for (;;) {
        switch (n = optget(argv, usage)) {
            ...
            case '1':
            case '2':
                if (opt_info.num <= 0) error(2, "field number must positive");
