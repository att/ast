...
static int namcomp(Dt_t *dp, Void_t *left, Void_t *right, Dtdisc_t *dsp) {
    ...
    char *r = (char *)right;
    char *suffix;
    if (suffix = strrchr(l, '.')) return (memcmp(l, r, suffix + 1 - l));
    return (strcmp(l, r));
}
...
static unsigned int is_omf(int fd) {
    ...
    if (read(fd, buff, 4) != 4) return (0);
    if (*buff == OMF_THEADR) return (buff[3]);
...
static Arfile_t *ar_getnode(Ardir_t *ar, const char *name) {
    ...
    if (cp = strrchr(name, '/')) name = cp + 1;
    fp = (Arfile_t *)dtmatch(dp, (void *)name);
    if (!fp) {
        size_t len = strlen(name) + 5;
        if (!(fp = newof(0, Arfile_t, 1, len))) return (0);
...
static int omfclose(Ardir_t *ar) {
...
static size_t displacement(struct Table *tp, size_t addr) {
    ...
    while (tp) {
        disp = tp->disp;
        if (addr > tp->offset) break;
        tp = tp->next;
    }
    return (disp);
}

static void omfload(Ardir_t *ar, unsigned char *base, unsigned char *last, struct Table *tp,
                    int dmars) {
    State_t *sp = (State_t *)ar->data;
    ...
    for (ip = (int *)cp; (unsigned char *)(ip + 6) < end; ip += 7) {
        ...
        if (base + len >= last) break;
        if (!(fp = ar_getnode(ar, (char *)base + len))) return;
        ...
        if (ip[6]) fp->alias = (char *)base + ip[6];
    }
    while (tp) {
        tpnext = tp->next;
        free((void *)tp);
...
static int omfopen(Ardir_t *ar, char *buf, size_t size) {
    State_t *state;
    int pagesize;
    ...
    if (ar->fd >= 0 && (size <= 0 || *((unsigned char *)buf) != OMF_LIBHDR)) return -1;
    if (!(state = newof(0, State_t, 1, 0))) return -1;
    ar->data = (void *)state;
    if (!(state->dict = dtopen(&namdisc, Dtoset))) goto nope;
    if (ar->fd < 0) return 0;
    size = (size_t)ar->st.st_size;
    if (!(addr = (unsigned char *)malloc(size))) goto nope;
    addrstart = addr;
    state->addr = (void *)addr;
    if (lseek(ar->fd, (off_t)0, SEEK_SET) < 0) goto nope;
    if (read(ar->fd, (void *)addr, size) < 0) goto nope;
    addrmax = addr + size;
    addr = readint(addr + 1, &n, 0);
    pagesize = n + 3;
    while ((addr += n) < addrmax) {
        if (*addr == OMF_LIBDHD) return (0);
        cp = addr;
        type = *addr;
        addr = readint(addr + 1, &n, 0);
        if ((type & ~1) == OMF_MODEND) {
            if (special) {
                if (base) omfload(ar, base, last, tp, dmars);
                special = 0;
            } else {
                if (!(fp = ar_getnode(ar, name))) return (0);
            ...
            dmars = (type & 1);
            len = (n - 1) - (cp - addr);
            if (!(tpnew = newof(0, struct Table, 1, 0))) return (0);
            tpnew->next = tp;
            tp = tpnew;
            if (!base)
                base = last = cp;
...
static Ardirent_t *omfnext(Ardir_t *ar) {
    ...
    if (!(ar->flags & ARDIR_FORCE) && fp->alias) fp->st.name = fp->alias;
    return (&fp->st);
}
...
int omfinsert(Ardir_t *ar, const char *name, int op) {
    ...
    if (fp) {
        ...
        if (op & ARDIR_DELETE) return -1;
        if (!(fp = (Arfile_t *)ar_getnode(ar, fname))) return -1;
    }
    ...
    if (m) fp->alias = (char *)name;
    if (suffix) memcpy(fp->suffix, suffix, sizeof(fp->suffix));
    return (ret);
}

const char *omfspecial(Ardir_t *ar) { return (FILENAME); }

/*
 * changef
 */

static int omfchange(Ardir_t *ar, Ardirent_t *ent) {
    State_t *sp = (State_t *)ar->data;
...
Ardirmeth_t ar_omf = {"omf",     "omf archive", omfopen,  omfnext,    omfchange,
