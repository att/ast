...
static int localclose(Ardir_t *ar) {
    State_t *state;
    int r;
...
static int localopen(Ardir_t *ar, char *buf, size_t n) {
    State_t *state;
    char *cmd;
    int c;

    if (!(ar->flags & ARDIR_LOCAL)) return -1;
    if (!(state = newof(0, State_t, 1, 0))) return -1;
    ar->data = (void *)state;
    cmd = sfprints("${ARDIR:-ar} ${ARDIRFLAGS:-tv} '%s' 2>/dev/null", ar->path);
    if (!(state->sp = sfpopen(NULL, cmd, "r")) || (c = sfgetc(state->sp)) == EOF ||
        sfungetc(state->sp, c) == EOF) {
        localclose(ar);
        return -1;
    }
    return 0;
...
static Ardirent_t *localnext(Ardir_t *ar) {
        ...
        if (state->count++ || !strmatch(t, SYMDIR_local))
            while (s = strchr(s, ' ')) {
                if (isupper(*++s) && islower(s[1]) && islower(s[2]) && s[3] == ' ') {
                    ...
                    if (!*e) {
                        if ((n = strlen(t)) > ar->truncate) ar->truncate = n;
                        break;
                    }
                } else
    ...
    if (sferror(state->sp)) ar->error = errno;
    return 0;
}

Ardirmeth_t ar_local = {"local", "local ar archive", localopen,    localnext, 0, 0,
