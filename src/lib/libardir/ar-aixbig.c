...
static int aixbigclose(Ardir_t *ar) {
    State_t *state;
...
static int aixbigopen(Ardir_t *ar, char *buf, size_t n) {
    Header_t *hdr;
    State_t *state;
    long m;

    if (n <= sizeof(Header_t)) return -1;
    hdr = (Header_t *)buf;
    if (memcmp(hdr->fl_magic, MAGIC, MAGIC_SIZE)) return -1;
    if (!(state = newof(0, State_t, 1, 0))) return -1;
    ar->data = (void *)state;
    if (sfsscanf(hdr->fl_gstoff, "%ld", &m) != 1) goto nope;
    if (sfsscanf(hdr->fl_fstmoff, "%ld", &m) != 1) goto nope;
    state->offset = m;
    if (sfsscanf(hdr->fl_memoff, "%ld", &m) != 1) goto nope;
    state->last = m;
    return 0;
nope:
    aixbigclose(ar);
...
static Ardirent_t *aixbignext(Ardir_t *ar) {
    State_t *state = (State_t *)ar->data;
    long n;
    unsigned long u;

    if ((state->current = state->offset) >= state->last) return 0;
...
static int aixbigchange(Ardir_t *ar, Ardirent_t *ent) {
    State_t *state = (State_t *)ar->data;
    off_t o;
...
Ardirmeth_t ar_aixbig = {"aixbig", "aixbig archive", aixbigopen,    aixbignext, aixbigchange, 0,
