...
static int portclose(Ardir_t *ar) {
    State_t *state;
    int r;
    ...
    if (!ar || !(state = (State_t *)ar->data))
        ...
        if (state->touch && state->patch >= 0) {
            if (lseek(ar->fd, state->patch, SEEK_SET) != state->patch)
                ...
                sfsprintf(buf, sizeof(buf), "%-*lu", sizeof(buf) - 1,
                          (unsigned long)time((time_t *)0) + 5);
                if (write(ar->fd, buf, sizeof(buf) - 1) != (sizeof(buf) - 1)) r = -1;
            }
        }
...
static int portopen(Ardir_t *ar, char *buf, size_t n) {
    long size;
    size_t i;
    Header_t *hdr;
    State_t *state;
    ...
    if (n < (MAGIC_SIZE + sizeof(Header_t))) return -1;
    if (memcmp(buf, MAGIC, MAGIC_SIZE)) return -1;
    ...
    if (hdr->ar_fmag[0] != FMAG_port_0 || hdr->ar_fmag[1] != FMAG_port_1) return -1;
#if __pdp11__ || pdp11
    ar->error = ENOSYS;
    return -1;
#else
    ...
    if (name[0] == '#' && name[1] == '1' && name[2] == TERM_port) {
        ...
        if (n < (MAGIC_SIZE + sizeof(Header_t) + i)) return -1;
        name = (char *)(hdr + 1);
        ar->truncate = 0;
    }
    if (strmatch(name, SYMDIR_port) || strmatch(name, SYMDIR_rand) && (ar->flags |= ARDIR_RANLIB)) {
        if (sfsscanf(hdr->ar_size, "%ld", &size) != 1) goto nope;
        ...
        if ((ar->flags & ARDIR_RANLIB) &&
            (sfsscanf(hdr->ar_date, "%lu", &ar->symtime) != 1 ||
             (unsigned long)ar->st.st_mtime >
                 ar->symtime + (strmatch(name, SYMDIR_strict) ? 0 : SYMDIR_age)))
            ar->symtime = 0;
    ...
    if (lseek(ar->fd, state->offset, SEEK_SET) < 0) goto nope;
    hdr = &state->header;
    while (read(ar->fd, (char *)hdr, sizeof(state->header)) == sizeof(state->header) &&
           hdr->ar_name[0] == TERM_port) {
        if (sfsscanf(hdr->ar_size, "%ld", &size) != 1) goto nope;
        size += (size & 01);
        if (!state->names && hdr->ar_name[1] == TERM_port &&
            (hdr->ar_name[2] == ' ' || hdr->ar_name[2] == TERM_port && hdr->ar_name[3] == ' ')) {
            /*
...
static int ar_uid_gid(Ardir_t *ar, char *b, long *p) {
    int i;
    long n;
...
static Ardirent_t *portnext(Ardir_t *ar) {
    ...
    if (ar_uid_gid(ar, state->header.ar_uid, &n)) return 0;
    ar->dirent.uid = n;
    if (ar_uid_gid(ar, state->header.ar_gid, &n)) return 0;
    ar->dirent.gid = n;
    ...
    if (ar->dirent.name[0] == '#' && ar->dirent.name[1] == '1' && ar->dirent.name[2] == TERM_port) {
        ...
        for (s = ar->dirent.name + strlen(ar->dirent.name);
             s > ar->dirent.name && (*(s - 1) == TERM_port || *(s - 1) == TERM_rand); s--)
            ;
        *s = 0;
    }
    if (state->separator)
...
static int portchange(Ardir_t *ar, Ardirent_t *ent) {
    State_t *state = (State_t *)ar->data;
    off_t o;
    char buf[sizeof(state->header.ar_date) + 1];
...
Ardirmeth_t ar_port = {"portable", "portable archive", portopen,    portnext, portchange, 0,
