...
static int pdpclose(Ardir_t *ar) {
    State_t *state;

    if (ar && (state = (State_t *)ar->data)) free(state);
    return 0;
}
...
static int pdpopen(Ardir_t *ar, char *buf, size_t n) {
    int swap;
    State_t *state;

    if (n <= MAGIC_SIZE) return -1;
...
static Ardirent_t *pdpnext(Ardir_t *ar) {
    ...
    ar->dirent.mtime =
        swapget(state->swap, (char *)&state->header.ar_date, sizeof(state->header.ar_date));
    ar->dirent.uid =
        swapget(state->swap, (char *)&state->header.ar_uid, sizeof(state->header.ar_uid));
    ar->dirent.gid =
        swapget(state->swap, (char *)&state->header.ar_gid, sizeof(state->header.ar_gid));
    ar->dirent.mode =
        swapget(state->swap, (char *)&state->header.ar_mode, sizeof(state->header.ar_mode));
    ar->dirent.offset = state->offset += sizeof(state->header);
    ar->dirent.size =
        swapget(state->swap, (char *)&state->header.ar_size, sizeof(state->header.ar_size));
...
static int pdpchange(Ardir_t *ar, Ardirent_t *ent) {
    State_t *state = (State_t *)ar->data;
    off_t o;
    ...
    swapput(state->swap, (char *)&state->header.ar_date, sizeof(state->header.ar_date),
            (intmax_t)ent->mtime);
...
Ardirmeth_t ar_pdp11 = {"pdp11", "pdp11 archive", pdpopen,      pdpnext, pdpchange, 0,
