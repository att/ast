...
static int s5r0close(Ardir_t *ar) {
    State_t *state;
    int r;
    Header_t header;
    ...
    if (!ar || !(state = (State_t *)ar->data))
        ...
        if (state->touch && state->patch >= 0) {
            if (lseek(ar->fd, state->patch, SEEK_SET) != state->patch)
                ...
                swapput(0, (char *)&header.ar_date, sizeof(header.ar_date),
...
static int s5r0open(Ardir_t *ar, char *buf, size_t n) {
    Header_t *hdr;
    State_t *state;

    if (n <= sizeof(Header_t)) return -1;
    hdr = (Header_t *)buf;
    if (memcmp(hdr->ar_magic, MAGIC, MAGIC_SIZE)) return -1;
    if (!(state = newof(0, State_t, 1, 0))) return -1;
    ar->data = (void *)state;
    state->patch = offsetof(Header_t, ar_date);
    state->offset =
        sizeof(Header_t) + swapget(0, hdr->ar_syms, sizeof(hdr->ar_syms)) * sizeof(Symbol_t);
    ar->truncate = 15;
    return 0;
}
...
static Ardirent_t *s5r0next(Ardir_t *ar) {
...
static int s5r0change(Ardir_t *ar, Ardirent_t *ent) {
    State_t *state = (State_t *)ar->data;
    off_t o;
    ...
    swapput(0, (char *)&state->member.arf_date, sizeof(state->member.arf_date),
            (intmax_t)ent->mtime);
...
Ardirmeth_t ar_s5r0 = {
    "s5r0",      "system V release 0 archive", s5r0open, s5r0next, s5r0change, 0, 0, s5r0close,
    ar_s5r0_next};
