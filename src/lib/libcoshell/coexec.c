...
static Cojob_t *service(Coshell_t *co, Coservice_t *cs, Cojob_t *cj, int flags, Sfio_t *sp) {
    ...
    if (flags & CO_DEBUG) {
        for (a = cs->argv; *a; a++) sfprintf(sp, " %s", *a);
        if (!(s = costash(sp))) goto nospace;
        errormsg(state.lib, ERROR_LIBRARY | 2, "service %s:%s", cs->path, s);
    }
    if (pipe(fds) < 0) {
        errormsg(state.lib, ERROR_LIBRARY | ERROR_SYSTEM | 2, "%s: cannot allocate service pipe",
                 cs->name);
        return 0;
    }
    ...
    ops[2] = 0;
    proc = procopen(cs->path, cs->argv, NULL, ops, PROC_DAEMON | PROC_IGNORE);
    if (!proc) {
        errormsg(state.lib, ERROR_LIBRARY | ERROR_SYSTEM | 2, "%s: cannot connect to %s service",
                 cs->path, cs->name);
        close(fds[0]);
        close(fds[1]);
        return 0;
    ...
    s = costash(sp);
    if (!s) goto bad;
    if (write(cs->fd, s, n) != n || sfpoll(&co->msgfp, 1, 5 * 1000) <= 0) goto bad;
    cj->pid = 0;
    cj->status = 0;
    cj->local = 0;
    cj->service = cs;
...
static Cojob_t *request(Coshell_t *co, Cojob_t *cj, Coservice_t *cs, const char *action,
                        int flags) {
    ssize_t n;
    ssize_t i;
    Sfio_t *sp;
    ...
    if (!cs->fd && !service(co, cs, cj, flags, sp)) goto bad;
    if (!cs->pid) goto bad;
    if (flags & CO_DEBUG) {
        errormsg(state.lib, ERROR_LIBRARY | 2, "job %d commands:\n\n%s %s\n", cj->id, cs->name,
                 action);
    }
    if (!(flags & CO_SILENT)) sfprintf(sfstderr, "+ %s %s\n", cs->name, action);
    sfprintf(sp, "id=%d %s\n", cj->id, action);
    n = sfstrtell(sp);
    action = sfstrbase(sp);
    while ((i = write(cs->fd, action, n)) > 0 && (n -= i) > 0) action += i;
    sfstrclose(sp);
    if (n) goto bad;
    sfclose(sp);
    cj->pid = 0;
...
Cojob_t *coexec(Coshell_t *co, const char *action, int flags, const char *out, const char *err,
                const char *att) {
    Cojob_t *cj;
    Sfio_t *sp;
    Coservice_t *cs;
    ...
    for (cs = co->service; cs; cs = cs->next) {
        for (s = cs->name, t = (char *)action; *s && *s == *t; s++, t++)
            ;
        if (!*s && *t == ' ') return request(co, cj, cs, t + 1, flags);
    }
    cj->flags &= ~CO_SERVICE;
    ...
    if (co->flags & CO_SERVER) {
        ...
        sfprintf(sp, "#%05d\ne %d %d %s %s %s", 0, cj->id, cj->flags, state.pwd, out, err);
        if (att) {
        ...
        if (flags & CO_DEBUG) sfprintf(sp, "set -x\n");
        sfprintf(sp, "%s%s\necho x %d $? >&$%s\n", env, action, cj->id, CO_ENV_MSGFD);
    } else if (flags & CO_KSH) {
        sfprintf(sp,
                 "{\ntrap 'set %s$?; trap \"\" 0; IFS=\"\n\"; print -u$%s x %d $1 $(times); exit "
                 "$1' 0 HUP INT QUIT TERM%s\n%s%s%s",
                 (flags & CO_SILENT) ? "" : "+x ", CO_ENV_MSGFD, cj->id,
                 (flags & CO_IGNORE) ? "" : " ERR", env, n > CO_MAXEVAL ? "" : "eval '",
        ...
        sfprintf(sp, "%c\n%s%sset -%s%s\n", og, env, n > CO_MAXEVAL ? "" : "eval '",
                 (flags & CO_IGNORE) ? "" : "e", (flags & CO_SILENT) ? "" : "x");
        if (n > CO_MAXEVAL) {
        ...
        if (!(co->mode & CO_MODE_SEPARATE)) {
            if (flags & CO_OSH) {
                sfprintf(sp, " && echo x %d 0 >&$%s || echo x %d $? >&$%s", cj->id, CO_ENV_MSGFD,
                         cj->id, CO_ENV_MSGFD);
            } else {
                sfprintf(sp, " && echo x %d 0 `times` >&$%s || echo x %d $? `times` >&$%s", cj->id,
                         CO_ENV_MSGFD, cj->id, CO_ENV_MSGFD);
            }
        }
        if (!(co->mode & CO_MODE_SEPARATE)) {
            sfprintf(sp, " &\necho j %d $! >&$%s\n", cj->id, CO_ENV_MSGFD);
    ...
    n = sfstrtell(sp);
    if (!costash(sp)) return 0;
    if (flags & CO_SERVER) sfprintf(sp, "#%05d\n", n - 7);
    ...
    if (co->mode & CO_MODE_SEPARATE) {
        ...
        while ((i = write(co->cmdfd, s, n)) > 0 && (n -= i) > 0) s += i;
        sfstrclose(sp);
        if (n) return 0;
        ...
        co->running++;
        co->total++;
        if (co->mode & CO_MODE_ACK) cj = cowait(co, cj, -1);
    }
    return cj;
}
