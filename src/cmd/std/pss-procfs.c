...
static int procfs_init(Pss_t *pss) {
    register State_t *state;
    int fd;

    sfsprintf(pss->buf, sizeof(pss->buf), _PS_path_num, (unsigned long)1, _PS_status);
    if ((fd = open(pss->buf, O_RDONLY | O_BINARY)) < 0) return -1;
    close(fd);
...
static int procfs_done(Pss_t *pss) {
...
static int procfs_read(Pss_t *pss, Pss_id_t pid) {
    register State_t *state = (State_t *)pss->data;
    struct dirent *ent;
    char *e;
    ...
    if (pid)
        ...
        do {
            if (!(ent = readdir(state->dir))) return 0;
            pss->pid = (Pss_id_t)strtol(ent->d_name, &e, 10);
        } while (*e);
    return 1;
}

static int procfs_part(register Pss_t *pss, register Pssent_t *pe) {
    register State_t *state = (State_t *)pss->data;
    ...
    if ((fd = open(pss->buf, O_RDONLY | O_BINARY)) < 0) {
        if (pss->disc->errorf && ((pss->disc->flags & PSS_VERBOSE) || errno != ENOENT))
            (*pss->disc->errorf)(pss, pss->disc, ERROR_SYSTEM | 2, "%lu: cannot stat process",
                                 pss->pid);
        return 0;
    }
    ...
    if ((n = read(fd, pss->buf, sizeof(pss->buf))) <= 0 || fstat(fd, &st)) {
        ...
        if (n < _PS_scan_count) {
            ...
            n = sfsscanf(pss->buf, _PS_scan_format, _PS_scan_args(pr));
            if (n < _PS_scan_count)
                error(1, "%lu: scan count %d, expected at least %d", (unsigned long)pss->pid, n,
                      _PS_scan_count);
        }
#ifdef _PS_scan_fix
        _PS_scan_fix(pr, pe);
        ...
        if (!pss->boot) {
            ...
            if (!(pss->hz = (int)strtol(astconf("CLK_TCK", NULL, NULL), NULL, 0))) pss->hz = PR_HZ;
        }
    ...
    if (n < 0) {
        if (pss->disc->errorf)
            (*pss->disc->errorf)(pss, pss->disc, ERROR_SYSTEM | 2, "%lu: cannot get process info",
                                 pss->pid);
        return 0;
    }
#if _UTS
    if (pr->pr_pri > 99)
        ...
        pr->pr_pri = 0;
    pr->pr_ttydev =
        makedev((pr->pr_ttydev >> 18) & ((1 << 14) - 1), pr->pr_ttydev & ((1 << 18) - 1));
#endif
#if PSS_gid
    pe->gid = pr->pr_gid;
#endif
...
static int procfs_full(register Pss_t *pss, register Pssent_t *pe) {
    register State_t *state = (State_t *)pss->data;
    register struct prpsinfo *pr = &state->pr;
...
static Pssmeth_t procfs_method = {"/proc",
