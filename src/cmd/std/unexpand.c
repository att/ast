...
static int *gettabs(const char *arg, int *ntab) {
    ...
    while (1) {
        ...
        if (cp == arg) error(ERROR_exit(1), "%c - invalid character in tablist", *cp);
        if (tablist[n] <= old) error(ERROR_exit(1), "tab stops must be increasing");
        old = tablist[n++];
    }
    *ntab = n;
    return (tablist);
...
static int expand(Sfio_t *in, Sfio_t *out, int tablist[], int tabmax, int type, int initial) {
    ...
    while (buff = cp = sfreserve(in, SF_UNBOUND, 0)) {
        ...
        while (1) {
            ...
            switch (n) {
                case S_SPACE:
                    if (tabmax == 0 || cp == first + 1) {
                        ...
                        n = 1;
                        cp -= 1;
                        if (tabmax == 0) tabspace -= (cp - first) % tabspace;
                        while (cp[n] == ' ') n++;
                        i = cp - buff;
                        ...
                        if (cp[n] == 0 && cp + n + 1 == cpend && savec == ' ') {
                            ...
                            register int c;
                            if (i) sfwrite(out, buff, i);
                            i = 0;
                            n++;
                            while ((c = sfgetc(in)) == ' ') n++;
                            if (c != EOF) sfungetc(in, c);
                            buff = cp + n;
                        }
                        cp += n;
            ...
            if (cp >= cpend) {
                if ((n = cp - buff) > 0) sfwrite(out, buff, n);
                n = cp - first;
                break;
            }
        }
