...
"[-?\n@(#)$Id: ls (AT&T Research) 2013-08-11 $\n]"
USAGE_LICENSE
	...
	if ((files = state->list.count.files) > 0)
	{
			...
			for (p = ent->fts_link; p; p = p->fts_link)
				if (p->fts_number != INVISIBLE)
				{
					if (!mbwide())
						...
						for (s = p->fts_name, w = 0; i = mbchar(s);)
							if (i < 0)
							{
								s++;
								w++;
		...
		else
		{
			...
			if (files > (vecsiz - 1))
			{
				...
				if (!(vec = newof(vec, FTSENT*, vecsiz, 0)))
					error(3, "out of space");
			}
			x = vec;
			i = 0;
			...
			if ((state->lsflags & LS_ACROSS) && n > 1)
			{
				...
				for (j = 0; j < c; j++)
				{
					siz[k] = 0;
					for (l = 0, r = j; l < n && r < i; r += c, l++)
						if (siz[k] < x[r]->fts_namelen)
							siz[k] = x[r]->fts_namelen;
					siz[k] += a + BETWEEN;
					k++;
				}
				for (j = 0; j <= i; j += c)
					for (l = 0, w = j; l < k && w < i; l++, w++)
						pr(state, x[w], l < (k - 1) && w < (i - 1) ? siz[l] : 0);
			}
			else
			{
				o = 0;
				...
				while (j < i)
				{
					siz[k] = 0;
					for (l = 0; l < n && j < i; j++, l++)
						if (siz[k] < x[j]->fts_namelen)
							siz[k] = x[j]->fts_namelen;
					siz[k] += a + BETWEEN;
					k++;
				}
				for (j = 0; j < n; j++)
					for (l = 0, w = j; l < k && w < i; l++, w += n)
						pr(state, x[w], l < (k - 1) && w < (i - n) ? siz[l] : 0);
			}
		}
	}
...
b_ls(int argc, register char** argv, Shbltin_t* context)
{
	...
	if (fts = fts_open(argv, state.ftsflags, order))
	{
		fts->fts_handle = &state;
		sfprintf(sfstdout, "=== top ===\n");
		if (ent = fts_children(fts, 0))
		{
			FTSENT*	x;

			ent->fts_parent->fts_link = ent;
			for (x = ent->fts_link; x; x = x->fts_link)
				if (x->fts_info == FTS_DOT)
				{
					x->fts_info &= ~FTS_DD;
					fts_set(NiL, x, FTS_SKIP);
				}
			ls(&state, ent->fts_parent);
			for (x = ent->fts_link; x; x = x->fts_link)
				if (!(x->fts_info & FTS_D))
					fts_set(NiL, x, FTS_SKIP);
			ent->fts_parent->fts_link = 0;
		}
		sfprintf(sfstdout, "=== rem ===\n");
...
int main(int argc, char** argv) { return b_ls(argc, argv, 0); }
