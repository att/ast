...
static int getprocs_init(Pss_t *pss) {
...
static int getprocs_read(Pss_t *pss, Pss_id_t pid) {
    ...
    if (pid) {
        ...
        state->index = 0;
        if ((state->count =
                 getprocs(state->entry, sizeof(state->entry[0]), NULL, 0, &pss->pid, 1)) != 1)
            return -1;
        state->last = 0;
    } else
        while (state->index >= state->count) {
            ...
            state->index = 0;
            state->count = getprocs(state->entry, sizeof(state->entry[0]), NULL, 0, &pss->pid,
            ...
            if (!state->entry[0].pi_pid) state->index++;
        }
    state->pr = state->entry + state->index++;
    return 1;
}

static int getprocs_part(register Pss_t *pss, register Pssent_t *pe) {
    register State_t *state = (State_t *)pss->data;
...
static int getprocs_full(register Pss_t *pss, register Pssent_t *pe) {
    register State_t *state = (State_t *)pss->data;
