...
#define PSS_default \
    (PSS_pgrp | PSS_pid | PSS_ppid | PSS_sid | PSS_state | PSS_tgrp | PSS_tty | PSS_uid)
...
static int ps_init(Pss_t *pss) {
    ...
    if ((s = getenv("_AST_PSS_ps")) && *s == 'd') state->debug = 1;
    fields = 0;
    for (po = pso; po->field; po++) fields |= po->field;
    pss->meth->fields = state->fields = fields;
    pss->data = state;
    return 1;
}

static int ps_done(Pss_t *pss) {
    register State_t *state = (State_t *)pss->data;

    if (state->ps) sfclose(state->ps);
    return 1;
}

static int ps_read(Pss_t *pss, Pss_id_t pid) {
    register State_t *state = (State_t *)pss->data;
    register Pso_t *po;
    register char *s;
    register char *e;
...
static unsigned long number(char *s, char **p, int base) {
    unsigned long n;
    char *e;
...
static int ps_part(register Pss_t *pss, register Pssent_t *pe) {
            ...
            switch (po->field) {
                ...
                case PSS_start:
                    ...
                    *s = c;
                    ...
                    break;
                case PSS_state:
                    if ((pe->state = *(e = s)) == PSS_ZOMBIE) *e = 0;
                    break;
                case PSS_tgrp:
