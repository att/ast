...
int main(int argc, register char **argv) {
        ...
        switch (optget(argv, usage)) {
            ...
            case 't':
                if (streq(opt_info.arg, "access") || streq(opt_info.arg, "atime") ||
                    ...
                    use = CTIME;
                else if (streq(opt_info.arg, "modify") || streq(opt_info.arg, "mtime") ||
                         streq(opt_info.arg, "modification"))
                    use = MTIME;
                else {
                    reference = 0;
                    t = tmxdate(opt_info.arg, &e, TMX_NOW);
                    if (*e) error(3, "%s: invalid date specification", e);
                    up = &uv;
                    tmx2tv(t, up);
                }
                continue;
    ...
    if (reference) {
        if (stat(reference, &st)) error(ERROR_SYSTEM | 3, "%s: not found", reference);
        if (use) {
            up = &uv;
            switch (use) {
                case ATIME:
        ...
        for (file = *argv; *file >= '0' && *file <= '9'; file++)
            ;
        if (((n = (file - *argv)) == 4 || n == 6 || n == 8 || n == 10 || n == 12) &&
            (!*file || *file == '.' && *(file + 1) >= '0' && *(file + 1) <= '9' &&
                           *(file + 2) >= '0' && *(file + 2) <= '9' && !*(file + 3))) {
            t = tmxdate(file = *argv++, &e, TMX_NOW);
            if (*e) error(3, "%s: invalid date specification", file);
            up = &uv;
            tmx2tv(t, up);
        }
    }
