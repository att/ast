...
static void list_locale(Sfio_t *sp, Keyword_t *key, Lc_t *lc, unsigned int flags) {
    register int i;
    int n;
    char *fmt;
    char *sep;
...
static void number(Sfio_t *sp, register Keyword_t *key, int i, unsigned int flags) {
    state.output = 1;
    if (flags & LC_indent) sfputc(sp, '\t');
    if (flags & LC_keyword)
        sfprintf(sp, "%s=%d\n", key->name, i);
    else
        sfprintf(sp, "%d\n", i);
...
static void value(Sfio_t *sp, register const char *s, register unsigned int flags) {
    register int c;
    register int u;
...
static void string(Sfio_t *sp, register Keyword_t *key, char **v, int n, unsigned int flags) {
    char **e;
    register const Lc_attribute_list_t *a;

    if (flags & LC_indent) sfputc(sp, '\t');
    if (flags & LC_keyword) sfprintf(sp, "%s=", key->name);
    if ((flags & LC_keyword) || n != 1) flags |= LC_quote;
    if (n) {
        value(sp, *v, flags);
        flags |= LC_quote;
        e = v + n - 1;
...
static void extract(Sfio_t *sp, register Keyword_t *key, void *data, unsigned int flags) {
    register int i;
    char *s;
    char **v;
    Lc_t *lc;
...
static void list_all(Sfio_t *sp, register Lc_t *lc, unsigned long flags) {
    if (!lc) lc = (Lc_t *)lcinfo(LC_CTYPE)->lc;
    if (!state.sep)
        state.sep = 1;
    else
        sfputc(sp, '\n');
    if (flags & LC_category) sfprintf(sp, "LC_ALL\n");
    if (flags & LC_indent) sfputc(sp, '\t');
    if (flags & LC_keyword) {
        flags |= LC_quote;
        sfprintf(sp, "locale=");
...
static void list_keyword(Sfio_t *sp, register Keyword_t *key, char *value, unsigned int flags) {
    register int i;
    register int j;
    register int n;
    ...
    switch (key->index) {
        ...
        case AST_LC_NUMERIC:
            if (!state.conv) state.conv = localeconv();
        ...
        case AST_LC_ALL:
            if (value) {
                if (streq(value, "-")) value = 0;
                if (!setlocale(key->offset, value)) error(1, "%s: invalid locale", value);
                state.conv = 0;
            } else {
                if (key->type == AST_LC_ALL) {
                    ...
                    i = n = key->type;
                if (state.all) list_all(sp, NULL, flags);
                for (; i <= n; i++) {
                    f = flags;
...
static int scan(Sfio_t *sp, Keyword_t *key, unsigned long flags) {
    register Lc_t *lc = 0;

    while (lc = lcscan(lc)) {
        switch (flags & (LC_defined | LC_undefined)) {
            case LC_defined:
                if (!lc->index && !setlocale(LC_MONETARY, lc->name)) continue;
                break;
            case LC_undefined:
                if (lc->index || setlocale(LC_MONETARY, lc->name)) continue;
                break;
...
int main(int argc, char **argv) {
        ...
        if (!flags) {
            ...
            for (i = 1; i < AST_LC_COUNT; i++) {
                ...
                sfprintf(sfstdout, "%s=%s\n", state.categories[i].name,
                         fmtquote(s, "\"", "\"", strlen(s),
                                  (value || !getenv(state.categories[i].name)) ? FMT_ALWAYS : 0));
            }
    ...
    if (!(state.dict = dtopen(&state.disc, Dtoset))) error(3, "out of space [dictionary]");
    for (i = 0; i < elementsof(keywords); i++) dtinsert(state.dict, keywords + i);
    for (i = 0; i < AST_LC_COUNT; i++) {
        if (!(key = newof(0, Keyword_t, 1, 0))) error(3, "out of space [keyword]");
    ...
    if (!error_info.errors && !state.output)
        list_keyword(sfstdout, (Keyword_t *)dtmatch(state.dict, state.categories[0].name), NULL,
                     flags);
    return error_info.errors != 0;
}
