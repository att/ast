 ...
 */
static struct op *getop(struct op **prev, Sfoff_t size, size_t repeat, int flags, int re) {
    struct op *op;
...
static struct op *getexpr(struct op **prev, const char *arg) {
    char *cp = (char *)arg;
    char *ep;
    int n;
    struct op *op;
    ...
    if (op = getop(prev, 0, 1, *cp == '/' ? OP_SEARCH : OP_SKIP, 1)) {
        ...
        if (*cp) {
            ...
            if (*ep) error(ERROR_exit(1), "%s: invalid offset", cp);
        }
    }
    return op;
}
...
static struct fname *setfname(const char *prefix, char *format, int suflen, int low, int high) {
    struct fname *fp;
    int flen;
    int slen;
    int len;
    ...
    if (fp = newof(0, struct fname, 1, len)) {
        ...
        *cp-- = 0;
        ...
        if (cp = strrchr(fp->fname, '/')) {
            ...
            cp = fp->fname;
            if (len > flen) flen = (int)strtol(astconf("NAME_MAX", ".", NULL), NULL, 0);
        }
        if (len > flen) error(ERROR_exit(1), "%s: filename too long", prefix);
    }
    return fp;
}
...
static char *getfname(struct fname *fp) {
    register char *cp = fp->last;

    if (fp->format) return sfprints(fp->format, fp->count++);
    while (++(*cp) > fp->high) {
        *cp-- = fp->low;
        if (cp < fp->suffix) {
...
static void removeall(struct fname *fp) {
...
static int msize(Sfio_t *in, long len) {
    ...
    if (sfsize(in) - off <= len) return len;
    while (nlen == 0 && n > 0) {
        n -= BLK_SIZE;
        if (n < 0) n = 0;
        sfseek(in, off + n, SEEK_SET);
        if (!(dp = cp = sfreserve(in, BLK_SIZE, 0))) return len;
        m = BLK_SIZE;
        while (m-- > 0) {
            if (*cp++ == '\n') nlen = n + (cp - dp);
        }
    }
    if (n > 0) sfseek(in, off, SEEK_SET);
    return nlen ? nlen : len;
}

static int split(Sfio_t *in, struct fname *fp, struct op *op, int flags) {
    register char *cp;
    register char *s;
    Sfoff_t len;
    Sfoff_t z;
    ...
    while (op) {
        if (op->flags == OP_LINES) len = op->size;
        repeat = op->repeat;
        do {
            if (op->flags != OP_SKIP) {
                if (!(cp = getfname(fp))) goto err;
            ...
            if (op->flags == OP_ABSOLUTE || op->flags == OP_LINES) {
                if (op->flags == OP_ABSOLUTE) len = op->size - lineno;
                if (peek) {
                    if ((n = sfputr(out, peek, delim)) <= 0) goto done;
                    peek = 0;
                    if (len > 0) len--;
                    lineno++;
                }
                if (len) {
                    ...
                    if ((n = sfmove(in, out, z, delim)) < z || n < 0) goto done;
                    lineno += n;
                }
            } else {
                if (peek) {
                    if (out && (n = sfputr(out, peek, delim)) <= 0) goto done;
                    lineno++;
                    peek = 0;
                }
                while (s = sfgetr(in, delim, 1)) {
                    if (!(c = regexec(op->re, s, 0, NULL, 0))) break;
                    lineno++;
                    if (c != REG_NOMATCH) {
                        regfatal(op->re, 2, c);
                        goto err;
                    }
                    if (out && (n = sfputr(out, s, delim)) <= 0) goto done;
                }
                if (!(peek = s)) {
            ...
            if (out) {
                ...
                if (!(flags & S_FLAG)) sfprintf(sfstdout, "%I*d\n", sizeof(size), size);
                sfclose(out);
                out = 0;
                if ((flags & Z_FLAG) && size <= 0) remove(cp);
            }
        } while (!repeat || --repeat);
        op = op->next;
    }
...
int main(int argc, char **argv) {
    ...
    for (;;) {
        switch (optget(argv, usage)) {
            ...
            case 'l':
                flags &= ~(B_FLAG | M_FLAG);
                if ((size = opt_info.number) <= 0) error(1, "%s: invalid size", opt_info.arg);
                continue;
            case 'k':
                flags |= K_FLAG;
                continue;
