...
#undef PR_CTIME
#define PR_CTIME(p)                                                                               \
    (((p)->rusage_children.ru_utime.tv_sec + (p)->rusage_children.ru_stime.tv_sec) * PR_HZ +      \
     ((((p)->rusage_children.ru_utime.tv_usec + (p)->rusage_children.ru_stime.tv_usec) * PR_HZ) / \
      1000000))
#undef PR_START
#define PR_START(p) ((p)->start_time)
#undef PR_TIME
#define PR_TIME(p)                                                                        \
    (((p)->rusage_self.ru_utime.tv_sec + (p)->rusage_self.ru_stime.tv_sec) * PR_HZ +      \
     ((((p)->rusage_self.ru_utime.tv_usec + (p)->rusage_self.ru_stime.tv_usec) * PR_HZ) / \
      1000000))
...
static int cygwin_init(Pss_t *pss) {
    cygwin_internal(CW_LOCK_PINFO, 1000);
    return 1;
}

static int cygwin_done(Pss_t *pss) {
    cygwin_internal(CW_UNLOCK_PINFO);
    return 1;
}

static int cygwin_read(Pss_t *pss, Pss_id_t pid) {
    if (pid)
        pss->pid = pid;
    else {
        if (!pss->data)
...
static int cygwin_part(register Pss_t *pss, register Pssent_t *pe) {
...
static int cygwin_full(register Pss_t *pss, register Pssent_t *pe) {
    ...
    if (pe->state != PSS_ZOMBIE) {
        if (fields & PSS_args) {
            ...
            s = pss->buf;
            if ((i = strlen(s)) > 4 && !strcasecmp(s + i - 4, ".exe")) *(s + i - 4) = 0;
            pe->args = s;
        }
...
static char *cygwin_ttyname(Pss_t *pss, register Pssent_t *pe) {
    if (pe->tty < 0) return "?";
    if (pe->tty == (Pss_dev_t)TTY_CONSOLE) return "con";
    sfsprintf(pss->buf, sizeof(pss->buf), "%I*d", sizeof(pe->tty), pe->tty);
    return pss->buf;
}

static Pss_dev_t cygwin_ttymap(Pss_t *pss, Pss_dev_t dev) { return minor(dev); }

static Pssmeth_t cygwin_method = {"cygwin",
