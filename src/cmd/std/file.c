...
static int type(Magic_t *mp, char *file, const char *pattern, register Magicdisc_t *disc) {
    char *s;
    char *e;
    Sfio_t *fp;
    struct stat *sp;
    ...
    sp = ((disc->flags & MAGIC_PHYSICAL) ? lstat(file, &st) : stat(file, &st)) ? (struct stat *)0
                                                                               : &st;
    ...
    if (!pattern) {
        ...
        if (!(disc->flags & MAGIC_SILENT)) sfprintf(sfstdout, "%s\n", file);
        return 1;
    }
    return 0;
}
...
int main(int argc, register char **argv) {
    ...
    disc.flags = 0;
    disc.errorf = errorf;
    if (!(mp = magicopen(&disc))) error(3, "out of space");
    for (;;) {
        switch (optget(argv, usage)) {
            ...
            case 'c':
                ...
                continue;
            case 'd':
                if (magicload(mp, NULL, 0)) error(3, "cannot load default magic file");
    ...
    if (disc.flags & MAGIC_LIST) {
        ...
        hit = 0;
        if (!list && !*argv) list = sfstdin;
        if (list)
            while (p = sfgetr(list, '\n', 1))
                if (*p) hit |= type(mp, p, pattern, &disc);
        while (p = *argv++)
            if (*p) hit |= type(mp, p, pattern, &disc);
    }
    return !hit;
