...
static int pstat_init(Pss_t *pss) {
...
static int pstat_read(Pss_t *pss, Pss_id_t pid) {
    register State_t *state = (State_t *)pss->data;
    int count;

    if (pid) {
        if ((count = pstat_getproc(state->pst, sizeof(state->pst[0]), 0, pid)) != 1) return -1;
        state->pp = state->pst;
        state->pe = state->pp + count;
        state->last = 0;
    } else if (state->pp >= state->pe) {
        count = state->pe ? (state->pe - 1)->pst_idx + 1 : 0;
        if ((count = pstat_getproc(state->pst, sizeof(state->pst[0]), elementsof(state->pst),
                                   count)) < 0)
            return -1;
        if (!count) return 0;
        state->pp = state->pst;
...
static int pstat_part(register Pss_t *pss, register Pssent_t *pe) {
...
static int pstat_full(register Pss_t *pss, register Pssent_t *pe) {
    register State_t *state = (State_t *)pss->data;
    register struct pst_status *pr = state->pr;
...
static Pssmeth_t pstat_method = {"pstat",
