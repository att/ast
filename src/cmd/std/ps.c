...
static Key_t keys[] = {
    ...
    {"cmd", "CMD", "Command path with arguments.", PSS_args, KEY_cmd, -32, 0, 0, 0, 0, KEY_comm},
    {"comm", "COMMAND", "Command file base name.", PSS_command, KEY_comm, -24, 0, 0, 0, 0, KEY_cmd},
    {"cpu", "%CPU", "Cpu percent usage.", PSS_cpu, KEY_cpu, 4},
    {"etime", "ELAPSED", "Elapsed time since start.", 0, KEY_etime, 7},
    {"flags", "F", "State flags (octal and additive).", PSS_flags, KEY_flags, 3},
    {"gid", "GROUP", "Numeric group id.", PSS_gid, KEY_gid, 8, 0, 0, 0, 0, KEY_group},
    {"group", "GROUP", "Group id name.", PSS_gid, KEY_group, 8, 0, 0, 0, 0, KEY_gid},
    ...
    {"pgrp", "PGRP", "Process group id.", PSS_pgrp, KEY_pgrp, 5, PID_MAX, 1, 0, 0},
    ...
    {"rss", "RSS", "Resident page set size in kilobytes.", PSS_rss, KEY_rss, 5},
    {"sid", "SID", "Session id.", PSS_sid, KEY_sid, 5, PID_MAX, 1},
    {"size", "SIZE", "Virtual memory size in kilobytes.", PSS_size, KEY_size, 6},
    ...
    {"uid", "USER", "Numeric user id.", PSS_uid, KEY_uid, 8, 0, 0, 0, 0, KEY_user},
    {"user", "USER", "User id name.", PSS_uid, KEY_user, 8, 0, 0, 0, 0, KEY_uid},
...
static int optinfo(Opt_t *op, Sfio_t *sp, const char *s, Optdisc_t *dp) {
    ...
    if (streq(s, "formats"))
        for (i = 1; i < elementsof(keys); i++) {
            ...
            if (keys[i].head)
                sfprintf(sp, "%s The title string is \b%s\b and the default width is %d.%s]",
                         keys[i].desc, keys[i].head, keys[i].width,
...
static int ttyid(const char *name) { return pssttydev(state.pss, name); }
...
static int key(void *handle, register Sffmt_t *fp, const char *arg, char **ps, Sflong_t *pn) {
    ...
    if (!kp->head && (state.pss->meth->fields & keys[kp->index].field)) kp = &keys[kp->index];
    if (kp->macro && !kp->disable) {
        kp->disable = 1;
        sfkeyprintf(state.mac, handle, kp->macro, key, NULL);
        if (!(s = sfstruse(state.mac))) error(ERROR_SYSTEM | 3, "out of space");
        kp->disable = 0;
    } else if (!pp) {
        ...
        kp->width = fp->width;
        if (fp->flags & SFFMT_LEFT) kp->width = -kp->width;
        fp->fmt = 's';
        *ps = arg && (arg = (const char *)strdup(arg)) ? (char *)arg : kp->head;
        ...
        switch (kp->index) {
            case KEY_addr:
                if (pp->ps->state == PSS_ZOMBIE) goto zombie;
                n = (long)pp->ps->addr;
                goto number;
            case KEY_class:
                if (pp->ps->state == PSS_ZOMBIE) goto zombie;
            ...
            branch:
                ...
                if ((j = pp->level) > 0) {
                    ...
                    if (!(s = sfstruse(state.wrk))) error(ERROR_SYSTEM | 3, "out of space");
                    if (state.escape) fmtesc(s + i);
                }
                break;
            case KEY_cpu:
                if (pp->ps->state == PSS_ZOMBIE) goto zombie;
            ...
            percent:
                ...
                if (!(s = sfstruse(state.tmp))) error(ERROR_SYSTEM | 3, "out of space");
                break;
            number:
                if (state.hex) {
                    fp->fmt = 'x';
                    if (!kp->hex) fp->flags |= SFFMT_ZERO;
                }
                break;
        }
        if (s)
...
static void ps(Ps_t *pp) {
    ...
    for (kp = state.fields; kp; kp = kp->next) {
        switch (kp->index) {
            case KEY_addr:
                if (pr->state == PSS_ZOMBIE) goto zombie;
                n = (long)pr->addr;
                goto hex;
            case KEY_class:
                if (pr->state == PSS_ZOMBIE) goto zombie;
...
static void kids(register Ps_t *pp, int level) {
    if (state.tree) {
        pp->level = level;
        ps(pp);
        if (level > 0) state.branch[level - 1] = pp->sibling != 0;
        if (level < elementsof(state.branch) - 1) level++;
    } else
        ps(pp);
    for (pp = pp->children; pp; pp = pp->sibling) kids(pp, level);
}
...
static void list(void) {
    ...
    if (state.children || state.parents) {
                ...
                if (!(pp->ps->pss & (PSS_EXPLICIT | PSS_MATCHED | PSS_PARENT)) && !pp->seen) {
                    ...
                    do {
                        ...
                        if (xp->ps->pss & (PSS_ANCESTOR | PSS_EXPLICIT | PSS_MATCHED)) {
                            ...
                            do {
                                xp->ps->pss |= PSS_PARENT;
                            } while ((xp = (Ps_t *)dtmatch(state.bypid, &xp->ps->ppid)) &&
                                     !xp->ps->pss);
                            break;
                        }
                    } while (xp->ps->ppid != xp->ps->pid &&
        ...
        for (pp = (Ps_t *)dtfirst(state.byorder); pp; pp = (Ps_t *)dtnext(state.byorder, pp))
            if (pp->ps->pss) {
                if (pp->ps->ppid != pp->ps->pid &&
                    ...
                    (xp->ps->pss & (PSS_EXPLICIT | PSS_MATCHED | PSS_PARENT))) {
                    xp->ps->pss |= PSS_CHILD;
                    if (xp->lastchild)
        ...
        for (pp = rp; pp; pp = pp->root) kids(pp, 0);
    } else {
        /*
         * list by order
         */
...
static void head(void) {
    ...
    if (state.fields) {
        while (state.fields->skip) state.fields = state.fields->next;
        kp = state.fields;
        while (kp->next) {
            if (!kp->next->skip)
                kp = kp->next;
        ...
        n = 0;
        for (kp = state.fields; kp; kp = kp->next) {
            if ((kp->prec = kp->width) < 0) kp->prec = -kp->prec;
            if (*kp->head) n = 1;
        }
        kp = state.lastfield;
        if (kp->width < 0) kp->width = 0;
        if (kp->index == KEY_cmd) kp->prec = 80;
        if (n && state.heading) {
            for (kp = state.fields; kp; kp = kp->next)
...
static int byorder(Dt_t *dt, void *a, void *b, Dtdisc_t *disc) {
    ...
    if (pa->ps->pgrp > pb->ps->pgrp) return 1;
    if (i = (pa->ps->pgrp == pa->ps->pid) - (pb->ps->pgrp == pb->ps->pid)) return i;
    if (pa->ps->start < pb->ps->start) return -1;
    if (pa->ps->start > pb->ps->start) return 1;
    if (pa->ps->pid < pb->ps->pid) return -1;
    if (pa->ps->pid > pb->ps->pid) return 1;
    return 0;
}
...
static void addpid(Pssent_t *pe, register char *s) {
    register char *t;
    register Ps_t *pp;
    register int c;
    char *e;
    ...
    do {
        ...
        if (pe) {
            ...
            if (!dtsearch(state.byorder, pp)) {
                if (!(pp->ps = psssave(state.pss, pe))) break;
...
static void addid(register char *s, int index, int (*getid)(const char *)) {
    register char *t;
    register int c;
    char *e;
    long n;
    ...
    for (mp = state.pssdisc.match; mp && mp->field != field; mp = mp->next)
        ;
    if (!mp) {
        if (!(mp = newof(0, Pssmatch_t, 1, 0))) error(ERROR_SYSTEM | 3, "out of space");
    ...
    do {
        ...
        for (t = s; *s && !isspace(*s) && *s != ','; s++)
            ;
        if ((c = s - t) >= sizeof(state.buf)) c = sizeof(state.buf) - 1;
        ...
        if (!(dp = newof(0, Pssdata_t, 1, 0))) error(ERROR_SYSTEM | 3, "out of space");
        dp->next = mp->data;
        mp->data = dp;
        dp->data = n;
    } while (*s++);
...
static void addkey(const char *k, int ignore) {
    ...
    if (streq(s, "?")) {
        ...
        for (kp = keys + 1; kp < keys + elementsof(keys); kp++) {
            ...
            sfprintf(
                sfstdout, "%-8s %-8s %s%s%s\n", kp->name, ap->head, ap->desc,
                ap == kp ? "" : " [alias]",
                (!ap->field || (state.pss->meth->fields & ap->field)) ? "" : " [not available]");
        }
        exit(0);
    }
    do {
        ...
        if (*s == '=') {
            for (t = ++s; *s && !isspace(*s) && *s != ','; s++)
                ;
            w = s - t;
            if (!(kp->head = newof(0, char, w, 1))) error(ERROR_SYSTEM | 3, "out of space");
            memcpy(kp->head, t, w);
            if (w < c) w = c;
            if (w > kp->width) kp->width = w;
        } else if (c >= strlen(kp->head))
            kp->width = c;
        if (!(state.pss->meth->fields & kp->field)) {
            if (!ignore) error(1, "%s: not available on this system", kp->name);
            continue;
        }
...
static void pushpids(void *argv, int argc) {
    register List_t *p;

    if (!(p = newof(0, List_t, 1, 0))) error(ERROR_SYSTEM | 3, "out of space");
    p->argv = (char **)argv;
    p->argc = argc;
    p->next = state.pids;
    state.pids = p;
...
static void poppids(void) {
    ...
    while (p = state.pids) {
        state.pids = p->next;
        if (i = p->argc)
            while (--i >= 0) addpid(NULL, p->argv[i]);
        else
...
int main(int argc, register char **argv) {
    ...
    if (!(state.keys = dtopen(&kd, Dtset)) || !(state.bypid = dtopen(&nd, Dtset)) ||
    ...
    for (;;) {
        switch (optget(argv, usage)) {
            ...
            case 'D':
                ...
                if (kp->macro = s) stresc(s);
                continue;
            case 'E':
                state.escape = opt_info.num;
                continue;
            case 'F':
                if (sfstrtell(fmt)) sfputc(fmt, ' ');
    ...
    if (sfstrtell(fmt)) {
        sfputc(fmt, '\n');
        if (!(state.format = sfstruse(fmt))) error(ERROR_SYSTEM | 3, "out of space");
    } else {
        sfclose(fmt);
        fmt = 0;
        if (!state.fields) addkey(fields_default, 1);
    }
    head();
    ...
    if (*argv) pushpids(argv, argc);
    if (state.children || state.parents || !state.pids) {
        if (state.children || state.parents) state.pssdisc.flags |= PSS_UNMATCHED;
        if (!state.pids && !state.pssdisc.match &&
            !(state.pssdisc.flags &
              (PSS_ALL | PSS_ATTACHED | PSS_DETACHED | PSS_LEADER | PSS_NOLEADER))) {
        ...
        poppids();
        while (pe = pssread(state.pss, PSS_SCAN)) addpid(pe, NULL);
        if (state.children || state.parents)
            for (pp = (Ps_t *)dtfirst(state.byorder); pp; pp = (Ps_t *)dtnext(state.byorder, pp))
    ...
    if (state.fields) state.lastfield->sep = newline;
    list();
    pssclose(state.pss);
