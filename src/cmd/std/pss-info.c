...
static int info_init(Pss_t *pss) {
    ...
    if (read(fd, state->ps, n * sizeof(state->ps[0])) != n * sizeof(state->ps[0])) {
        if (pss->disc->errorf)
            (*pss->disc->errorf)(pss, pss->disc, ERROR_SYSTEM | 2, "%s: %lu entry read error", mem,
                                 n);
        goto bad;
    }
    state->mem = fd;
...
static int info_read(Pss_t *pss, Pss_id_t pid) {
    register State_t *state = (State_t *)pss->data;
    int count;

    if (pid) {
        for (state->pp = state->ps; state->pp < state->pe; state->pp++)
            if (state->pp->pstate != PRFREE && state->pp->pid == pid) break;
    } else
        while (state->pp < state->pe && state->pp->pstate == PRFREE) state->pp++;
    if (state->pp >= state->pe) return 0;
    state->pr = state->pp++;
    pss->pid = state->pr->pid;
    return 1;
}

static int info_part(register Pss_t *pss, register Pssent_t *pe) {
...
static int info_full(register Pss_t *pss, register Pssent_t *pe) {
    register State_t *state = (State_t *)pss->data;
    register struct pentry *pr = state->pr;
...
static int info_done(register Pss_t *pss) {
    register State_t *state = (State_t *)pss->data;
...
static Pssmeth_t info_method = {"info",
                                "[-version?@(#)$Id: pss info (AT&T Research) 2005-02-11 $\n]"
                                "[-author?Glenn Fowler <gsf@research.att.com>]",
                                PSS_args | PSS_command | PSS_flags | PSS_gid | PSS_nice | PSS_pgrp |
                                    PSS_pid | PSS_ppid | PSS_pri | PSS_sid | PSS_size | PSS_state |
                                    PSS_time | PSS_tty | PSS_uid,
                                info_init,
                                info_read,
                                info_part,
                                info_full,
