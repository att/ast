...
static int visit(register Node_t *x) {
    register List_t *p;
    int cycle = 0;
...
static void tsort(Sfio_t *ip) {
    ...
    if (sfvalue(ip)) error(ERROR_warn(1), "last line incomplete");
    if (head) error(ERROR_exit(1), "odd data");
    if (pos = hashscan(tab, 0)) {
        while (hashnext(pos)) visit((Node_t *)pos->bucket);
        hashdone(pos);
    } else
