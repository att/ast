...
static void fini(int code) {
    if (state.in.fp != sfstdin) sfclose(state.in.fp);
...
int main(int argc, char **argv) {
    ...
    if (!(sp = sfstropen())) error(ERROR_SYSTEM | 3, "out of space");
    sfputr(sp, usage1, '\n');
    ...
    sfputr(sp, usage2, '\n');
    if (!(usage = sfstruse(sp))) error(ERROR_SYSTEM | 3, "out of space");
    if (signal(SIGINT, SIG_IGN) != SIG_IGN) signal(SIGINT, interrupt);
    while (f = optget(argv, usage)) {
        if (f > 0) {
            if (f == '?') error(ERROR_USAGE | 4, "%s", opt_info.arg);
            if (f = ':') error(2, "%s", opt_info.arg);
            continue;
        }
        ...
        switch (op->type) {
            ...
            case CONV:
                do {
                    ...
                    vp = (Operand_t *)strsearch(&state.conv_begin,
                                                &state.conv_end - &state.conv_begin + 1,
                                                sizeof(Operand_t), stracmp, v, NULL);
                    if (e) *e++ = ',';
                    if (!vp) error(3, "%s: unknown %s value", v, op->name);
            ...
            case NUMBER:
                ...
                for (;;) {
                    ...
                    if (n < 0) error(3, "%s: %s must be >= 0", v, op->name);
                    if (!c) {
                        c = 1;
                        op->value.number = n;
                    } else
                ...
                if (*v) error(3, "%s: %s: invalid numeric expression", op->name, opt_info.arg);
                break;
        }
    }
    ...
    switch ((long)(state.conv.value.number & (A2E | A2I | A2N | A2O | E2A | I2A | N2A | O2A))) {
        ...
        case O2A:
            ...
            break;
        default:
            error(3, "only one of %s={%s,%s,%s} may be specified", state.conv.name,
                  state.ascii.value.string, state.ebcdic.value.string, state.ibm.value.string);
    }
    if (streq(state.from.value.string, state.to.value.string))
        state.cvt = (iconv_t)(-1);
    else if ((state.cvt = iconv_open(state.to.value.string, state.from.value.string)) ==
             (iconv_t)(-1))
        error(3, "cannot convert from %s to %s",
              *state.from.value.string ? state.from.value.string : ccmapname(CC_NATIVE),
              *state.to.value.string ? state.to.value.string : ccmapname(CC_NATIVE));
    ...
    if ((state.conv.value.number & (BLOCK | UNBLOCK)) == (BLOCK | UNBLOCK))
        error(3, "only one of %s=%s and %s=%s may be specified", state.conv.name,
              state.block.value.string, state.conv.name, state.unblock.value.string);
    if ((state.conv.value.number & (SYNC | UNBLOCK)) == (SYNC | UNBLOCK)) {
        state.conv.value.number &= ~SYNC;
        error(1, "%s=%s ignored for %s=%s", state.conv.name, state.sync.value.string,
              state.conv.name, state.unblock.value.string);
    }
    ...
    if (state.oseek.value.number && (state.conv.value.number & NOTRUNC)) {
        ...
        error(1, "%s ignored for %s=%s", state.oseek.name, state.conv.name,
              state.notrunc.value.string);
    }
    if ((state.conv.value.number & (LCASE | UCASE)) == (LCASE | UCASE))
        error(3, "only one of %s=%s and %s=%s may be specified", state.conv.name,
              state.lcase.value.string, state.conv.name, state.ucase.value.string);
    if (state.conv.value.number & (BLOCK | UNBLOCK)) {
        if (!state.cbs.value.number)
            error(3, "%s must be specified for %s=%s", state.cbs.name, state.conv.name,
                  (state.conv.value.number & BLOCK) ? state.block.value.string
    ...
    if (n = state.bs.value.number) state.ibs.value.number = state.obs.value.number = n;
    ...
    if (n = state.skip.value.number) {
        ...
        if (sfseek(state.in.fp, n, SEEK_SET) != n) {
            do {
                if (!sfreserve(
                        state.in.fp,
                        state.in.special && state.ibs.value.number < n ? state.ibs.value.number : n,
                        0)) {
            ...
            if (n > 0) error(3, "%s: cannot seek past end of file", state.ifn.value.string);
        }
    }
    if (n = state.oseek.value.number) {
        ...
        if (sfseek(state.out.fp, n, SEEK_SET) != n) {
            ...
            while (n > 0 && (c = sfwrite(state.out.fp, state.buffer,
                                         state.out.special && state.obs.value.number < n
                                             ? state.obs.value.number
                                             : n)) > 0)
                n -= c;
    ...
    if (state.silent.value.number) state.iconv.errorf = 0;
    ...
    if (!state.obs.value.number) state.obs.value.number = BS;
    if (state.conv.value.number & BLOCK) {
        ...
        if (!(c = state.ibs.value.number)) c = BS;
        f = state.conv.value.number;
        if (!state.in.special) f &= ~NOERROR;
        if (!(r = state.count.value.number)) r = -1;
        partial = 0;
        while (state.in.complete != r) {
            ...
            if (!b) {
                if (!m) break;
                error(ERROR_SYSTEM | ((f & NOERROR) ? 2 : 3), "%s: read error",
            ...
            while (state.in.complete != r) {
                ...
                if (m >= c) {
                    ...
                    n = m;
                    if (state.in.special) state.in.partial++;
                ...
                if (f & SWAP) swapmem(state.swap.value.number, s, s, n);
                if (state.cvt != (iconv_t)(-1)) {
                    cb = s;
                    cc = n;
                    state.iconv.errors = 0;
                    if ((d = iconv_write(state.cvt, state.tmp, &cb, &cc, &state.iconv)) < 0) d = 0;
