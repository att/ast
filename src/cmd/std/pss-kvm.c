...
static int kvm_readf(Pss_t *pss, Pss_id_t pid) {
    register State_t *state = (State_t *)pss->data;
    int count;

    if (pid) {
        if (!(state->kp = kvm_getprocs(state->kd, KERN_PROC_PID, pid, &count))) return -1;
        state->ke = state->kp + count;
    } else if (!state->kp) {
        if (!(state->kp = kvm_getprocs(state->kd, KERN_PROC_ALL, 0, &count))) return -1;
        state->ke = state->kp + count;
    }
    if (state->kp >= state->ke) return 0;
...
static int kvm_part(register Pss_t *pss, register Pssent_t *pe) {
    register State_t *state = (State_t *)pss->data;
...
static int kvm_full(register Pss_t *pss, register Pssent_t *pe) {
    register State_t *state = (State_t *)pss->data;
...
static Pssmeth_t kvm_method = {"kvm",
