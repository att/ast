...
#define BLOCKS(n) \
    (Count_t)((blocksize == LS_BLOCKSIZE) ? (n) : (((n)*LS_BLOCKSIZE + blocksize - 1) / blocksize))
...
static void mark(Dt_t *dict, Hit_t *key, FTSENT *ent) {
    Hit_t *hit;
...
int main(int argc, register char **argv) {
    ...
    if (!(dict = dtopen(&disc, Dtset))) error(3, "not enough space for file id dictionary");
    if (blocksize)
        scale = 0;
    else
        blocksize = LS_BLOCKSIZE;
    if (logical = !(flags & (FTS_META | FTS_PHYSICAL))) flags |= FTS_PHYSICAL;
    multiple = argv[0] && argv[1];
    dirs = logical || multiple;
    if (!(fts = fts_open(argv, flags, NULL))) error(ERROR_system(1), "%s: not found", argv[1]);
    while (ent = fts_read(fts)) {
        ...
        switch (ent->fts_info) {
            case FTS_NS:
                if (!silent) error(ERROR_SYSTEM | 2, "%s: not found", ent->fts_path);
                continue;
            case FTS_D:
                if (!(ent->fts_pointer = newof(0, Count_t, 1, 0)))
                    error(ERROR_SYSTEM | 3, "out of space");
                if (dirs) mark(dict, &hit, ent);
                continue;
            case FTS_DC:
                if (!silent) error(2, "%s: directory causes cycle", ent->fts_path);
                continue;
            case FTS_DNR:
                if (!silent) error(ERROR_SYSTEM | 2, "%s: cannot read directory", ent->fts_path);
                break;
            case FTS_DNX:
                if (!silent) error(ERROR_SYSTEM | 2, "%s: cannot search directory", ent->fts_path);
                fts_set(NULL, ent, FTS_SKIP);
                break;
            case FTS_DP:
                if (ent->fts_pointer) {
            ...
            case FTS_SL:
                ...
                /*FALLTHROUGH*/
            default:
                if (ent->fts_statp->st_nlink > 1 || dirs && !ent->fts_level) mark(dict, &hit, ent);
                if (!all) list = 0;
                break;
        }
        ...
        count += b;
        n += b;
        if (ent->fts_parent->fts_pointer) *(Count_t *)ent->fts_parent->fts_pointer += n;
