...
int b_mime(int argc, char **argv, Shbltin_t *context) {
    ...
    argc -= opt_info.index;
    argv += opt_info.index;
    if (error_info.errors || argc > 1 && argc < 3) error(ERROR_usage(2), "%s", optusage(NULL));
    disc.version = MIME_VERSION;
    disc.flags = 0;
    disc.errorf = errorf;
    if (!(mp = mimeopen(&disc))) error(ERROR_exit(1), "mime library error");
    ...
    if (argc <= 1)
        ...
        for (argc = 3; s = argv[argc]; argc++) sfputr(sp, s, ';');
        if (!(s = sfstruse(sp))) error(ERROR_exit(1), "out of space");
