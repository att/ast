...
int b_dlls(int argc, char **argv, Shbltin_t *context) {
    int i;
    int r;
    int flags;
    int only;
    ...
    r = 0;
    if (flags & LIST_INFO) {
        if (!(dli = dllinfo())) error(2, "cannot determine native dll info");
        sfprintf(sfstdout, "sibling=(%s", dli->sibling[0]);
        for (i = 1; dli->sibling[i]; i++) sfprintf(sfstdout, " %s", dli->sibling[i]);
        sfprintf(sfstdout, ") env=%s prefix=%s suffix=%s flags=", dli->env, dli->prefix,
                 dli->suffix);
        i = 0;
    ...
    if (flags &= (LIST_BASE | LIST_PATH | LIST_LONG)) {
        for (i = 0; i < elementsof(arg); i++)
            if (arg[i] = *argv) argv++;
        if (only && !*argv) error(ERROR_usage(2), "%s", optusage(NULL));
        r = 1;
        for (;;) {
            ...
            if (!r || !arg[0] || streq(arg[0], "-") || !arg[1] || streq(arg[1], "-")) break;
            arg[0] = 0;
        }
    }
