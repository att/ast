...
static int look(Sfio_t *fp, char *prefix, char *maxprefix, int flags) {
    Sfoff_t low;
    Sfoff_t mid;
    Sfoff_t high;
    int n;
        ...
        *dp = 0;
        ...
        if (maxprefix) {
            ...
            while (n = *cp++)
                if (isalnum(n) || isspace(n)) *dp++ = n;
            *dp = 0;
            if ((n = strlen(maxprefix)) < len) n = len;
        } else
            n = len;
        buff = (void *)malloc(n + 1);
    } else
        n = len = strlen(prefix);
    if (maxprefix && (*compare)(prefix, maxprefix, n) > 0) return 1;
    if (flags & H_FLAG)
        while (sfgetr(fp, '\n', 0) && sfvalue(fp) > 1)
            ;
    if ((low = sfseek(fp, (Sfoff_t)0, SEEK_CUR)) < 0 ||
        ...
        if (maxprefix) {
            prefix = maxprefix;
            len = strlen(prefix);
            if (n && (*compare)(prefix, EXTRACT(flags, cp, buff, len), len) >= 0) n = 0;
        }
        found = !n;
        while (!n) {
...
int b_look(int argc, char **argv, Shbltin_t *context) {
    ...
    argv += opt_info.index;
    if (error_info.errors || !(bp = *argv++)) error(ERROR_usage(2), "%s", optusage(NULL));
    if (file = *argv) {
        argv++;
    ...
    if (!file) {
        ...
        if (!file) error(ERROR_system(1), "%s: not found", dict[0]);
        flags |= (D_FLAG | F_FLAG);
    }
    n = 0;
    do {
