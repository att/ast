...
static void what(const char *file, Sfio_t *ip, Sfio_t *op) {
    ...
    if (intro = !state.match) sfprintf(op, "%s:\n", file);
    if (buf = (unsigned char *)sfreserve(ip, SF_UNBOUND, 0)) {
        skip = state.skip;
        if ((mid = sfvalue(ip)) <= (index = 3)) goto next;
        e = buf + mid;
        for (;;) {
            ...
            next:
                ...
                if (!(buf = (unsigned char *)sfreserve(ip, SF_UNBOUND, 0))) goto done;
                if ((mid = sfvalue(ip)) <= (index = 3)) goto next;
                e = buf + mid;
                switch (skip[buf[0]]) {
                    case HIT:
                        if (buf[0] == ')' && s[2] == '#' && s[1] == '(' && s[0] == '@' ||
                            buf[0] == ':' && s[2] == 'd' && s[1] == 'I' && s[0] == '$') {
                            index = 0;
                            s = buf + 1;
                            goto hit;
                        }
                        break;
                    case 1:
                        if (buf[1] == ')' && buf[0] == '#' && s[2] == '(' && s[1] == '@' ||
                            buf[1] == ':' && buf[0] == 'd' && s[2] == 'I' && s[1] == '$') {
                            index = 1;
                            s = buf + 2;
                            goto hit;
                        }
                        break;
                    case 2:
                        if (buf[2] == ')' && buf[1] == '#' && buf[0] == '(' && s[2] == '@' ||
                            buf[2] == ':' && buf[1] == 'd' && buf[0] == 'I' && s[2] == '$') {
                            index = 2;
                            s = buf + 3;
                            goto hit;
                        }
                ...
                hit:
                    ...
                    for (;;) {
                        if (s >= e) {
                            ...
                            t = "";
                            if (!(buf = (unsigned char *)sfreserve(ip, SF_UNBOUND, 0))) goto list;
                ...
                if ((index += 4) >= mid) goto next;
            }
        }
    }
done:
    if (sfvalue(ip)) error(ERROR_system(0), "%s: read error", file);
}
...
int b_what(int argc, char **argv, Shbltin_t *context) {
    ...
    for (n = 0; n <= UCHAR_MAX; n++) state.skip[n] = 4;
