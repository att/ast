...
static int prheader(Pr_t *pp) {
    ...
    sfputc(pp->outfile, '\n');
    sfputc(pp->outfile, '\n');
    if (pp->offset) sfwrite(pp->outfile, pp->margin, pp->offlen);
    if (sfprintf(pp->outfile, "%s  %s Page %d\n\n\n", pp->date, header, pp->pageno) < 0) return -1;
    return 0;
}
...
static int prtrailer(Pr_t *pp, int line) {
    ...
    if (pp->flags & F_FLAG) {
        if (sfputc(pp->outfile, '\f') < 0) return -1;
    } else {
...
static void prpage(Pr_t *pp) {
    ...
    while (1) {
        ...
        if (out && tflag) {
            if (!sfreserve(pp->infile, 0, 0) || sfvalue(pp->infile) < 0) break;
            sfnputc(pp->outfile, '\n', HDRSZ);
        }
    }
    if (out && tflag) prtrailer(pp, n);
}
...
static int outspaces(Pr_t *pp, int spaces, int col) {
    ...
    if (pp->ogap) {
        ...
        while (spaces >= col) {
            if (sfputc(pp->outfile, pp->otab) < 0) return -1;
            n++;
            spaces -= col;
            col = pp->ogap;
        }
    ...
    if (spaces > 0)
        if (sfnputc(pp->outfile, ' ', spaces) < 0) return -1;
    return n += spaces;
}
...
static int outcol(Pr_t *pp, char *buff, int size, int spaces) {
    ...
    if (pp->igap || pp->ogap) {
        ...
        buffend = cp + size;
        if (size = spaces) {
            if (pp->ogap) omod = pp->ogap - (pp->colno - size) % pp->ogap;
            if (n = state[*(unsigned char *)cp++]) goto skip;
        ...
        while (1) {
            ...
            if (n == S_NL) break;
            if (sfwrite(pp->outfile, buff, --size) < 0) return -1;
            if (pp->width && col >= pp->width) return 1;
            size = 0;
            if (pp->ogap) omod = pp->ogap - (pp->colno + col) % pp->ogap;
        skip:
    ...
    if (size > 0 && sfwrite(pp->outfile, buff, size) < 0) return -1;
    return 0;
}
...
static int prline(Pr_t *pp) {
    ...
    while (cp = sfgetr(pp->infile, '\n', 0)) {
        ...
        if (line >= pp->pagelen) {
            line = 0;
            pp->pageno++;
            if (!(pp->flags & T_FLAG) && prheader(pp) < 0) return -1;
        }
        if (pp->offset) sfwrite(pp->outfile, pp->margin, pp->offlen);
        if (pp->numwidth) sfprintf(pp->outfile, "%*d%c", pp->numwidth, ++pp->lineno, pp->nchar);
        if (outcol(pp, cp, n, 0) < 0) return -1;
    ...
    if (!(pp->flags & T_FLAG)) prtrailer(pp, line);
    return 0;
}
...
static int outpage(Pr_t *pp, int n) {
    ...
    if (!(pp->flags & T_FLAG)) prheader(pp);
    if (pp->flags & A_FLAG)
        incr = 1;
    else
    ...
    while (1) {
        ...
        for (old = 0, j = pp->columns; --j >= 0;) {
            ...
            if (j == 0)
                ...
                c = ' ';
            cp[size - 1] = c;
            if ((old = outcol(pp, cp, size, old)) < 0) return -1;
            pp->colno += pp->width;
            if (j > 0 && pp->schar) {
        ...
        if (++line >= pp->pagelen) break;
        if (pp->flags & D_FLAG) sfputc(pp->outfile, '\n');
    }
    if (!(pp->flags & T_FLAG)) prtrailer(pp, line);
    return 0;
}
...
static int prcol(Pr_t *pp) {
    ...
    do {
        ...
        *next = pp->fieldptr = pp->fieldbuff;
        ...
        while (1) {
            if (fp && (cp = sfgetr(fp, '\n', 0))) {
                if ((size = sfvalue(fp)) > pp->width) size = pp->width;
                if (!skip) memcpy(pp->fieldptr, cp, size);
            } else if (pp->streams) {
                if (fp) {
            ...
            pp->fieldptr += size;
            *++next = pp->fieldptr;
            if (next >= nextmax) break;
            if (pp->streams) {
                if (++nstream >= pp->columns) nstream = 0;
        ...
        pp->fieldlast = next;
        if ((n = next - pp->fieldlist) > 0) r = outpage(pp, n);
    } while (cp);
    return r;
}
...
static ssize_t c_read(Sfio_t *fp, void *buf, size_t n, Sfdisc_t *dp) {
    ...
    if (pp->control.cur >= pp->control.end) {
        ...
        if ((z = sfrd(fp, pp->control.buf, n, dp)) <= 0) return z;
...
int b_pr(int argc, char **argv, Shbltin_t *context) {
            ...
            case 'e':
                ...
                if (opt_info.arg) {
                    ...
                    if (*opt_info.arg) {
                        ...
                        if (l < 0 || *opt_info.arg) error(2, "-%c: requires positive integer", n);
                        if (n == 'e')
                            pp->igap = l;
                        else if (n == 'i')
                            pp->ogap = l;
    ...
    pp->outfile = sfstdout;
    if (pp->pagelen <= (2 * HDRSZ)) pp->flags |= T_FLAG;
    if (!(pp->flags & T_FLAG)) pp->pagelen -= (2 * HDRSZ);
    pp->pageodd = (pp->pagelen & 1);
    if (pp->flags & D_FLAG) pp->pagelen = (pp->pagelen + 1) / 2;
    if (pp->flags & M_FLAG) {
    ...
    if (pp->columns > 0) {
        if (pp->columns == 1) {
            ...
            if (pp->igap == 0) pp->igap = TABSZ;
            if (pp->ogap == 0) pp->ogap = TABSZ;
            if ((n = pp->width) == 0)
#if 0
...
#endif
                n -= pp->numwidth;
            if ((pp->width = ((n + 1) / pp->columns)) <= 0) error(ERROR_exit(1), "width too small");
    ...
    if (pp->igap) pp->state[pp->itab] = S_TAB;
    pp->state[' '] |= S_SPACE;
    pp->state['\n'] = S_NL;
    if (pp->ogap) pp->omod = (pp->numwidth + pp->numwidth > 0) % pp->ogap;
    if (pp->offset) {
    ...
    do {
        ...
        if (!cp || strcmp(cp, "-") == 0)
            ...
            if (!(pp->flags & R_FLAG)) error(ERROR_system(0), "%s: cannot open", cp);
            error_info.errors = 1;
            continue;
        }
        ...
        if (pp->streams)
            ...
            if (!(pp->flags & T_FLAG)) {
                if (pp->flags & X_FLAG)
                    ...
                    tmfmt(pp->date, DATESZ, "%b %e %H:%M %Y",
                          fstat(sffileno(fp), &statb) ? (time_t *)0 : &statb.st_mtime);
            }
            ...
            if (pp->columns)
                prcol(pp);
            else if (pp->width || pp->igap || pp->ogap || pp->offset || pp->numwidth ||
                     (pp->flags & D_FLAG))
                prline(pp);
            else
                prpage(pp);
    ...
    if (pp->control.buf) free(pp->control.buf);
