...
static int strings(Sfio_t *in, Sfio_t *out, register int width, char *format, register int flags) {
    ...
    if (format) {
        ...
        if ((offset = sfseek(in, (off_t)0, SEEK_CUR)) < 0) offset = 0;
        offset--;
    }
                    ...
                    if (format) {
                        if (flags & (MULTIBYTE | MULTILINE)) {
                            if (sfprintf(out, fmt, sizeof(offset), offset - (inend - inp) - n) < 0)
                                return 0;
                            n = outstring(out, (char *)inp - ((flags & MULTIBYTE) + 1) * n - 1, n,
                                          flags);
                        } else
                            n = sfprintf(out, fmt, sizeof(offset), offset - (inend - inp) - n, n,
                                         inp - n - 1);
    ...
    for (;;) {
        switch (optget(argv, usage)) {
            ...
            case 'o':
                ...
                continue;
            case 't':
                for (cp = opt_info.arg; (n = *cp) == '+' || n == '-' || n == '.' || isdigit(n);
                     cp++)
                    ;
