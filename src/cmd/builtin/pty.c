...
static Proc_t *runcmd(char **argv, int slave, int session) {
    long ops[4];
...
static int process(Sfio_t *mp, Sfio_t *lp, int delay, int timeout) {
    ...
    do {
        ...
        if ((n = sfpoll(sps, i, t)) <= 0) {
            if (n < 0) error(ERROR_SYSTEM | 2, "poll failed");
            break;
        }
        for (i = t = 0; i < n; i++) {
...
static int match(char *pattern, char *text, int must) {
    ...
    if (!text) {
        if (must) error(2, "expected \"%s\", got EOF", pattern);
        return 0;
    }
    ...
    if (regexec(re, text, 0, NULL, 0)) {
        if (must) error(2, "expected \"%s\", got \"%s\"", pattern, fmtesq(text, "\""));
        return 0;
    }
    return 1;
}
...
static char *masterline(Sfio_t *mp, Sfio_t *lp, char *prompt, int must, int timeout, Master_t *bp) {
    ...
    if (prompt) promptlen = sfsprintf(promptbuf, sizeof(promptbuf), prompt, ++bp->line);
again:
    if (prompt) {
        ...
        *bp->cur = 0;
        ...
        if (bp->restore >= 0) *bp->cur = bp->restore;
        r = bp->cur;
        bp->restore = *bp->nxt;
        *bp->nxt = 0;
        if (bp->nxt >= bp->end) {
...
int b_pty(int argc, char **argv, Shbltin_t *context) {
    int master;
    int slave;
    int fd;
    int drop;
    ...
    if (lp && sfclose(lp)) error(ERROR_system(1), "%s: write error", log);
