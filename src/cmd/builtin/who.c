...
static void outline(Sfio_t *out, register struct utmpx *up, register State_t *sp) {
    ...
    if (sp->flags & FLAG_T) {
        int state = '?';
        if (r >= 0) state = (statb.st_mode & S_IWOTH) ? '+' : '-';
        sfprintf(out, "%-*.*s %c %-*.*s %.12s ", sp->siz_user, sp->siz_user, up->ut_user, state,
                 sp->siz_line, sp->siz_line, up->ut_line, date + 4);
    } else
        sfprintf(out, "%-*.*s %-*.*s %.12s ", sp->siz_user, sp->siz_user, up->ut_user, sp->siz_line,
                 sp->siz_line, up->ut_line, date + 4);
    if (sp->flags & FLAG_U) {
        time_t t = time(0) - 30;
        sfsprintf(line, sizeof(line), "/dev/%s\0", up->ut_line);
...
static int who(Sfio_t *in, Sfio_t *out, int flags) {
    ...
    if (state.siz_user > 24) state.siz_user = 24;
    state.siz_line = sizeof(ut.ut_line);
    if (state.siz_line > 8) state.siz_line = 8;
#if _mem_ut_host_utmp || _mem_ut_host_utmpx
    state.siz_host = sizeof(ut.ut_host);
    if (state.siz_host > 30) state.siz_host = 30;
#endif
    if (state.flags & FLAG_Q) state.flags = FLAG_Q;
    if ((state.flags & FLAG_M) && ((line = ttyname(0)) || (line = ttyname(2)))) line += 5;
    if (state.flags & FLAG_H) {
        char *t = "";
        if (state.flags & FLAG_T) t = "MESG";
        if (state.flags & FLAG_U)
            sfprintf(out, "%-*.*s %2s %-*.*s %.12s %6s     %s\n", state.siz_user - 3,
                     state.siz_user - 3, "USER", t, state.siz_line, state.siz_line, "LINE",
                     "LOGIN-TIME", "IDLE", "FROM");
        else
    ...
    while (sfread(in, &ut, sizeof(struct utmpx)) > 0) {
        if (skip(ut)) continue;
        if (line && strcmp(line, ut.ut_line)) continue;
    ...
    if (state.flags & FLAG_Q) sfprintf(out, "\n# users=%d\n", count);
    return (0);
}
...
int b_who(int argc, char **argv, Shbltin_t *context) {
    ...
    if (error_info.errors || argc) error(ERROR_usage(2), "%s", optusage((char *)0));
    if (!(sp = sfopen((Sfio_t *)0, UTMP, "r"))) error(ERROR_system(1), "%s: cannot open", UTMP);
