...
static int optinfo(Opt_t *op, Sfio_t *sp, const char *s, Optdisc_t *dp) {
    ...
    switch (*s) {
        case 'm':
            for (mp = uumeth(NULL); mp->name; mp++) sfprintf(sp, "[+%s|%s]", mp->name, mp->alias);
            break;
    }
    return 0;
}
...
int main(int argc, register char **argv) {
    ...
    argv += opt_info.index;
    if ((ipath = *argv) && *++argv) opath = *argv++;
    if (error_info.errors || *argv) error(ERROR_usage(2), "%s", optusage(NULL));
    if (!(meth = uumeth(encoding))) error(ERROR_exit(1), "%s: unknown method", encoding);
