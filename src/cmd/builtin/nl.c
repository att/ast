...
int b_nl(int argc, char **argv, Shbltin_t *context) {
    ...
    while (n = optget(argv, usage)) switch (n) {
            ...
            case 'd':
                ...
                if (opt_info.arg[1]) nl.delim2 = opt_info.arg[1];
                break;
            case 'f':
            case 'b':
            case 'h':
                ...
                switch (*opt_info.arg) {
                    ...
                    case 'p':
                        ...
                        if (regcomp(&re[m], opt_info.arg + 1, REG_NOSUB))
                            error(2, "-%c: invalid basic regular expression %s", n,
                                  opt_info.arg + 1);
                        break;
                }
                break;
    ...
    for (m = 0; m < 3; m++) {
        if (nl.section[m] == (void *)&re[m]) regfree(&re[m]);
    }
    return (n ? 1 : 0);
}
