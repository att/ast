...
static void highlight(Sfio_t *sp, const char *s, int n, int so, int eo) {
...
static int hit(State_t *state, const char *prefix, int line, const char *s, size_t len) {
    state->hit->hits++;
    if (state->query || state->list) return -1;
    if (!state->count) {
        if (state->prefix) sfprintf(sfstdout, "%s:", prefix);
        if (state->number && line) sfprintf(sfstdout, "%d:", line);
        if (state->label) sfprintf(sfstdout, "%s:", state->hit->string);
        if (state->pos)
