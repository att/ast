...
static void dbm_get(struct dbm_array *ap) {
    ...
    if (data.dsize && (val = (char *)data.dptr)) {
        if (!ap->header.hdr.type && data.dsize > 1 && *val == 0 && val[1]) {
            *val = '(';
            if (!ap->strbuf) ap->strbuf = sfstropen();
            ap->node.nvname = ap->key.dptr;
        ...
        ap->cur = &ap->node;
        if (ap->header.hdr.type) nv_setsize(ap->cur, data.dsize);
    } else {
        int err;
        ap->cur = 0;
        if (err = dbm_error(ap->dbm)) {
            dbm_clearerr(ap->dbm);
            error(ERROR_system(err), "Unable to get key %.*s", ap->key.dsize - ap->addzero,
                  ap->key.dptr);
        }
    }
}
...
static void dbm_put(struct dbm_array *ap) {
    ...
    if (ap->node.nvsize) {
        ...
        if (nv_isvtree(&ap->node) && *val == '(') *val = 0;
        data.dptr = (char *)val;
    }
...
static void *dbm_associative(register Namval_t *np, const char *sp, int mode) {
    ...
    switch (mode) {
        ...
        case NV_ASETSUB:
            ...
            /* FALL THROUGH*/
        case NV_ACURRENT:
            if (ap->pos) dbm_get(ap);
            if (ap->cur) ap->cur->nvprivate = (char *)np;
            return ((void *)ap->cur);
        case NV_ANAME:
            if (ap->cur && ap->cur != &ap->node) ap->cur = &ap->node;
            if (ap->cur) return ((void *)ap->cur->nvname);
            return ((void *)0);
        default:
            if (sp) {
                if (sp == (char *)np) {
                ...
                if (!ap->init) {
                    ...
                    if (ap->node.nvsize == 0) ap->node.nvalue = ap->val;
                    ap->key = dbm_firstkey(ap->dbm);
                ...
                if (ap->cur) dbm_setname(ap);
                if (mode & NV_AADD) {
                    if (ap->shp->subshell)
                        sfprintf(sfstderr,
                                 "subshell=%d subscript=%s will be modified, need to save \n",
                                 ap->shp->subshell, sp);
                    ap->modified = 1;
                    if (!ap->cur) {
                        ap->header.nelem++;
                ...
                if (ap->pos != &ap->node && !(ap->header.nelem & ARRAY_SCAN)) ap->pos = 0;
            }
            if (ap->cur) {
                ap->cur = &ap->node;
...
static int dbm_create(int argc, char **argv, Shbltin_t *context) {
    ...
    while ((fds[n] = open("/dev/null", NV_RDONLY)) < 10) n++;
    if (!error_info.errors &&
        !(db = dbm_open(dbfile, oflags,
    ...
    nv_unset(np);
    if (zflag && (oflags & O_CREAT)) nv_onattr(np, NV_ZFILL);
    ...
    if (tname) {
        ...
        free(tmp);
        if (!tp) error(ERROR_exit(1), "%s: unknown type", tname);
        nv_settype(np, tp, 0);
        nv_settype(&ap->node, ap->header.hdr.type, 0);
        ap->node.nvsize = tp->nvsize;
    }
    if (!(oflags & O_RDWR)) nv_onattr(np, NV_RDONLY);
