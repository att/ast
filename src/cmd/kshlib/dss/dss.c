...
static int query(int argc, char *argv[], Shbltin_t *bp) {
    ...
    char *savedata;
    size_t savesize;
    while (pp && pp->index != n) pp = pp->next;
    if (!pp || argv[1]) {
        if (!pp) {
        ...
        if (pp->expr.query && qp->end) (*qp->end)(cx, &pp->expr, (void *)0, cx->disc);
    ...
    if (!bp->vnode->nvalue || !dp->sp || !(data = dp->sp->rp)) return (1);
    savedata = data->data;
    savesize = data->size;
    data->data = bp->vnode->nvalue;
    data->size = nv_size(bp->vnode);
    if (qp->sel) {
        if ((n = (*qp->sel)(cx, &pp->expr, data, cx->disc)) <= 0) return (n < 0 ? 2 : 1);
    }
    if (qp->act) return (-(*qp->act)(cx, &pp->expr, data, cx->disc));
    data->data = savedata;
    data->size = savesize;
    return (0);
}
...
static void put_child(Namval_t *np, const char *val, int flags, Namfun_t *fp) {
    ...
    if ((*tp->member->setf)(pp->cx, &in, &ret, &op, NULL, NULL, pp->cx->disc) == 0)
        nv_putval(pp->hdr.np, (char *)&ret.value.number,
...
static Namval_t *type_child(register Namval_t *np, Namfun_t *fp) {
    ...
    Cxvariable_t *vp = vnode(np);
    if (!pp->dss || vp->type == sp->type_number || vp->type == sp->type_string) return (0);
    return (typenode(vp->type->name, 0));
}

/* for child variables of compound variables */
static const Namdisc_t child_disc = {sizeof(Namchld_t), put_child, get_child, nget_child, 0, 0, 0,
                                     name_child,        0,         type_child};

/* for child variables of methods */
static const Namdisc_t mchild_disc = {
    sizeof(Namchld_t), put_mchild, get_mchild, nget_mchild, 0, 0, 0, name_child, 0, type_child};
...
static Namval_t *node(Cxvariable_t *vp, struct parent *dp) {
    ...
    if (!dp->hdr.nodes) {
        struct type *tp;
        struct parent *pp;
        if (!(dp->hdr.nodes = (char *)calloc(dp->hdr.numnodes, NV_MINSZ))) return (0);
        ...
        if (mp != dp->hdr.np && pp && pp->hdr.fun.disc == &parent_disc && pp->hdr.nodes)
            ...
            mp = 0;
        if (mp && !nv_isnull(mp)) nv_clone(mp, np, 0);
    }
    if (!vp) return (np ? np : nv_namptr(dp->hdr.nodes, 0));
    ...
    if (!np->nvname) {
        ...
        if (!dp->dss && vp->type->base && (mp = typenode(vp->type->name, 0)) &&
            ...
            if (nfp) nv_disc(np, nfp, NV_FIRST);
            if (((struct type *)fp)->details.nvalue) nv_setvtree(np);
        }
        np->nvname = (char *)vp;
    }
    return (np);
...
static Namfun_t *clone_parent(Namval_t *np, Namval_t *mp, int flags, Namfun_t *fp) {
    ...
    if (tp = (struct type *)nv_hasdisc(mp, &type_disc)) tp->pfun = &dp->fun;
    dp->data = 0;
    dp->np = mp;
    dp->nodes = 0;
...
static Namval_t *create_parent(Namval_t *np, const char *name, int flag, Namfun_t *fp) {
    ...
    char *last = (char *)name;
    int c;
    if (!name) return (dp->hdr.np);
    while ((c = *last) && c != '.' && c != '=' && c != '[' && c != '+') last++;
    if (c) *last = 0;
...
static void put_parent(Namval_t *np, const char *val, int flag, Namfun_t *fp) {
    ...
    if (!pp->dss || (flag & NV_NOFREE)) nv_putv(np, val, flag, fp);
}
...
static int read_parent(register Namval_t *np, Sfio_t *iop, int delim, Namfun_t *fp) {
    ...
    if (iop) {
        ...
        sp->np = np;
        sp->mp = 0;
        if (!(sp->rp = dssfread(sp->fp))) return (1);
        if (ap) {
...
static int write_parent(register Namval_t *np, Sfio_t *iop, int delim, Namfun_t *fp) {
    ...
    if (!sp->rp) sp->rp = (Dssrecord_t *)calloc(1, sizeof(Dssrecord_t));
...
sfprintf(sfstderr,"%x: first=%s index=%d\n",vp,vp->name,vp->header.index);
    ...
    if (!vp) return ((Namval_t *)0);
    np = node(vp, dp);
    return (np);
}

static const Namdisc_t parent_disc = {sizeof(struct parent), put_parent,   0, 0,           0,
 ...
 */
static Namval_t *add_discipline(const char *typename, const char *name,
                                int (*fun)(int, char *[], Shbltin_t *), void *context) {
    Namval_t *mp;
    int offset = stktell(stkstd);
    sfputc(stkstd, 0);
...
static void put_type(Namval_t *np, const char *val, int flag, Namfun_t *fp) {
    ...
    if (val && !(flag & NV_INTEGER) && tp->type->internalf && !cxisstring(tp->type)) {
        ...
        cop.type = tp->type;
        if ((*tp->type->internalf)(tp->cx, tp->type, NULL, NULL, &cop, val, size, Vmregion,
                                   &Dssdisc) < 0)
            errormsg(SH_DICT, ERROR_exit(1), "%s: cannot covert to type dss.%s", val,
                     tp->type->name);
...
static char *get_type(register Namval_t *np, Namfun_t *fp) {
    ...
    if (!tp->type->externalf) return (nv_getv(np, fp));
    if (nv_isattr(np, NV_INTEGER))
        ...
        if ((n = nv_size(np)) == 0) n = strlen(cval.string.data);
        cval.string.size = n;
    }
    buf = getbuf(32);
    if (nv_hasdisc(np, &child_disc)) vp = vnode(np);
    if (dp = (struct parent *)nv_hasdisc(np, &parent_disc)) mp = node((Cxvariable_t *)0, dp);
    if (!(format = nv_getval(mp)) && vp) format = vp->format.details;
    for (i = 0; i < 2; i++) {
...
static Namfun_t *clone_type(Namval_t *np, Namval_t *mp, int flags, Namfun_t *fp) {
    struct type *tp = (struct type *)fp;
    if (!fp->next) pushtype(np, tp->type, fp);
    if (fp->next) {
...
static Namval_t *next_type(register Namval_t *np, Dt_t *root, Namfun_t *fp) {
    ...
    if (!root && (!fpn || fpn->disc != &parent_disc)) {
        ...
        if (fp->next == fpn) return (&((struct type *)fp)->details);
    }
    if (tp->pfun) return ((*tp->pfun->disc->nextf)(np, root, tp->pfun));
    return (0);
}

static char *setdisc_type(Namval_t *np, const char *event, Namval_t *action, Namfun_t *fp) {
    struct type *tp = (struct type *)fp;
    int n = -1;
    const char *name = event ? event : (const char *)action;
    if (name) {
        if (strcmp(name, "match") == 0 && tp->type->match) n = 0;
    }
    if (!event) {
        if (!action && tp->type->match) return ("match");
        n = -1;
    }
    if (n < 0) return (nv_setdisc(np, event, action, fp));
    if (action == np) {
...
static Namval_t *create_type(Namval_t *np, const char *name, int flag, Namfun_t *fp) {
    ...
    while (fp = fp->next)
        if (fp->disc && fp->disc->createf) {
            if (np = (*fp->disc->createf)(np, name, flag, fp)) tp->fun.last = fp->last;
            return (np);
        }
    if (memcmp(name, NV_DATA, sizeof(NV_DATA) - 1) == 0 &&
        (name[sizeof(NV_DATA) - 1] == 0 || strchr("+=[", name[sizeof(NV_DATA) - 1]))) {
...
static const Namdisc_t type_disc = {0,           put_type,   get_type, 0,        setdisc_type,
...
static void mktype(Namval_t *np, Cxtype_t *tp, Cx_t *cx) {
    ...
    if (tp->match) pp->bltins[0] = add_discipline(pp->type->name, "match", match, pp);
...
static Namval_t *create_dss(Namval_t *np, const char *name, int flag, Namfun_t *fp) {
    ...
    if (fp && !(cp = strchr(name, '.'))) fp->last = (char *)name + strlen(name);
    mp = typenode(name, 0);
    if (cp) {
        Namfun_t *fq;
        *cp = '.';
        ...
        for (fq = mp->nvfun; fq && !(fq->disc && fq->disc->createf); fq->next)
            ;
        if (!fq) return (0);
        if (np = (*fq->disc->createf)(mp, cp + 1, flag, fq)) fp->last = fq->last;
        return (np);
    }
    if (mp) return (mp);
    ...
    for (n = 0, vp = (Cxvariable_t *)dtfirst(dict); vp; vp = (Cxvariable_t *)dtnext(dict, vp)) {
        Namval_t *qp;
        if (strcmp(vp->name, "dss") == 0) continue;
#if 0
vp->header.index = n;
sfprintf(sfstderr,"tname=%s name=%s n=%d index=%d\n",name,vp->name,n,vp->header.index);
    ...
    if (!(dp = newof((struct parent *)0, struct parent, 1, 0))) return (0);
    dp->hdr.numnodes = n + 1;
    ...
    if (dss) {
        ...
        for (i = 0; i < n; i++) add_discipline(name, dp->hdr.names[i], query, &dp->hdr.names[i]);
    }
    nv_disc(mp, &dp->hdr.fun, NV_FIRST);
    mp->nvalue = 0;
    if (dss) nv_onattr(mp, NV_BINARY | NV_RAW);
    memset(&optdisc, 0, sizeof(optdisc));
    optdisc.dss.optdisc.infof = dssoptinfo;
...
static int loadlib(int argc, char *argv[], Shbltin_t *bp) {
    ...
    if (error_info.errors || !*argv) errormsg(SH_DICT, ERROR_usage(2), "%s", optusage((char *)0));
    while (name = *argv++)
        if (!dssload(name, dp)) error_info.errors = 1;
    return (error_info.errors);
}
...
static Cxvalue_t *getvalue(Namval_t *np, Namfun_t *fp, Cxoperand_t *valp) {
    if (fp && fp->disc == &child_disc) {
        ...
        if (nv_isattr(np, NV_INTEGER))
            ...
            if (!valp->value.string.size) valp->value.string.size = strlen(valp->value.string.data);
        }
    }
    return (&valp->value);
}
...
static int match(int argc, char *argv[], Shbltin_t *bp) {
    ...
    int n, flag = 0;
    if (comp = bp->data) {
        if (!(bp->flags & SH_END_OPTIM)) goto exec;
    ...
    if (!tptr) tptr = tp->cx->state->type_string;
    comp = (*mp->compf)(tp->cx, tp->type, tptr, valp, &Dssdisc);
    if (!comp) return (3);
    if (bp->flags & SH_BEGIN_OPTIM) bp->data = comp;
exec:
    if (bp->vnode) fp = nv_hasdisc(bp->vnode, &child_disc);
    valp = getvalue(bp->vnode, fp, &cval);
    n = (*mp->execf)(tp->cx, comp, tptr, valp, &Dssdisc);
    if (!bp->data) (*mp->freef)(tp->cx, comp, &Dssdisc);
    return (n < 0 ? 4 : !n);
}
...
void lib_init(int flag, void *context) {
    ...
    for (level = 0; level < 2; level++) {
        char *cp;
        n = 0;
        for (lib = dsslib(NULL, DSS_VERBOSE, &Dssdisc); lib;
             lib = (Dsslib_t *)dtnext(state->cx->libraries, lib)) {
            int m;
            if (!lib->queries) continue;
            for (i = 0; name = lib->queries[i].name; i++) {
                if (strcmp(name, "null") == 0) continue;
                if (strcmp(name, "return") == 0) continue;
                if (strcmp(name, "print") == 0) continue;
                m = strlen(name) + 1;
                if (level == 0) {
                    n++;
                    len += m;
