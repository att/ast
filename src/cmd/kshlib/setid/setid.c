...
extern int b_setid(int argc, char **argv, Shbltin_t *context) {
    char *rg;
    char *eg;
    char *ru;
    char *eu;
    ...
    for (;;) {
        switch (optget(argv, usage)) {
            case 'g':
                rg = opt_info.arg;
                if ((rgid = strgid(rg)) < 0) error(2, "%s: unknown group", rg);
                continue;
            case 'G':
                eg = opt_info.arg;
                if ((egid = strgid(eg)) < 0) error(2, "%s: unknown group", eg);
                continue;
            case 'u':
                ru = opt_info.arg;
                if ((ruid = struid(ru)) < 0) error(2, "%s: unknown user", ru);
                continue;
            case 'U':
                eu = opt_info.arg;
                if ((euid = struid(eu)) < 0) error(2, "%s: unknown user", eu);
                continue;
            case '?':
...
#else
    if (rgid >= 0 && setgid(rgid)) error(ERROR_SYSTEM | 2, "setgid(%s) failed", rg);
    if (egid >= 0 && setegid(egid)) error(ERROR_SYSTEM | 2, "setegid(%s) failed", eg);
#endif
#if _lib_setreuid
    if ((ruid >= 0 || euid >= 0) && setreuid(ruid, euid))
        error(ERROR_SYSTEM | 2, "setreuid(%s,%s) failed", ru, eu);
#else
    if (ruid >= 0 && setuid(ruid)) error(ERROR_SYSTEM | 2, "setuid(%s) failed", ru);
    if (euid >= 0 && seteuid(euid)) error(ERROR_SYSTEM | 2, "seteuid(%s) failed", eu);
#endif
