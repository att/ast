...
static void put_time(Namval_t *np, const char *val, int flag, Namfun_t *nfp) {
    ...
    if (val) {
        ...
        if (flag & NV_INTEGER) {
            ...
            if (*last) errormsg(SH_DICT, ERROR_exit(1), "%s: invalid date/time string", val);
        }
...
static Namval_t *create_time(Namval_t *np, const char *name, int flags, Namfun_t *nfp) {
    struct dctime *dp = (struct dctime *)nfp;
    if (strcmp(name, "format")) return ((Namval_t *)0);
    return (dp->format);
}
...
static Shfield_t *sh_findfield(Shfield_t *ftable, int nelem, const char *name) {
    ...
    for (i = 0; i < nelem; i++, fp++) {
        if (memcmp(fp->name, name, n) == 0 && fp->name[n] == 0) return (fp);
    }
    return (0);
}
...
static Namval_t *sh_newnode(register Shfield_t *fp, Namval_t *np) {
    ...
    if (fp->size < 0) val = *(char **)val;
    nv_putval(nq, val, fp->flags | NV_NOFREE);
    if (fp->make) (*fp->make)(nq);
    return (nq);
}
...
static void genvalue(Sfio_t *out, Shclass_t *sp, int indent, Namval_t *npar) {
...
#endif
        if (!(np = nodes[i]) && out) np = sh_newnode(fp, npar);
        if (np) {
            isarray = 0;
            ...
            while (1) {
                ...
                if (!(fmtq = nv_getval(np)) || !(fmtq = sh_fmtq(fmtq))) fmtq = "";
                sfputr(out, fmtq, '\n');
    ...
    if (out) {
        if (indent > 1) sfnputc(out, '\t', indent - 1);
        sfputc(out, ')');
    }
}
...
static Shfield_t filefield[] = {
    {"atime", NV_INTEGER | NV_RDONLY, offsetof(struct stat, st_atime), sizeof(Sp->st_atime),
     make_time},
    {"ctime", NV_INTEGER | NV_RDONLY, offsetof(struct stat, st_ctime), sizeof(Sp->st_ctime),
     make_time},
    ...
    {"mode", NV_INTEGER | NV_RDONLY, offsetof(struct stat, st_mode), sizeof(Sp->st_mode),
     make_mode},
    {"mtime", NV_INTEGER | NV_RDONLY, offsetof(struct stat, st_mtime), sizeof(Sp->st_mtime),
     make_time},
...
static Shclass_t Fileclass = {sizeof(filefield) / sizeof(*filefield), sizeof(struct filedata),
                              filefield};
...
extern int b_open(int argc, char *argv[], Shbltin_t *context) {
    ...
    if (!(flags & (letterbit('r') | letterbit('w')))) {
        ...
        if (fd < 0) errormsg(SH_DICT, ERROR_system(1), "%s: open failed", argv[1]);
    }
    if (!(flags & letterbit('i'))) fcntl(fd, F_SETFL, 0);
