...
static struct ppkeyword keys[] = {"char",        T_CHAR,      "int",    T_INT,     "sfstderr",
                                  T_STDERR,      "stderr",    T_STDERR, "_STDIO_", T_STDIO,
                                  "_TRANSLATE_", T_TRANSLATE, 0,        0};

static int msgppargs(char **argv, int last) {
    for (;;) {
...
int main(int argc, char **argv) {
    ...
    ppop(PP_INIT);
    if (!(tmp = sfstropen())) error(ERROR_SYSTEM | 3, "out of space");
    x = 0;
    for (;;) {
        c = pplex();
    again:
        switch (c) {
            ...
            case T_ID:
                ...
                if (x > 0) {
                    ...
                    if ((c = pplex()) == '(' && (c = pplex()) == T_INT && (c = pplex()) == T_ID &&
                        (c = pplex()) == ',' && (c = pplex()) == T_CHAR && (c = pplex()) == '*')
                        sfprintf(sfstdout, "cmd %s\n", s + 2);
                    else
                        goto again;
                } else {
                    if ((c = pplex()) == '[') {
                        if (ppisinteger(c = pplex())) c = pplex();
                        if (c != ']') goto again;
                        c = pplex();
                    }
                    if (c == '=' && (c = pplex()) == T_STRING && !strmatch(pp.token, OMIT)) {
                        ...
                        if (!(s = sfstruse(tmp))) error(ERROR_SYSTEM | 3, "out of space");
                        ppinput(s, "string", 0);
                    } else
                        goto again;
                }
