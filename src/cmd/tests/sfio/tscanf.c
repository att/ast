...
static int extf(Sfio_t *sp, Void_t *vp, Sffmt_t *dp) {
...
tmain() {
    ...
    if (n != 0) terror("Bad %%[ scanning1");
    n = sfsscanf("123", "%#[a-z]%d", str, &i);
    if (n != 2 || str[0] != 0 || i != 123) terror("Bad %%[ scanning2");

    str[0] = str[1] = str[2] = str[3] = str[4] = str[5] = str[6] = str[7] = 'x';
    c[0] = c[1] = c[2] = c[3] = 'x';
    cl[0] = cl[1] = cl[2] = cl[3] = cl[4] = cl[5] = cl[6] = cl[7] = 'x';

    i = -1;
    if (sfsscanf("123456789", "%.*.*d", 4, 10, &i) != 1) terror("Bad %%d scanning");
    if (i != 1234) terror("Expected 1234, got %d", i);

    i = -1;
    if (sfsscanf("0", "%i", &i) != 1 || i != 0) terror("Bad %%i scanning1");
    i = -1;
    if (sfsscanf("0x", "%1i%c", &i, c) != 2 || i != 0 || c[0] != 'x') terror("Bad %%i scanning2");
    i = -1;
    if (sfsscanf("0x1", "%i", &i) != 1 || i != 1) terror("Bad %%i scanning3");
    i = -1;
    if (sfsscanf("07", "%i", &i) != 1 || i != 7) terror("Bad %%i scanning4");
    i = -1;
    if (sfsscanf("08", "%i%i", &i, &j) != 2 || i != 0 || j != 8) terror("Bad %%i scanning5");

    sfsscanf("1234567890", "%4I*s%2I*c%4I*[0-9]", 4, str, 2, c, 6, cl);

    if (strcmp(str, "123") != 0) terror("Bad s");
    ...
    if (f != 4 || d != 6) terror("Bad return values f=%f d=%f", f, d);
    ...
    for (k = 2; k <= 64; ++k) {
        ...
        for (i = 0; i < 1000; ++i) sfprintf(sf, "%#..*d\n", k, i);
        sfseek(sf, (Sfoff_t)0, 0);
        for (i = 0; i < 1000; ++i) {
            if (sfscanf(sf, "%i", &j) != 1) terror("Scanf failed");
    ...
    s = sfprints("%p", sf);
    sfsscanf(s, "%p", &vp);
    if (vp != (Void_t *)sf) terror("Wrong pointer scan");
    ...
    k = 0;
    if (sfsscanf(" %1", "%%%d", &k) != 1 || k != 1) terror("%%%% does not skip leading space");
    k = 0;
    if (sfsscanf("%1", "%*[%]%d", &k) != 1 || k != 1) terror("%%*%% failed");
    k = 0;
    if (sfsscanf(" %1", "%*[%]%d", &k) == 1 && k == 1) terror("%%*%% skips leading space");
