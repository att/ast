...
tmain() {
...
#ifdef SIGPIPE
    if ((handler = signal(SIGPIPE, SIG_DFL)) == SIG_DFL) terror("Wrong signal handler");
    if ((handler = signal(SIGPIPE, handler)) != SIG_DFL) terror("Weird signal handling");
#endif
    ...
    if (os != endos) terror("Does not match all data, left=%s", os);
