...
tmain() {
    ...
    /* our real work */
    if (!(f = sfopen(NULL, tstfile("sf", 0), "w"))) terror("Can't open to write");

    for (n = 0; n < sizeof(buf); ++n) buf[n] = '0' + (n % 10);

    for (n = 0; n < 10; ++n) sfwrite(f, buf, sizeof(buf));

    if (!(f = sfopen(f, tstfile("sf", 0), "r"))) terror("Can't open to read");

    for (n = 0; n < 10; ++n) {
        if ((r = sfread(f, buf2, sizeof(buf))) != sizeof(buf)) terror("Bad read size=%d", r);
        if (strncmp(buf, buf2, sizeof(buf)) != 0) terror("Get wrong data");
    }
