...
tmain() {
    ...
    if (sfread(fr, buf, 10) != 10) terror("Can't read data from pipe");
    SYNC sfwrite(fw, "0123456789", 10);
    ...
    SYNC if (!(s = sfgetr(fr, '\n', 1)) || strcmp(s, "abc")) terror("sfgetr failed");
    if (to) alarm(0);
    ...
    if (to) alarm(0);
    if (strcmp(s, "111")) terror("sfgetr got wrong string");

    if (to) alarm(4);
    SYNC if (sfmove(fr, sfstdout, (Sfoff_t)2, '\n') != 2) terror("sfmove failed");
    ...
    if (to) alarm(4);
    SYNC if (sfwrite(fw, "0123456789", 11) != 11) terror("Bad write to pipe2");
    SYNC if (!(s = sfreserve(fr, 11, 0))) terror("Bad peek size %d, expect 11", sfvalue(fr));
    if (to) alarm(0);
    if (strncmp(s, "0123456789", 10)) terror("Bad peek str %s", s);

    /* test for handling pipe error */
    if (pipe(fd) < 0) terror("Can't create pipe");
    close(fd[0]);
    if (!(fw = sfnew(NULL, NULL, sizeof(buf), fd[1], SF_WRITE))) terror("Can't open stream");
