...
static pid_t makeprocess(char *proc, int num) {
    ...
    if ((pid = fork()) < 0)
        ...
        argv[i++] = text;
        argv[i] = 0;
        if (execv(proc, argv) < 0) terror("Process[num=%d]: Could not execv() %s", num, proc);
    }
    return -1;
}
...
static int pingpong(char *procnum) {
    ...
    num = atoi(procnum);
    if ((pid = getpid()) < 0) terror("Process[num=%d]: can't get process id", num);
    ...
    tinfo("Process[num=%d,pid=%d]: move %d (%s -> %s)", num, pid, n_move,
          num % 2 == 0 ? "map" : "shm", num % 2 == 0 ? "shm" : "map");
...
tmain() {
    ...
    if ((ppid = getpid()) < 0) terror("Can't get process id");
    ...
    if (twait(cpid, N_CONCUR)) terror("workload subprocess error");
    ...
    for (k = 0; k < N_OBJ; ++k) {
        ...
        if (om && os) terror("Parent[pid=%d]: object %d is in both dictionaries", ppid, k);
        if (!om && !os) terror("Parent[pid=%d]: object %d is in neither dictionary", ppid, k);
    }
