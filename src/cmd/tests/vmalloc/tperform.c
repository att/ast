...
void *emphemeral(void *arg) /* an empheral thread, allocate a few times, free then return */
{
    ...
    for (k = 0; k < Empcount; ++k) {
        ...
        if (!(blk = (void *)malloc(sz))) terror("Malloc failed");
...
void *simulate(void *arg) {
    ...
    for (k = 0; k < nalloc; ++k) {
        ...
        for (up = list[k].free; up; up = next) {
            ...
            if (k == nalloc - 1 || (up->type == 0 && (RANDOM() % 100) >= Smallre) ||
            ...
            } else /* realloc: new size's range is [up->size/2, 2*up->size] */
            {
                ...
                if ((a = (unsigned long)(list[k].data) % ALIGNMENT) != 0)
                    terror("Thread %d: block=%#0x mod %d == %d", thread, list[k].data, ALIGNMENT,
                           a);
        ...
        if (!(list[k].data = malloc(sz))) terror("Thread %d: failed to malloc(%d)", thread, sz);
        if ((a = (unsigned long)(list[k].data) % ALIGNMENT) != 0) {
            if (warn_align)
        ...
        if (Empperiod > 0 && k > 0 && k % Empperiod == 0)
            if (pthread_join(ethread, 0) != 0) terror("Can't wait for emphemeral thread");
    }
...
tmain() {
    ...
    tinfo("Nstep=%d Nthread=%d ProbSmall=%d, Emphe(p=%d,n=%d)", Nalloc, Nthread, Small, Empperiod,
          Empcount);
