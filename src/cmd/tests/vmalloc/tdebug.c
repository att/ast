...
#define AFTER 7
    ...
    if (!(vm = vmopen(Vmdcheap, Vmdebug, VM_DBCHECK))) terror("Can't open region handle");

    if (pipe(pfd) < 0) terror("Can't puff a pipe");
    vmdebug(pfd[1]);

    for (k = 0; k < 10; ++k) {
        if (!(addr[k] = vmalloc(vm, ALLOCSZ))) terror("Allocation of block[%d] failed", k);
    ...
    if (vmfree(vm, addr[0]) < 0) terror("Failed freeing a block");
    vmfree(vm, addr[0]);
    if ((n = read(pfd[0], buf, sizeof(buf))) <= 0) terror("No corruption messages written");
    buf[n] = 0;
    tinfo(buf);
    ...
    if (vmfree(vm, addr[2]) < 0) terror("Failed freeing a block");
    if ((n = read(pfd[0], buf, sizeof(buf))) <= 0) terror("No corruption messages written");
    buf[n] = 0;
    tinfo(buf);
    ...
    if ((addr[4] = vmresize(vm, addr[4], 256, VM_RSMOVE | VM_RSCOPY)) == NULL)
        terror("Failed resizing");
    addr[4][256 + AFTER] = 0;
    if (vmfree(vm, addr[4]) < 0) terror("Failed freeing a block");
    if ((n = read(pfd[0], buf, sizeof(buf))) <= 0) terror("No corruption messages written");
    buf[n] = 0;
    tinfo(buf);
