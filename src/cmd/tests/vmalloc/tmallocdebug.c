...
tmain() {
    ...
    if ((addr[2] = realloc(addr[2], 256)) == NULL) terror("Failed resizing");
    ...
    free(addr[3]);
    if ((addr[3] = realloc(addr[3], 256)) != NULL) terror("Resizing a free block succeeded");
    TMESSAGE();

    /* corrupting a byte in back */
    if ((addr[4] = realloc(addr[4], 256)) == NULL) terror("Failed resizing");
    addr[4][256 + AFTER] = 0;
    free(addr[4]);
    TMESSAGE();
