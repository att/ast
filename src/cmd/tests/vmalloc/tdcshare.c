...
tmain() {
    ...
    if (k = tchild()) {
        ...
        if (!(dc = vmdcshare(shmfile, 1, MAPSIZE, 0)) || !(shm = vmopen(dc, Vmbest, 0)))
        ...
        if (!(dc = vmdcshare(mapfile, -1, MAPSIZE, 0)) || !(map = vmopen(dc, Vmbest, 0)))
        ...
        if (!(dc = vmdcshare(shmfile, 1, MAPSIZE, -1)) || !(shm = vmopen(dc, Vmbest, 0)))
        ...
        if (!(dc = vmdcshare(mapfile, -1, MAPSIZE, -1)) || !(map = vmopen(dc, Vmbest, 0)))
        ...
        switch ((cpid = fork())) /* make a child process */
        {
            ...
            case 0:
                ...
                arg[3] = mapfile;
                arg[4] = 0;
                if (execv(arg[0], arg) < 0) terror("Could not exec child process");
            case -1:
