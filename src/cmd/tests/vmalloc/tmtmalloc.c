...
static void *dummy(unsigned int i) { return NULL; }
...
static void run_test(void) {
    ...
    */
    ...
    tinfo("Thread %u adjusted timing: %d.%06d seconds for %d requests of %d bytes", pthread_self(),
          adjusted.tv_sec, adjusted.tv_usec, total_iterations, request_size);
...
tmain() {
    ...
    */
    for (i = 1; i <= thread_count; i++) pthread_join(thread[i], NULL);
