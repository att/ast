...
static int working(char *store, int type, ssize_t size) {
    ...
    if (!(dc = vmdcshare(store, type, size, 1)) || !(vm = vmopen(dc, Vmbest, 0)))
        terror("Can't open region based on %s", store);
    tinfo("Region successfully opened", store);
    if (vmstat(vm, &vmst) < 0) terror("vmstat failed");
    ...
    for (k = f = 0; k < N_ALLOC; ++k) {
        ...
        nbusy += 1;
        if (k < (N_ALLOC - 1) && (random() % 100) != 0) continue;

        for (; f <= k; ++f) /* free anything that should be freed */
        {
            if (!Piece[f].free) continue;
            if (vmfree(vm, Piece[f].addr) < 0) terror("Vmfree [f=%d] failed", f);
            nfree += 1;
            nbusy -= 1;
            Piece[f].free = 0;
    ...
    if (vmstat(vm, &vmst) < 0) terror("vmstat failed");
    tinfo("extent=%d busy=(expect=%d,actual=%d,z=%d) free=(n=%d,z=%d)", vmst.extent, nbusy,
          vmst.n_busy, vmst.s_busy, vmst.n_free, vmst.s_free);
...
tmain() {
    ...
    if (!(dc = vmdcshare(store, -1, size, -1)) || !(vm = vmopen(dc, Vmbest, 0)))
        terror("Can't open shared region");

    if (working(store, -1, size) < 0) terror("Failed simulation");

    if (vmstat(vm, &vmst) < 0) terror("Can't get statistics for region");
    tinfo("Statistics: extent=%d busy=(n=%d,z=%d) free=(n=%d,z=%d)", vmst.extent, vmst.n_busy,
          vmst.s_busy, vmst.n_free, vmst.s_free);
    if (vmst.n_busy != (N_ALLOC - N_FREE))
        terror("Number of busy block %d is not the expected %d", vmst.n_busy, (N_ALLOC - N_FREE));
