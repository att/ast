...
static int working(char *store, char *typeascii, char *pnumascii, ssize_t size) {
    ...
    if (!(dc = vmdcshare(store, type, size, 0)) || !(vm = vmopen(dc, Vmbest, 0)))
    ...
    n_busy = 0;
    for (k = 0; k < N_ALLOC; ++k)
        if (Piece[k].addr) n_busy += 1;

    tinfo("Process %d[pid=%d]: (n_alloc=%d, n_free=%d, n_busy=%d) simulation completed", pnum, pid,
          n_alloc, n_free, n_busy);
...
tmain() {
    ...
    if (k = tchild()) return working(argv[k], argv[k + 1], argv[k + 2], size);
    ...
    for (type = -1; type < 2; type += 2) /* -1 is mmap, 1 is shm */
    {
        ...
        if (!vm) terror("Can't open shared region");

        for (k = 0; k < N_PROC; ++k) proc[k] = makeprocess(argv[0], store, type, k);

        for (f = 0; f < N_PROC; ++f) {
            if ((pid = wait(0)) <= 0) terror("wait failed");
            for (k = 0; k < N_PROC; ++k)
                if (proc[k] == pid) break;
            if (k >= N_PROC) terror("Unknown child process %d", pid);
        }
