...
tmain() {
    ...
    for (n = 0; n < SEGN; ++n) {
        if (!(data[n][0] = vmalloc(vm, (SEGZ / SEGN) / 2 + 32))) terror("vmalloc failed");
        if (!(data[n][1] = vmalloc(vm, (SEGZ / SEGN) / 2 + 32))) terror("vmalloc failed");
        if (!(data[n][2] = vmalloc(vm, (SEGZ / SEGN) / 2 + 32))) terror("vmalloc failed");
        if (!(data[n][3] = vmalloc(vm, 128))) terror("vmalloc failed");
...
#define NBLOCK (SEGZ / (ZBLOCK + 64))
    {
        ...
        for (n = 0; n < NBLOCK; ++n)
            if (!(blk[n] = vmalloc(vm, ZBLOCK))) terror("vmalloc failed");
        ...
        for (n = 0; n < NBLOCK; n += 2) /* free all blocks to remerge free areas */
            vmfree(vm, blk[n]);
        if (!(big = vmalloc(vm, SEGZ / 4))) terror("vmalloc failed");
