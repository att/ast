...
tmain() {
    ...
    if (!(vm2 = vmopen(&Disc, Vmbest, 0))) terror("Failed to open vm2");
    if ((Vmuchar_t *)vm2 < Buf || (Vmuchar_t *)vm2 >= Endbuf)
        terror("Failed to get vm2 memory to be ours");

    if (!(vm3 = vmopen(&Disc, Vmbest, 0))) terror("Failed to open vm3");
    if ((Vmuchar_t *)vm3 < Buf || (Vmuchar_t *)vm3 >= Endbuf)
        terror("Failed to get vm3 memory to be ours");

    Disc.exceptf = NULL;
    if (!(vm4 = vmopen(&Disc, Vmbest, 0))) terror("Failed to open vm4");
    if ((Vmuchar_t *)vm4 >= Buf && (Vmuchar_t *)vm4 < Endbuf)
        terror("vm4 memory should not be ours");

    if (!(m1 = vmalloc(vm1, 1024))) terror("vmalloc failed on m1");
    if (!(m2 = vmalloc(vm2, 1024))) terror("vmalloc failed on m2");
    if (!(m3 = vmalloc(vm3, 1024))) terror("vmalloc failed on m3");
    if (!(m4 = vmalloc(vm4, 1024))) terror("vmalloc failed on m4");

    if (!(m1 = vmresize(vm1, m1, 4 * 1024, VM_RSMOVE | VM_RSCOPY))) terror("vmresize failed on m1");
    if (!(m2 = vmresize(vm2, m2, 4 * 1024, VM_RSMOVE | VM_RSCOPY))) terror("vmresize failed on m2");
    if (!(m3 = vmresize(vm3, m3, 4 * 1024, VM_RSMOVE | VM_RSCOPY))) terror("vmresize failed on m3");
    if (!(m4 = vmresize(vm4, m4, 4 * 1024, VM_RSMOVE | VM_RSCOPY))) terror("vmresize failed on m4");

    vmsegwalk(NULL, walk, &Walk);
    if (Walk != Count) terror("Wrong walk count");
    tinfo("Number of segments: %d", Count);

    if (vmfree(vm1, m1) < 0) terror("vmfree failed on m1");
    if (vmfree(vm2, m2) < 0) terror("vmfree failed on m2");
    if (vmfree(vm3, m3) < 0) terror("vmfree failed on m3");
    if (vmfree(vm4, m4) < 0) terror("vmfree failed on m4");
