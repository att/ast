...
tmain() {
    ...
    if (!(vm = vmopen(&Disc, Vmbest, 0))) terror("Can't open");
    ...
    for (i = 0; i < 10; ++i)
        if (vmfree(vm, addr[i]) < 0) terror("can't free an element?");

    if (!(addr[0] = vmalloc(vm, 100))) terror("Can't alloc");

    for (i = 0; i < 10; ++i) addr[i] = vmalloc(vm, 100);

    if (vmresize(vm, addr[0], 90, 1) != addr[0]) terror("Location should not have changed");
