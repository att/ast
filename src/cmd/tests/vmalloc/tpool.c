...
tmain() {
    ...
    if (!(vm = vmopen(&Disc, Vmpool, 0))) terror("Can't open");
    ...
    for (i = 0; i < 10; ++i) addr[i] = vmalloc(vm, 15);

    if (vmresize(vm, addr[0], 16, 1) != NULL) terror("Resize to a different size succeeds?");
