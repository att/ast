...
static void *worker(void *arg) {
    ...
    if (w->object) free(w->object);

    for (i = 0; i < w->iterations; ++i) {
        char *obj;

        if (!(obj = (char *)malloc(w->objsize))) terror("malloc failed");
...
tmain() {
    ...
    for (i = 0; i < nthreads; ++i)
        if (!(objs[i] = (char *)malloc(objsize))) terror("Can't allocate objs[%d]", i);
    ...
    for (i = 0; i < nthreads; ++i)
        if ((rv = pthread_join(thread[i], &status)) != 0) terror("Failed waiting for thread %d", i);
