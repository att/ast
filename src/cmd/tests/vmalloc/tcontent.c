...
tmain() {
    ...
    tinfo("Nstep=%d Nthread=%d Small(min=%d,max=%d,life=%d) Large(min=%d,max=%d,life=%d) ", Nalloc,
          Nthread, Smalllo, Smallhi, Smalllf, Largelo, Largehi, Largelf);
    ...
    for (i = 0; i < Nthread; ++i) {
        for (k = 0; k < Tdata[i].nalloc; ++k) {
            if (Tdata[i].list[k].size == 0) continue;
            if ((ch = Tdata[i].list[k].data[0]) == CH_MALLOC || ch == CH_REALLOC) {
                for (t = 1; t < Tdata[i].list[k].size; ++t)
                    if (Tdata[i].list[k].data[t] != ch) terror("Corrupted allocated data");
            } else
