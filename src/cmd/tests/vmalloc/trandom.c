...
#define TIME(nk, kk, zz) \
    ((nk = kk + (RAND() % TMRANGE(zz)) + 1), (nk = nk <= kk ? kk + 1 : nk > N_OBJ ? N_OBJ : nk))
...
tmain() {
    ...
    if (!(huge = vmalloc(Vmregion, hugesz))) terror("Can't allocate block");
    ...
    for (k = 0; k < N_OBJ; ++k) {
        ...
        if (STAT(k)) {
            if (vmstat(Vmregion, &sb) < 0) terror("Vmstat failed");
            tinfo("Arena: busy=(%u,%u) free=(%u,%u) extent=%u #segs=%d", sb.n_busy, sb.s_busy,
                  sb.n_free, sb.s_free, sb.extent, sb.n_seg);
        }
        ...
        o->size = ALLOCSIZE();
        if (!(o->obj = vmalloc(Vmregion, o->size))) terror("Vmalloc failed");
        TIME(p, k, o->size);
        o->next = List[p];
        List[p] = o;
    }

    if (vmdbcheck(Vmregion) < 0) terror("Corrupted region");

    if (vmstat(Vmregion, &sb) < 0) terror("Vmstat failed");
    tinfo("After simulation loop: Busy=(%u,%u) Free=(%u,%u) Extent=%u #segs=%d", sb.n_busy,
          sb.s_busy, sb.n_free, sb.s_free, sb.extent, sb.n_seg);

    /* now free all left-overs */
    for (o = List[N_OBJ]; o; o = o->next) vmfree(Vmregion, o->obj);
    vmfree(Vmregion, huge);

    if (vmstat(Vmregion, &sb) < 0) terror("Vmstat failed");
    tinfo("After freeing all: Busy=(%u,%u) Free=(%u,%u) Extent=%u #segs=%d", sb.n_busy, sb.s_busy,
          sb.n_free, sb.s_free, sb.extent, sb.n_seg);

    if (!(huge = vmalloc(Vmregion, 10))) terror("Vmalloc failed");
    if (vmstat(Vmregion, &sb) < 0) terror("Vmstat failed");
    tinfo("Allocating a small block: Busy=(%u,%u) Free=(%u,%u) Extent=%u #segs=%d", sb.n_busy,
          sb.s_busy, sb.n_free, sb.s_free, sb.extent, sb.n_seg);
