...
static void put_chtype(Namval_t *np, const char *val, int flag, Namfun_t *fp) {
    ...
    if (!val) {
        ...
        if (mp->nvalue.cp >= pp->ttype->data &&
            ...
            if (np->nvalue.cp != mp->nvalue.cp) memcpy((char *)np->nvalue.cp, mp->nvalue.cp, dsize);
...
static Namfun_t *clone_type(Namval_t *np, Namval_t *mp, int flags, Namfun_t *fp) {
    ...
    for (i = dp->numnodes; --i >= 0;) {
        ...
        if (fixnode(dp, pp, i, nrp, NV_TYPE | (flags & NV_IARRAY))) {
            ...
            if (nv_type(np) != np || nv_getnum(nq) == 0) nq->nvalue.cp = Empty;
        }
...
Namval_t *nv_mktype(Namval_t **nodes, int numnodes) {
    ...
    if (qp) mnodes = newof(NiL, Namval_t *, nd + 1, 0);
    nd = 0;
            ...
            for (r = 0; r < dp->numnodes; r++) {
                ...
                if (nr->nvalue.cp) {
                    ...
                    if (xp && nr->nvalue.cp >= xp->ptype->data &&
                        nr->nvalue.cp < xp->ptype->data + xp->ptype->fun.dsize) {
