...
int sh_close(int fd) {
    ...
    if (r = (shp->fdstatus[fd] >> 8)) close(r);
...
int sh_pipe(int pv[]) {
    ...
    sh_iovalidfd(shp, fd[0]);
    sh_iovalidfd(shp, fd[1]);
#if _pipe_socketpair && !_stream_peek
        ...
        if (*fname || (iof & (IODOC | IOSTRG)) == (IODOC | IOSTRG)) {
            if (iof & IODOC) {
                ...
                if (iof & IOAPP) {
                    ...
                    io_op[2] = ';';
                    o_mode |= O_TRUNC;
                    if (tname = io_usename(shp, fname, &perm, fn, 0)) o_mode |= O_EXCL;
                } else {
                    o_mode |= O_TRUNC;
                    if (iof & IOCLOB)
                        io_op[2] = '|';
...
int sh_iocheckfd(Shell_t *shp, int fd, int fn) {
    ...
    if (!(n & (IOREAD | IOWRITE))) {
#ifdef F_GETFL
        if ((flags = fcntl(fd, F_GETFL, 0)) < 0) return (shp->fdstatus[fd] = IOCLOSE);
        if ((flags & O_ACCMODE) != O_WRONLY) n |= IOREAD;
        if ((flags & O_ACCMODE) != O_RDONLY) n |= IOWRITE;
#else
        struct stat statb;
        if ((flags = fstat(fd, &statb)) < 0) return (shp->fdstatus[fd] = IOCLOSE);
        n |= (IOREAD | IOWRITE);
...
int sh_fcntl(int fd, int op, ...) {
    ...
    if (newfd >= 0) switch (op) {
            ...
            case F_DUPFD_CLOEXEC: {
                ...
                if (newfd >= shp->gd->lim.open_max) sh_iovalidfd(shp, newfd);
                if (op == F_DUPFD) {
