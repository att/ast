...
static Namarr_t *array_scope(Namval_t *np, Namarr_t *ap, int flags) {
    ...
    if (!(aq = newof(NIL(Namarr_t *), Namarr_t, 1, size - sizeof(Namarr_t)))) return (0);
    memcpy(aq, ap, size);
    aq->hdr.nofree &= ~1;
...
#if SHOPT_FIXEDARRAY
    else if ((struct fixed_array *)ap->fixed) {
...
static bool array_unscope(Namval_t *np, Namarr_t *ap) {
    ...
    if (is_associative(ap)) (*ap->fun)(np, NIL(char *), NV_AFREE);
...
#if SHOPT_FIXEDARRAY
    else if ((struct fixed_array *)arp->fixed) {
        fp = (struct fixed_array *)arp->fixed;
        if (!fp->data) array_fixed_setdata(np, arp, fp);
        up = &np->nvalue;
        if (fp->ptr) {
...
bool nv_arrayisset(Namval_t *np, Namarr_t *arp) {
    ...
    if (is_associative(ap)) return (np = nv_opensub(np)) && !nv_isnull(np);
...
static Namval_t *array_find(Namval_t *np, Namarr_t *arp, int flag) {
    ...
    if (wasundef) {
        ...
        if (flag & ARRAY_DELETE) {
            ...
            nv_putsub(
                np, NIL(char *), 0,
                ARRAY_SCAN | ARRAY_NOSCOPE | (ap->header.fixed ? (ARRAY_UNDEF | ARRAY_FIXED) : 0));
#else
    ...
    if (is_associative(ap)) {
        ...
        if (!mp) {
            ...
            if (wasundef) nv_putsub(mp, NIL(char *), 0, ARRAY_UNDEF);
            return mp;
        } else {
            up = &mp->nvalue;
            if (nv_isvtree(mp)) {
        ...
        if (fp->ptr) {
            if (!fp->data) array_fixed_setdata(np, &ap->header, fp);
        ...
        if (up->np && array_isbit(ap->bits, ap->cur, ARRAY_CHILD)) {
            if (wasundef && nv_isarray(up->np)) nv_putsub(up->np, NIL(char *), 0, ARRAY_UNDEF);
            return up->np;
        }
        if (flag & ARRAY_ASSIGN) {
    ...
    if (!string && !nv_isattr(np, NV_ARRAY)) {
        ...
        if (fp && (!ap->scope || data != fp->data)) {
            if (fp->ptr) {
                ...
                char **cp = (char **)fp->data;
                while (n-- > 0) {
                    if (cp && *cp != Empty) free(*cp);
                    cp++;
                }
            }
            free((void *)fp->data);
            if (data) fp->data = data;
        } else
#endif  // SHOPT_FIXEDARRAY
...
static Namarr_t *nv_changearray(Namval_t *np, void *(*fun)(Namval_t *, const char *, int)) {
    ...
    if (!fun || !(ap = nv_arrayptr(np)) || is_associative(ap)) return (NIL(Namarr_t *));
...
bool nv_nextsub(Namval_t *np) {
    ...
    if (is_associative(ap)) {
        if ((*ap->header.fun)(np, NIL(char *), NV_ANEXT)) return (true);
        ...
        if (ap->header.flags & ARRAY_FIXED) {
            while (++fp->curi < fp->nelem) {
                ...
                if (fp->ptr && *(((char **)fp->data) + fp->curi)) return (true);
            }
        ...
        if (fp->level) {
            dot = --fp->dim;
            while ((dot + 1) < fp->ndim) fp->cur[++dot] = 0;
            fp->level--;
            fp->curi = 0;
        } else {
            ap->header.flags &= ~(ARRAY_SCAN | ARRAY_NOCHILD);
        ...
        if (aq->val[dot].cp) {
            ...
            if (array_isbit(aq->bits, dot, ARRAY_CHILD)) {
                ...
                if (nv_isarray(mp)) {
                    ...
                    if (aq && aq->header.nelem) nv_putsub(mp, NIL(char *), 0, ARRAY_SCAN);
                }
            }
            return true;
        }
...
Namval_t *nv_putsub(Namval_t *np, char *sp, long size, int flags) {
...
#endif  // SHOPT_FIXEDARRAY
    {
        ...
        if (!ap || size >= ap->maxi) {
            if (size == 0 && !(flags & ARRAY_FILL)) return (NIL(Namval_t *));
...
#if SHOPT_FIXEDARRAY
    fp = (struct fixed_array *)ap->header.fixed;
    if (fp) {
        if (!fp->data) return (np);
        if (flags & ARRAY_UNDEF) {
            fp->dim = 0;
            fp->curi = 0;
            for (size = fp->ndim; --size >= 0;) fp->cur[size] = 0;
            ...
            while (fp->dim < fp->ndim) fp->cur[fp->dim++] = 0;
...
int nv_arrfixed(Namval_t *np, Sfio_t *out, int flag, char *dim) {
    ...
    if (flag) {
        if (out) {
            for (n = 0; n < fp->dim; n++) sfprintf(out, "[%d]", fp->cur[n]);
        }
        if (dim) *dim = fp->dim;
        return (fp->curi);
    }
    if (out) {
        for (n = 0; n < fp->ndim; n++) sfprintf(out, "[%d]", fp->max[n]);
    }
    fp->dim = 0;
    return fp->curi;
}
...
static bool array_fixed_init(Namval_t *np, char *sub, char *cp) {
    ...
    if (!(ap = newof(NIL(Namarr_t *), Namarr_t, 1, sz))) return (false);
    ...
    for (sz = 1; --n >= 0;) sz = fp->incr[n] = sz * fp->max[n + 1];
...
static char *array_fixed(Namval_t *np, char *sub, char *cp, int mode) {
    ...
    if (!fp->data) array_fixed_setdata(np, ap, fp);
    ...
    *cp++ = ']';
    ...
    for (n++, ep = cp; *ep == '['; ep = cp, n++) {
        if (n >= fp->ndim) errormsg(SH_DICT, ERROR_exit(1), e_subscript, nv_name(np));
        cp = nv_endsubscript(np, ep, 0, np->nvshell);
        cp[-1] = 0;
...
char *nv_getsub(Namval_t *np) {
    ...
    if (!np || !(ap = (struct index_array *)nv_arrayptr(np))) return (NIL(char *));
    if (is_associative(ap)) return ((char *)((*ap->header.fun)(np, NIL(char *), NV_ANAME)));
    if (ap->xp) {
        np = nv_namptr(ap->xp, 0);
        np->nvalue.s = ap->cur;
        return (nv_getval(np));
...
void *nv_associative(Namval_t *np, const char *sp, int mode) {
    ...
    switch (mode) {
        ...
        case NV_ANAME: {
            if (ap->cur) {
                if (!shp->instance && nv_isnull(ap->cur)) return (NIL(void *));
