...
int tty_alt(int fd) {
    ...
    if (ioctl(fd, TIOCGETC, &l_ttychars) == SYSERR) return -1;
    if (ioctl(fd, TIOCLGET, &l_mask) == SYSERR) return -1;
    ttychars = l_ttychars;
    mask = LCRTBS | LCRTERA | LCTLECH | LPENDIN | LCRTKIL;
    if ((l_mask | mask) != l_mask) l_changed = L_MASK;
    if (ioctl(fd, TIOCLBIS, &mask) == SYSERR) return -1;
    if (ttychars.t_brkc != ESC) {
        ttychars.t_brkc = ESC;
        l_changed |= T_CHARS;
        if (ioctl(fd, TIOCSETC, &ttychars) == SYSERR) return -1;
    }
    return 0;
}
#else  // TIOCGETC
