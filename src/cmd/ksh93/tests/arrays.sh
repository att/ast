...
if [[ $x != zero ]]
then
    err_exit '$x is not element 0'
fi

if [[ ${x[0]} != zero ]]
then
    err_exit '${x[0] is not element 0'
fi

if (( ${#x[0]} != 4 ))
then
    err_exit "length of ${x[0]} is not 4"
fi

if (( ${#x[@]} != 6  ))
then
    err_exit 'number of elements of x is not 6'
fi

if [[ ${x[2]} != two  ]]
then
    err_exit ' element two is not 2'
fi

if [[ ${x[@]:2:1} != two  ]]
then
    err_exit ' ${x[@]:2:1} is not two'
fi
...
if [[ ${y[0]} != zero ]]
then
    err_exit '${y[0] is not element 0'
fi

if (( ${#y[@]} != 7  ))
then
    err_exit 'number of elements of y is not 7'
fi
...
if [[ ${y[0]} != nine ]]
then
    err_exit '${y[0] is not nine'
fi

unset y[4]
if (( ${#y[@]} != 6  ))
then
    err_exit 'number of elements of y is not 6'
fi

if (( ${#y[4]} != 0  ))
then
    err_exit 'string length of unset element is not 0'
fi

unset foo
if (( ${#foo[@]} != 0  ))
then
    err_exit 'number of elements of unset variable foo is not 0'
fi

foo=''
if (( ${#foo[0]} != 0  ))
then
    err_exit 'string length of null element is not 0'
fi

if (( ${#foo[@]} != 1  ))
then
    err_exit 'number of elements of null variable foo is not 1'
fi
...
if (( ${#foo[@]} != 0  ))
then
    err_exit 'number of elements of left in variable foo is not 0'
fi
...
if (( ${#foo[@]} != 0  ))
then
    err_exit 'number of elements of left in variable foo again is not 0'
fi

fun
if (( ${#xxx[@]} != 2  ))
then
    err_exit 'number of elements of left in variable xxx is not 2'
fi

fun
if (( ${#xxx[@]} != 2  ))
then
    err_exit 'number of elements of left in variable xxx again is not 2'
fi

set -A foo -- "${x[@]}"
if (( ${#foo[@]} != 6  ))
then
    err_exit 'number of elements of foo is not 6'
fi

if (( ${#PWD[@]} != 1  ))
then
    err_exit 'number of elements of PWD is not 1'
fi
...
if (( ${#x[@]} != 2  ))
then
    err_exit 'number of elements of x is not 2'
fi

s[1]=1 c[1]=foo
if [[ ${c[s[1]]} != foo ]]
then
    err_exit 'c[1]=foo s[1]=1; ${c[s[1]]} != foo'
fi
...
if (( ${#s[@]} != 2  ))
then
    err_exit 'number of elements of  is not 2'
fi
...
if   [[ ${y[@]} != "$z" ]]
then
    err_exit 'error in array conversion from int to R12'
fi

if   (( ${#y[@]} != 3  ))
then
    err_exit 'error in count of array conversion from int to R12'
fi
...
if [[ ${y[1]} != 4 ]]
then
    err_exit 'arithmetic expressions in typeset not working'
fi
...
if [[ ${foo[0]} != bar ]]
then
    err_exit 'initial value not preserved when typecast to associative'
fi
...
if [[ ${#foo[*]} != 3 ]]
then
    err_exit 'conversion of indexed to associative array failed'
fi
...
if [[ ${!xxx[@]} != 0 ]]
then
    err_exit '${!xxx[@]} for scalar not 0'
fi

if [[ ${11} != k ]]
then
    err_exit '${11} not working'
fi

if [[ ${@:4:1} != d ]]
then
    err_exit '${@:4:1} not working'
fi
...
then
    err_exit '${!foovar@} does not expand correctly'
fi

if [[ ${!foovar1} != foovar1 ]]
then
    err_exit '${!foovar1} != foovar1'
fi
...
if (( i != 1))
then
    err_exit 'execution trace side effects with array subscripts'
fi
...
if (( ${#list[@]} != 0))
then
    err_exit '$(set -A list ...) leaves side effects'
fi
...
if [[ ${list[1]} != bar ]]
then
    err_exit 'array not restored after subshell'
fi
...
then
    err_exit '${xpath[@]##*/} not applied to each element'
fi
...
if [[ ${foo[@]:n} != six ]]
then
    err_exit 'array offset of -1 not working'
fi
...
if [[ ${#foo[@]} != 2 ]]
then
    err_exit 'array getting unset before right hand side evaluation'
fi
...
then
    err_exit 'exporting indexed array not exporting 0-th element'
fi
...
if [[ ${!foo[@]} != two ]]
then
    err_exit 'error in subscript names'
fi
