...
if [[ $x != 'hi there' ]]
then
    err_exit "$x not the same as 'hi there'"
fi
...
then
    err_exit "nested \$(print -r - '') differs from ''"
fi
...
then
    err_exit "\${x-\$(print -r - '')} differs from ''"
fi
...
if [[ $x != foo  ]]
then
    err_exit "((cmd)|(cmd)) failed"
fi

x=$(print -r -- "\"$HOME\"")
if [[ $x != '"'$HOME'"' ]]
then
    err_exit "nested double quotes failed"
fi
...
if [[ $(print -r -- "a\*b") !=  'a\*b' ]]
then
    err_exit '$(print -r -- "a\*b") differs from  a\*b'
fi

unset x
if [[ $(print -r -- "a\*b$x") !=  'a\*b' ]]
then
    err_exit '$(print -r -- "a\*b$x") differs from  a\*b'
fi
...
if (( $# !=3 ))
then
    err_exit '${x+foo bar bam} does not yield three arguments'
fi

set -- ${x+foo "bar bam"}
if (( $# !=2 ))
then
    err_exit '${x+foo "bar bam"} does not yield two arguments'
fi
...
if (( $# !=3 ))
then
    err_exit '${x+foo $x bam} does not yield three arguments'
fi
...
if (( $# !=1 ))
then
    err_exit '${x+"foo $x bam"} does not yield one argument'
fi
...
if (( $# !=2 ))
then
    err_exit '${x+foo "$x "bam} does not yield two arguments'
fi
...
if [[ $x != "'" ]]
then
    err_exit 'line continuation in expanded double strings not working'
fi
...
if (( $# != 2 ))
then
    err_exit 'field splitting error'
fi
...
if (( ${#x} != (15*73-3) ))
then
    err_exit "length of x, ${#x}, is incorrect should be $((15*73-3))"
fi

x='$hi'
if [[ $x\$ != '$hi$' ]]
then
    err_exit ' $x\$, with x=$hi, does not expand to $hi$'
fi
...
if [[ $# != 2 ]]
then
    err_exit 'word splitting after command substitution not working'
fi

unset q
if [[ "${q:+'}q${q:+'}" != q ]]
then
    err_exit 'expansion of "{q:+'\''}" not correct when q unset'
fi

q=1
if [[ "${q:+'}q${q:+'}" != "'q'" ]]
then
    err_exit 'expansion of "{q:+'\''}" not correct when q set'
fi
...
if [[ $2 != 'b\' ]]
then
    err_exit "field splitting of $x with IFS=$IFS not working"
fi
...
if [[ $(print -r -- $x) != "$x" ]]
then
    err_exit '$x, where x=\\(..\\)|&\|\|\\&\\| not working'
fi

x='\\('
if [[ $(print -r -- a${x}b) != a"${x}"b ]]
then
    err_exit 'a${x}b, where x=\\( not working'
fi

x=
if [[ $(print -r -- $x'\\1') != '\\1' ]]
then
    err_exit 'backreference inside single quotes broken'
fi
...
if (( $# != 1 ))
then
    err_exit '"$@" not preserving nulls'
fi

x=
if [[ $(print -r s"!\2${x}\1\a!") != 's!\2\1\a!' ]]
then
    err_exit  'print -r s"!\2${x}\1\a!" not equal s!\2\1\a!'
fi

if [[ $(print -r $'foo\n\n\n') != foo ]]
then
    err_exit 'trailing newlines on comsubstitution not removed'
fi
...
if [[ $(print -r -- {\$foo}) != '{$foo}' ]]
then
    err_exit '{\$foo}' not expanded correctly
fi
