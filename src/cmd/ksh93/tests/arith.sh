...
if (( (i?0:1) ))
then
    err_exit '(( (i?0:1) )) failed'
fi

if (( (1 || 1 && 0) != 1 ))
then
    err_exit '( (1 || 1 && 0) != 1) failed'
fi

if (( (_=1)+(_x=0)-_ ))
then
    err_exit '(_=1)+(_x=0)-_ failed'
fi

if ((  (3^6) != 5))
then
    err_exit '((3^6) != 5) failed'
fi

integer x=1
if (( (x=-x) != -1 ))
then
    err_exit '(x=-x) != -1 failed'
fi

i=2
if (( 1$(($i))3 != 123 ))
then
    err_exit ' 1$(($i))3 failed'
fi
...
then
    err_exit 'cos*cos +sin*sin > 1.01'
fi

if (( point.x*point.x + point.y*point.y < .99 ))
then
    err_exit 'cos*cos +sin*sin < .99'
fi

if [[ $((y=x=1.5)) != 1 ]]
then
    err_exit 'typecast not working in arithmetic evaluation'
fi
...
if [[ $? == 0 ]]
then
    err_exit 'postincrement of floating point allowed'
fi

( ((++x))  ) 2>/dev/null
if [[ $? == 0 ]]
then
    err_exit 'preincrement of floating point allowed'
fi
...
if [[ $? == 0 ]]
then
    err_exit 'floating point allowed with % operator'
fi

x=.125
if [[ $(( 4 * x/2 )) != 0.25 ]]
then
    err_exit '(( 4 * x/2 )) is not 0.25, with x=.125'
fi
...
if [[ $z != "08" ]]
then
    err_exit "typeset -RZ2 leading 0 decimal not working [z=$z]"
fi

{ z=$(typeset -RZ3 z3; (( z3 = 8 )); print $z3) ;} 2>/dev/null
if [[ $z != "008" ]]
then
    err_exit "typeset -RZ3 leading 0 decimal not working [z=$z]"
fi
...
if [[ $z != 011 ]]
then
    err_exit "leading 0's in -Z not treated as decimal"
fi
...
if [[ $(newscope) != 2.5 ]]
then
    err_exit "arithmetic using wrong scope"
fi
unset x
...
if [[ $x != 11 ]]
then
    err_exit "constant index array arithmetic failure"
fi

(( x = $empty y[3] + y[4] ))
if [[ $x != 11 ]]
then
    err_exit "empty constant index array arithmetic failure"
fi

(( x = y[i] + y[i+1] ))
if [[ $x != 11 ]]
then
    err_exit "variable subscript index array arithmetic failure"
fi
...
if [[ $x != 11 ]]
then
    err_exit "nested subscript index array arithmetic failure"
fi
...
if [[ $x != 11 ]]
then
    err_exit "constant associative array arithmetic failure"
fi

(( x = y[$three] + y[$four] ))
if [[ $x != 11 ]]
then
    err_exit "variable subscript associative array arithmetic failure"
fi
...
if { (( z%2 != 1));} 2> /dev/null
then
    err_exit '% not working on floating point'
fi
...
if (( x < 1e20 ))
then
    err_exit 'large integer constants not working'
fi
...
if [[ $(foobar y) != 5 ]]
then
    err_exit 'name references in arithmetic statements in functions broken'
fi

if (( 2**3 != pow(2,3) ))
then
    err_exit '2**3 not working'
fi
...
if (( x != 11 ))
then
    err_exit '-Z3 not treated as decimal'
fi
...
if [[ $x != 42#18 ]]
then
    err_exit 'display of unsigned integers in non-decimal bases wrong'
fi
...
then
    err_exit 'parsing error with x$((10))=foo'
fi
...
do
    [[ $( ( $SHELL -c "( print -r -- \$(($expr)) )" ) 2>/dev/null ) == 2 ]] || err_exit "invalid value for: $expr"
done
...
then
    ...
    err_exit 'Inf and NaN not working'
fi
...
for i in 1
do
    (($x == 1)) || err_exit 'arithmetic in for loop with $x, where $x is hex constant not working'
done
