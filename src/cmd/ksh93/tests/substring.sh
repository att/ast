...
if [[ ${string1:1:4} != home ]]
then
    err_exit "string1:1:4"
fi

if [[ ${string1: -5:4} != bcab ]]
then
    err_exit "string1: -5:4"
fi

if [[ ${string1:1:j} != home ]]
then
    err_exit "string1:1:j"
fi

if [[ ${string1:(j?1:0):j} != home ]]
then
    err_exit "string1:(j?1:0):j"
fi

if [[ ${string1%*zzz*} != "$string1" ]]
then
    err_exit "string1%*zzz*"
fi
...
if [[ ${foo#a[b*} != abc ]]
then
    err_exit "abc#a[b*} != abc"
fi

if [[ ${foo//[0-9]/bar} != abc ]]
then
    err_exit '${foo//[0-9]/bar} not expanding correctly'
fi
...
if [[ ${foo/[0-9]?/""} != a3b456c ]]
then
    err_exit '${foo/[0-9]?/""} not expanding correctly'
fi
...
then
    err_exit '${foo/#a/b} not expanding correctly'
fi

if [[ ${foo/#?/b} != b123b456c ]]
then
    err_exit '${foo/#?/b} not expanding correctly'
fi

if [[ ${foo/%c/b} != a123b456b ]]
then
    err_exit '${foo/%c/b} not expanding correctly'
fi
...
do
    ...
    then
        if [[ $string != $pattern ]]
        then
            err_exit "$pattern does not match $string"
        fi

        if [[ ${string##$pattern} != "" ]]
        then
            err_exit "\${$string##$pattern} not null"
        fi
        ...
        if [[ $string == $pattern ]]
        then
            err_exit "$pattern matches $string"
        fi
    fi
...
if [[ $yy != b/c/d/e ]]
then
    err_exit '${xx#*/} != a/b/c/d/e when xx=a/b/c/d/e'
fi

if [[ ${xx//\//\\} != 'a\b\c\d\e' ]]
then
    err_exit '${xx//\//\\} not working'
fi
...
then
     err_exit '${xx//%28/abc\)} not working'
fi
...
then
    err_exit 'single quoting / in replacements failed'
fi

if [[ ${foo//"/"/_} != one_two_three ]]
then
    err_exit 'double quoting / in replacements failed'
fi

if [[ ${foo//\//_} != one_two_three ]]
then
    err_exit 'escaping / in replacements failed'
fi
...
if [[ ${val#dgk=} != base ]]
then
    err_exit 'export not restored after function call'
fi

if [[ $(myexport dgk fun fun2) != fun2 ]]
then
    err_exit 'export inside function not working with recursive function'
fi

val=$(export | grep "^dgk=")
if [[ ${val#dgk=} != base ]]
then
    err_exit 'export not restored after recursive function call'
fi

if [[ $(dgk=try3 myexport dgk) != try3 ]]
then
    err_exit 'name=value not added to export list with function call'
fi

val=$(export | grep "^dgk=")
if [[ ${val#dgk=} != base ]]
then
    err_exit 'export not restored name=value function call'
fi

unset zzz
if [[ $(myexport zzz fun) != fun ]]
then
    err_exit 'export inside function not working for zzz'
fi

if [[ $(export | grep "zzz=") ]]
then
    err_exit 'zzz exported after function call'
fi

set -- foo/bar bam/yes last/file/done
if [[ ${@/*\/@(*)/\1} != 'bar yes done' ]]
then
    err_exit '\1 not working with $@'
fi

var=(foo/bar bam/yes last/file/done)
if [[ ${var[@]/*\/@(*)/\1} != 'bar yes done' ]]
then
    err_exit '\1 not working with ${var[@]}'
fi

var='abc_d2ef.462abc %%'
if [[ ${var/+(\w)/Q} != 'Q.462abc %%' ]]
then
    err_exit '${var/+(\w)/Q} not workding'
fi
...
then
    err_exit '${.sh.match[1]:1:${#.sh.match[1]}} not expanding correctly'
fi
...
then
    LC_ALL=C.UTF-8 $SHELL -c b1=$'"\342\202\254\342\202\254\342\202\254\342\202\254w\342\202\254\342\202\254\342\202\254\342\202\254"; [[ ${b1:4:1} == w ]]' || err_exit 'multibyte ${var:offset:len} not working correctly'
fi
...
do
    ...
    if [[ "$o" != "$4" ]]
    then
        err_exit "i='$1'; \${i/~($2)$3/\\(\\0\\)} failed -- expected '$4', got '$o'"
    fi

    eval o="\${i/~($2)($3)/\\(\\1\\)}"
    if [[ "$o" != "$4" ]]
    then
        err_exit "i='$1'; \${i/~($2)($3)/\\(\\1\\)} failed -- expected '$4', got '$o'"
    fi
...
then
    ...
    err_exit '${##[0-9]} give syntax error'
fi
...
for v in "${arr[@]:5}"
do
    err_exit "\${arr[@]:5} should not generate $v" 
    break
done

for v in "${arr[@]:1:0}"
do
    err_exit "\${arr[@]:1:0} should not generate ${v:-empty_string}"
    break
done

for v in "${arr[@]:0:-1}"
do
    err_exit "\${arr[@]:0:-1} should not generate ${v:-empty_string}"
    break
done
...
for v in "${@:5}"
do
    err_exit "\${@:5} should not generate $v" 
    break
done

for v in "${@:1:0}"
do
    err_exit "\${@:1:0} should not generate ${v:-empty_string}"
    break
done

for v in "${@:0:-1}"
do
    err_exit "\${@:0:-1} should not generate ${v:-empty_string}"
    break
done
