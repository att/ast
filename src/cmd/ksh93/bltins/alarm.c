...
static_fn void *time_add(struct tevent *item, void *list) {
    ...
    if (!tp || item->milli < tp->milli) {
        ...
        while (tp->next && item->milli > tp->next->milli) tp = tp->next;
        item->next = tp->next;
        tp->next = item;
    }
    tp = item;
...
static_fn void *time_delete(struct tevent *item, void *list) {
    ...
    if (item == tp) {
        ...
        while (tp && tp->next != item) tp = tp->next;
        if (tp) tp->next = item->next;
    }
    if (item->timeout) timerdel((void *)item->timeout);
    return list;
}
...
static_fn void trap_timeout(void *handle) {
    ...
    if (sh_isstate(tp->sh, SH_TTYWAIT) && !tp->sh->bltinfun) sh_timetraps(tp->sh);
}
...
void sh_timetraps(Shell_t *shp) {
    ...
    while (1) {
        ...
        if (!(shp->sigflag[SIGALRM] & SH_SIGALRM)) break;
    }
}
...
static_fn void putval(Namval_t *np, const char *val, int flag, Namfun_t *fp) {
    ...
    if (val) {
        ...
        if (*val == '+') {
            ...
            d = strtod(val, &last);
            if (*last) {
                pp = sfprints("exact %s", val);
                if (pp) d = tmxdate(pp, &last, TMX_NOW);
                if (*last && (pp = sfprints("p%s", val))) d = tmxdate(pp, &last, TMX_NOW);
                d /= 1000000000;
                x = d;
                d -= now;
            }
        ...
        if (tp->timeout) shp->st.timetrap = time_delete(tp, shp->st.timetrap);
        if (tp->milli > 0) shp->st.timetrap = time_add(tp, shp->st.timetrap);
    } else {
        tp = (struct tevent *)nv_stack(np, (Namfun_t *)0);
        shp->st.timetrap = time_delete(tp, shp->st.timetrap);
        if (tp->action) nv_close(tp->action);
        nv_unset(np);
        free(fp);
    }
}
...
int b_alarm(int argc, char *argv[], Shbltin_t *context) {
    ...
    argc -= opt_info.index;
    argv += opt_info.index;
    if (error_info.errors) errormsg(SH_DICT, ERROR_usage(2), optusage((char *)0));
    if (argc == 0) {
