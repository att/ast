...
static int fdclose(Service_t *sp, int fd) {
    ...
    for (i = 0; i < npoll; i++) {
        if (file_list[i] == fd) {
            ...
            if (sp->actionf) (*sp->actionf)(sp, fd, 1);
            return 1;
        }
    }
    return 0;
...
static int fdnotify(int fd1, int fd2) {
    Service_t *sp;
    if (covered_fdnotify) (*covered_fdnotify)(fd1, fd2);
    if (fd2 != SH_FDCLOSE) {
        int i;
...
static void process_stream(Sfio_t *iop) {
    ...
    if (fd == sp->fd) {
        ...
        file_list[npoll++] = fd;
        if (fd >= 0) {
            if (sp->acceptf) r = (*sp->acceptf)(sp, fd);
        }
    } else if (sp->actionf) {
        service_list[fd] = 0;
...
static int waitnotify(int fd, long timeout, int rw) {
    ...
    if (fd >= 0) special = sh_fd2sfio(shp, fd);
    while (1) {
        pstream = poll_list;
        while (ready < nready) process_stream(pstream[ready++]);
        if (special) *pstream++ = special;
        for (i = 0; i < npoll; i++) {
            if (service_list[file_list[i]]) *pstream++ = sh_fd2sfio(shp, file_list[i]);
        }
#if 1
        for (i = 0; i < pstream - poll_list; i++) sfset(poll_list[i], SF_WRITE, 0);
#endif
        nready = ready = 0;
        errno = 0;
#ifdef DEBUG
        sfprintf(sfstderr, "before poll npoll=%d", pstream - poll_list);
        for (i = 0; i < pstream - poll_list; i++) sfprintf(sfstderr, " %d", sffileno(poll_list[i]));
        sfputc(sfstderr, '\n');
#endif
        nready = sfpoll(poll_list, pstream - poll_list, timeout);
#ifdef DEBUG
        sfprintf(sfstderr, "after poll nready=%d", nready);
        for (i = 0; i < nready; i++) sfprintf(sfstderr, " %d", sffileno(poll_list[i]));
        sfputc(sfstderr, '\n');
#endif
#if 1
        for (i = 0; i < pstream - poll_list; i++) sfset(poll_list[i], SF_WRITE, 1);
#endif
        if (nready <= 0) return (errno ? -1 : 0);
...
int b_mkservice(int argc, char **argv, Shbltin_t *context) {
    ...
    sp = newof(0, Service_t, 1, 0);
    if (!sp) error(ERROR_exit(1), "out of space");
    sp->acceptf = Accept;
    sp->actionf = Action;
    sp->sh = shp;
    sp->errorf = Error;
    ...
    fd = sh_open(path, O_SERVICE | O_RDWR);
    if (fd <= 0) {
        free(sp);
        error(ERROR_exit(1), "%s: cannot start service", path);
    }
    sp->fd = fcntl(fd, F_DUPFD, 10);
    if (sp->fd >= 10) {
        close(fd);
    } else {
        sp->fd = fd;
...
int b_eloop(int argc, char **argv, Shbltin_t *context) {
    ...
    argv += opt_info.index;
    if (error_info.errors || *argv) error(ERROR_usage(2), optusage(NULL));
    while (1) {
        if (waitnotify(-1, timeout, 0) == 0) break;
