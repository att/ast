...
static Xl_t *translation(Xl_t *xp, register char *s) {
    register Xl_t *px;
    register char *t;
    char *d;
    char *e;
...
static int ccsfprintf(int from, int to, Sfio_t *sp, const char *format, ...) {
    va_list ap;
    Sfio_t *tp;
    char *s;
    int n;
...
int main(int argc, char **argv) {
    ...
    argv += opt_info.index;
    if (error_info.errors || !(catfile = *argv++)) error(ERROR_USAGE | 4, "%s", optusage(NULL));
    ...
    if (set) {
        ...
        if (!(mc = mcopen(sp))) error(ERROR_SYSTEM | 3, "%s: catalog content error", catfile);
        sfclose(sp);
        if (format) {
    ...
    if (!(tp = sfstropen())) error(ERROR_SYSTEM | 3, "out of space [string stream]");
        ...
        if (*s == '$') {
            ...
            if (streq(t, "delset")) {
                ...
                if (num < mc->num && mc->set[num].num)
                    for (i = 1; i <= mc->set[num].num; i++) mcput(mc, num, i, NULL);
            ...
            if (e != s) {
                ...
                if (!*s) {
                    ...
                    if (t > (s + 1) && *(t -= 2) == '\\') {
                        ...
                        if (!(s = sfstruse(tp))) error(ERROR_SYSTEM | 3, "out of space");
                    }
                    if (q) {
                        if (*s++ != q) {
                    ...
                    if (mcput(mc, set, num, s)) error(2, "(%d,%d): cannot add message", set, num);
                } else
    ...
    if (xp) {
        ...
        for (;;) {
            for (bp = 0, px = xp; px; px = px->next)
                if (px->date && (!bp || strcoll(bp->date, px->date) < 0)) bp = px;
            if (!bp) break;
            sfprintf(tp, "%s%s %s", t, bp->name, bp->date);
            t = ", ";
            bp->date = 0;
        }
        if (!(mc->translation = sfstruse(tp))) error(ERROR_SYSTEM | 3, "out of space");
    }
