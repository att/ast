...
static void init(void) {
    ...
    if (!(state.con = newof(0, Connection_t, state.fdtotal, 0))) error(3, "out of space [con]");
...
int main(int argc, char **argv) {
    ...
    while ((s = sfgetr(sfstdin, '\n', 0)) && sfvalue(sfstdin) > 1)
        switch (s[sfvalue(sfstdin) - 1] = 0, op = *s == ':' ? (s++, *s++) : '?') {
            case '#':
                break;
            case '?':
            case ':':
                ...
                break;
            case '=':
                if (!search(SET, s, NULL, NULL)) error(2, "%s: invalid host name", s);
                break;
            default:
