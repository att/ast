    ...
    (((p)->mode & SHELL_DISABLE)                                       \
         ...
         : ((p)->flags & IGN)                                          \
               ...
               : ((((p)->stat.load / (p)->scale) +                     \
                  ...
                  (p)->bias * (((p) == state.home && (p)->cpu == 1) ? 400 : 100) / (p)->rating))
...
static char *dupstring(char *s, const char *v) {
    if (v) {
        if (!(s = newof(s, char, strlen(v) + 1, 0))) error(3, "out of space [dupstring]");
        strcpy(s, v);
    } else if (s) {
        free(s);
        s = 0;
...
Coshell_t *search(int op, char *name, register Coattr_t *a, Coattr_t *d) {
    ...
    if (!(op & JOB)) {
        if (a->global.set) {
            ...
            if (a->global.set & SETBUSY) state.busy = a->global.busy;
            if (a->global.set & SETDISABLE) state.disable = a->global.disable;
            if (a->global.set & SETGRACE) state.grace = a->global.grace;
            if (a->global.set & SETIDENTIFY)
                state.identify = dupstring(state.identify, a->global.identify);
            if (a->global.set & SETMAXIDLE) state.maxidle = a->global.maxidle;
            if (a->global.set & SETMAXLOAD) state.maxload = a->global.maxload;
            if (a->global.set & SETMIGRATE)
                state.migrate = dupstring(state.migrate, a->global.migrate);
            if (a->global.set & SETPERCPU) state.percpu = a->global.percpu;
            if (a->global.set & SETPERHOST) state.perhost = a->global.perhost;
            if (a->global.set & SETPERSERVER) state.perserver = a->global.perserver;
            if (a->global.set & SETPERUSER) state.peruser = a->global.peruser;
            if (a->global.set & SETPOOL) state.pool = a->global.pool;
            ...
            if (a->global.set & SETSCHEDULE) {
                ...
                if (state.scheduler.fd > 0) close(state.scheduler.fd);
                if ((state.scheduler.fd = csopen(name, 0)) < 0 && *name != '/') {
                    sfprintf(state.string, "/dev/tcp/share/%s/trust", name);
                    if (!(name = sfstruse(state.string))) error(3, "out of space");
                    state.scheduler.fd = csopen(name, 0);
                }
                if (state.scheduler.fd > 0)
                    state.scheduler.name = dupstring(state.scheduler.name, name);
                else
            ...
            if (a->global.set & SETFILE) return (info(op, a->global.file));
            if (a->global.set &
                (SETBUSY | SETMAXIDLE | SETPERCPU | SETPERHOST | SETPERSERVER | SETPERUSER))
                jobcheck(NULL);
        }
        if ((op & (DEF | NEW)) && a->set && !(a->set & SETNAME)) return (info(op, NULL));
    }
    ...
    if (op & JOB) {
        ...
        if (cs.time - (unsigned long)tt > LOST) {
            tt = cs.time;
            state.tm = tmmake(&tt);
            if (!state.tm->tm_wday) state.tm->tm_wday = 7;
        }
        ...
        do {
            if ((matched = match(ap, a, op)) && ap->access && !ap->home &&
                ...
                ap->mode &= ~SHELL_DENIED;
            message((-6, "search: %s name=%s misc=%s matched=%d", ap->name,
                     (a->set & SETNAME) ? a->name : "*",
                     ((a->set | op) & (SETMISC | DEF | NEW | SET)) == SETMISC ? a->misc : "*",
                     matched));
            ...
            if (ap->fd) {
                ...
                if (ap->fd > 0) {
                    if (!ap->home &&
                        (!xp || ap->temp < PCT(xp->temp, H_TEMP) || (ap->mode & SHELL_DENIED) ||
                         matched && xm && PCT(ap->temp, H_TEMP) < xp->temp &&
                             ap->rank > xp->rank)) {
                        xp = ap;
                        xm = matched;
                    }
                ...
                if (ap != state.shell &&
                    (ap->override || !IDLE(ap->stat.idle, ap->idle) &&
                                         (!ap->bypass || !(bypass = miscmatch(ap, ap->bypass))))) {
            ...
            if (matched) {
                ...
                if (ap->fd > 0) {
                    v = RNK(ap, a);
                    if (v < sv &&
                        ap->running < (state.perhost ? state.perhost : ap->cpu * state.percpu) &&
                        (!state.maxload || (ap->stat.load / ap->scale) < state.maxload) &&
        ...
        message((-4, "open=%d override=%d sp=%s mp=%s xp=%s dp=%s dt=%s", nopen, noverride,
                 sp ? sp->name : "*", mp ? mp->name : "*", xp ? xp->name : "*", dp ? dp->name : "*",
                 dp && dt > cs.time ? fmtelapsed(dt - cs.time, 1) : "*"));
        if (xp && xp != sp) {
            if (!xp->running &&
                (nopen - xp->cpu >= state.pool + noverride ||
                 state.open - xp->cpu >= state.fdtotal / 2 ||
                 xp == dp && cs.time > dt && (!xm || !mp || mp->rank < PCT(xp->rank, H_RANK)))) {
                dp = 0;
                xp->mode &= ~SHELL_CLOSE;
                shellclose(xp, -1);
            } else if (xp != dp) {
...
empty:
    ...
    if (!(sp = newof(0, Coshell_t, 1, 0))) error(3, "out of space [%s]", a->name);
...
found:
    ...
    if (!sp->update && !(op & GET) ||
        (a->set & (SETBIAS | SETCPU | SETIDLE | SETIGNORE | SETRATING | SETSCALE)) &&
...
void update(register Coshell_t *sp) {
    register long n;
    ...
    if (csstat(sp->name, &sp->stat)) {
        ...
        if (sp->stat.up < 0) {
            ...
            if (!sp->rating) sp->rating = 1;
            n += W_LOAD * (((sp->stat.load / sp->scale) + CPU(sp->cpu, W_JOB * LOAD)) / T_LOAD) *
                 sp->bias / sp->rating;
            if (state.maxidle) {
                if (sp->idle) {
            ...
            if (n > PCT(X_RANK, C_RANK)) {
                if (n >= 2 * X_RANK)
                    ...
                    n = PCT(X_RANK, C_RANK) + (n - PCT(X_RANK, C_RANK)) /
        ...
        message((-4, "%s: %s=%s idle=%s load=%s users=%d rank=%s", sp->name,
                 sp->stat.up < 0 ? "down" : "up",
                 fmtelapsed(sp->stat.up < 0 ? -sp->stat.up : sp->stat.up, 1),
                 fmtelapsed(sp->stat.idle, 1), fmtfloat(sp->stat.load), sp->stat.users,
                 fmtfloat(sp->rank)));
    }
}
...
Coshell_t *info(int op, char *file) {
    register Sfio_t *fp;
    register char *s;
    struct stat st;
    ...
    if (!file && op == SET) {
        if (!(file = apath)) {
            if (!(file = sfstrrsrv(state.string, PATH_MAX)) ||
                !pathaccess(csvar(CS_VAR_TRUST, 1), csvar(CS_VAR_SHARE, 0), CS_SVC_ACCESS,
                            PATH_REGULAR, file, PATH_MAX) ||
                !(file = strdup(file))) {
    ...
    if ((op & NEW) && (sfset(fp, 0, 0) & SF_STRING)) op &= ~DEF;
    while (s = sfgetr(fp, '\n', 1)) search(op, s, NULL, NULL);
    sfclose(fp);
    return (state.shell);
}
...
int byname(const char *a, const char *b) {
...
int byrank(const char *a, const char *b) {
...
int bytemp(const char *a, const char *b) {
