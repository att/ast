...
#define STAT1(p)                                        \
    do {                                                \
        if ((p)->update <= cs.time + UPDATE) update(p); \
    } while (0)
...
static void copystring(char *s, int m, const char *v, int n) {
    if (n <= 0) n = strlen(v);
    if (n <= 0 || n == 1 && *v == '-') {
        n = 1;
        s[0] = '*';
...
static char *savestring(String_t *p, const char *v, int n) {
    if (n <= 0 || n == 1 && *v == '-') return (0);
    if (n >= p->size) {
        p->size = roundof(n + 1, 32);
        if (!(p->data = newof(p->data, char, p->size, 0))) error(3, "out of space [savestring]");
    }
    memcpy(p->data, v, n);
    p->data[n] = 0;
    return (p->data);
...
static int miscop(unsigned long n1, int op, unsigned long n2) {
    switch (op) {
        case EQ:
            return (n1 == n2);
        case NE:
...
static int misccmp(char *vp, int op, char *sb, char *se) {
    int c;
    int r;
...
static long miscget(Coshell_t *sp, const char *as, int set, char **e, char *rep) {
        ...
        if (!set) {
            ...
            if (op) {
                ...
                if ((c = *cp) == '"' || c == '\'') {
                    ...
                    if (*se == '@') {
                        ...
                        c = *cp;
                        *cp = 0;
                        tsp = search(GET, se, NULL, NULL);
                        *cp = c;
                        ee = cp;
                        if (tsp) {
                            c = *st;
                            *st = 0;
                            if ((st - sb) == 4 && (*sb == 'n' && !strncmp(sb, "name", 4) ||
                                                   *sb == 't' && !strncmp(sb, "type", 4))) {
                                sb = *sb == 'n' ? tsp->name : tsp->type;
                                se = sb + strlen(sb);
                            } else if (miscget(tsp, sb, 1, &sb, NULL))
            ...
            switch (x) {
                case HASHKEY4('b', 'i', 'a', 's'):
                    return (osp ? osp->bias == sp->bias
                                : op ? miscop(sp->bias, op, strton(cp, e, NULL, 100)) : sp->bias);
                case HASHKEY3('c', 'p', 'u'):
                ...
                case HASHKEY4('i', 'd', 'l', 'e'):
                    STATS(sp, osp);
                    return (osp ? osp->stat.idle == sp->stat.idle
                ...
                case HASHKEY4('l', 'o', 'a', 'd'):
                    STATS(sp, osp);
                    return (
                        osp ? (osp->stat.load / osp->scale) == (sp->stat.load / sp->scale)
                            : op ? miscop(sp->stat.load / sp->scale, op, strton(cp, e, NULL, 100))
                                 : sp->stat.load / sp->scale);
                case HASHKEY3('m', 'i', 'n'):
                    return (state.tm->tm_min);
                case HASHKEY6('m', 'i', 'n', 'i', 'd', 'l'):
                    return (osp ? osp->idle == sp->idle
                                : op ? miscop(sp->idle, op, strelapsed(cp, e, 1)) : sp->idle);
                ...
                case HASHKEY6('r', 'a', 't', 'i', 'n', 'g'):
                    return (osp ? osp->rating == sp->rating
                                : op ? miscop(sp->rating, op, strton(cp, e, NULL, 100))
                                     : sp->rating);
        ...
        for (;;) {
            ...
            while (*np++ == *cp++)
                if (np >= s) {
                    ...
                    if (*cp++ == '=') {
                        ...
                        if (osp) {
                            if (!miscget(osp, bp, 1, &bp, NULL)) return (0);
                            for (np = cp; *np && *np != ' '; np++)
                                ;
                            return (!strncmp(bp, cp, np - cp));
                        }
                        set = strton(cp, &bp, NULL, 0);
                        if (*bp && *bp != ' ') set = 1;
                        return (set);
                    }
                    break;
            ...
            for (;;) {
                if (!*cp) {
                    ...
                    if (osp) return (!miscget(osp, bp, 1, &bp, NULL));
                    return (0);
                }
                if (*cp++ == ' ') break;
            }
...
void attributes(register char *s, register Coattr_t *p, Coattr_t *d) {
    register char *b;
    register char *v;
    char *e;
    char *r;
    ...
    if (s) {
        ...
        for (;;) {
            ...
            switch (x) {
                ...
                case HASHKEY4('b', 'i', 'a', 's'):
                    p->set |= SETBIAS;
                    p->bias = sp ? sp->bias : strton(v, NULL, NULL, 100);
                    continue;
                case HASHKEY4('b', 'u', 's', 'y'):
                    if (!sp) {
                        p->global.set |= SETBUSY;
                        p->global.busy = strelapsed(v, NULL, 1);
                    }
                    continue;
                case HASHKEY6('b', 'y', 'p', 'a', 's', 's'):
                    ...
                    continue;
                case HASHKEY3('c', 'p', 'u'):
                    if (p->cpu = sp ? sp->cpu : (int)strtol(v, NULL, 0)) p->set |= SETCPU;
                    continue;
                case HASHKEY5('d', 'e', 'b', 'u', 'g'):
                    if (!sp) {
                        p->global.set |= SETDEBUG;
                        p->global.debug = -strton(v, NULL, NULL, 0);
                    }
                    continue;
                case HASHKEY6('d', 'i', 's', 'a', 'b', 'l'):
                    if (!sp) {
                        p->global.set |= SETDISABLE;
                        p->global.disable = strelapsed(v, NULL, 1);
                    }
                    continue;
                ...
                case HASHKEY5('g', 'r', 'a', 'c', 'e'):
                    if (!sp) {
                        ...
                        p->global.grace = strelapsed(v, NULL, 1);
                    }
                    continue;
                ...
                case HASHKEY6('m', 'i', 'n', 'i', 'd', 'l'):
                    p->set |= SETIDLE;
                    p->idle = sp ? sp->idle : strelapsed(v, NULL, 1);
                    continue;
                case HASHKEY6('i', 'g', 'n', 'o', 'r', 'e'):
                    p->set |= SETIGNORE;
                    p->ignore = sp ? (sp->flags & IGN) : (int)strtol(v, NULL, 0);
                    continue;
                ...
                case HASHKEY6('m', 'a', 'x', 'i', 'd', 'l'):
                    if (!sp) {
                        p->global.maxidle = (int)strtol(v, NULL, 0);
                        p->global.set |= SETMAXIDLE;
                    }
                    continue;
                case HASHKEY6('m', 'a', 'x', 'l', 'o', 'a'):
                    if (!sp && (p->global.maxload = strton(v, NULL, NULL, 100)) >= 0)
                ...
                case HASHKEY6('p', 'e', 'r', 'c', 'p', 'u'):
                    if (!sp && (p->global.percpu = (int)strtol(v, NULL, 0)) > 0) {
                ...
                case HASHKEY6('p', 'e', 'r', 'h', 'o', 's'):
                    if (!sp && (p->global.perhost = (int)strtol(v, NULL, 0)) > 0) {
                ...
                case HASHKEY6('p', 'e', 'r', 's', 'e', 'r'):
                    if (!sp && (p->global.perserver = (int)strtol(v, NULL, 0)) >= 0) {
                ...
                case HASHKEY6('p', 'e', 'r', 'u', 's', 'e'):
                    if (!sp && (p->global.peruser = (int)strtol(v, NULL, 0)) > 0) {
                ...
                case HASHKEY4('p', 'o', 'o', 'l'):
                    if (!sp && (p->global.pool = (int)strtol(v, NULL, 0)) > 0)
                ...
                case HASHKEY6('p', 'r', 'o', 'f', 'i', 'l'):
                    ...
                    continue;
                case HASHKEY6('r', 'a', 't', 'i', 'n', 'g'):
                    if (p->rating = sp ? sp->rating : strton(v, NULL, NULL, 100))
                        p->set |= SETRATING;
                    continue;
                ...
                case HASHKEY5('s', 'c', 'a', 'l', 'e'):
                    if (p->scale = sp ? sp->scale : (int)strtol(v, NULL, 0)) p->set |= SETSCALE;
                    continue;
                ...
                case HASHKEY4('s', 't', 'a', 't'):
                    if ((v - b) > 6 && *(b + 4) == '.') {
                        if ((--v - (b += HASHKEYMAX - 1)) > HASHKEYMAX) v = b + HASHKEYMAX;
                        while (b < v) {
                            if (islower(q = *b++))
                                x = HASHKEYPART(x, q);
                            else {
                                if (isalnum(q) || q == '_') x = 0;
                                break;
                            }
                        }
                        switch (x) {
                            case HASHKEY4('i', 'd', 'l', 'e'):
                                ...
                                p->stat.idle = strelapsed(v, NULL, 1);
                                break;
                            case HASHKEY4('l', 'o', 'a', 'd'):
                                p->global.set |= SETLOAD;
                                p->stat.load = strton(v, NULL, NULL, 100);
                                break;
                            case HASHKEY6('u', 'p', 'd', 'a', 't', 'e'):
                                p->global.set |= SETUPDATE;
                                p->stat.up = cs.time + strelapsed(v, NULL, 1);
                                break;
                            case HASHKEY5('u', 's', 'e', 'r', 's'):
                                p->global.set |= SETUSERS;
                                p->stat.users = strton(v, NULL, NULL, 0);
                                break;
                        }
                        continue;
                    }
...
void miscadd(Coshell_t *sp, register char *s) {
    char *tp;
...
static long misceval(const char *s, char **e, void *handle) {
    if (!s) {
        message((-3, "attribute: %s", *e));
        return (0);
    }
    return (miscget((Coshell_t *)handle, s, 0, e, NULL));
}
...
int miscmatch(Coshell_t *sp, char *p) { return (strexpr(p, NULL, misceval, sp) != 0); }
