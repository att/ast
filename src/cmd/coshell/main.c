...
static void *init(void *handle, int fdmax) {
    register int n;
    register char *s;
    ...
    if ((n = getgroups(0, NULL)) <= 0) n = getconf("NGROUPS_MAX");
    if (!(state.gids = newof(0, gid_t, n + 2, 0))) error(3, "out of space [gids]");
    if ((n = getgroups(n, state.gids + 1)) < 0) n = 0;
    state.gids[n + 1] = -1;
    state.gids[0] = getegid();
    state.uid = geteuid();
    n = state.fdtotal / 2;
    if (!(state.job = state.jobnext = newof(0, Cojob_t, n, 0))) error(3, "out of space [job]");
    ...
    state.profile =
        strdup("{ . ./.profile; eval test -f \\$ENV \\&\\& . \\$ENV; } >/dev/null 2>&1 </dev/null");
    ...
    if (state.perserver <= 0 || state.perserver > n) state.perserver = n;
    if (state.peruser <= 0 && (state.peruser = (3 * (state.pool - 1)) / 2) <= 0) state.peruser = 1;
    if (state.percpu <= 0 && (state.percpu = state.peruser / 4) <= 2) state.percpu = 2;
    ...
    if (!(s = state.sh)) {
        s = state.buf;
        for (n = 0;;) {
            if (pathpath(coshell[n], NULL, PATH_ABSOLUTE | PATH_REGULAR | PATH_EXECUTE, s,
                         state.buflen))
                break;
            if (++n >= elementsof(coshell)) error(3, "shell not found");
        }
    }
    ...
    if (!(state.sh = strdup(s))) error(3, "out of space [shell path]");
    ...
    if (!(state.remote = strdup(CS_REMOTE_SHELL))) error(3, "out of space [remote shell path]");
...
static int nonblocking(int fd) {
    ...
    if ((flags = fcntl(fd, F_GETFL)) < 0) return flags;
...
static int service(void *handle, register int fd) {
    register int n;
    register int i;
    char *s;
    char *t;
    ...
    switch (state.con[fd].type) {
        ...
        case SHELL:
            ...
            if (sp && sp->running) jobcheck(sp);
            break;
        case DEST:
            if (csread(fd, s = state.buf, 13, CS_EXACT) != 13 || s[0] != '#' ||
                (jp = state.job + (int)strtol(s + 1, NULL, 10)) > state.jobmax ||
                fstat(state.con[fd].info.pass.fd = (int)strtol(s + 7, NULL, 10), &st))
                drop(fd);
            else {
        ...
        case INIT:
            if (csread(fd, s = cmd, 7, CS_EXACT) != 7 || s[0] != '#' ||
                (i = (int)strtol(s + 1, NULL, 10)) &&
                    (i < 0 || i > state.buflen || csread(fd, state.buf, i, CS_EXACT) != i)) {
                drop(fd);
                break;
            }
                    ...
                    if (strneq(s, CO_ENV_ATTRIBUTES, sizeof(CO_ENV_ATTRIBUTES) - 1) &&
                        ...
                        if (streq(s, CO_OPT_COMMAND))
                            ...
                            state.con[fd].info.user.flags |= USER_DUP;
                                 ...
                                 (sp = search(GET, s + sizeof(CO_OPT_HOME), NULL, NULL)))
                            (state.con[fd].info.user.home = sp)->home++;
                        else if (strneq(s, CO_OPT_INDIRECT, sizeof(CO_OPT_INDIRECT) - 1) &&
            ...
            if (state.indirect.con &&
                (state.con[fd].info.user.flags & (USER_IDENT | USER_INIT)) == USER_IDENT) {
            ...
            if (state.indirect.con) {
                if (!state.con[fd].info.user.pump ||
                    (state.con[fd].info.user.fds[1] = csopen(state.con[fd].info.user.pump, 0)) <
                        0 ||
                ...
                if (state.con[fd].info.user.flags & USER_DUP)
                    state.con[fd].info.user.fds[2] = state.con[fd].info.user.fds[1];
                              ...
                              csopen(state.con[fd].info.user.pump, 0)) < 0 ||
                    ...
                    state.con[state.con[fd].info.user.fds[2]].type = UERR;
                       ...
                       !fstat(state.con[fd].info.user.fds[2], &ts) && st.st_ino == ts.st_ino &&
                       st.st_dev == ts.st_dev) {
            ...
            if (state.con[fd].info.user.flags & USER_IDENT) {
                s = state.buf;
                s += sfsprintf(s, state.buflen - (s - state.buf), "%s,%s", CO_OPT_SERVER,
                               CO_OPT_ACK);
                if (state.indirect.con)
                    s += sfsprintf(s, state.buflen - (s - state.buf), ",%s", CO_OPT_INDIRECT);
        ...
        case PUMP:
            ...
            break;
        case SCHED:
            if ((i = read(fd, state.buf, state.buflen)) <= 0) drop(fd);
            /*HERE*/
            break;
        case USER:
            if (csread(fd, cmd, 7, CS_EXACT) != 7 || cmd[0] != '#' ||
                (n = (int)strtol(cmd + 1, NULL, 10)) <= 0) {
                drop(fd);
                break;
            }
            if (n > state.buflen) {
            ...
            state.cmds++;
            n = error_info.errors;
            if (!(state.home = state.con[fd].info.user.home)) state.home = state.shell;
            switch (i = *state.buf) {
                case 'e':
                case 'E':
...
static int pump(void *handle, register int fd) {
    register int n;
    register int pd;
    ...
    if (!(pd = *pass)) {
        if ((n -= 7) < 0 || s[0] != '#' || (pd = (int)strtol(s + 1, NULL, 10)) < 1 || pd > 2)
            goto drop;
...
int main(int argc, char **argv) {
    ...
    opt_info.argv = argv;
    error_info.id = CO_ID;
    if (pathcheck(PATHCHECK, error_info.id, &state.check)) exit(1);
    state.version = strdup(fmtident(usage));
    error(-1, "%s", state.version);
    if (!(state.string = sfstropen())) error(3, "out of space [string]");
    state.buflen = 8 * CHUNK;
    if (!(state.buf = newof(0, char, state.buflen, 0))) error(3, "out of space [buf]");
    ...
    if (!(state.service = strdup(s))) error(3, "out of space [service]");
    ...
    if ((s = *argv) && strneq(s, "/dev/fd/", 8)) {
        ...
        if (i = (int)strtol(s + 8, NULL, 0)) {
            close(0);
            if (dup(i)) error(ERROR_SYSTEM | 3, "%s: cannot open", s);
        }
    }
    ...
    if ((s = strrchr(*opt_info.argv, '.')) && streq(s + 1, CS_SVC_SUFFIX)) {
        ...
        error(ERROR_USAGE | 4, "[connect-stream] [-hjqQs[aelpst]] [-r host [cmd]] | + [info]");
    if (n != SERVER) {
        if ((fd = csopen(state.service, CS_OPEN_TEST)) < 0) {
            if (errno == ENOENT)
                error(3, "%s: server not running", state.service);
            else
                error(ERROR_SYSTEM | 3, "%s: cannot open connect stream", state.service);
        }
        if (!state.indirect.con) {
            ...
            if (n == COMMAND)
                i--;
                     ...
                     (state.indirect.msg = (int)strtol(s, &t, 0)) <= 0 || *t)
                state.indirect.msg = i - 1;
            fds[i - 1] = state.indirect.msg;
        }
        errno = EINVAL;
        s = state.buf;
        if ((state.indirect.con || !cssend(fd, fds, i)) && csread(fd, s, 7, CS_EXACT) == 7 &&
            s[0] == '#' && !(errno = (int)strtol(s + 1, NULL, 10)))
            do {
                if (state.indirect.con) {
                    ...
                    exit(0);
                s += 7;
                s += sfsprintf(s, state.buflen - (s - state.buf), "%s=label=%s", CO_ENV_ATTRIBUTES,
                               CO_ID);
                if ((t = getenv(CO_ENV_ATTRIBUTES)) && *t)
                    s += sfsprintf(s, state.buflen - (s - state.buf), ",%s", t);
                if (state.indirect.con) {
                    struct stat st;
                    struct stat ts;
                    ...
                    s += sfsprintf(s, state.buflen - (s - state.buf), "\n%s=%s", CO_OPT_HOME,
                                   csname(0));
                    if (!fstat(1, &st) && !fstat(2, &ts) && st.st_ino == ts.st_ino &&
                        st.st_dev == ts.st_dev) {
                        s += sfsprintf(s, state.buflen - (s - state.buf), "\n%s", CO_OPT_DUP);
                        d = 1;
                    } else
                ...
                if (n == COMMAND) {
                    if (state.indirect.con) {
                        for (; d > 0 && csrecv(pfd, NULL, fds, 1) == 1 &&
                               csread(fds[0], s, 7, CS_EXACT) == 7 && s[0] == '#';
                             d--)
                            if ((n = (int)strtol(s + 1, NULL, 10)) == 1)
