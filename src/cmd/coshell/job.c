...
void drop(register int fd) {
    ...
    switch (state.con[fd].type) {
        ...
        case INIT:
            ...
            if (sp = state.con[fd].info.user.home) sp->home--;
            if (state.con[fd].info.user.pump) free(state.con[fd].info.user.pump);
            if (state.con[fd].info.user.expr) free(state.con[fd].info.user.expr);
            break;
        case PASS:
            ...
            if (jp->pid > 0) {
                    ...
                    if (sp->stat.idle < state.busy && (!sp->bypass || !miscmatch(sp, sp->bypass))) {
                        if (!jp->busy && state.grace)
                            ...
                            if (state.migrate) {
                                ...
                                error(ERROR_OUTPUT | 2, state.con[jp->fd].info.user.fds[2],
                                      "%s: job=%d pid=%d %s", sp->name, jp - state.job, jp->pid,
                                      state.migrate);
                                n = sfprintf(state.string, "job=%d; pid=%d; host=%s; type=%s; %s\n",
                                             jp - state.job, jp->pid, sp->name, sp->type,
                                             state.migrate);
...
#ifdef SIGSTOP
                                error(ERROR_OUTPUT | 2, state.con[jp->fd].info.user.fds[2],
                                      "%s: job=%d pid=%d stopped", sp->name, jp - state.job,
                                      jp->pid);
                                jobkill(jp, SIGSTOP);
#else
                                error(ERROR_OUTPUT | 2, state.con[jp->fd].info.user.fds[2],
                                      "%s: job=%d pid=%d is a hog", sp->name, jp - state.job,
                                      jp->pid);
#endif
                                jp->busy = HOG;
                            }
...
#ifdef SIGCONT
                    error(ERROR_OUTPUT | 2, state.con[jp->fd].info.user.fds[2],
                          "job %d pid %d restarted on %s", jp - state.job, jp->pid, sp->name);
                    jobkill(jp, SIGCONT);
#else
                    error(ERROR_OUTPUT | 2, state.con[jp->fd].info.user.fds[2],
                          "job %d pid %d is no longer hogging %s", jp - state.job, jp->pid,
                          sp->name);
                    jp->busy = 0;
#endif
                }
                ...
                if (jp->pid == QUEUE && (sp->fd > 0 || sp == &state.wait)) {
                    if (state.running < (state.perserver + state.jobwait) &&
                        state.con[jp->fd].info.user.running < (state.peruser + 1) &&
                        (sp == &state.wait ||
                         sp->running <
                             ((state.perhost ? state.perhost : sp->cpu * state.percpu) + 1)))
                        shellexec(jp, jp->cmd, jp->fd);
                } else if (cs.time > jp->start + LOST) {
                    message((-4, "jobcheck: %s: possibly hung %s", sp->name,
...
void jobkill(Cojob_t *jp, int sig) {
    int n;
    char buf[128];
...
void jobdone(register Cojob_t *jp) {
    register int n;
    char buf[64];
    ...
    if (jp->fd > 0) {
        ...
        state.sys += jp->sys;
        if (state.disable &&
            (jp->sig == SIGKILL || jp->status > 128 && (jp->status % 128) == SIGKILL)) {
        ...
        n = sfsprintf(buf, sizeof(buf), "x %d %d %s %s\n", jp->rid, jp->status,
                      fmtelapsed(jp->user, CO_QUANT), fmtelapsed(jp->sys, CO_QUANT));
        if (cswrite(state.con[jp->fd].info.user.fds[0], buf, n) != n) drop(jp->fd);
    }
