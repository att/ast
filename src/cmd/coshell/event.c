...
static const Request_t requests[] = {
    "a*ll",   REQ_all,   1, 1,  "c*lear",  REQ_clear,   1, -1, "d*elete", REQ_delete, 1, -1,
    "e*xit",  REQ_exit,  0, 0,  "hold",    REQ_hold,    0, -1, "i*nfo",   REQ_info,   0, 0,
    "l*ist",  REQ_list,  0, 1,  "n*ext",   REQ_next,    0, 0,  "q*uit",   REQ_exit,   0, 0,
    "r*aise", REQ_raise, 1, -1, "release", REQ_release, 0, -1, "s*et",    REQ_set,    0, 0,
    "stop",   REQ_stop,  0, 0,  "t*est",   REQ_test,    1, 1,  "w*ait",   REQ_wait,   1, 1,
};
...
static void log(State_t *state, Connection_t *con, int type, const char *format, ...) {
    va_list ap;
    char *s;
    int n;

    va_start(ap, format);
    if (format) sfvprintf(state->tmp, format, ap);
    va_end(ap);
    if (type) {
        if (!(s = sfstruse(state->tmp))) error(ERROR_SYSTEM | 3, "out of space");
        if (type != 'I' && state->log && state->logf)
            sfprintf(state->logf, "%s (%03d) %c %s\n", fmttime("%K", time(NULL)), con ? con->fd : 0,
                     toupper(type), s);
        if (con && type != 'R' && type != 'S') {
            if (type != 'L' || !con->quiet) debug_printf(con->fd, "%c %s\n", toupper(type), s);
            if (type == 'W')
                con->code |= 1;
            else if (type == 'E')
                con->code |= 2;
...
static int acceptf(Css_t *css, Cssfd_t *fp, Csid_t *ip, char **av, Cssdisc_t *disc) {
    register State_t *state = (State_t *)disc;
    register Connection_t *con;

    if (!(con = newof(0, Connection_t, 1, 0))) return -1;
    fp->data = con;
    con->id = *ip;
    con->waiting = 0;
    con->fd = fp->fd;
...
static int notify(State_t *state, Event_t *ep) {
    Connection_t *cp;
    Waiting_t *wp;
    char *s;
    size_t n;
    ...
    for (cp = (Connection_t *)dtfirst(state->connections); cp;
        ...
        if (cp->waiting && (wp = (Waiting_t *)dtmatch(cp->waiting, &ep))) {
            if (wp->id >= 0) {
                ...
                if (!(s = sfstruse(state->usrf))) error(ERROR_SYSTEM | 3, "out of space");
                write(cp->fd, s, n);
            } else if (!cp->quiet)
...
static int post(State_t *state, Connection_t *con, const char *name, int id) {
    Event_t *ep;
    Waiting_t *wp;
    ...
    if (ep = dtmatch(state->events, name)) {
        if (dtmatch(con->waiting, &ep)) return 0;
...
static int info(State_t *state, Connection_t *con, Css_t *css) {
    Connection_t *cp;
    Waiting_t *wp;
    int n;

    log(state, con, 'I', "info server='%s' version=%d.%d host=%s pid=%d uid=%d gid=%d",
        fmtident(usage), EVENT_MAJOR, EVENT_MINOR, csname(css->state, 0), getpid(), geteuid(),
        getegid());
...
static int apply(State_t *state, Connection_t *con, int id, int index, datum key, datum val,
                 Data_t *dat) {
    Event_t *e;
    int n;
        ...
        case REQ_release:
            if (dat->flags & DATA_hold) {
                ...
                if (dat->raise) {
                    ...
                    if (e = (Event_t *)dtmatch(state->events, key.dptr)) notify(state, e);
...
static int request(State_t *state, Connection_t *con, int id, int index, char **a,
                   unsigned long older, unsigned long newer) {
    char *s;
    int i;
    Event_t *e;
    datum key;
    ...
    while (s = *a++)
        if (i = regcomp(&re, s, REG_SHELL | REG_AUGMENTED | REG_LEFT | REG_RIGHT)) {
            ...
            if (key.dsize >= sizeof(e->name)) s[(key.dsize = sizeof(e->name)) - 1] = 0;
            val = dbm_fetch(state->dbm, key);
            if (val.dptr) {
                if (val.dsize > sizeof(dat)) val.dsize = sizeof(dat);
                swapmem(state->swap, val.dptr, &dat, sizeof(dat));
            } else
                memset(&dat, 0, sizeof(dat));
            if (apply(state, con, id, index, key, val, &dat)) return -1;
        } else {
        rescan:
            for (key = dbm_firstkey(state->dbm); key.dptr; key = dbm_nextkey(state->dbm))
                if (EVENT(key.dptr) && !regexec(&re, key.dptr, 0, NULL, 0)) {
                    ...
                    if ((!older || dat.time < older) && (!newer || dat.time > newer)) {
                        if ((i = apply(state, con, id, index, key, val, &dat)) < 0) return -1;
                        if (i > 0) goto rescan;
                    }
                }
        }
...
static unsigned long date(State_t *state, Connection_t *con, const char *s) {
    unsigned long t;
    char *e;
    datum key;
    datum val;
...
static int actionf(register Css_t *css, register Cssfd_t *fp, Cssdisc_t *disc) {
    register State_t *state = (State_t *)disc;
    register Connection_t *con;
    char *s;
    char *t;
    ...
    switch (fp->status) {
        case CS_POLL_CLOSE:
            if (con = (Connection_t *)fp->data) dtdelete(state->connections, con);
            return 0;
        case CS_POLL_READ:
            ...
            if (tokscan(state->req, NULL, " %v ", state->cmd, elementsof(state->cmd) - 1) > 0) {
                ...
                s = *(a = q);
                if (!(r = (Request_t *)strpsearch(requests, elementsof(requests),
                                                  sizeof(requests[0]), s, NULL)))
                        ...
                        switch (optget(a, usage)) {
                            case 'e':
                                if (r->index == REQ_set) {
                                    ...
                                    if (*t) {
                                        log(state, con, 'E', "%s: invalid elapsed time expression",
                                            opt_info.arg);
                                        err = 1;
                                        break;
                                    }
                                ...
                                continue;
                            case 'l':
                                if (r->index == REQ_set) state->log = opt_info.num;
                                continue;
                            case 'n':
                    ...
                    if (!err) {
                        ...
                        if (r->min && n < r->min)
                            ...
                            log(state, con, 'E', "%s: at most %d argument%s expected", s, r->max,
                                r->max == 1 ? "" : "s");
                        else if (r->min == r->max && n != r->max)
                            log(state, con, 'E', "%s: %d argument%s expected", s, r->max,
                                r->max == 1 ? "" : "s");
                        else
                            switch (r->index) {
                                case REQ_all:
                                    ...
                                    if (x->waiting) {
                                        ...
                                        x->quiet = 1;
                                        a = state->cmd;
                                        for (w = (Waiting_t *)dtfirst(x->waiting); w;
                                             w = (Waiting_t *)dtnext(x->waiting, w)) {
                                            if (a >= &state->cmd[elementsof(state->cmd) - 1]) {
                                                *a = 0;
                                                if (request(state, x, -1, REQ_raise, a = state->cmd,
                                                            older, newer))
                                                    break;
                                            }
                                            *a++ = w->event->name;
                                        ...
                                        if (a > state->cmd) {
                                            *a = 0;
                                            request(state, x, -1, REQ_raise, state->cmd, older,
                                                    newer);
                                        }
                                        x->quiet = n;
                                    }
                                ...
                                case REQ_hold:
                                    ...
                                    } else if (request(state, con, id, r->index, a, older, newer))
                                        return -1;
                                    break;
                                case REQ_list:
                                    con->all = 1;
                                ...
                                case REQ_next:
                                    ...
                                    list:
                                        if (EVENT(con->list.dptr) &&
                                            (con->all ||
                                             !regexec(&con->re, con->list.dptr, 0, NULL, 0))) {
                                            val = dbm_fetch(state->dbm, con->list);
                                            if (val.dsize > sizeof(data)) val.dsize = sizeof(data);
                                            swapmem(state->swap, val.dptr, &data, val.dsize);
                                            if ((!con->older || data.time < con->older) &&
                                                (!con->newer || data.time > con->newer)) {
                                                log(state, con, 'I', "event %s %s %d%s%s",
                                                    con->list.dptr, fmttime("%K", data.time),
                                                    data.raise,
                                                    (data.flags & DATA_clear) ? " CLEAR" : "",
                                ...
                                case REQ_release:
                                    if (!*a) {
                                        ...
                                        while (key.dptr) {
                                            ...
                                            if (val.dsize > sizeof(data)) val.dsize = sizeof(data);
                                            swapmem(state->swap, val.dptr, &data, val.dsize);
                                            if (!(data.flags & (DATA_clear | DATA_hold)) &&
                ...
                if (sfstrtell(state->usrf)) {
                    ...
                    n = sfstrtell(state->usrf);
                    if (!(s = sfstruse(state->usrf))) error(ERROR_SYSTEM | 3, "out of space");
                    if (cswrite(css->state, fp->fd, s, n) != n) return -1;
                }
            }
            return 1;
    }
...
static int exceptf(Css_t *css, unsigned long op, unsigned long arg, Cssdisc_t *disc) {
    register State_t *state = (State_t *)disc;
...
static void confree(Dt_t *dt, void *obj, Dtdisc_t *disc) {
...
static void waitfree(Dt_t *dt, void *obj, Dtdisc_t *disc) {
...
static void db(register State_t *state) {
    datum key;
    datum val;
    Data_t data;
    uint32_t u4;

    if (!(state->dbm = dbm_open(state->path, O_RDWR | O_CREAT,
    ...
    if (val.dptr) {
        if (val.dsize != sizeof(data))
            error(3, "%s: invalid db -- data size %d, expected %d", state->path, val.dsize,
                  sizeof(data));
        memcpy(&data, val.dptr, val.dsize);
        if (memcmp(&data.expire, ident_name, sizeof(data.expire)))
            error(3, "%s: %s: invalid db -- ident mismatch, expected %s", state->path, ident_key,
                  ident_name);
        u4 = IDENT_SWAP;
...
int main(int argc, char **argv) {
    ...
    if (!(state.usrf = sfstropen()) || !(state.tmp = sfstropen())) error(3, "out of space [buf]");
    ...
    for (;;) {
        switch (optget(argv, usage)) {
            ...
            case 'e':
                ...
                if (*s) error(2, "%s: invalid elapsed time expression", opt_info.arg);
                continue;
            case 'i':
                server = 1;
                continue;
    ...
    if (!(state.service = strdup(sfstruse(state.usrf)))) error(3, "out of space [service]");
    ...
    if (server) {
        ...
        state.disc.flags =
            CSS_DAEMON | CSS_LOG | CSS_ERROR | CSS_INTERRUPT | CSS_WAKEUP | CSS_PRESERVE;
        ...
        if (!(css = cssopen(state.service, &state.disc))) return 1;
        umask(S_IWOTH);
