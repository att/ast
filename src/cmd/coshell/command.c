...
static int cmdsend(int fd, int c, register char *s, register int n, int *code) {
    int i;
    int m;
    int x;
    CSPOLL fds[5];
    ...
    if (cswrite(fd, s, n) != n) error(ERROR_SYSTEM | 3, "message write error");
    if (c == 'Q') return (0);
    s = cmd;
    ...
    if (state.indirect.con) {
        ...
        if (fds[n].fd = state.indirect.err) fds[n++].events = CS_POLL_READ;
        ...
        while (--i > n)
            if (fds[i].events) close(fds[i].fd);
    }
    return (1);
}
...
static void cmdkill(int fd, int sig) {
    int n;
    char buf[32];
    ...
    for (;;) {
        ...
        switch (c) {
            ...
            case 'R':
                ...
                sfprintf(state.string, "#%05d\ne %d %d %s %s %s", 0, 1, c == 'r' ? CO_SILENT : 0,
                         NULL, NULL, NULL);
                if (!att[0] || att[0] == '-' && !att[1])
                    sfprintf(state.string, " %s", NULL);
                else
                ...
                if (ap) {
                    while (s = *ap++) t += sfsprintf(t, state.buf + state.buflen - t, " %s", s);
                    ap--;
                }
...
static char *ffmtmode(int fd, int must) {
    char *p;
    struct stat st;
...
void server(int fd, int op, int sub, int arg, char *dat) {
    register char *s;
    register int n;
    ...
    switch (op) {
            ...
            if (dat) {
                ...
                while (s = tokread(t))
                    if (!search(SET, s, NULL, NULL))
                        error(ERROR_OUTPUT | 2, con[fd].info.user.fds[2], "%s: invalid host name",
                              s);
                tokclose(t);
            }
            break;
        case 'c':
            if (dat) {
                t = tokopen(dat, 1);
                while (s = tokread(t)) {
                    if (!(sp = search(GET, s, NULL, NULL)))
                        error(ERROR_OUTPUT | 2, con[fd].info.user.fds[2], "%s: not found", s);
                    else if (!sp->fd)
                        error(ERROR_OUTPUT | 2, con[fd].info.user.fds[2], "%s: host not open", s);
                    else
        ...
        case 'd':
            ...
            message((error_info.trace, "level=%d output=%s", -arg,
                     dat ? dat : cspath(state.indirect.con ? 2 : con[fd].info.user.fds[2], 0)));
            error_info.trace = arg;
            if (dat) {
                if ((n = open(dat, O_WRONLY | O_CREAT | O_TRUNC,
                    ...
                    switch (con[n].type) {
                        ...
                        case PASS:
                            if (jp = con[n].info.pass.job)
                                sfprintf(state.string, "%3d  pass  %-3d %s %d%s\n", n,
                                         con[n].info.pass.fd, jp->shell->name, jp->pid,
                                         con[n].info.pass.serialize ? " serialize" : "");
                            else
                                sfprintf(state.string, "%3d  pass  %-3d zombie\n", n,
                                         con[n].info.pass.fd);
                            break;
                        case POLL:
                        ...
                        case USER:
                            if (con[n].info.user.fds[0] == n)
                                sfprintf(state.string, "%3d  query %-3d %-3d %s\n", n,
                                         con[n].info.user.fds[1], con[n].info.user.fds[2],
                                         (sp = con[n].info.user.home) ? sp->name : "");
                            else
                                sfprintf(state.string, "%3d  user  %-3d %-3d %-3d %s\n", n,
                                         con[n].info.user.fds[0], con[n].info.user.fds[1],
                        ...
                        path:
                            if (v = ffmtmode(n, 1))
                                sfprintf(state.string, "%3d  %s  %s %s\n", n, t, v, cspath(n, 0));
                            else if (con[n].type)
        ...
        case 'g':
            ...
            if (state.identify) sfprintf(state.string, "  identify  %s\n", fmtesc(state.identify));
            if (state.migrate) sfprintf(state.string, "   migrate  %s\n", state.migrate);
            ...
            sfprintf(state.string, "    access  %-7s",
                     state.access < cs.time ? "EXPIRED" : fmtelapsed(state.access - cs.time, 1));
        ...
        case 'j':
            for (jp = state.job; jp <= state.jobmax; jp++)
                if (jp->pid) {
                    ...
                    sfprintf(state.string, "%3d%4d%4d%6s%7s%c%-16s %s%s%s", jp - state.job, jp->fd,
                             jp->rid, t, fmtelapsed(cs.time - jp->start, 1),
                             jp->ref <= 0 ? '*' : jp->lost ? (jp->lost > cs.time ? '?' : '!') : ' ',
                             jp->shell->name, v, *v ? " " : "", jp->label);
            ...
            if (arg == NOARG)
                ...
                switch (sub) {
                    ...
                    case '-':
                        ...
                        break;
                    default:
                        error(ERROR_OUTPUT | 2, con[fd].info.user.fds[2], "%c: invalid signal id",
                              sub);
                        break;
                }
            break;
        case 'l':
            ...
            sp = state.shell;
            if (dat && *dat)
                attributes(dat, &attr, NULL);
            else
                attr = con[fd].info.user.attr;
            do {
                if (match(sp, &attr, 0)) *sv++ = sp;
            ...
            if (n = sv - state.shellv) {
                *sv = 0;
                ...
                while (sp = *sv++) sfprintf(state.string, "%s\n", sp->name);
            }
            break;
        case 'o':
            if (dat) {
                t = tokopen(dat, 1);
                while (s = tokread(t)) {
                    if (!(sp = search(NEW, s, NULL, NULL)))
                        error(ERROR_OUTPUT | 2, con[fd].info.user.fds[2], "%s: invalid host name",
                              s);
                    else if (sp->fd)
                        error(ERROR_OUTPUT | 2, con[fd].info.user.fds[2], "%s: host already open",
                              s);
                    else
        ...
        case 's':
            ...
            do {
                if (sub == 'a' || sub == 'e' || sp->fd ||
                    (sub == 'l' || sub == 's') && sp->temp && !(sp->flags & IGN))
                    *sv++ = sp;
            } while ((sp = sp->next) != state.shell);
            if (n = sv - state.shellv) {
                *sv = 0;
                strsort(
                    (char **)(sv = state.shellv), n,
                    sub == 'a' || sub == 'e' || sub == 'p' ? byname : sub == 't' ? bytemp : byrank);
                switch (sub) {
                    case 'a':
                        sfprintf(state.string,
                                 "HOST         CONF\n%-12.12s busy=%s grace=%s name=%s percpu=%d "
                                 "perserver=%d peruser=%d pool=%d\n",
                                 CS_HOST_LOCAL, fmtelapsed(state.busy, 1),
                                 fmtelapsed(state.grace, 1), state.shell->name, state.percpu,
                                 state.perserver, state.peruser, state.pool);
                        break;
                    case 'o':
                    case 's':
                    case 't':
                        sfprintf(state.string,
                                 "CON OPEN USERS     UP CONNECT UPDATE OVERRIDE   IDLE   TEMP     "
                                 "RANK HOST\n");
                        break;
                    case 'p':
                    ...
                    default:
                        sfprintf(state.string,
                                 "CON JOBS TOTAL   USER    SYS   IDLE CPU  LOAD RATING  BIAS TYPE  "
                                 "   HOST\n");
                        break;
                }
                while (sp = *sv++) {
                    switch (sub) {
                        case 'a':
                            sfprintf(state.string, "%-12.12s rating=%s", sp->name,
                                     fmtfloat(sp->rating));
                            if (sp->flags & IGN) sfprintf(state.string, " ignore=1");
                            if (sp->bias != BIAS)
                                sfprintf(state.string, " bias=%s", fmtfloat(sp->bias));
                            if (sp->cpu != 1) sfprintf(state.string, " cpu=%d", sp->cpu);
                            if (sp->scale != sp->cpu)
                            ...
                            if (sp->access) sfprintf(state.string, " access=%s", sp->access);
                            if (sp->bypass) sfprintf(state.string, " bypass=%s", sp->bypass);
                            if (sp->shell[0]) sfprintf(state.string, " shell=%s", sp->shell);
                            sfprintf(state.string, " %s\n", sp->misc);
                            break;
                        case 'p':
                        ...
                        default:
                            ...
                            if (sub == 'o' || sub == 's' || sub == 't')
                                sfprintf(
                                    state.string, "%3s%c%4d%6d%7s%8s%7s%7s/%d%7s%7s%9s %s\n", t,
                                    sp->fd > 0 && state.con[sp->fd].error ? '?' : ' ', sp->open,
                                    sp->stat.users,
                                    ...
                                    fmtelapsed(sp->override > cs.time ? sp->override - cs.time : 0,
                                               1),
                                    sp->home, fmtelapsed(sp->idle, 1),
                                    fmtfloat(sp->temp >> (CHAR_BIT * sizeof(sp->temp) / 2)),
                                    fmtfloat(sp->rank), sp->name);
                            else
                                sfprintf(
                                    state.string, "%3s %4d%6d%7s%7s%7s%c%3d%6s%7s%6s %-8.8s %s\n",
                                    t, sp->running, sp->total, fmtelapsed(sp->user, CO_QUANT),
                                        ...
                                        : (sp->mode & SHELL_DISABLE)
                                              ...
                                              : (sp->mode & SHELL_DENIED)
                                                    ...
                                                    : sp->idle && sp->stat.idle < sp->idle
                                                          ? (sp->home ? '~' : '*')
                                                          : ' ',
                                    sp->cpu, fmtfloat(sp->stat.load / sp->scale),
                                    fmtfloat(sp->rating), fmtfloat(sp->bias), sp->type, sp->name);
                            break;
                    }
                }
            }
        ...
        case 't':
            ...
            sfprintf(state.string, "%6s%7s%8s%6d%7s%7s%7s%7s%8s%6s%7s\n", num[0], num[1], num[2],
                     state.cmds, fmtelapsed(cs.time - state.start, 1), fmtelapsed(state.real, 1),
                     fmtelapsed(state.user, CO_QUANT), fmtelapsed(state.sys, CO_QUANT), num[3],
                     fmtfloat(tot.stat.load), fmtfloat(tot.rating));
            break;
        case 'u':
            for (n = 0; n <= state.fdtotal; n++)
                if (con[n].type == USER) {
                    if (!hdr) {
                        hdr = 1;
                        sfprintf(state.string,
                                 "CON   PID JOBS TOTAL TTY                          LABEL\n");
                    }
                    sfprintf(state.string, "%3d%6d%5d%6d %-28s %s\n", n, con[n].info.user.pid,
                             con[n].info.user.running, con[n].info.user.total,
                             cspath(con[n].info.user.fds[2], 0), con[n].info.user.attr.label);
                }
            break;
        case 'v':
            sfprintf(state.string, "%s\n", state.version);
