...
static void quote(register Sfio_t *sp, register char *s) {
    register int c;
...
int shellopen(register Coshell_t *sp, int fd) {
    char *sh;
    Proc_t *proc;
    Sfio_t *xp;
    Sfio_t *vp;
    ...
    if (!sp) {
        ...
        if (!(sh = sfstruse(xp))) goto nospace;
        if (sp == state.shell) {
            sfprintf(vp,
                     "%s=%s %s=%s %s='%s %s' COINIT='%s' %s /dev/fd/4 >/dev/null 2>&1 3<%s 4<&3 "
                     "5>&- 6>&- 7>&- 8>&- 9>&- &",
                     CO_ENV_HOST, sp->name, CO_ENV_TYPE, sp->type, CO_ENV_SHELL, opt_info.argv[0],
                     state.service, (sp->flags & SETRATING) ? "rating=0;" : "", sh, state.mesg);
            av[0] = "sh";
            av[1] = "-c";
        } else {
            sfprintf(vp,
                     "%s -c 'trap \"\" HUP; %s=%s %s=%s %s= COINIT='\\''%s%s'\\'' %s /dev/fd/4 "
                     ">/dev/null 2>&1 3<%s 4<&3 5>&- 6>&- 7>&- 8>&- 9>&- &'",
                     sh, CO_ENV_HOST, sp->name, CO_ENV_TYPE, sp->type, CO_ENV_SHELL,
                     (sp->flags & SETRATING) ? "rating=0;" : "", state.profile ? state.profile : "",
                     sh, state.mesg);
            av[0] = sh = state.remote;
            av[1] = sp->name;
        }
        if (!(av[2] = sfstruse(vp))) goto nospace;
        av[3] = 0;
    ...
    return -1;
nospace:
    if (fd >= 0) error(ERROR_OUTPUT | 2, fd, "out of space");
    if (xp) sfstrclose(xp);
    if (vp) sfstrclose(vp);
    return -1;
}
...
void shellclose(register Coshell_t *sp, int fd) {
    register Cojob_t *jp;
...
void shellexec(Cojob_t *jp, char *msg, int fd) {
    ...
    con = state.con;
    cmd = strdup(msg);
    if (tokscan(msg, &end, "%s %d %d %s %s %s %s %s %s", NULL, &id, &flags, &pwd, &out, &err, &att,
                &env, &act) != 9) {
    ...
    if (!jp || !(sp = jp->shell) || sp == &state.wait) {
        ...
        } else if (!(sp = search((flags & CO_LOCAL) ? DEF | JOB : JOB, att, &attr,
                                 &con[fd].info.user.attr))) {
            if (s = con[fd].info.user.expr) {
                if (att)
                    error(ERROR_OUTPUT | 2, con[fd].info.user.fds[2], "%s && %s: invalid host", s,
                          att);
                else
                    error(ERROR_OUTPUT | 2, con[fd].info.user.fds[2], "%s: invalid host", s);
            } else if (att)
                error(ERROR_OUTPUT | 2, con[fd].info.user.fds[2], "%s: invalid host", att);
            else
                error(ERROR_OUTPUT | 2, con[fd].info.user.fds[2], "all hosts closed");
            goto noexec;
        }
    ...
    sfprintf(state.string,
             "{\ntrap 'set %s$?; trap \"\" 0; print -u3 x %d $1 $(times); 3>&-; exit $1' 0 HUP INT "
             "QUIT TERM%s\n",
             (flags & CO_SILENT) ? "" : "+x ", jp - state.job, (flags & CO_IGNORE) ? "" : " ERR");
    if (!out) {
    ...
    sfprintf(state.string, "%s\ntypeset -i%d ___=${!:-$$}\nexport %s=%..*u${___#%d#}\neval '%s",
             env ? env : "", TEMPBASE, CO_ENV_TEMP, TEMPBASE, sp->addr, TEMPBASE,
    ...
    if (cswrite(con[fd].info.user.fds[0], state.buf, n) != n) goto noexec;
    return;
nojob:
    if (cmd) free(cmd);
...
noexec:
    ...
    if (cswrite(con[fd].info.user.fds[0], state.buf, n) != n) drop(fd);
}
