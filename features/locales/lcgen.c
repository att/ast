...
static Link_t *enter(Table_t *tab, Link_t *v) {
    ...
    for (p = 0, x = tab->root; x; p = x, x = x->next)
        if (!strcmp(x->code, v->code)) return x;
    if (p)
        p->next = v;
    else
        tab->root = v;
...
int main(int argc, char **argv) {
    ...
    command = argv[0];
    const char *tab = _LC_TAB;
    const char *hdr = _LC_HDR;
    const char *lib = _LC_LIB;
    FILE *tf;
    FILE *hf;
    FILE *lf;
    ...
    fprintf(lf, "#include \"config_ast.h\"\n");
    fprintf(lf, "#include \"lclib.h\"\n");
    fprintf(lf, "#include \"lclang.h\"\n");
    fprintf(lf, "\n");

    while ((s = fgets(buf, sizeof(buf), tf))) {
        line++;
        while (isspace(*s)) s++;
        if (!*s || *s == '#') continue;
        b = s;
        ...
        switch (type) {
                    ...
                    while (*(b = s)) {
                        ...
                        if (c == ':') {
                            for (b = s; *s && *s != '|'; s++)
                                ;
                            if (*s) *s++ = 0;
                            if (!strcmp(b, "primary")) tp->primary = 1;
                        }
                    }
                    if (territory_language_max < i) territory_language_max = i;
                }
            ...
            case LANGUAGE:
                ...
                if (s) {
                    ...
                    if (language_attribute_max < i) language_attribute_max = i;
                    fprintf(lf, "};\n");
                }
    ...
    if (!language_attribute_max) language_attribute_max = 1;
    if (!territory_language_max) territory_language_max = 1;
    fprintf(hf, "/* : : generated by %s : : */\n", command);
    ...
    for (lp = (Language_t *)state.language.root; lp; lp = (Language_t *)lp->link.next) {
        ...
        if (lp->attributes) {
            ...
            for (; i < language_attribute_max; i++) fprintf(lf, "0,");
        }
        fprintf(lf, "}\n},\n");
    }
    ...
    for (tp = (Territory_t *)state.territory.root; tp; tp = (Territory_t *)tp->link.next) {
        ...
        for (; i < territory_language_max; i++) fprintf(lf, "0,");
        fprintf(lf, "}\n");
        fprintf(lf, "#ifdef _LC_TERRITORY_PRIVATE_\n");
        fprintf(lf, ",{");
        for (i = 0, ll = tp->languages; ll; ll = ll->next, i++)
